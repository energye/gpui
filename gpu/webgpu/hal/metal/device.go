...
import (
	...
	"github.com/energye/gpui/gpu/shader/msl"
...
func (d *Device) CreateBuffer(desc *hal.BufferDescriptor) (hal.Buffer, error) {
	...
	copyDst := desc.Usage&types.BufferUsageCopyDst != 0
...
func (d *Device) CreateShaderModule(desc *hal.ShaderModuleDescriptor) (hal.ShaderModule, error) {
	...
	if desc.Source.WGSL != "" { //nolint:nestif // WGSL→MSL pipeline is sequential; splitting would scatter coupled logic
		...
		// Parse WGSL to AST
		ast, err := shader.Parse(desc.Source.WGSL)
		if err != nil {
			return nil, fmt.Errorf("metal: failed to parse WGSL: %w", err)
		}

		// Lower AST to IR
		irModule, err := shader.LowerWithSource(ast, desc.Source.WGSL)
		if err != nil {
...
func (d *Device) buildVertexDescriptor(buffers []types.VertexBufferLayout) (ID, error) {
