...
import (
	...
	"github.com/energye/gpui/gpu/shader/dxil"
	"github.com/energye/gpui/gpu/shader/hlsl"
	...
	"github.com/energye/gpui/gpu/webgpu/hal/dx12/d3dcompile"
...
func newDevice(instance *Instance, adapterPtr unsafe.Pointer, featureLevel d3d12.D3D_FEATURE_LEVEL) (*Device, error) {
	...
	if instance.flags&types.InstanceFlagsDebug != 0 {
	...
	if dev.useDXIL {
		hal.Logger().Info("dx12: device created (DXIL direct compilation via naga dxil backend)",
			...
			"debugLayer", instance.flags&types.InstanceFlagsDebug != 0,
		)
	} else {
...
func (d *Device) CreateTextureView(texture hal.Texture, desc *hal.TextureViewDescriptor) (hal.TextureView, error) {
	...
	if tex.usage&types.TextureUsageRenderAttachment != 0 && !isDepthFormat(viewFormat) {
		...
		if isMultisampled && viewDim == types.TextureViewDimension2D {
...
func (d *Device) CreateBindGroup(desc *hal.BindGroupDescriptor) (hal.BindGroup, error) {
	...
	var viewEntries []types.BindGroupEntry // CBV, SRV, UAV
	...
	for _, layoutEntry := range layout.entries {
		...
		for _, bgEntry := range desc.Entries {
			...
			if bufBinding, ok := bgEntry.Resource.(types.BufferBinding); ok && bufBinding.Buffer != 0 {
...
func (d *Device) populateBindGroupDescriptors(bg *BindGroup, totalViewDescs uint32, viewEntries []types.BindGroupEntry, samplerPoolIndices []uint32) error {
...
func (d *Device) writeViewDescriptorsBatched(cpuStart d3d12.D3D12_CPU_DESCRIPTOR_HANDLE, entries []types.BindGroupEntry) error {
...
func (d *Device) compileWGSLModule(wgslSource string, nagaOpts *hlsl.Options, module *ShaderModule) error {
	...
	ast, err := shader.Parse(wgslSource)
	if err != nil {
		return fmt.Errorf("WGSL parse: %w", err)
	}

	// Step 2: Lower to IR
	irModule, err := shader.LowerWithSource(ast, wgslSource)
	if err != nil {
