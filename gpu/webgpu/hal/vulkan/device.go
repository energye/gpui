...
import (
	...
	"github.com/energye/gpui/gpu/webgpu/hal/vulkan/memory"
...
func (d *Device) CreateBuffer(desc *hal.BufferDescriptor) (hal.Buffer, error) {
	...
	if desc.Usage&(types.BufferUsageMapRead|types.BufferUsageMapWrite) != 0 || desc.MappedAtCreation {
		memUsage = memory.UsageHostAccess
		if desc.Usage&types.BufferUsageMapWrite != 0 || desc.MappedAtCreation {
...
func (d *Device) CreateTexture(desc *hal.TextureDescriptor) (hal.Texture, error) {
	...
	if desc.Dimension == types.TextureDimension2D && arrayLayers >= 6 {
...
func (d *Device) updateDescriptorSet(set vk.DescriptorSet, entries []types.BindGroupEntry, bindingTypes map[uint32]vk.DescriptorType) error {
	if len(entries) == 0 {
		return nil
	}
...
func (d *Device) CreateShaderModule(desc *hal.ShaderModuleDescriptor) (hal.ShaderModule, error) {
	...
	case desc.Source.WGSL != "":
		spirvBytes, err := shader.Compile(desc.Source.WGSL)
		if err != nil {
