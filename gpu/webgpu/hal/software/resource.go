...
func (r *Resource) CurrentUsage() types.TextureUsage { return 0 }
...
func formatBytesPerPixel(f types.TextureFormat) uint64 {
...
// CurrentUsage returns 0 — Software backend has no resource state tracking.
func (t *Texture) CurrentUsage() types.TextureUsage { return 0 }
...
func (t *Texture) Clear(color types.Color) {
	...
	default:
		c0, c1, c2, c3 := r, g, b, a
		if t.format == types.TextureFormatBGRA8Unorm || t.format == types.TextureFormatBGRA8UnormSrgb {
			c0, c2 = b, r
		}
...
func (s *Surface) GetFramebuffer() []byte {
	...
	if s.format == types.TextureFormatBGRA8Unorm ||
		s.format == types.TextureFormatBGRA8UnormSrgb {
