...
import (
	...
	"github.com/energye/gpui/gpu/webgpu/hal/software/raster"
	"github.com/energye/gpui/gpu/webgpu/hal/software/shader"
)
...
func (r *RenderPassEncoder) blitStateValid() bool {
	...
	for _, t := range desc.Fragment.Targets {
		if t.WriteMask != types.ColorWriteMaskAll {
			return false
		}
...
func isBlitSafeBlend(b *types.BlendState) bool {
	if b == nil {
		return true
	}
	srcOver := b.Color.SrcFactor == types.BlendFactorOne &&
		b.Color.DstFactor == types.BlendFactorOneMinusSrcAlpha
	src := b.Color.SrcFactor == types.BlendFactorOne &&
		b.Color.DstFactor == types.BlendFactorZero
	return srcOver || src
}
...
func needsSwizzle(src, dst types.TextureFormat) bool {
	srcBGRA := src == types.TextureFormatBGRA8Unorm || src == types.TextureFormatBGRA8UnormSrgb
	dstBGRA := dst == types.TextureFormatBGRA8Unorm || dst == types.TextureFormatBGRA8UnormSrgb
...
func isBGRA(fmt types.TextureFormat) bool {
	return fmt == types.TextureFormatBGRA8Unorm || fmt == types.TextureFormatBGRA8UnormSrgb
}
...
func (r *RenderPassEncoder) fetchTriangles(
	...
	var posAttr *types.VertexAttribute
	var extraAttrs []types.VertexAttribute
	...
	for inst := uint32(0); inst < instanceCount; inst++ {
		...
		for vert := uint32(0); vert < vertexCount; vert++ {
			...
			for _, li := range locationInputs {
				...
				var idx uint32
				if ab.layout.StepMode == types.VertexStepModeInstance {
					idx = instanceID
				} else {
					idx = vertexID
				}
...
func filterModeToShader(mode types.FilterMode) uint32 {
	switch mode {
...
func wrapModeToShader(mode types.AddressMode) uint32 {
	switch mode {
...
func readVertexAttribute(data []byte, offset uint64, format types.VertexFormat) []float32 {
...
func (r *RenderPassEncoder) hasVertexColors(layouts []types.VertexBufferLayout) bool {
	if len(layouts) == 0 {
		return false
	}
	for _, attr := range layouts[0].Attributes {
		...
		switch attr.Format {
		case types.VertexFormatFloat32x3, types.VertexFormatFloat32x4, types.VertexFormatUnorm8x4:
			return true
		}
	}
	return false
...
func (r *RenderPassEncoder) configureRasterPipeline(pipe *raster.Pipeline) {
	...
	depthEnabled := ds.DepthCompare != types.CompareFunctionUndefined &&
		ds.DepthCompare != types.CompareFunctionAlways
...
func isStencilEnabled(sf hal.StencilFaceState) bool {
	...
	if sf.Compare == types.CompareFunctionAlways &&
	...
	if sf.Compare == types.CompareFunctionUndefined {
		return false
	}
	return true
}
...
func convertCompareFunction(cf types.CompareFunction) raster.CompareFunc {
	switch cf {
