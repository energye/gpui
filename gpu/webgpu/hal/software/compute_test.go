...
func TestSoftwareComputeMultipleWorkgroups(t *testing.T) {
	...
	inputBuf, _ := dev.CreateBuffer(&hal.BufferDescriptor{Size: bufSize, Usage: types.BufferUsageStorage})
	outputBuf, _ := dev.CreateBuffer(&hal.BufferDescriptor{Size: bufSize, Usage: types.BufferUsageStorage})
	defer dev.DestroyBuffer(inputBuf)
	defer dev.DestroyBuffer(outputBuf)
	...
	bg, _ := dev.CreateBindGroup(&hal.BindGroupDescriptor{
		Entries: []types.BindGroupEntry{
			{Binding: 0, Resource: types.BufferBinding{Buffer: inputBuf.NativeHandle()}},
			{Binding: 1, Resource: types.BufferBinding{Buffer: outputBuf.NativeHandle()}},
		},
	})
	defer dev.DestroyBindGroup(bg)
