	...
	if desc.DepthStencilAttachment != nil { //nolint:nestif // sequential attachment init
		if dsView, ok := desc.DepthStencilAttachment.View.(*TextureView); ok && dsView.texture != nil {
			...
			if w > 0 && h > 0 {
				...
				if desc.DepthStencilAttachment.StencilLoadOp == types.LoadOpClear {
...
func (r *RenderPassEncoder) clearDepthStencilAttachment() {
	...
	if ds == nil || ds.DepthLoadOp != types.LoadOpClear {
		return
	}
	...
	view.texture.Clear(types.Color{R: float64(val), G: float64(val), B: float64(val), A: 1.0})
}
...
func (r *RenderPassEncoder) SetIndexBuffer(buf hal.Buffer, format types.IndexFormat, offset uint64) {
	if b, ok := buf.(*Buffer); ok {
		r.indexBuffer = b
...
func (r *RenderPassEncoder) resolveIndices(indexCount, firstIndex uint32, baseVertex int32) []uint32 {
	indexSize := uint64(2)
	if r.indexFormat == types.IndexFormatUint32 {
		indexSize = 4
	}
