...
func TestRenderPassSetBindGroup(t *testing.T) {
	...
	bg, _ := dev.CreateBindGroup(&hal.BindGroupDescriptor{
		...
		Entries: []types.BindGroupEntry{
			{
				Binding:  0,
				Resource: types.TextureViewBinding{TextureView: view.NativeHandle()},
			},
		},
	})
...
func TestDrawTexturedQuad(t *testing.T) {
	...
	src := srcTex.(*Texture)
	src.Clear(types.Color{R: 1, G: 0, B: 0, A: 1})
	...
	bg, _ := dev.CreateBindGroup(&hal.BindGroupDescriptor{
		Entries: []types.BindGroupEntry{
			{
				Binding:  0,
				Resource: types.TextureViewBinding{TextureView: srcView.NativeHandle()},
			},
		},
	})
	defer dev.DestroyBindGroup(bg)
...
func TestDrawClearsBeforeBlit(t *testing.T) {
	...
	src := srcTex.(*Texture)
	src.Clear(types.Color{R: 0, G: 1, B: 0, A: 1})
	...
	defer dev.DestroyTexture(dstTex)
	dstTex.(*Texture).Clear(types.Color{R: 1, G: 1, B: 1, A: 1})
	...
	bg, _ := dev.CreateBindGroup(&hal.BindGroupDescriptor{
		Entries: []types.BindGroupEntry{
			{Binding: 0, Resource: types.TextureViewBinding{TextureView: srcView.NativeHandle()}},
		},
	})
	defer dev.DestroyBindGroup(bg)
		...
		ColorAttachments: []hal.RenderPassColorAttachment{
			{
				View:       dstView,
				LoadOp:     types.LoadOpClear,
				ClearValue: types.Color{R: 0, G: 0, B: 1, A: 1}, // clear to blue
			},
		},
	})
	pass.SetPipeline(pipeline)
...
func TestDrawWithoutPipeline(t *testing.T) {
	...
	pass := enc.BeginRenderPass(&hal.RenderPassDescriptor{
		ColorAttachments: []hal.RenderPassColorAttachment{
			{View: dstView, LoadOp: types.LoadOpClear, ClearValue: types.Color{R: 1, G: 0, B: 0, A: 1}},
		},
	})
...
func TestDrawWithoutTexture(t *testing.T) {
	...
	bg, _ := dev.CreateBindGroup(&hal.BindGroupDescriptor{
		Entries: []types.BindGroupEntry{
			{Binding: 0, Resource: types.BufferBinding{Buffer: buf.NativeHandle(), Size: 64}},
		},
	})
	defer dev.DestroyBindGroup(bg)
	...
	pass := enc.BeginRenderPass(&hal.RenderPassDescriptor{
		ColorAttachments: []hal.RenderPassColorAttachment{
			{View: dstView, LoadOp: types.LoadOpClear, ClearValue: types.Color{R: 0, G: 0, B: 1, A: 1}},
		},
	})
	pass.SetPipeline(pipeline)
	pass.SetBindGroup(0, bg, nil)
...
func TestDrawBGRAToRGBAConversion(t *testing.T) {
	...
	pass := enc.BeginRenderPass(&hal.RenderPassDescriptor{
		ColorAttachments: []hal.RenderPassColorAttachment{
			{View: dstView, LoadOp: types.LoadOpLoad},
		},
	})
	pass.SetPipeline(pipeline)
	pass.SetBindGroup(0, bg, nil)
...
func TestDrawWithSurfaceTexture(t *testing.T) {
	...
	defer dev.DestroyTexture(srcTex)
	srcTex.(*Texture).Clear(types.Color{R: 0, G: 1, B: 0, A: 1})
	...
	pass := enc.BeginRenderPass(&hal.RenderPassDescriptor{
		ColorAttachments: []hal.RenderPassColorAttachment{
			{View: surfView, LoadOp: types.LoadOpLoad},
		},
	})
	pass.SetPipeline(pipeline)
	pass.SetBindGroup(0, bg, nil)
...
func TestDrawMultipleBindGroups(t *testing.T) {
	...
	defer dev.DestroyTexture(srcTex)
	srcTex.(*Texture).Clear(types.Color{R: 0.5, G: 0.5, B: 0.5, A: 1})
...
func TestDrawTriangleWithVertexColors(t *testing.T) {
			...
			Buffers: []types.VertexBufferLayout{
				{
					...
					Attributes: []types.VertexAttribute{
						{Format: types.VertexFormatFloat32x3, Offset: 0, ShaderLocation: 0},
						{Format: types.VertexFormatFloat32x4, Offset: 12, ShaderLocation: 1},
					},
				},
			},
		},
	...
	pass := enc.BeginRenderPass(&hal.RenderPassDescriptor{
		ColorAttachments: []hal.RenderPassColorAttachment{
			{View: dstView, LoadOp: types.LoadOpClear, ClearValue: types.Color{}},
		},
	})
	pass.SetPipeline(pipeline)
	pass.SetVertexBuffer(0, vb, 0)
...
func TestDrawClearBeforeDraw(t *testing.T) {
	...
	defer dev.DestroyTexture(dstTex)
	// Pre-fill with red.
	dstTex.(*Texture).Clear(types.Color{R: 1, G: 0, B: 0, A: 1})
...
func TestSetIndexBuffer(t *testing.T) {
	...
	pass.SetIndexBuffer(buf, types.IndexFormatUint16, 10)

	if encoder.indexBuffer == nil {
		t.Error("index buffer should be set")
	}
	if encoder.indexFormat != types.IndexFormatUint16 {
...
`
	...
	// --- Create buffers ---
	usage := types.BufferUsageStorage | types.BufferUsageVertex | types.BufferUsageCopyDst
	...
	bg, err := dev.CreateBindGroup(&hal.BindGroupDescriptor{
		Label: "compute-bg",
		Entries: []types.BindGroupEntry{
			{Binding: 0, Resource: types.BufferBinding{Buffer: bufA.NativeHandle(), Size: bufSize}},
			{Binding: 1, Resource: types.BufferBinding{Buffer: bufB.NativeHandle(), Size: bufSize}},
			{Binding: 2, Resource: types.BufferBinding{Buffer: uniformBuf.NativeHandle(), Size: 8}},
		},
	})
	if err != nil {
	...
	rp, err := dev.CreateRenderPipeline(&hal.RenderPipelineDescriptor{
		...
		Primitive: types.PrimitiveState{Topology: types.PrimitiveTopologyTriangleStrip},
	...
	rpEnc := enc2.BeginRenderPass(&hal.RenderPassDescriptor{
		ColorAttachments: []hal.RenderPassColorAttachment{{
			View: texView, LoadOp: types.LoadOpClear, StoreOp: types.StoreOpStore,
...
func TestScissorBlitClipsToRect(t *testing.T) {
	...
	defer dev.DestroyTexture(srcTex)
	srcTex.(*Texture).Clear(types.Color{R: 1, G: 0, B: 0, A: 1})
	...
	defer dev.DestroyTexture(dstTex)
	dstTex.(*Texture).Clear(types.Color{R: 0, G: 0, B: 1, A: 1})
