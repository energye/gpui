...
import (
	...
	"github.com/energye/gpui/gpu/shader/glsl"
...
func queryFeatures(exts map[string]bool, glMajor, glMinor int, isES bool, glCtx *gl.Context) types.Features {
	var features types.Features

	// Depth32FloatStencil8 -- always available on GL 3.3+ / ES 3.0+
	features.Insert(types.FeatureDepth32FloatStencil8)

	// TextureAdapterSpecificFormatFeatures -- always reported
	features.Insert(types.FeatureTextureAdapterSpecificFormatFeatures)
	...
	if isES || hasExtension(exts, "GL_ARB_ES3_compatibility") {
		features.Insert(types.FeatureTextureCompressionETC2)
	}

	// ASTC
	if hasExtension(exts, "GL_OES_texture_compression_astc",
		"GL_KHR_texture_compression_astc_ldr") {
		features.Insert(types.FeatureTextureCompressionASTC)
	}
	...
	if hasExtension(exts, "GL_ARB_color_buffer_float",
		...
		features.Insert(types.FeatureFloat32Filterable)
	}
	...
	if hasExtension(exts, "GL_EXT_color_buffer_float", "GL_ARB_color_buffer_float") {
		features.Insert(types.FeatureRG11B10UfloatRenderable)
	}
	...
	if !isES && glMajor >= 4 && glMinor >= 2 &&
		...
		features.Insert(types.FeatureIndirectFirstInstance)
	}
...
func queryLimits(glCtx *gl.Context, glMajor, glMinor int, isES bool, exts map[string]bool) types.Limits {
...
func inferDeviceType(vendor, renderer string) types.DeviceType {
...
func queryTextureFormatCapabilities(
	...
	compressionCaps := func(feature types.Feature) hal.TextureFormatCapabilityFlags {
	...
	case types.TextureFormatR8Unorm, types.TextureFormatRG8Unorm:
		flags = filterableRenderable
	case types.TextureFormatR8Snorm, types.TextureFormatRG8Snorm:
		flags = base // filterable but not renderable
	case types.TextureFormatR8Uint, types.TextureFormatR8Sint,
		types.TextureFormatRG8Uint, types.TextureFormatRG8Sint:
		flags = renderable

	// --- 16-bit ---
	case types.TextureFormatR16Uint, types.TextureFormatR16Sint,
		types.TextureFormatRG16Uint, types.TextureFormatRG16Sint:
		flags = renderable
	case types.TextureFormatR16Float, types.TextureFormatRG16Float:
		flags = base | halfFloatRender

	// --- 32-bit ---
	case types.TextureFormatR32Uint, types.TextureFormatR32Sint:
	...
	case types.TextureFormatRGBA8Uint, types.TextureFormatRGBA8Sint:
		flags = renderable | storage
	case types.TextureFormatBGRA8Unorm, types.TextureFormatBGRA8UnormSrgb:
		flags = filterableRenderable
	...
	// --- 64-bit ---
	case types.TextureFormatRG32Uint, types.TextureFormatRG32Sint:
		flags = renderable
	case types.TextureFormatRG32Float:
		flags = base | floatRender

	// --- 128-bit ---
	case types.TextureFormatRGBA16Uint, types.TextureFormatRGBA16Sint:
		flags = renderable | storage
	case types.TextureFormatRGBA16Float:
		flags = base | storage | halfFloatRender
	case types.TextureFormatRGBA32Uint, types.TextureFormatRGBA32Sint:
		flags = renderable | storage
	...
	// --- BC compressed ---
	case types.TextureFormatBC1RGBAUnorm, types.TextureFormatBC1RGBAUnormSrgb,
		...
		flags = compressionCaps(types.FeatureTextureCompressionBC)

	// --- ETC2 compressed ---
	case types.TextureFormatETC2RGB8Unorm, types.TextureFormatETC2RGB8UnormSrgb,
		...
		flags = compressionCaps(types.FeatureTextureCompressionETC2)

	// --- ASTC compressed ---
	case types.TextureFormatASTC4x4Unorm, types.TextureFormatASTC4x4UnormSrgb,
		...
		flags = compressionCaps(types.FeatureTextureCompressionASTC)
