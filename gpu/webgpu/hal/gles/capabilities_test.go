...
func TestInferDeviceType(t *testing.T) {
	tests := []struct {
		...
		// CPU renderers
		{"Mesa", "llvmpipe (LLVM 15.0.7)", types.DeviceTypeCPU},
		{"Google Inc.", "SwiftShader Device", types.DeviceTypeCPU},
		{"Mesa", "Mesa Offscreen", types.DeviceTypeCPU},

		// Integrated via vendor
		{"Intel", "Intel(R) Iris(R) Xe Graphics", types.DeviceTypeIntegratedGPU},
		{"Qualcomm", "Adreno (TM) 730", types.DeviceTypeIntegratedGPU},

		// Integrated via renderer string
		{"ARM", "Mali-G710", types.DeviceTypeIntegratedGPU},
		{"NVIDIA Corporation", "NVIDIA Tegra X1", types.DeviceTypeIntegratedGPU},
		{"Apple", "Apple M1 Pro", types.DeviceTypeIntegratedGPU},

		// Default (likely discrete but we report Other)
		{"NVIDIA Corporation", "NVIDIA GeForce RTX 4090", types.DeviceTypeOther},
		{"ATI Technologies Inc.", "AMD Radeon RX 7900 XT", types.DeviceTypeOther},
	}
