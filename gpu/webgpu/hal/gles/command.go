...
func (e *CommandEncoder) TransitionBuffers(barriers []hal.BufferBarrier) {
	...
	for _, bar := range barriers {
		if bar.Usage.OldUsage&types.BufferUsageStorage != 0 {
...
func (e *CommandEncoder) TransitionTextures(barriers []hal.TextureBarrier) {
	var bits uint32
	for _, bar := range barriers {
		if bar.Usage.OldUsage&types.TextureUsageStorageBinding != 0 {
...
func (e *RenderPassEncoder) SetVertexBuffer(slot uint32, buffer hal.Buffer, offset uint64) {
	...
	var layout *types.VertexBufferLayout
...
func (e *RenderPassEncoder) Draw(vertexCount, instanceCount, firstVertex, firstInstance uint32) {
	topology := types.PrimitiveTopologyTriangleList // default
	if e.pipeline != nil {
...
type SetPipelineStateCommand struct {
	...
	colorWriteMask types.ColorWriteMask
	stencilRef     uint32
}
...
func (c *SetPipelineStateCommand) Execute(ctx *gl.Context) {
	...
	// Color write mask
	ctx.ColorMask(
		c.colorWriteMask&types.ColorWriteMaskRed != 0,
		c.colorWriteMask&types.ColorWriteMaskGreen != 0,
		c.colorWriteMask&types.ColorWriteMaskBlue != 0,
		c.colorWriteMask&types.ColorWriteMaskAlpha != 0,
	)
...
func (c *SetPipelineStateCommand) applyDepthStencilState(ctx *gl.Context) {
	...
	hasStencilOps := c.depthStencil.StencilFront.PassOp != hal.StencilOperationKeep ||
		...
		c.depthStencil.StencilFront.Compare != types.CompareFunctionAlways ||
		c.depthStencil.StencilBack.Compare != types.CompareFunctionAlways
...
func (c *SetBindGroupCommand) resolveBufferTarget(binding uint32, dynamicIdx *int) (uint32, int) {
	...
	for _, le := range c.group.layout.entries {
		...
		if le.Buffer.Type == types.BufferBindingTypeStorage ||
			le.Buffer.Type == types.BufferBindingTypeReadOnlyStorage {
...
type SetVertexBufferCommand struct {
	...
	buffer *Buffer
	offset uint64
	layout *types.VertexBufferLayout // from the render pipeline descriptor
}
...
func (c *SetVertexBufferCommand) Execute(ctx *gl.Context) {
	...
	var divisor uint32
	if c.layout.StepMode == types.VertexStepModeInstance {
		divisor = 1
	}
...
func vertexFormatToGL(format types.VertexFormat) (size int32, typ uint32, normalized bool) {
	switch format {
...
func blendFactorToGL(f types.BlendFactor) uint32 {
	switch f {
...
func blendOperationToGL(op types.BlendOperation) uint32 {
	switch op {
...
func primitiveTopologyToGL(topology types.PrimitiveTopology) uint32 {
	switch topology {
