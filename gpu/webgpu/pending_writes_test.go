...
func TestPendingWrites_WriteBufferNonBatching(t *testing.T) {
	...
	data := []byte{1, 2, 3, 4}
	if err := pw.writeBuffer(buf, types.BufferUsageCopyDst|types.BufferUsageVertex, 8, data); err != nil {
...
func TestPendingWrites_WriteBufferBatching(t *testing.T) {
	...
	usage := types.BufferUsageCopyDst | types.BufferUsageVertex | types.BufferUsageUniform
	if err := pw.writeBuffer(dstBuf, types.BufferUsageCopyDst|types.BufferUsageVertex|types.BufferUsageUniform, 16, data); err != nil {
		t.Fatalf("writeBuffer: %v", err)
	}
	...
	if err := pw2.writeBuffer(dstBuf, types.BufferUsageCopyDst|types.BufferUsageVertex, 0, data); err != nil {
		t.Fatalf("writeBuffer: %v", err)
	}
...
func TestPendingWrites_WriteBufferEmptyData(t *testing.T) {
	...
	if err := pw.writeBuffer(dstBuf, types.BufferUsageCopyDst, 0, []byte{}); err != nil {
...
func TestPendingWrites_HasPendingWork(t *testing.T) {
	...
	// Write something.
	dstBuf := &noop.Resource{}
	if err := pw.writeBuffer(dstBuf, types.BufferUsageCopyDst|types.BufferUsageVertex, 0, []byte{1, 2, 3, 4}); err != nil {
		t.Fatalf("writeBuffer: %v", err)
	}
...
func TestPendingWrites_DestroyCleanup(t *testing.T) {
	...
	dstBuf := &noop.Resource{}
	if err := pw.writeBuffer(dstBuf, types.BufferUsageCopyDst, 0, []byte{1}); err != nil {
		t.Fatalf("writeBuffer: %v", err)
	}
...
func TestPendingWrites_BufferReadUsage(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want:  types.BufferUsageVertex,
		},
		{
			name:  "index only",
			usage: types.BufferUsageIndex,
			want:  types.BufferUsageIndex,
		},
		{
			name:  "uniform only",
			usage: types.BufferUsageUniform,
			want:  types.BufferUsageUniform,
		},
		{
			name:  "indirect only",
			usage: types.BufferUsageIndirect,
			want:  types.BufferUsageIndirect,
		},
		{
			name:  "vertex + copyDst",
			usage: types.BufferUsageCopyDst | types.BufferUsageVertex,
		...
		},
		{
			...
			usage: types.BufferUsageCopySrc | types.BufferUsageCopyDst | types.BufferUsageUniform,
			want:  types.BufferUsageUniform,
		},
		{
			name:  "vertex + index + uniform",
			usage: types.BufferUsageVertex | types.BufferUsageIndex | types.BufferUsageUniform,
			want:  types.BufferUsageVertex | types.BufferUsageIndex | types.BufferUsageUniform,
		},
		{
		...
		},
		{
			...
			usage: types.BufferUsageCopySrc | types.BufferUsageCopyDst | types.BufferUsageMapWrite |
				types.BufferUsageVertex | types.BufferUsageIndex | types.BufferUsageUniform |
				types.BufferUsageStorage | types.BufferUsageIndirect,
			want: types.BufferUsageVertex | types.BufferUsageIndex | types.BufferUsageUniform | types.BufferUsageIndirect,
		},
		{
			name:  "zero usage",
			usage: 0,
...
func TestPendingWrites_MultipleWritesThenFlush(t *testing.T) {
	...
	if err := pw.writeBuffer(buf1, types.BufferUsageCopyDst|types.BufferUsageVertex, 0, data); err != nil {
		t.Fatalf("writeBuffer 1: %v", err)
	}
	if err := pw.writeBuffer(buf2, types.BufferUsageCopyDst|types.BufferUsageUniform, 0, data); err != nil {
...
func TestPendingWrites_ActivateReusesEncoder(t *testing.T) {
	...
	if err := pw.writeBuffer(&noop.Resource{}, types.BufferUsageCopyDst, 0, []byte{1}); err != nil {
		t.Fatalf("write 1: %v", err)
	}
	firstEncoder := pw.encoder

	// Second write should reuse the same encoder.
	if err := pw.writeBuffer(&noop.Resource{}, types.BufferUsageCopyDst, 0, []byte{2}); err != nil {
...
func TestPendingWrites_WriteBufferNonBatchingVerifiesData(t *testing.T) {
	...
	if err := pw.writeBuffer(buf, types.BufferUsageCopyDst, 4, data); err != nil {
		t.Fatalf("writeBuffer: %v", err)
	}
