...
func TestCopyTextureToTextureNilSrc(t *testing.T) {
	...
	tex, err := device.CreateTexture(&webgpu.TextureDescriptor{
		...
		Usage:         webgpu.TextureUsageCopyDst,
	})
	if err != nil {
...
func TestCopyTextureToTextureNilDst(t *testing.T) {
	...
	tex, err := device.CreateTexture(&webgpu.TextureDescriptor{
		...
		Dimension:     webgpu.TextureDimension2D,
		Format:        webgpu.TextureFormatRGBA8Unorm,
		Usage:         webgpu.TextureUsageCopySrc,
	})
	if err != nil {
		t.Fatalf("CreateTexture: %v", err)
	}
...
func TestCopyTextureToTextureValid(t *testing.T) {
	...
	srcTex, err := device.CreateTexture(&webgpu.TextureDescriptor{
		Label:         "copy-t2t-src",
		Size:          webgpu.Extent3D{Width: 4, Height: 4, DepthOrArrayLayers: 1},
		MipLevelCount: 1,
		SampleCount:   1,
	...
	dstTex, err := device.CreateTexture(&webgpu.TextureDescriptor{
		Label:         "copy-t2t-dst",
		Size:          webgpu.Extent3D{Width: 4, Height: 4, DepthOrArrayLayers: 1},
		MipLevelCount: 1,
		SampleCount:   1,
	...
	regions := []webgpu.TextureCopy{
		{
			Source:      webgpu.ImageCopyTexture{Texture: srcTex},
			Destination: webgpu.ImageCopyTexture{Texture: dstTex},
			Size:        webgpu.Extent3D{Width: 4, Height: 4, DepthOrArrayLayers: 1},
		},
	}
...
func TestComputePass_DirectWriteTrackedRefs(t *testing.T) {
	...
	layout, err := device.CreateBindGroupLayout(&webgpu.BindGroupLayoutDescriptor{
		...
		Entries: []webgpu.BindGroupLayoutEntry{
			{Binding: 0, Visibility: webgpu.ShaderStageCompute,
