...
type Instance struct {
	...
	halInstanceMap map[types.Backend]hal.Instance
...
func NewInstance(desc *types.InstanceDescriptor) *Instance {
	...
	i := &Instance{
		...
		adapters:       []AdapterID{},
		halInstances:   []hal.Instance{},
		halInstanceMap: make(map[types.Backend]hal.Instance),
		useMock:        false,
	}
...
func (i *Instance) enumerateRealAdapters(desc *types.InstanceDescriptor) bool {
	...
	for _, provider := range providers {
		...
		if provider.Variant() == types.BackendEmpty {
			...
			isNoop := len(adapters) > 0 && adapters[0].Info.DeviceType == types.DeviceTypeOther
			if isNoop {
				halInst.Destroy()
				continue
			}
...
func (i *Instance) RequestAdapter(options *types.RequestAdapterOptions) (AdapterID, error) { //nolint:gocognit // adapter selection with GPU preference logic
...
func (i *Instance) RequestAdapterWithSurface(options *types.RequestAdapterOptions, surfaceHint hal.Surface) (AdapterID, error) {
...
func (i *Instance) HALInstanceForBackend(backend types.Backend) hal.Instance {
	i.mu.RLock()
	defer i.mu.RUnlock()
...
func (i *Instance) HALInstanceMap() map[types.Backend]hal.Instance {
	i.mu.RLock()
	defer i.mu.RUnlock()
