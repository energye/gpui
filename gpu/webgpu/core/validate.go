...
func ValidateTextureDescriptor(desc *hal.TextureDescriptor, limits types.Limits) error {
	label := desc.Label
	w := desc.Size.Width
	h := desc.Size.Height
	...
	if desc.Dimension == types.TextureDimensionUndefined {
	...
	if desc.Usage == types.TextureUsageNone {
		return &CreateTextureError{
...
func validateTextureDimLimits(desc *hal.TextureDescriptor, label string, limits types.Limits) error {
	w := desc.Size.Width
	h := desc.Size.Height
	d := desc.Size.DepthOrArrayLayers
...
func validateTextureMultisample(desc *hal.TextureDescriptor, label string) error {
	...
	if desc.Usage.Contains(types.TextureUsageStorageBinding) {
		return &CreateTextureError{
...
func ValidateSamplerDescriptor(desc *hal.SamplerDescriptor) error {
	...
	if anisotropy > 1 {
		...
		if desc.MagFilter != types.FilterModeLinear ||
			desc.MinFilter != types.FilterModeLinear ||
			desc.MipmapFilter != types.FilterModeLinear {
			return &CreateSamplerError{
...
func ValidatePipelineLayoutDescriptor(desc *hal.PipelineLayoutDescriptor, limits types.Limits) error {
	label := desc.Label
...
func isDepthStencilFormat(f types.TextureFormat) bool {
	switch f {
...
func hasDepthAspect(f types.TextureFormat) bool {
	switch f {
...
func hasStencilAspect(f types.TextureFormat) bool {
	switch f {
	case types.TextureFormatStencil8,
		types.TextureFormatDepth24PlusStencil8,
		types.TextureFormatDepth32FloatStencil8:
...
func isDepthEnabled(ds *hal.DepthStencilState) bool {
	return ds.DepthCompare != types.CompareFunctionAlways || ds.DepthWriteEnabled
}
...
func stencilFaceIgnored(face hal.StencilFaceState) bool {
	return face.Compare == types.CompareFunctionAlways &&
...
func ValidateRenderPipelineDescriptor(desc *hal.RenderPipelineDescriptor, limits types.Limits) error {
	label := desc.Label
	...
	if desc.Fragment != nil {
		for i, ct := range desc.Fragment.Targets {
			if ct.Format != types.TextureFormatUndefined && isDepthStencilFormat(ct.Format) {
				return &CreateRenderPipelineError{
	...
	if desc.DepthStencil != nil {
		if desc.DepthStencil.Format != types.TextureFormatUndefined && !isDepthStencilFormat(desc.DepthStencil.Format) {
			return &CreateRenderPipelineError{
...
func validateFragmentStage(frag *hal.FragmentState, label string, limits types.Limits) error {
...
func ValidateBindGroupLayoutDescriptor(desc *hal.BindGroupLayoutDescriptor, limits types.Limits) error {
	label := desc.Label
...
func ValidateBindGroupDescriptor(
	...
	layoutByBinding := make(map[uint32]*types.BindGroupLayoutEntry, len(layoutEntries))
...
func validateBindGroupBufferEntries(
	label string,
	layoutByBinding map[uint32]*types.BindGroupLayoutEntry,
	bufferInfos []BindGroupBufferInfo,
	limits types.Limits,
) error {
...
type bufferBindingParams struct {
	requiredUsage    types.BufferUsage
...
func resolveBufferBindingParams(bindingType types.BufferBindingType, limits types.Limits) (bufferBindingParams, bool) {
	switch bindingType {
	case types.BufferBindingTypeUniform:
		return bufferBindingParams{
			requiredUsage:    types.BufferUsageUniform,
	...
	case types.BufferBindingTypeStorage, types.BufferBindingTypeReadOnlyStorage:
		return bufferBindingParams{
			requiredUsage:    types.BufferUsageStorage,
...
func validateSingleBufferEntry(
	label string,
	bufLayout *types.BufferBindingLayout,
...
func maxMips(dimension types.TextureDimension, width, height, depth uint32) uint32 {
	var maxDim uint32
	switch dimension {
	case types.TextureDimension1D:
		maxDim = width
