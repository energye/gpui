...
func newTestBuffer(usage types.BufferUsage, size uint64) *Buffer {
	return &Buffer{
		usage:       usage,
		size:        size,
...
func TestBeginMap_Success(t *testing.T) {
	buf := newTestBuffer(types.BufferUsageMapRead, 1024)
...
func TestBeginMap_InvalidMode(t *testing.T) {
	tests := []struct {
		...
		{"read without MAP_READ", types.BufferUsageMapWrite, MapModeInternalRead},
		{"write without MAP_WRITE", types.BufferUsageMapRead, MapModeInternalWrite},
		{"zero mode", types.BufferUsageMapRead, 0},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestUnmapBuffer_FromMapped(t *testing.T) {
	...
	buf := NewBuffer(mockBuffer{}, device, types.BufferUsageMapRead, 1024, "test")
