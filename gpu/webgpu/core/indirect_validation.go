...
func NewIndirectValidation(halDevice hal.Device, limits types.Limits) *IndirectValidation {
	if halDevice == nil {
		return nil
	}
...
func buildIndirectValidation(
	...
	b.dstBindGroupLayout, err = halDevice.CreateBindGroupLayout(&hal.BindGroupLayoutDescriptor{
		...
		Entries: []types.BindGroupLayoutEntry{
			{Binding: 0, Visibility: types.ShaderStageCompute, Buffer: &types.BufferBindingLayout{
				Type: types.BufferBindingTypeStorage, MinBindingSize: 12,
			}},
			{Binding: 1, Visibility: types.ShaderStageCompute, Buffer: &types.BufferBindingLayout{
				Type: types.BufferBindingTypeUniform, MinBindingSize: 8,
			}},
		},
	})
	if err != nil {
	...
	b.srcBindGroupLayout, err = halDevice.CreateBindGroupLayout(&hal.BindGroupLayoutDescriptor{
		...
		Entries: []types.BindGroupLayoutEntry{
			{Binding: 0, Visibility: types.ShaderStageCompute, Buffer: &types.BufferBindingLayout{
				Type: types.BufferBindingTypeReadOnlyStorage, MinBindingSize: 12,
			}},
		},
	})
	if err != nil {
