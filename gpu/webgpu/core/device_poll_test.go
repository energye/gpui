...
func TestRegisterPendingMap_Basic(t *testing.T) {
	...
	device := NewDevice(halDevice, &Adapter{}, types.Features(0), types.DefaultLimits(), "PollTest")

	buf := NewBuffer(mockBuffer{}, device, types.BufferUsageMapRead, 256, "buf")
...
func TestPollMaps_MultipleSubmissions(t *testing.T) {
	...
	buf1 := NewBuffer(mockBuffer{}, device, types.BufferUsageMapRead, 256, "buf1")
	buf2 := NewBuffer(mockBuffer{}, device, types.BufferUsageMapRead, 256, "buf2")
	buf3 := NewBuffer(mockBuffer{}, device, types.BufferUsageMapRead, 256, "buf3")
