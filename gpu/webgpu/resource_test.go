...
func TestBufferAccessorsTableDriven(t *testing.T) {
	...
	tests := []struct {
		...
		{"small_vertex", "small-vertex-buf", 64, webgpu.BufferUsageVertex | webgpu.BufferUsageCopyDst},
		{"large_storage", "large-storage-buf", 4096, webgpu.BufferUsageStorage | webgpu.BufferUsageCopyDst},
		{"uniform_buffer", "uniform-buf", 256, webgpu.BufferUsageUniform | webgpu.BufferUsageCopyDst},
		{"minimal_size", "min-buf", 4, webgpu.BufferUsageCopyDst},
		{"copysrc_copydst", "copy-buf", 128, webgpu.BufferUsageCopySrc | webgpu.BufferUsageCopyDst},
	}
...
func TestBufferMapStateUnmapped(t *testing.T) {
	...
	state := buf.MapState()
	if state != webgpu.MapStateUnmapped {
...
func TestBufferMapStateMappedAtCreation(t *testing.T) {
	...
	state := buf.MapState()
	if state != webgpu.MapStateMapped {
...
func TestBindGroupWithBufferResource(t *testing.T) {
	...
	bufLayoutEntry := webgpu.BindGroupLayoutEntry{
		Binding:    0,
		Visibility: webgpu.ShaderStageVertex | webgpu.ShaderStageFragment,
	...
	layout, err := device.CreateBindGroupLayout(&webgpu.BindGroupLayoutDescriptor{
		Label:   "bg-layout",
		Entries: []webgpu.BindGroupLayoutEntry{bufLayoutEntry},
	})
	if err != nil {
		t.Fatalf("CreateBindGroupLayout: %v", err)
	}
