...
import (
	...
	shader "github.com/energye/gpui/gpu/shader"
...
func (d *Device) CreateShaderModule(desc *ShaderModuleDescriptor) (*ShaderModule, error) {
	...
	if desc.WGSL != "" {
		ast, parseErr := shader.Parse(desc.WGSL)
		if parseErr == nil {
			irModule, lowerErr := shader.Lower(ast)
			if lowerErr == nil {
...
func (d *Device) CreateBindGroupLayout(desc *BindGroupLayoutDescriptor) (*BindGroupLayout, error) {
	...
	entriesCopy := make([]types.BindGroupLayoutEntry, len(desc.Entries))
