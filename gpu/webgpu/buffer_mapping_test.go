...
func createMapReadBuf(t *testing.T, device *webgpu.Device, size uint64) *webgpu.Buffer {
	t.Helper()
...
func TestBufferMapRoundTrip(t *testing.T) {
	...
	defer cancel()
	if err := buf.Map(ctx, webgpu.MapModeRead, 0, size); err != nil {
		t.Fatalf("Map: %v", err)
	}
	...
	if s := buf.MapState(); s != webgpu.MapStateUnmapped {
...
func TestBufferMapAlignment(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			defer cancel()
			err := buf.Map(ctx, webgpu.MapModeRead, c.offset, c.size)
			switch {
			case c.expectFailure && err == nil:
				_ = buf.Unmap()
				t.Fatalf("expected alignment error, got nil")
			case c.expectFailure && !errors.Is(err, webgpu.ErrMapAlignment):
				_ = buf.Unmap()
...
func TestBufferMapDoubleMapFails(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	if err := buf.Map(ctx, webgpu.MapModeRead, 0, 32); err != nil {
		t.Fatalf("first Map: %v", err)
	}
	if err := buf.Map(ctx, webgpu.MapModeRead, 0, 32); !errors.Is(err, webgpu.ErrMapAlreadyMapped) {
...
func TestBufferMapWrongUsageFails(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	if err := buf.Map(ctx, webgpu.MapModeRead, 0, 64); !errors.Is(err, webgpu.ErrMapInvalidMode) {
...
func TestBufferMappedRangeOverlap(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	if err := buf.Map(ctx, webgpu.MapModeRead, 0, 128); err != nil {
		t.Fatalf("Map: %v", err)
	}
	...
	if _, err := buf.MappedRange(32, 64); !errors.Is(err, webgpu.ErrMapRangeOverlap) {
...
func TestBufferUnmapIdempotency(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	if err := buf.Map(ctx, webgpu.MapModeRead, 0, 16); err != nil {
		t.Fatalf("Map: %v", err)
	}
	if err := buf.Unmap(); err != nil {
		t.Fatalf("first Unmap: %v", err)
	}
	if err := buf.Unmap(); !errors.Is(err, webgpu.ErrMapNotMapped) {
...
func TestBufferMapStateTransitions(t *testing.T) {
	...
	if s := buf.MapState(); s != webgpu.MapStateMapped {
...
func TestBufferMapContextCancel(t *testing.T) {
	...
	cancel()
	_ = buf.Map(ctx, webgpu.MapModeRead, 0, 32)
...
func TestBufferMapAsyncStatus(t *testing.T) {
	...
	pending, err := buf.MapAsync(webgpu.MapModeRead, 0, size)
	if err != nil {
