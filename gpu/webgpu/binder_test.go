...
func TestBindGroupLayoutIsCompatibleWithSameEntries(t *testing.T) {
	...
	layout1 := &BindGroupLayout{entries: make([]types.BindGroupLayoutEntry, len(entries))}
...
func TestBindGroupLayoutIsCompatibleWithDifferentEntries(t *testing.T) {
	tests := []struct {
		name     string
		entries1 []types.BindGroupLayoutEntry
		entries2 []types.BindGroupLayoutEntry
	}{
		{
			...
			entries2: []types.BindGroupLayoutEntry{
				{Binding: 1, Visibility: types.ShaderStageVertex},
			},
		},
		{
		...
		},
		{
			...
			entries2: []types.BindGroupLayoutEntry{
				{Binding: 0, Visibility: types.ShaderStageVertex, Buffer: &types.BufferBindingLayout{Type: types.BufferBindingTypeStorage}},
			},
		},
		{
			name: "different texture sample type",
			entries1: []types.BindGroupLayoutEntry{
				{Binding: 0, Visibility: types.ShaderStageFragment, Texture: &types.TextureBindingLayout{SampleType: types.TextureSampleTypeFloat}},
			},
			entries2: []types.BindGroupLayoutEntry{
				{Binding: 0, Visibility: types.ShaderStageFragment, Texture: &types.TextureBindingLayout{SampleType: types.TextureSampleTypeDepth}},
			},
		},
	}
...
func TestBindGroupLayoutIsCompatibleWithEmptyEntries(t *testing.T) {
	l1 := &BindGroupLayout{entries: []types.BindGroupLayoutEntry{}}
	l2 := &BindGroupLayout{entries: []types.BindGroupLayoutEntry{}}
	if !l1.isCompatibleWith(l2) {
...
func TestBinderCheckCompatibilityEntryByEntry(t *testing.T) {
	...
	pipelineLayout := &BindGroupLayout{entries: make([]types.BindGroupLayoutEntry, len(entries))}
...
func TestMakeLateSizedBufferGroups(t *testing.T) {
	...
	layouts := []*BindGroupLayout{
		{entries: []types.BindGroupLayoutEntry{
			{Binding: 0, Buffer: &types.BufferBindingLayout{MinBindingSize: 0}},                         // late: should get 64
			{Binding: 1, Buffer: &types.BufferBindingLayout{MinBindingSize: 256}},                       // NOT late: MinBindingSize != 0
			{Binding: 2, Buffer: &types.BufferBindingLayout{MinBindingSize: 0}},                         // late: should get 128
			{Binding: 3, Sampler: &types.SamplerBindingLayout{Type: types.SamplerBindingTypeFiltering}}, // not buffer
		}},
		{entries: []types.BindGroupLayoutEntry{
			{Binding: 0, Buffer: &types.BufferBindingLayout{MinBindingSize: 0}}, // late: should get 32
		}},
	}
