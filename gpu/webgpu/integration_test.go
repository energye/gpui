...
import (
	...
	_ "github.com/energye/gpui/gpu/webgpu/hal/software"
)
...
// by the caller.
func createTestDevice(t *testing.T) (*webgpu.Instance, *webgpu.Adapter, *webgpu.Device) {
	t.Helper()
...
func TestIntegrationCreateBuffer(t *testing.T) {
	...
	desc := &webgpu.BufferDescriptor{
		...
		Size:  1024,
		Usage: webgpu.BufferUsageStorage | webgpu.BufferUsageCopyDst | webgpu.BufferUsageCopySrc,
	}
...
`,
	...
	computePipeline, cpErr := device.CreateComputePipeline(&webgpu.ComputePipelineDescriptor{
...
func TestIntegrationQueueReadBuffer(t *testing.T) {
	...
	defer cancel()
	if err := buf.Map(ctx, webgpu.MapModeRead, 0, bufSize); err != nil {
		t.Fatalf("Map: %v", err)
	}
...
func TestIntegrationQueueWriteTexture(t *testing.T) {
	...
	tex, err := device.CreateTexture(&webgpu.TextureDescriptor{
		Label:         "write-texture",
		Size:          webgpu.Extent3D{Width: 2, Height: 2, DepthOrArrayLayers: 1},
		MipLevelCount: 1,
		SampleCount:   1,
		Dimension:     types.TextureDimension2D,
		Format:        webgpu.TextureFormatRGBA8Unorm,
		Usage:         webgpu.TextureUsageTextureBinding | webgpu.TextureUsageCopyDst,
		ViewFormats:   []webgpu.TextureFormat{webgpu.TextureFormatRGBA8Unorm},
	})
	if err != nil {
		t.Fatalf("CreateTexture: %v", err)
	}
	...
	copyTexture := &webgpu.ImageCopyTexture{
		Texture:  tex,
		MipLevel: 0,
