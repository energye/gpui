...
func TestNewDeviceFromHALBothNil(t *testing.T) {
	_, err := webgpu.NewDeviceFromHAL(nil, nil, 0, webgpu.DefaultLimits(), "both-nil")
	if err == nil {
...
func TestNewTextureFromHALFormats(t *testing.T) {
	...
	for _, tt := range formats {
		t.Run(tt.name, func(t *testing.T) {
			tex := webgpu.NewTextureFromHAL(nil, nil, tt.format)
			if tex == nil {
...
func TestNewSurfaceFromHALWithLabel(t *testing.T) {
	surface := webgpu.NewSurfaceFromHAL(nil, "labeled-surface")
	if surface == nil {
...
func TestCreateAndFinishMultipleEncoders(t *testing.T) {
	...
	var cmdBufs []*webgpu.CommandBuffer
...
func TestAllCreateMethodsOnReleasedDevice(t *testing.T) {
	...
	tests := []struct {
		...
		{"CreateBuffer", func() error {
			_, err := device.CreateBuffer(&webgpu.BufferDescriptor{Size: 64, Usage: webgpu.BufferUsageVertex})
			return err
		}},
		{"CreateTexture", func() error {
			_, err := device.CreateTexture(&webgpu.TextureDescriptor{
				Size: webgpu.Extent3D{Width: 4, Height: 4, DepthOrArrayLayers: 1}, MipLevelCount: 1, SampleCount: 1,
				Format: webgpu.TextureFormatRGBA8Unorm, Usage: webgpu.TextureUsageTextureBinding,
			})
			return err
		}},
		...
		{"CreateShaderModule", func() error {
			_, err := device.CreateShaderModule(&webgpu.ShaderModuleDescriptor{WGSL: "test"})
			return err
		}},
		...
		{"CreateComputePipeline", func() error {
			_, err := device.CreateComputePipeline(&webgpu.ComputePipelineDescriptor{EntryPoint: "main"})
			return err
		}},
