...
func BenchmarkPendingWrites_WriteBufferNonBatching(b *testing.B) {
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		_ = pw.writeBuffer(buf, types.BufferUsageUniform|types.BufferUsageCopyDst, 0, data)
	}
}
...
func BenchmarkBufferReadUsage(b *testing.B) {
	b.ReportAllocs()
	var sum types.BufferUsage
	for i := 0; i < b.N; i++ {
		sum |= bufferReadUsage(types.BufferUsageVertex | types.BufferUsageCopyDst | types.BufferUsageUniform)
	}
	_ = sum
}
