...
func createPipeline(device *webgpu.Device, bufs *bufferSet) (*pipelineSet, *webgpu.BindGroup, error) {
	...
	plLayout, err := device.CreatePipelineLayout(&webgpu.PipelineLayoutDescriptor{
		Label: "copy-pl", BindGroupLayouts: []*webgpu.BindGroupLayout{bgLayout},
	})
	if err != nil {
...
func dispatchAndReadBack(device *webgpu.Device, pipeline *webgpu.ComputePipeline, bindGroup *webgpu.BindGroup, bufs *bufferSet) ([]byte, error) {
	...
	defer cancel()
	if err := bufs.staging.Map(ctx, webgpu.MapModeRead, 0, bufSize); err != nil {
