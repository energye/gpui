...
func uploadInitialData(device *webgpu.Device, inputBuf, uniformBuf *webgpu.Buffer) error {
...
func createPipeline(device *webgpu.Device, shader *webgpu.ShaderModule, inputBuf, outputBuf, uniformBuf *webgpu.Buffer) (*webgpu.ComputePipeline, *webgpu.BindGroup, func(), error) {
	bgl, err := device.CreateBindGroupLayout(&webgpu.BindGroupLayoutDescriptor{
		...
		Entries: []webgpu.BindGroupLayoutEntry{
			...
			{Binding: 2, Visibility: webgpu.ShaderStageCompute, Buffer: &types.BufferBindingLayout{Type: types.BufferBindingTypeUniform, MinBindingSize: 8}},
		},
	})
	if err != nil {
...
func simulate(device *webgpu.Device, pipeline *webgpu.ComputePipeline, bg *webgpu.BindGroup, inputBuf, outputBuf, stagingBuf *webgpu.Buffer) error {
	...
	for step := 0; step < 10; step++ {
		...
		if err := stagingBuf.Map(readCtx, webgpu.MapModeRead, 0, bufSize); err != nil {
			cancelRead()
