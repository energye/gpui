...
func main() {
	...
	bgl, err := device.CreateBindGroupLayout(&webgpu.BindGroupLayoutDescriptor{
		...
		Entries: []webgpu.BindGroupLayoutEntry{
			{Binding: 0, Visibility: webgpu.ShaderStagesCompute, Buffer: &webgpu.BufferBindingLayout{Type: webgpu.BufferBindingTypeReadOnlyStorage}},
			{Binding: 1, Visibility: webgpu.ShaderStagesCompute, Buffer: &webgpu.BufferBindingLayout{Type: webgpu.BufferBindingTypeStorage}},
		},
	})
	if err != nil {
	...
	inputBuf, _ := device.CreateBuffer(&webgpu.BufferDescriptor{
		Label: "input",
	...
	outputBuf, _ := device.CreateBuffer(&webgpu.BufferDescriptor{
		Label: "output",
		Size:  uint64(len(inputData)),
		Usage: webgpu.BufferUsageStorage | webgpu.BufferUsageCopySrc,
	})

	readBuf, _ := device.CreateBuffer(&webgpu.BufferDescriptor{
		Label: "readback",
		Size:  uint64(len(inputData)),
	...
	// Read back results
	err = readBuf.MapAsync(webgpu.MapModeRead, 0, uint64(len(inputData)), func(status webgpu.BufferMapAsyncStatus) {
		if status != webgpu.BufferMapAsyncStatusSuccess {
