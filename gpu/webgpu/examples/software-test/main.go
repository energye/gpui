...
func testCompute(device *webgpu.Device) error { //nolint:gocyclo,cyclop,funlen // linear test flow
...
`
	...
	pipeLayout, err := device.CreatePipelineLayout(&webgpu.PipelineLayoutDescriptor{
		Label:            "compute-pipe",
	...
	defer cancel()
	if err := stagingBuf.Map(ctx, webgpu.MapModeRead, 0, n*4); err != nil {
