...
func TestTransitionTexturesWithBarrier(t *testing.T) {
	...
	tex, err := device.CreateTexture(&webgpu.TextureDescriptor{
		...
		Dimension:     webgpu.TextureDimension2D,
		Format:        webgpu.TextureFormatRGBA8Unorm,
		Usage:         webgpu.TextureUsageCopySrc | webgpu.TextureUsageRenderAttachment,
	})
	if err != nil {
		t.Fatalf("CreateTexture: %v", err)
	}
	...
	barriers := []webgpu.TextureBarrier{
		{
			Texture: tex,
			Range: webgpu.TextureRange{
				BaseMipLevel:    0,
				MipLevelCount:   1,
				BaseArrayLayer:  0,
				ArrayLayerCount: 1,
			...
			Usage: webgpu.TextureUsageTransition{
				OldUsage: webgpu.TextureUsageRenderAttachment,
				NewUsage: webgpu.TextureUsageCopySrc,
			},
		},
	}
