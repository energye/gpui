...
func setupMapBench(b *testing.B) (*webgpu.Instance, *webgpu.Adapter, *webgpu.Device, *webgpu.Buffer) {
	b.Helper()
...
func BenchmarkBufferMapReadPrimary(b *testing.B) {
	...
	_ = buf.Map(ctx, webgpu.MapModeRead, 0, size)
...
func BenchmarkBufferMapAsyncEscapeHatch(b *testing.B) {
	...
	// Warm up.
	pending, _ := buf.MapAsync(webgpu.MapModeRead, 0, size)
