...
func BuildTextureViewDescriptor(
	...
	if dimension != types.TextureViewDimensionUndefined {
...
func BuildSamplerDescriptor(
	label string,
	addressModeU, addressModeV, addressModeW types.AddressMode,
	magFilter, minFilter types.FilterMode,
	...
	if addressModeU != types.AddressModeUndefined {
		desc.Set("addressModeU", AddressModeToJS(addressModeU))
	}
	if addressModeV != types.AddressModeUndefined {
		desc.Set("addressModeV", AddressModeToJS(addressModeV))
	}
	if addressModeW != types.AddressModeUndefined {
		desc.Set("addressModeW", AddressModeToJS(addressModeW))
	}

	if magFilter != types.FilterModeUndefined {
		desc.Set("magFilter", FilterModeToJS(magFilter))
	}
	if minFilter != types.FilterModeUndefined {
		desc.Set("minFilter", FilterModeToJS(minFilter))
	}
	if mipmapFilter != types.FilterModeUndefined {
