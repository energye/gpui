...
import (
	...
	_ "github.com/energye/gpui/gpu/webgpu/hal/noop"
)

// --- helpers ---

// newInstance creates a fresh Instance for tests.
func newInstance(t *testing.T) *webgpu.Instance {
	t.Helper()
	inst, err := webgpu.CreateInstance(nil)
	if err != nil {
...
func newAdapter(t *testing.T) (*webgpu.Instance, *webgpu.Adapter) {
	t.Helper()
	inst := newInstance(t)
...
func newDevice(t *testing.T) (*webgpu.Instance, *webgpu.Adapter, *webgpu.Device) {
	t.Helper()
...
func requireHAL(t *testing.T, device *webgpu.Device) {
	t.Helper()
	if device.Queue() == nil {
...
func TestCreateInstanceWithDescriptor(t *testing.T) {
	tests := []struct {
		name     string
		backends webgpu.Backends
	}{
		{"all backends", webgpu.BackendsAll},
		{"primary backends", webgpu.BackendsPrimary},
		{"vulkan only", webgpu.BackendsVulkan},
	}
...
func TestRequestDeviceWithDescriptor(t *testing.T) {
	...
	device, err := adapter.RequestDevice(&webgpu.DeviceDescriptor{
		Label:          "test-device",
		RequiredLimits: webgpu.DefaultLimits(),
	})
	if err != nil {
...
func TestDeviceCreateBuffer(t *testing.T) {
	...
	buf, err := device.CreateBuffer(&webgpu.BufferDescriptor{
		Label: "test-buffer",
		Size:  256,
		Usage: webgpu.BufferUsageVertex | webgpu.BufferUsageCopyDst,
	})
	if err != nil {
...
func TestDeviceCreateTexture(t *testing.T) {
	...
	tex, err := device.CreateTexture(&webgpu.TextureDescriptor{
		...
		Usage:         webgpu.TextureUsageTextureBinding | webgpu.TextureUsageCopyDst,
	})
	if err != nil {
	...
	if tex.Format() != webgpu.TextureFormatRGBA8Unorm {
...
func TestDeviceCreateSampler(t *testing.T) {
	...
	sampler, err := device.CreateSampler(&webgpu.SamplerDescriptor{
		Label: "test-sampler",
	})
	if err != nil {
...
func TestDeviceCreateBindGroupLayout(t *testing.T) {
	...
	layout, err := device.CreateBindGroupLayout(&webgpu.BindGroupLayoutDescriptor{
		Label:   "test-bgl",
...
func TestCommandEncoderBeginComputePass(t *testing.T) {
	...
	pass, err := encoder.BeginComputePass(&webgpu.ComputePassDescriptor{
...
func TestDeviceCreateRenderPipeline(t *testing.T) {
	...
	vertShader, err := device.CreateShaderModule(&webgpu.ShaderModuleDescriptor{
		Label: "vert-shader",
		WGSL:  "@vertex fn vs_main() -> @builtin(position) vec4f { return vec4f(0.0); }",
	})
	if err != nil {
...
func TestDeviceErrorScopeAllFilters(t *testing.T) {
	...
	filters := []struct {
		name   string
		filter webgpu.ErrorFilter
	}{
		{"Validation", webgpu.ErrorFilterValidation},
		{"OutOfMemory", webgpu.ErrorFilterOutOfMemory},
		{"Internal", webgpu.ErrorFilterInternal},
	}
...
func TestCreateBindGroupNilLayout(t *testing.T) {
	...
	_, err := device.CreateBindGroup(&webgpu.BindGroupDescriptor{
		Label:  "nil-layout",
		Layout: nil,
	})
	if err == nil {
...
func TestCreatePipelineLayoutNilBindGroupLayout(t *testing.T) {
	...
	_, err := device.CreatePipelineLayout(&webgpu.PipelineLayoutDescriptor{
		Label:            "nil-bgl-element",
		BindGroupLayouts: []*webgpu.BindGroupLayout{nil},
	})
	if err == nil {
...
func TestErrorSentinels(t *testing.T) {
	...
	sentinels := []struct {
		...
		{"ErrReleased", webgpu.ErrReleased},
		{"ErrNoAdapters", webgpu.ErrNoAdapters},
		{"ErrNoBackends", webgpu.ErrNoBackends},
	}
...
func TestErrorSentinelMessages(t *testing.T) {
	tests := []struct {
		...
		{"ErrReleased", webgpu.ErrReleased, "released"},
		{"ErrNoAdapters", webgpu.ErrNoAdapters, "adapter"},
		{"ErrNoBackends", webgpu.ErrNoBackends, "backend"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			msg := tt.err.Error()
...
func TestHALErrorSentinels(t *testing.T) {
	...
	halErrors := []struct {
		...
		{"ErrDeviceLost", webgpu.ErrDeviceLost},
		{"ErrOutOfMemory", webgpu.ErrOutOfMemory},
		{"ErrSurfaceLost", webgpu.ErrSurfaceLost},
		{"ErrSurfaceOutdated", webgpu.ErrSurfaceOutdated},
		{"ErrTimeout", webgpu.ErrTimeout},
	}
...
func TestBackendConstants(t *testing.T) {
	...
	backends := []struct {
		...
		{"Vulkan", webgpu.BackendVulkan},
		{"Metal", webgpu.BackendMetal},
		{"DX12", webgpu.BackendDX12},
		{"GL", webgpu.BackendGL},
	}

	seen := make(map[webgpu.Backend]string)
...
func TestBufferUsageConstants(t *testing.T) {
	...
	usages := []struct {
		name string
		u    webgpu.BufferUsage
	}{
		{"MapRead", webgpu.BufferUsageMapRead},
		{"MapWrite", webgpu.BufferUsageMapWrite},
		{"CopySrc", webgpu.BufferUsageCopySrc},
		{"CopyDst", webgpu.BufferUsageCopyDst},
		{"Index", webgpu.BufferUsageIndex},
		{"Vertex", webgpu.BufferUsageVertex},
		{"Uniform", webgpu.BufferUsageUniform},
		{"Storage", webgpu.BufferUsageStorage},
		{"Indirect", webgpu.BufferUsageIndirect},
		{"QueryResolve", webgpu.BufferUsageQueryResolve},
	}
	...
	combined := webgpu.BufferUsageVertex | webgpu.BufferUsageCopyDst
	if combined == webgpu.BufferUsageVertex || combined == webgpu.BufferUsageCopyDst {
...
func TestTextureUsageConstants(t *testing.T) {
	usages := []struct {
		name string
		u    webgpu.TextureUsage
	}{
		{"CopySrc", webgpu.TextureUsageCopySrc},
		{"CopyDst", webgpu.TextureUsageCopyDst},
		{"TextureBinding", webgpu.TextureUsageTextureBinding},
		{"StorageBinding", webgpu.TextureUsageStorageBinding},
		{"RenderAttachment", webgpu.TextureUsageRenderAttachment},
	}
...
func TestShaderStageConstants(t *testing.T) {
	stages := []struct {
		name string
		s    webgpu.ShaderStages
	}{
		{"Vertex", webgpu.ShaderStageVertex},
		{"Fragment", webgpu.ShaderStageFragment},
		{"Compute", webgpu.ShaderStageCompute},
	}
...
func TestPowerPreferenceConstants(t *testing.T) {
	...
	if webgpu.PowerPreferenceNone != 0 {
		t.Error("PowerPreferenceNone should be 0")
	}
	// Others should be distinct.
	if webgpu.PowerPreferenceLowPower == webgpu.PowerPreferenceHighPerformance {
...
func TestErrorFilterConstants(t *testing.T) {
	...
	filters := []webgpu.ErrorFilter{
		webgpu.ErrorFilterValidation,
		webgpu.ErrorFilterOutOfMemory,
		webgpu.ErrorFilterInternal,
	}

	seen := make(map[webgpu.ErrorFilter]bool)
	for _, f := range filters {
		if seen[f] {
...
func TestPresentModeConstants(t *testing.T) {
	modes := []struct {
		name string
		m    webgpu.PresentMode
	}{
		{"Immediate", webgpu.PresentModeImmediate},
		{"Mailbox", webgpu.PresentModeMailbox},
		{"Fifo", webgpu.PresentModeFifo},
		{"FifoRelaxed", webgpu.PresentModeFifoRelaxed},
	}

	seen := make(map[webgpu.PresentMode]string)
	for _, m := range modes {
...
func TestTextureFormatConstants(t *testing.T) {
	formats := []struct {
		name string
		f    webgpu.TextureFormat
	}{
		{"RGBA8Unorm", webgpu.TextureFormatRGBA8Unorm},
		{"RGBA8UnormSrgb", webgpu.TextureFormatRGBA8UnormSrgb},
		{"BGRA8Unorm", webgpu.TextureFormatBGRA8Unorm},
		{"BGRA8UnormSrgb", webgpu.TextureFormatBGRA8UnormSrgb},
		{"Depth24Plus", webgpu.TextureFormatDepth24Plus},
		{"Depth32Float", webgpu.TextureFormatDepth32Float},
	}
...
func newEncoderWithRenderPass(t *testing.T) (*webgpu.Device, *webgpu.CommandEncoder, *webgpu.RenderPassEncoder) {
	t.Helper()
	_, _, device := newDevice(t)
	requireHAL(t, device)
...
func newEncoderWithComputePass(t *testing.T) (*webgpu.Device, *webgpu.CommandEncoder, *webgpu.ComputePassEncoder) {
	t.Helper()
	_, _, device := newDevice(t)
	requireHAL(t, device)
...
func TestRenderPassDrawIndirectWithoutPipelineDeferredError(t *testing.T) {
	...
	buf, bufErr := device.CreateBuffer(&webgpu.BufferDescriptor{
		Label: "indirect-buf",
		Size:  16,
		Usage: webgpu.BufferUsageIndirect,
	})
	if bufErr != nil {
...
func TestMaxBindGroupsConstant(t *testing.T) {
	if webgpu.MaxBindGroups != 8 {
		t.Errorf("MaxBindGroups = %d, want 8", webgpu.MaxBindGroups)
	}
}
...
func TestCopyBufferToBufferNilSrcDeferredError(t *testing.T) {
	...
	dstBuf, err := device.CreateBuffer(&webgpu.BufferDescriptor{
		Label: "dst-buf",
		Size:  64,
		Usage: webgpu.BufferUsageCopyDst,
	})
	if err != nil {
		t.Fatalf("CreateBuffer: %v", err)
	}
...
func TestCopyBufferToBufferNilDstDeferredError(t *testing.T) {
	...
	buf, err := device.CreateBuffer(&webgpu.BufferDescriptor{
		Label: "src-buf",
		Size:  64,
		Usage: webgpu.BufferUsageCopySrc,
	})
	if err != nil {
		t.Fatalf("CreateBuffer: %v", err)
	}
...
func TestRenderPassDrawIndexedWithIndexBuffer(t *testing.T) {
	...
	idxBuf, bufErr := device.CreateBuffer(&webgpu.BufferDescriptor{
		Label: "idx-buf",
		Size:  64,
		Usage: webgpu.BufferUsageIndex,
	})
	if bufErr != nil {
		t.Fatalf("CreateBuffer: %v", bufErr)
	}
...
func TestDrawMissingPipelineSentinel(t *testing.T) {
	...
	if !errors.Is(err, webgpu.ErrDrawMissingPipeline) {
...
func TestDrawMissingBindGroupSentinel(t *testing.T) {
	...
	pipeline := &webgpu.RenderPipeline{}
	pipeline.SetTestRequiredVertexBuffers(0)
	pipeline.SetTestBindGroupLayouts([]*webgpu.BindGroupLayout{layout})
	pass.SetPipeline(pipeline)
	...
	if !errors.Is(err, webgpu.ErrDrawMissingBindGroup) {
...
func TestDrawIncompatibleBindGroupSentinel(t *testing.T) {
	...
	expectedLayout := &webgpu.BindGroupLayout{}
	expectedLayout.SetTestEntries([]types.BindGroupLayoutEntry{
		{Binding: 0, Visibility: types.ShaderStageVertex},
	})
	wrongLayout := &webgpu.BindGroupLayout{}
	wrongLayout.SetTestEntries([]types.BindGroupLayoutEntry{
		{Binding: 0, Visibility: types.ShaderStageFragment},
	})

	pipeline := &webgpu.RenderPipeline{}
	pipeline.SetTestRequiredVertexBuffers(0)
	pipeline.SetTestBindGroupLayouts([]*webgpu.BindGroupLayout{expectedLayout})
	pass.SetPipeline(pipeline)
	...
	if !errors.Is(err, webgpu.ErrDrawIncompatibleBindGroup) {
...
func TestDrawMissingVertexBufferSentinel(t *testing.T) {
	...
	if !errors.Is(err, webgpu.ErrDrawMissingVertexBuffer) {
...
func TestDrawMissingIndexBufferSentinel(t *testing.T) {
	...
	if !errors.Is(err, webgpu.ErrDrawMissingIndexBuffer) {
...
func TestDispatchMissingPipelineSentinel(t *testing.T) {
	...
	if !errors.Is(err, webgpu.ErrDispatchMissingPipeline) {
...
func TestDispatchMissingBindGroupSentinel(t *testing.T) {
	...
	if !errors.Is(err, webgpu.ErrDispatchMissingBindGroup) {
...
func TestDispatchIncompatibleBindGroupSentinel(t *testing.T) {
	...
	expectedLayout := &webgpu.BindGroupLayout{}
	expectedLayout.SetTestEntries([]types.BindGroupLayoutEntry{
		{Binding: 0, Visibility: types.ShaderStageCompute},
	})
	wrongLayout := &webgpu.BindGroupLayout{}
	wrongLayout.SetTestEntries([]types.BindGroupLayoutEntry{
		{Binding: 0, Visibility: types.ShaderStageFragment},
	...
	if !errors.Is(err, webgpu.ErrDispatchIncompatibleBindGroup) {
...
func TestDispatchWorkgroupCountSentinel(t *testing.T) {
	...
	if !errors.Is(err, webgpu.ErrDispatchWorkgroupCountExceeded) {
...
func TestDrawSentinelErrorsAreDistinct(t *testing.T) {
	sentinels := []struct {
		...
		{"ErrDrawMissingPipeline", webgpu.ErrDrawMissingPipeline},
		{"ErrDrawMissingBindGroup", webgpu.ErrDrawMissingBindGroup},
		{"ErrDrawIncompatibleBindGroup", webgpu.ErrDrawIncompatibleBindGroup},
		{"ErrDrawMissingVertexBuffer", webgpu.ErrDrawMissingVertexBuffer},
		{"ErrDrawMissingIndexBuffer", webgpu.ErrDrawMissingIndexBuffer},
		{"ErrDrawMissingBlendConstant", webgpu.ErrDrawMissingBlendConstant},
		{"ErrDrawLateBufferTooSmall", webgpu.ErrDrawLateBufferTooSmall},
		{"ErrDispatchMissingPipeline", webgpu.ErrDispatchMissingPipeline},
		{"ErrDispatchMissingBindGroup", webgpu.ErrDispatchMissingBindGroup},
		{"ErrDispatchIncompatibleBindGroup", webgpu.ErrDispatchIncompatibleBindGroup},
		{"ErrDispatchLateBufferTooSmall", webgpu.ErrDispatchLateBufferTooSmall},
		{"ErrDispatchWorkgroupCountExceeded", webgpu.ErrDispatchWorkgroupCountExceeded},
		{"ErrDrawIndexFormatMismatch", webgpu.ErrDrawIndexFormatMismatch},
		{"ErrDrawIndirectBufferUsage", webgpu.ErrDrawIndirectBufferUsage},
		{"ErrDrawIndirectOffsetAlignment", webgpu.ErrDrawIndirectOffsetAlignment},
		{"ErrDispatchIndirectBufferUsage", webgpu.ErrDispatchIndirectBufferUsage},
		{"ErrDispatchIndirectOffsetAlignment", webgpu.ErrDispatchIndirectOffsetAlignment},
	}
...
func TestDrawIndexedFormatMismatchSentinel(t *testing.T) {
	...
	pass.SetIndexBuffer(idxBuf, types.IndexFormatUint16, 0)
	...
	if !errors.Is(err, webgpu.ErrDrawIndexFormatMismatch) {
...
func TestDrawIndexedFormatMatchesStripFormat(t *testing.T) {
	...
	pass.SetIndexBuffer(idxBuf, types.IndexFormatUint16, 0) // matches pipeline
...
func TestDrawIndexedIndirectFormatMismatchSentinel(t *testing.T) {
	...
	pass.SetIndexBuffer(idxBuf, types.IndexFormatUint32, 0) // mismatch: buffer=Uint32, pipeline=Uint16

	indirectBuf, bufErr := device.CreateBuffer(&webgpu.BufferDescriptor{
		Label: "indirect-buf",
		Size:  20,
		Usage: webgpu.BufferUsageIndirect,
	})
...
func TestDrawIndirectBufferMissingUsageSentinel(t *testing.T) {
	...
	buf, bufErr := device.CreateBuffer(&webgpu.BufferDescriptor{
		...
		Size:  16,
		Usage: webgpu.BufferUsageStorage,
	})
	if bufErr != nil {
		t.Fatalf("CreateBuffer: %v", bufErr)
	}
	...
	if !errors.Is(err, webgpu.ErrDrawIndirectBufferUsage) {
...
func TestDrawIndirectOffsetUnalignedSentinel(t *testing.T) {
	...
	if !errors.Is(err, webgpu.ErrDrawIndirectOffsetAlignment) {
...
func TestDrawIndexedIndirectBufferMissingUsageSentinel(t *testing.T) {
	...
	// Buffer WITHOUT Indirect usage.
	indBuf, bufErr := device.CreateBuffer(&webgpu.BufferDescriptor{
		Label: "no-indirect",
		Size:  20,
		Usage: webgpu.BufferUsageStorage,
	})
...
func TestDispatchIndirectBufferMissingUsageSentinel(t *testing.T) {
	...
	if !errors.Is(err, webgpu.ErrDispatchIndirectBufferUsage) {
...
func TestDispatchIndirectOffsetUnalignedSentinel(t *testing.T) {
	...
	if !errors.Is(err, webgpu.ErrDispatchIndirectOffsetAlignment) {
