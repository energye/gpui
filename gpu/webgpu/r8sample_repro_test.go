...
func TestR8SampleCoordRepro(t *testing.T) {
	runSampleCoordTest(t, types.TextureFormatR8Unorm, 1)
}

// TestRGBA8SampleCoordControl is the control: RGBA8 sampling must also be 1:1.
func TestRGBA8SampleCoordControl(t *testing.T) {
	runSampleCoordTest(t, types.TextureFormatRGBA8Unorm, 4)
}

func runSampleCoordTest(t *testing.T, format webgpu.TextureFormat, bytesPerPixel int) {
	instance, err := webgpu.CreateInstance(&webgpu.InstanceDescriptor{Backends: webgpu.BackendsPrimary})
	if err != nil {
		t.Skipf("CreateInstance: %v", err)
	}
	defer instance.Release()
	adapter, err := instance.RequestAdapter(&webgpu.RequestAdapterOptions{PowerPreference: webgpu.PowerPreferenceHighPerformance})
	if err != nil {
		t.Skipf("RequestAdapter: %v", err)
	}
	defer adapter.Release()
	device, err := adapter.RequestDevice(&webgpu.DeviceDescriptor{Label: "r8repro"})
	if err != nil {
	...
	tex, err := device.CreateTexture(&webgpu.TextureDescriptor{
		...
		Size:          webgpu.Extent3D{Width: texW, Height: texH, DepthOrArrayLayers: 1},
		MipLevelCount: 1,
		SampleCount:   1,
	...
	if err := q.WriteTexture(
		...
		data,
		&webgpu.ImageDataLayout{Offset: 0, BytesPerRow: uint32(texW * bytesPerPixel), RowsPerImage: texH},
		&webgpu.Extent3D{Width: texW, Height: texH, DepthOrArrayLayers: 1},
	); err != nil {
	...
	shader, err := device.CreateShaderModule(&webgpu.ShaderModuleDescriptor{Label: "sample", WGSL: sampleShader})
	if err != nil {
	...
	bgl, err := device.CreateBindGroupLayout(&webgpu.BindGroupLayoutDescriptor{
		Entries: []types.BindGroupLayoutEntry{
			{Binding: 0, Visibility: types.ShaderStageFragment, Texture: &types.TextureBindingLayout{SampleType: types.TextureSampleTypeFloat, ViewDimension: types.TextureViewDimension2D}},
	...
	defer bgl.Release()
	pl, err := device.CreatePipelineLayout(&webgpu.PipelineLayoutDescriptor{BindGroupLayouts: []*webgpu.BindGroupLayout{bgl}})
	if err != nil {
	...
	pipeline, err := device.CreateRenderPipeline(&webgpu.RenderPipelineDescriptor{
		...
		Fragment: &webgpu.FragmentState{
			Module:     shader,
			EntryPoint: "fs_main",
			Targets:    []types.ColorTargetState{{Format: types.TextureFormatRGBA8Unorm, WriteMask: types.ColorWriteMaskAll}},
		},
	...
	outTex, err := device.CreateTexture(&webgpu.TextureDescriptor{
		Label:         "out",
		Size:          webgpu.Extent3D{Width: outW, Height: outH, DepthOrArrayLayers: 1},
	...
	enc.TransitionTextures([]webgpu.TextureBarrier{{Texture: outTex, Usage: webgpu.TextureUsageTransition{OldUsage: types.TextureUsageRenderAttachment, NewUsage: types.TextureUsageCopySrc}}})

	staging, err := device.CreateBuffer(&webgpu.BufferDescriptor{Size: outW * 4, Usage: types.BufferUsageMapRead | types.BufferUsageCopyDst})
	if err != nil {
		t.Fatalf("CreateBuffer(staging): %v", err)
	}
	defer staging.Release()
	enc.CopyTextureToBuffer(outTex, staging, []webgpu.BufferTextureCopy{{
		BufferLayout: webgpu.ImageDataLayout{Offset: 0, BytesPerRow: outW * 4, RowsPerImage: outH},
		TextureBase:  webgpu.ImageCopyTexture{Texture: outTex, MipLevel: 0},
		Size:         webgpu.Extent3D{Width: outW, Height: outH, DepthOrArrayLayers: 1},
	}})
	cb, err := enc.Finish()
	if err != nil {
	...
	if err := staging.Map(context.Background(), webgpu.MapModeRead, 0, outW*4); err != nil {
		t.Fatalf("Map: %v", err)
	}
...
func TestDrawIndexedRenders(t *testing.T) {
	...
	vbuf, _ := device.CreateBuffer(&webgpu.BufferDescriptor{Size: uint64(len(vbytes)), Usage: types.BufferUsageVertex | types.BufferUsageCopyDst})
	defer vbuf.Release()
	q.WriteBuffer(vbuf, 0, vbytes)
	ibuf, _ := device.CreateBuffer(&webgpu.BufferDescriptor{Size: uint64(len(ibytes)), Usage: types.BufferUsageIndex | types.BufferUsageCopyDst})
	defer ibuf.Release()
	q.WriteBuffer(ibuf, 0, ibytes)

	shader, err := device.CreateShaderModule(&webgpu.ShaderModuleDescriptor{WGSL: indexedShader})
	if err != nil {
		t.Fatalf("CreateShaderModule: %v", err)
	}
	defer shader.Release()
	pl, _ := device.CreatePipelineLayout(&webgpu.PipelineLayoutDescriptor{})
	defer pl.Release()
	...
	pipeline, err := device.CreateRenderPipeline(&webgpu.RenderPipelineDescriptor{
		...
		Vertex: webgpu.VertexState{
			...
			Buffers: []types.VertexBufferLayout{{
				ArrayStride: 8, StepMode: types.VertexStepModeVertex,
				Attributes: []types.VertexAttribute{{Format: types.VertexFormatFloat32x2, Offset: 0, ShaderLocation: 0}},
			}},
		},
		Fragment: &webgpu.FragmentState{
	...
	outTex, _ := device.CreateTexture(&webgpu.TextureDescriptor{
		Size: webgpu.Extent3D{Width: W, Height: H, DepthOrArrayLayers: 1}, MipLevelCount: 1, SampleCount: 1,
		Dimension: types.TextureDimension2D, Format: types.TextureFormatRGBA8Unorm,
		Usage: types.TextureUsageRenderAttachment | types.TextureUsageCopySrc,
	})
	defer outTex.Release()
	...
	enc, _ := device.CreateCommandEncoder(&webgpu.CommandEncoderDescriptor{})
	rp, _ := enc.BeginRenderPass(&webgpu.RenderPassDescriptor{
		ColorAttachments: []webgpu.RenderPassColorAttachment{{
			View: outView, LoadOp: types.LoadOpClear, StoreOp: types.StoreOpStore,
			ClearValue: types.Color{R: 0, G: 0, B: 0, A: 1},
		}},
	...
	rp.SetIndexBuffer(ibuf, types.IndexFormatUint16, 0)
	rp.DrawIndexed(6, 1, 0, 0, 0)
	_ = rp.End()
	enc.TransitionTextures([]webgpu.TextureBarrier{{Texture: outTex, Usage: webgpu.TextureUsageTransition{OldUsage: types.TextureUsageRenderAttachment, NewUsage: types.TextureUsageCopySrc}}})
	staging, _ := device.CreateBuffer(&webgpu.BufferDescriptor{Size: W * H * 4, Usage: types.BufferUsageMapRead | types.BufferUsageCopyDst})
	defer staging.Release()
	enc.CopyTextureToBuffer(outTex, staging, []webgpu.BufferTextureCopy{{
	...
	cb, _ := enc.Finish()
	q.Submit(cb)
	if err := staging.Map(context.Background(), webgpu.MapModeRead, 0, W*H*4); err != nil {
		t.Fatalf("Map: %v", err)
	}
