...
func TestMapPendingStatusIdempotent(t *testing.T) {
	...
	pending, err := buf.MapAsync(webgpu.MapModeWrite, 0, 32)
	if err != nil {
		t.Fatalf("MapAsync: %v", err)
	}
	defer pending.Release()
...
func TestBufferMapRangeOverflow(t *testing.T) {
	...
	_, err = buf.MapAsync(webgpu.MapModeRead, 32, 64)
	if !errors.Is(err, webgpu.ErrMapRangeOverflow) {
...
func TestBufferMapAlignmentErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := buf.MapAsync(webgpu.MapModeRead, tt.offset, tt.size)
...
func TestCoreErrToTypedMapping(t *testing.T) {
	...
	allErrors := []struct {
		...
		{"AlreadyPending", webgpu.ErrMapAlreadyPending},
		{"AlreadyMapped", webgpu.ErrMapAlreadyMapped},
		{"NotMapped", webgpu.ErrMapNotMapped},
		{"RangeOverlap", webgpu.ErrMapRangeOverlap},
		{"RangeDetached", webgpu.ErrMapRangeDetached},
		{"Alignment", webgpu.ErrMapAlignment},
		{"Canceled", webgpu.ErrMapCanceled},
		{"Destroyed", webgpu.ErrBufferDestroyed},
		{"DeviceLost", webgpu.ErrMapDeviceLost},
		{"InvalidMode", webgpu.ErrMapInvalidMode},
		{"RangeOverflow", webgpu.ErrMapRangeOverflow},
	}
...
func TestBufferMapWriteMode(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	err = buf.Map(ctx, webgpu.MapModeWrite, 0, 64)
	if err != nil {
