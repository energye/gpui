...
func TestDevicePollOnNilDevice(t *testing.T) {
	...
	result := device.Poll(webgpu.PollPoll)
	if result {
...
func TestDevicePollWait(t *testing.T) {
	...
	result := device.Poll(webgpu.PollWait)
...
func TestDeviceCreateTextureView(t *testing.T) {
	...
	tex, err := device.CreateTexture(&webgpu.TextureDescriptor{
		Label:         "view-texture",
		Size:          webgpu.Extent3D{Width: 16, Height: 16, DepthOrArrayLayers: 1},
		MipLevelCount: 1,
		SampleCount:   1,
...
func TestDeviceCreateTextureViewWithDescriptor(t *testing.T) {
	...
	tex, err := device.CreateTexture(&webgpu.TextureDescriptor{
		...
		Size:          webgpu.Extent3D{Width: 32, Height: 32, DepthOrArrayLayers: 1},
		MipLevelCount: 1,
		SampleCount:   1,
		Dimension:     webgpu.TextureDimension2D,
		Format:        webgpu.TextureFormatRGBA8Unorm,
...
func TestNewDeviceFromHALNilDevice(t *testing.T) {
	_, err := webgpu.NewDeviceFromHAL(nil, nil, 0, webgpu.DefaultLimits(), "test")
	if err == nil {
