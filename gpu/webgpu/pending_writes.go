...
type pendingWrites struct {
	...
	dstBuffers map[hal.Buffer]types.BufferUsage
...
func newPendingWrites(halDevice hal.Device, halQueue hal.Queue, pool *encoderPool) *pendingWrites {
	pw := &pendingWrites{
		halDevice:    halDevice,
		halQueue:     halQueue,
		dstBuffers:   make(map[hal.Buffer]types.BufferUsage),
...
func (pw *pendingWrites) writeBuffer(buffer hal.Buffer, usage types.BufferUsage, offset uint64, data []byte) error {
	pw.mu.Lock()
	defer pw.mu.Unlock()
...
func (pw *pendingWrites) writeTexture(
	...
	if currentUsage != types.TextureUsageCopyDst {
...
func bufferReadUsage(usage types.BufferUsage) types.BufferUsage {
	...
	readMask := types.BufferUsageVertex | types.BufferUsageIndex |
		types.BufferUsageUniform | types.BufferUsageIndirect
			...
			if readUsage != 0 {
				barriers = append(barriers, hal.BufferBarrier{
					...
					Usage: hal.BufferUsageTransition{
						OldUsage: types.BufferUsageCopyDst,
						NewUsage: readUsage,
					},
				})
			}
