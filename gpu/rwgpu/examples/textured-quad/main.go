...
type App struct {
	...
	indexBuffer    *rwgpu.Buffer
	texture        *rwgpu.Texture
	textureView    *rwgpu.TextureView
	sampler        *rwgpu.Sampler
	bindGroupLyt   *rwgpu.BindGroupLayout
...
func (app *App) createTexture() error {
	...
	// Create texture
	textureDesc := rwgpu.TextureDescriptor{
		Label:     "",
		Usage:     rwgpu.TextureUsageTextureBinding | rwgpu.TextureUsageCopyDst,
		Dimension: rwgpu.TextureDimension2D,
		Size: rwgpu.Extent3D{
			Width:              size,
			Height:             size,
			DepthOrArrayLayers: 1,
		},
		Format:        rwgpu.TextureFormatRGBA8Unorm,
		MipLevelCount: 1,
		SampleCount:   1,
	}
	...
	destInfo := rwgpu.ImageCopyTexture{
		Texture:  app.texture,
		MipLevel: 0,
		Origin:   rwgpu.Origin3D{X: 0, Y: 0, Z: 0},
		Aspect:   rwgpu.TextureAspectAll,
	}

	layout := rwgpu.ImageDataLayout{
		Offset:       0,
		BytesPerRow:  alignedBytesPerRow,
		RowsPerImage: size,
	}

	extent := rwgpu.Extent3D{
		Width:              size,
		Height:             size,
		DepthOrArrayLayers: 1,
	}
...
func (app *App) createBindGroup() error {
	...
	layoutEntries := []rwgpu.BindGroupLayoutEntry{
		{
			Binding:    0,
			Visibility: rwgpu.ShaderStageFragment,
			Sampler: &rwgpu.SamplerBindingLayout{
				Type: rwgpu.SamplerBindingTypeFiltering,
			},
		},
		{
			Binding:    1,
			Visibility: rwgpu.ShaderStageFragment,
			Texture: &rwgpu.TextureBindingLayout{
				SampleType:    rwgpu.TextureSampleTypeFloat,
				ViewDimension: rwgpu.TextureViewDimension2D,
			},
		},
	}
	...
	entries := []rwgpu.BindGroupEntry{
		rwgpu.SamplerBindingEntry(0, app.sampler),
		rwgpu.TextureBindingEntry(1, app.textureView),
	}
...
func (app *App) createBuffers() error {
	...
	app.indexBuffer, _ = app.device.CreateBuffer(&rwgpu.BufferDescriptor{
		Label:            "",
		Usage:            rwgpu.BufferUsageIndex | rwgpu.BufferUsageCopyDst,
...
func (app *App) createPipeline() error {
	...
	pipelineLayout, _ := app.device.CreatePipelineLayoutSimple([]*rwgpu.BindGroupLayout{app.bindGroupLyt})
	...
	attributes := []rwgpu.VertexAttribute{
		{
			Format:         rwgpu.VertexFormatFloat32x2, // position: vec2f
			Offset:         0,
			ShaderLocation: 0,
		},
		{
			Format:         rwgpu.VertexFormatFloat32x2, // uv: vec2f
...
func (app *App) renderQuad(encoder *rwgpu.CommandEncoder, view *rwgpu.TextureView) error {
	...
	pass.SetIndexBuffer(app.indexBuffer, rwgpu.IndexFormatUint16, 0, uint64(6*2)) // 6 indices * 2 bytes
