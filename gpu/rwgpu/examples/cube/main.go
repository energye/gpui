...
type App struct {
	hwnd             windows.HWND
	hinstance        windows.Handle
	instance         *rwgpu.Instance
	adapter          *rwgpu.Adapter
	device           *rwgpu.Device
	queue            *rwgpu.Queue
	surface          *rwgpu.Surface
	pipeline         *rwgpu.RenderPipeline
	vertexBuffer     *rwgpu.Buffer
	uniformBuffer    *rwgpu.Buffer
	bindGroupLayout  *rwgpu.BindGroupLayout
	bindGroup        *rwgpu.BindGroup
	depthTexture     *rwgpu.Texture
	depthTextureView *rwgpu.TextureView
	width            uint32
	height           uint32
	running          bool
	needsRecreate    bool
	surfaceTex       *rwgpu.SurfaceTexture
	surfaceTexView   *rwgpu.TextureView
	startTime        time.Time
}
...
func (app *App) configureSurface() error {
	_ = app.surface.Configure(app.device, &rwgpu.SurfaceConfiguration{
		Format:      rwgpu.TextureFormatBGRA8Unorm,
		Usage:       rwgpu.TextureUsageRenderAttachment,
		Width:       app.width,
		Height:      app.height,
		AlphaMode:   rwgpu.CompositeAlphaModeOpaque,
		PresentMode: rwgpu.PresentModeFifo,
	})
...
func (app *App) createVertexBuffer() error {
	...
	app.vertexBuffer, _ = app.device.CreateBuffer(&rwgpu.BufferDescriptor{
		Label:            "",
		Usage:            rwgpu.BufferUsageVertex | rwgpu.BufferUsageCopyDst,
...
func (app *App) createUniformBuffer() error {
	...
	app.uniformBuffer, _ = app.device.CreateBuffer(&rwgpu.BufferDescriptor{
		Label:            "",
		Usage:            rwgpu.BufferUsageUniform | rwgpu.BufferUsageCopyDst,
...
func (app *App) createBindGroupLayout() error {
	entries := []rwgpu.BindGroupLayoutEntry{
		{
			Binding:    0,
			Visibility: rwgpu.ShaderStageVertex,
			Buffer: &rwgpu.BufferBindingLayout{
				Type:           rwgpu.BufferBindingTypeUniform,
...
func (app *App) createBindGroup() error {
	entries := []rwgpu.BindGroupEntry{
		rwgpu.BufferBindingEntry(0, app.uniformBuffer, 0, 64),
	}
...
func (app *App) createPipelineLayout() (*rwgpu.PipelineLayout, error) {
	pipelineLayout, _ := app.device.CreatePipelineLayoutSimple([]*rwgpu.BindGroupLayout{app.bindGroupLayout})
...
func (app *App) createPipeline() error {
	...
	attributes := []rwgpu.VertexAttribute{
		{
			Format:         rwgpu.VertexFormatFloat32x3, // position: vec3f
			Offset:         0,
			ShaderLocation: 0,
		},
		{
			Format:         rwgpu.VertexFormatFloat32x3, // color: vec3f
	...
	pipeline, _ := app.device.CreateRenderPipeline(&rwgpu.RenderPipelineDescriptor{
		Label:  "",
		Layout: pipelineLayout,
		Vertex: rwgpu.VertexState{
			Module:     shader,
			EntryPoint: "vs_main",
			Buffers: []rwgpu.VertexBufferLayout{{
				ArrayStride:    24, // 6 floats * 4 bytes = 24 bytes per vertex
				StepMode:       rwgpu.VertexStepModeVertex,
				AttributeCount: 2,
				Attributes:     &attributes[0],
			}},
		},
		Primitive: rwgpu.PrimitiveState{
			Topology:  rwgpu.PrimitiveTopologyTriangleList,
			FrontFace: rwgpu.FrontFaceCCW,
			CullMode:  rwgpu.CullModeBack, // Enable back-face culling for cube
		},
		DepthStencil: &rwgpu.DepthStencilState{
			Format:            rwgpu.TextureFormatDepth24Plus,
			DepthWriteEnabled: true,
			DepthCompare:      rwgpu.CompareFunctionLess,
			StencilFront: rwgpu.StencilFaceState{
				Compare:     rwgpu.CompareFunctionAlways,
				FailOp:      rwgpu.StencilOperationKeep,
				DepthFailOp: rwgpu.StencilOperationKeep,
				PassOp:      rwgpu.StencilOperationKeep,
			},
		...
		Multisample: rwgpu.MultisampleState{
			Count: 1,
			Mask:  0xFFFFFFFF,
		},
		Fragment: &rwgpu.FragmentState{
			Module:     shader,
			EntryPoint: "fs_main",
			Targets: []rwgpu.ColorTargetState{{
				Format:    rwgpu.TextureFormatBGRA8Unorm,
				WriteMask: rwgpu.ColorWriteMaskAll,
			}},
		},
	})
...
func (app *App) updateUniformBuffer() {
	...
	modelY := rwgpu.Mat4RotateY(angleY)
	modelX := rwgpu.Mat4RotateX(angleX)
	model := modelY.Mul(modelX)

	// View matrix: camera at (0, 0, 3) looking at origin
	view := rwgpu.Mat4LookAt(
		rwgpu.Vec3{X: 0, Y: 0, Z: 3},
		rwgpu.Vec3{X: 0, Y: 0, Z: 0},
		rwgpu.Vec3{X: 0, Y: 1, Z: 0},
	)

	// Projection matrix: perspective with 45° FOV
	aspect := float32(app.width) / float32(app.height)
	projection := rwgpu.Mat4Perspective(45.0*math.Pi/180.0, aspect, 0.1, 100.0)
...
func (app *App) acquireSurfaceTexture() error {
	...
	if err != nil {
		...
		if err == rwgpu.ErrSurfaceLost || err == rwgpu.ErrSurfaceNeedsReconfigure {
			app.needsRecreate = true
			return nil
		}
...
func (app *App) renderCube(encoder *rwgpu.CommandEncoder, view *rwgpu.TextureView) error {
	pass, _ := encoder.BeginRenderPass(&rwgpu.RenderPassDescriptor{
		Label: "Cube Render Pass",
		ColorAttachments: []rwgpu.RenderPassColorAttachment{{
			View:    view,
			LoadOp:  rwgpu.LoadOpClear,
			StoreOp: rwgpu.StoreOpStore,
		...
		DepthStencilAttachment: &rwgpu.RenderPassDepthStencilAttachment{
			View:              app.depthTextureView,
			DepthLoadOp:       rwgpu.LoadOpClear,
			DepthStoreOp:      rwgpu.StoreOpStore,
			DepthClearValue:   1.0,
			DepthReadOnly:     false,
			StencilLoadOp:     rwgpu.LoadOpClear,
			StencilStoreOp:    rwgpu.StoreOpStore,
			StencilClearValue: 0,
