...
- **MVP Matrices**: Model-View-Projection transformation using `rwgpu.Mat4*` helpers
...
projection := rwgpu.Mat4Perspective(45.0*π/180.0, aspect, 0.1, 100.0)
...
Uses the helper function from `rwgpu.Device`:
```go
depthTexture := device.CreateDepthTexture(width, height, rwgpu.TextureFormatDepth24Plus)
