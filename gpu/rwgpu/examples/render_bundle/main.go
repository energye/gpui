...
type App struct {
	...
	renderBundle   *rwgpu.RenderBundle
	width          uint32
	height         uint32
	running        bool
	needsRecreate  bool
...
func (app *App) createRenderBundle() error {
	...
	colorFormats := []rwgpu.TextureFormat{rwgpu.TextureFormatBGRA8Unorm}
	bundleEncoder := app.device.CreateRenderBundleEncoderSimple(
		colorFormats,
		rwgpu.TextureFormatUndefined, // no depth
		1,                            // sample count
	)
...
func (app *App) render() error {
	...
	pass.ExecuteBundles([]*rwgpu.RenderBundle{app.renderBundle})
