...
func main() {
	...
	vertexBuffer, err := device.CreateBuffer(&rwgpu.BufferDescriptor{
	...
	instanceBuffer, err := device.CreateBuffer(&rwgpu.BufferDescriptor{
		Usage:            rwgpu.BufferUsageVertex | rwgpu.BufferUsageCopyDst,
	...
	vertexAttributes := []rwgpu.VertexAttribute{
		{Format: rwgpu.VertexFormatFloat32x2, Offset: 0, ShaderLocation: 0}, // position
		{Format: rwgpu.VertexFormatFloat32x3, Offset: 8, ShaderLocation: 1}, // color
	}

	// Define vertex attributes for per-instance buffer
	instanceAttributes := []rwgpu.VertexAttribute{
		{Format: rwgpu.VertexFormatFloat32x2, Offset: 0, ShaderLocation: 2}, // offset
		{Format: rwgpu.VertexFormatFloat32, Offset: 8, ShaderLocation: 3},   // scale
	}
	...
	pipeline, err := device.CreateRenderPipeline(&rwgpu.RenderPipelineDescriptor{
		Vertex: rwgpu.VertexState{
			...
			Buffers: []rwgpu.VertexBufferLayout{
				...
				// Per-instance buffer (slot 1)
				{
					...
					StepMode:       rwgpu.VertexStepModeInstance, // Key: advances per instance, not per vertex
		...
		Fragment: &rwgpu.FragmentState{
			...
			Targets: []rwgpu.ColorTargetState{
				{Format: rwgpu.TextureFormatRGBA8Unorm, WriteMask: rwgpu.ColorWriteMaskAll},
			},
		},
	...
	outputTexture, err := device.CreateTexture(&rwgpu.TextureDescriptor{
		Size:   rwgpu.Extent3D{Width: 256, Height: 256, DepthOrArrayLayers: 1},
		Format: rwgpu.TextureFormatRGBA8Unorm,
		Usage:  rwgpu.TextureUsageRenderAttachment | rwgpu.TextureUsageCopySrc,
	})
	if err != nil {
	...
	// Begin render pass
	renderPass, err := encoder.BeginRenderPass(&rwgpu.RenderPassDescriptor{
		ColorAttachments: []rwgpu.RenderPassColorAttachment{
			{
				View:       outputView,
				LoadOp:     rwgpu.LoadOpClear,
				StoreOp:    rwgpu.StoreOpStore,
				ClearValue: rwgpu.Color{R: 0.1, G: 0.1, B: 0.1, A: 1.0},
			},
		},
	})
	if err != nil {
