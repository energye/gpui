...
func main() {
	...
	device.PushErrorScope(rwgpu.ErrorFilterValidation)
	...
	fmt.Println("\n=== Example 2: Nested Error Scopes ===")
	device.PushErrorScope(rwgpu.ErrorFilterValidation) // Outer scope
	fmt.Println("Pushed outer validation scope")

	device.PushErrorScope(rwgpu.ErrorFilterValidation) // Inner scope
	...
	device.PushErrorScope(rwgpu.ErrorFilterOutOfMemory)
	fmt.Println("Monitoring for out-of-memory errors...")
	errType, _ = device.PopErrorScope(instance) //nolint:staticcheck // example demonstrates deprecated API
	fmt.Printf("Out-of-memory filter: errType=%v\n", errType)

	// Catch internal errors
	device.PushErrorScope(rwgpu.ErrorFilterInternal)
