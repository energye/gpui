...
func (app *App) createBuffers() error {
	...
	indirectArgs := rwgpu.DrawIndirectArgs{
	...
	app.indirectBuffer, _ = app.device.CreateBuffer(&rwgpu.BufferDescriptor{
		Usage:            rwgpu.BufferUsageIndirect | rwgpu.BufferUsageCopyDst,
	...
	if indirectPtr != nil {
		*(*rwgpu.DrawIndirectArgs)(indirectPtr) = indirectArgs
	}
...
func (app *App) createPipeline() error {
	...
	// Vertex attributes
	vertexAttributes := []rwgpu.VertexAttribute{
		{
			Format:         rwgpu.VertexFormatFloat32x2,
			Offset:         0,
			ShaderLocation: 0, // position
		},
		{
			Format:         rwgpu.VertexFormatFloat32x3,
			Offset:         8, // After position (2 * 4 bytes)
			ShaderLocation: 1, // color
		},
	}

	vertexBufferLayout := rwgpu.VertexBufferLayout{
	...
	pipeline, _ := app.device.CreateRenderPipeline(&rwgpu.RenderPipelineDescriptor{
		Vertex: rwgpu.VertexState{
			Module:     shader,
			EntryPoint: "vs_main",
			Buffers:    []rwgpu.VertexBufferLayout{vertexBufferLayout},
		},
