...
type App struct {
	hwnd             windows.HWND
	hinstance        windows.Handle
	instance         *rrwgpu.Instance
	adapter          *rrwgpu.Adapter
	device           *rrwgpu.Device
	queue            *rrwgpu.Queue
	surface          *rrwgpu.Surface
	pipeline         *rrwgpu.RenderPipeline
	vertexBuffer     *rrwgpu.Buffer
	uniformBuffer    *rrwgpu.Buffer
	bindGroupLayout  *rrwgpu.BindGroupLayout
	bindGroup        *rrwgpu.BindGroup
	extraTexture     *rrwgpu.Texture
	extraTextureView *rrwgpu.TextureView
	width            uint32
	height           uint32
	running          bool
	needsRecreate    bool
	surfaceTex       *rrwgpu.SurfaceTexture
...
func (app *App) createExtraTexture() error {
	app.extraTexture, _ = app.device.CreateTexture(&rwgpu.TextureDescriptor{
		Label:     "",
		Usage:     rwgpu.TextureUsageRenderAttachment | rwgpu.TextureUsageTextureBinding,
...
func getVertexAttributes() []rwgpu.VertexAttribute {
	return []rwgpu.VertexAttribute{
		{
...
func createPipelineDescriptor(
	layout *rwgpu.PipelineLayout,
	shader *rwgpu.ShaderModule,
	attributes *rwgpu.VertexAttribute,
) *rwgpu.RenderPipelineDescriptor {
	return &rwgpu.RenderPipelineDescriptor{
		Label:  "",
		Layout: layout,
...
func (app *App) renderTriangle(encoder *rwgpu.CommandEncoder, view *rwgpu.TextureView) error {
