...
func main() { //nolint:gocyclo,cyclop // example: sequential GPU setup is inherently linear
	...
	storageBuffer, err := device.CreateBuffer(&rwgpu.BufferDescriptor{
	...
	readbackBuffer, err := device.CreateBuffer(&rwgpu.BufferDescriptor{
	...
	bindGroup, err := device.CreateBindGroupSimple(bindGroupLayout, []rwgpu.BindGroupEntry{
		rwgpu.BufferBindingEntry(0, storageBuffer, 0, bufferSize),
	})
	if err != nil {
	...
	mapPending, err := readbackBuffer.MapAsync(rwgpu.MapModeRead, 0, bufferSize)
	if err != nil {
