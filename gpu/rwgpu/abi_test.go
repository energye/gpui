...
func TestABIStructSizes(t *testing.T) {
	...
	tests := []struct {
		...
		{"gputypes.Extent3D", unsafe.Sizeof(types.Extent3D{}), 12},
		{"gputypes.Origin3D", unsafe.Sizeof(types.Origin3D{}), 12},
...
func TestABIGputypesEnumAlignment(t *testing.T) {
	t.Run("TextureFormat_common", func(t *testing.T) {
		...
		// These expected values are from gputypes v0.3.0 (github.com/energye/gpui/gpu/types).
	...
	t.Run("TextureUsage_bitflags", func(t *testing.T) {
		...
		tests := []struct {
			...
			{"None", uint64(types.TextureUsageNone), 0x0000000000000000},
			{"CopySrc", uint64(types.TextureUsageCopySrc), 0x0000000000000001},
			{"CopyDst", uint64(types.TextureUsageCopyDst), 0x0000000000000002},
			{"TextureBinding", uint64(types.TextureUsageTextureBinding), 0x0000000000000004},
			{"StorageBinding", uint64(types.TextureUsageStorageBinding), 0x0000000000000008},
			{"RenderAttachment", uint64(types.TextureUsageRenderAttachment), 0x0000000000000010},
		}
		for _, tt := range tests {
			tt := tt
			t.Run(tt.name, func(t *testing.T) {
	...
	t.Run("BufferUsage_bitflags", func(t *testing.T) {
		...
		tests := []struct {
			...
			{"None", uint64(types.BufferUsageNone), 0x0000000000000000},
			{"MapRead", uint64(types.BufferUsageMapRead), 0x0000000000000001},
			{"MapWrite", uint64(types.BufferUsageMapWrite), 0x0000000000000002},
			{"CopySrc", uint64(types.BufferUsageCopySrc), 0x0000000000000004},
			{"CopyDst", uint64(types.BufferUsageCopyDst), 0x0000000000000008},
			{"Index", uint64(types.BufferUsageIndex), 0x0000000000000010},
			{"Vertex", uint64(types.BufferUsageVertex), 0x0000000000000020},
			{"Uniform", uint64(types.BufferUsageUniform), 0x0000000000000040},
			{"Storage", uint64(types.BufferUsageStorage), 0x0000000000000080},
			{"Indirect", uint64(types.BufferUsageIndirect), 0x0000000000000100},
			{"QueryResolve", uint64(types.BufferUsageQueryResolve), 0x0000000000000200},
		}
		for _, tt := range tests {
			tt := tt
			t.Run(tt.name, func(t *testing.T) {
	...
	t.Run("ShaderStage_bitflags", func(t *testing.T) {
		...
		tests := []struct {
			...
			{"None", uint32(types.ShaderStageNone), 0x00000000},
			{"Vertex", uint32(types.ShaderStageVertex), 0x00000001},
			{"Fragment", uint32(types.ShaderStageFragment), 0x00000002},
			{"Compute", uint32(types.ShaderStageCompute), 0x00000004},
		}
		runEnumTests(t, tests)
	})
}
