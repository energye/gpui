...
func TestDebugColorTargetState(t *testing.T) {
	...
	target := colorTargetStateWire{
		nextInChain: 0,
		format:      uint32(types.TextureFormatBGRA8Unorm), // Should be 27 (0x1B)
		writeMask:   uint64(types.ColorWriteMaskAll),       // Should be 15 (0xF)
	}
...
func TestDebugRenderPipelineBytes(t *testing.T) {
	...
	t.Logf("gputypes.TextureFormatBGRA8Unorm = %d (0x%X)", types.TextureFormatBGRA8Unorm, types.TextureFormatBGRA8Unorm)
	t.Logf("gputypes.TextureFormatRG11B10Ufloat = %d (0x%X)", types.TextureFormatRG11B10Ufloat, types.TextureFormatRG11B10Ufloat)

	// Verify the direct cast: gputypes v0.3.0 BGRA8Unorm = 0x1B = 27, v29 BGRA8Unorm = 0x1B = 27 (match)
	converted := uint32(types.TextureFormatBGRA8Unorm)
