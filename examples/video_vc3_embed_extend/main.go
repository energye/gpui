// Command video_vc3_embed_extend is the VW3 VC3 combo real-window:
// VR8 embed + VR9 registry + VR4 playback, all three on one screen.
//
...
// three readable unsupported cases with zero crash) plus the embed cold
// gate (demux 1280x720, params, cold decode to Ended, monotonic stamps,
// non-black variance).
...
func main() {
	...
	if secsSet {
		wrkit.RequireMinRun(secs, "VC3")
	} else {
		secs = 30
	}
	wrkit.EnsureUIFace()

	st := loadCombo("三合一组合", resolveClip("vr2_720p.mp4"))
	...
	} else if st.regPass != st.regTotal || st.regTotal == 0 {
		gateErr = fmt.Sprintf("注册用例 %d/%d 没全过", st.regPass, st.regTotal)
	} else {
		gateOK = 1
		yuvReady = 1
	}

	win, err := platform.Open(platform.Options{Width: winW, Height: winH, Title: "gpui video_vc3_embed_extend — 三合一组合", Decorations: true})
	if err != nil {
	...
	shell := wrkit.NewShell(winW, winH, "VC3 三合一 — 内嵌+注册表+播放同屏", []string{
		"视频嵌场景，与文字浮层合成",
		"同片走注册表播出+灰桩可插拔",
		"不支持的报人话错，零崩溃",
		"浮层盖一角，中途改尺寸",
	...
	if gateErr != "" {
		statusText = "组合失败：" + shortErr(gateErr, 56)
		sr, sg, sb = 0.95, 0.4, 0.35
	}
	shell.Body.LabelAt("VC3 三合一组合", 22, 20, 12, 0.92, 0.94, 0.98)
	...
	// Live picture: same 720p clip looping inside the scene. One 1280x720
	// buffer for the whole run (never per-frame alloc); two targets share
	// it to prove scaling sits in the display rect — decode stays full
	// 1280x720 (§2.8: no decode-side downscale).
	...
	if live != nil {
		...
		shell.Body.LabelAt("同源第二尺寸（注册表同片）", 13, 520, 180, 0.6, 0.8, 0.95)
		...
		shell.Body.LabelAt(fmt.Sprintf("注册 %d/%d 灰桩R=%d 坏例人话", st.regPass, st.regTotal, st.stubR), 12, 520, 360, 0.72, 0.8, 0.9)
	...
	app := embedder.NewPipelineApp(host, shell.Root, embedder.PipelineOptions{
		...
		OnEvent: func(ev platform.Event) {
			...
			case platform.EventClose:
				fmt.Fprintf(os.Stderr, "video_vc3_embed_extend: 关闭 (%s)\n", win.Backend())
	...
	app.Scheduler().Tickers().Add(&ticker{on: func(dt float64) {
		...
		shell.UpdateHUD("VC3", phaseCN(phase), app, gatePreview,
			fmt.Sprintf("注册=%d/%d %s", st.regPass, st.regTotal, resizeNote),
	...
	comboOK := 0
	if gateErr == "" && embedOK == 1 && st.regPass == st.regTotal && st.regTotal > 0 {
		comboOK = 1
	} else if gateErr == "" {
		gateErr = fmt.Sprintf("组合不全: 内嵌%d 注册%d/%d", embedOK, st.regPass, st.regTotal)
	}
	if gateErr != "" {
		gateOK = 0
		yuvReady = 0
	...
	rep := buildReport(snap, app.PresentCount(), elapsed, st, gateOK, comboOK, yuvReady, gateErr, liveShown, lastVar, embedOK, scaleOK, clipOK, overlayOK, resizeOK, liveStats, hotTick)
	...
	if app.PresentCount() < 1 {
		fmt.Fprintln(os.Stderr, "FAIL: 一次都没上屏(VC3需要真窗口)")
		os.Exit(1)
	}
	fpsWall := 0.0
	if elapsed > 0.001 {
	...
	if gateErr != "" || gateOK != 1 || comboOK != 1 {
		fmt.Fprintf(os.Stderr, "FAIL: 组合门禁: %s\n", gateErr)
		os.Exit(1)
	}
	...
	fmt.Fprintf(os.Stderr, "video_vc3_embed_extend: 通过 注册=%d/%d 内嵌=%d 直播=%d 改尺寸=%s 上屏=%d 用时=%.1f秒\n",
		st.regPass, st.regTotal, embedOK, liveShown, resizeNote, app.PresentCount(), elapsed)
}
...
// report covers VIDEO_DECODE_PLAN 2.2 families A-J plus VC3 combo extras.
type report struct {
...
func buildReport(snap scheduler.FrameMetrics, presents int64, elapsed float64, st *comboState, gateOK, comboOK, yuvReady int, gateErr string, liveShown int64, lastVar float64, embedOK, scaleOK, clipOK, overlayOK, resizeOK int, liveStats govideo.Stats, hotTick int) report {
	fpsWall := 0.0
	if elapsed > 0.001 {
	...
	return report{
		AbilityID: "VC3", Scenario: "video_vc3_embed_extend", ElapsedSec: elapsed,
		...
		ComboOK: comboOK, EmbedOK: embedOK, ScaleOK: scaleOK, ClipOK: clipOK, OverlayOK: overlayOK, ResizeOK: resizeOK,
		RegistryCasesPass: st.regPass, RegistryCasesTotal: st.regTotal,
		...
		NANote: "vc3: VR8 embed + VR9 registry + VR4 playback on one screen; scale via target rect, clip via ClipRRect, opacity via RenderOpacity, containers/codecs/colors via tables, core flow names no format",
	}
}
...
func checkBaseline(raw []byte) error {
	...
	for _, k := range []string{"combo_ok", "yuv_ready", "frames_split"} {
