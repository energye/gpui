...
func main() {
	out := flag.String("out", "tmp/render_visual_images.png", "PNG output path (default: tmp/render_visual_images.png)")
	flag.Parse()
	if *out == "" {
...
func drawExamples(dc *render.Context, img *render.ImageBuf) {
	...
	// nearest example above so sharpness is directly comparable.
	drawText(dc, 550, 400)
	dc.DrawImageEx(img, render.DrawImageOptions{
		X:             550,
		Y:             430,
