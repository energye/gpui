// Command ui_wr_r15_soak is the R15 single-ability soak real window:
// UI/raster 所有权长跑 — 全主路径组合（静态+动画+滚动+浮层）300s 持续运行。
//
// §2 R15 thesis: 长时间运行不泄漏不崩溃 — 600 行虚拟列表持续滚动（滚动），
// 3 层嵌套 boundary + 密集静态面板（静态），每帧变色热点（动画），周期性
// 浮层面板开合（浮层），PhaseClock 60s 周期循环变化 5 轮。结束时判无崩 +
// hitch_rate 合理 + CPU/RSS 稳定 + 内容始终正确（像素探针/Golden）。
//
// Gates (§2 R15 行 + §2.5 Soak 长跑 档 + §2.2 全族；硬，不许放):
// present_count≥1, fps_interval≥55, p95≤22ms, hitch≤5/min, cpu_pct_avg≤85
// 且非双 0, rss_slope≤30000, vsync_source 必有, boundary_skip>0,
// bind≪item_count, scroll_rerecord>0, overlay 周期开合≥3 次, 像素断言 4/4,
// Golden 静态掩码零容差（次跑起）。运行中无逐帧读回：像素采样只读终帧 PNG
// 文件，不碰 GPU backbuffer。
//
...
const (
	winW, winH   = 1200, 800
	closeSeconds = 300 // §2.5 Soak 长跑 档：R15 = 300s
...
func cyclicPhase(elapsed float64) (string, float64) {
	cycleT := math.Mod(elapsed, cyclePeriod)
	switch {
...
func main() {
	...
	win, err := platform.Open(platform.Options{Width: winW, Height: winH,
		Title: "gpui ui_wr_r15_soak — 全主路径 300s 长跑", Decorations: true, Resizable: true})
	if err != nil {
	...
	shell := wrkit.NewShell(winW, winH, "R15 全主路径长跑 — 300s 不泄漏不崩溃", []string{
		...
		"终帧像素探针 + Golden 静态掩码",
		"HUD 实时 fps/p95/skip/bind/overlay",
	})
	...
	snapDir := os.Getenv("R15_SNAP_DIR")
	if snapDir == "" {
		snapDir = "/tmp/r15_soak"
	}
	...
	app := embedder.NewPipelineApp(host, shell.Root, embedder.PipelineOptions{
		...
		SnapshotPath: func() string {
			if secsSet {
				return filepath.Join(snapDir, "r15_final.png")
			}
			return ""
		}(),
		OnEvent: func(ev platform.Event) {
			if ev.Type == platform.EventClose {
				fmt.Fprintf(os.Stderr, "ui_wr_r15_soak: close (%s)\n", win.Backend())
			}
	...
	// NOTE: no explicit SetPresentPolicy — R15 runs the W6 retained default.
	ov := overlay.New()
	app.SetOverlay(ov)
	...
	app.Scheduler().Tickers().Add(&wrsoak.Ticker{On: func(dt float64) {
		...
		shell.UpdateHUD("R15", phase, app, gateOK,
	...
	wrsoak.RunPixelChecks("ui_wr_r15_soak", winW, wrsoak.LoadImage(filepath.Join(snapDir, "r15_final.png")), finalChecks, pixelResult)
	...
	goldenDiffPct, goldenTotalPx, goldenFirstRun := wrsoak.EvaluateGolden("ui_wr_r15_soak", snapDir, "r15_final.png", "r15_final_base.png", goldenRects, winW)

	report := wrgate.BuildReport(wrgate.BuildInput{
		AbilityID:     "R15",
		Scenario:      "ui_wr_r15_soak",
		Snap:          snap,
		...
		Extra: map[string]any{
			...
			"covered":                     "静态 nesting + 持续滚动 + 每帧动画热点 + 周期浮层 + 60s 相位循环 ×5",
			"impl_correctness":            "终帧探针：嵌套内层色不变、滚入行按索引色精确、浮层区关闭无残留、静区文字密度达标",
			"impl_dirty":                  "稳态每帧只重录新挂载 cell + banner + hot + 相位翻转 nest chip + 浮层开合；dense 与 nest 外/中层持续 skip",
			"impl_cache":                  "600 行只绑一小窗；相位翻转只脏内层 chip",
	...
	if wrsoak.FpsOf(snap.AvgFrameIntervalMs) < 55 {
		fmt.Fprintf(os.Stderr, "FAIL: fps_interval=%.1f < 55 (持续 tick R15)", wrsoak.FpsOf(snap.AvgFrameIntervalMs))
		os.Exit(1)
	}
	...
	if scriptedTotal != 4 || scriptedOK != scriptedTotal {
		fmt.Fprintf(os.Stderr, "FAIL: scripted=%d/%d want 4/4 (像素断言)\n", scriptedOK, scriptedTotal)
		os.Exit(1)
	}
	...
	fmt.Fprintf(os.Stderr, "ui_wr_r15_soak: OK presents=%d fps=%.1f p95=%.1f hitch=%.2f/min bind=%d/%d scroll_rr=%d ov_opens=%d slope=%.0fKB/min cpu=%.1f skip=%d scripted=%d/%d golden=%.4f%%(%dpx first=%v) vsync=%s elapsed=%.1fs\n",
...
// Assertion plumbing (pixel/Golden/tick) is shared with C10 in examples/wrsoak.
