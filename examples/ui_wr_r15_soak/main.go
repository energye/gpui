...
func main() {
	...
	runPixelChecks(loadImage(filepath.Join(snapDir, "r15_final.png")), finalChecks, pixelResult)
	...
	// 族 A: 持续 tick 60fps 档。
	if fpsOf(snap) < 55 {
		fmt.Fprintf(os.Stderr, "FAIL: fps_interval=%.1f < 55 (持续 tick R15)", fpsOf(snap))
		os.Exit(1)
	}
...
func runPixelChecks(img image.Image, checks []pixelCheck, result map[string]bool) {
	...
	for _, c := range checks {
		...
		if c.pt != nil {
			...
			fmt.Fprintf(os.Stderr, "ui_wr_r15_soak: pixel %-32s @(%4.0f,%4.0f) got=(%.3f,%.3f,%.3f) want=(%.3f,%.3f,%.3f) ok=%v\n",
				c.desc, c.pt.x, c.pt.y, r, g, b, c.pt.want[0], c.pt.want[1], c.pt.want[2], ok)
		} else if c.text != nil {
			n := textPixels(img, dpr, *c.text, c.base)
			ok = img != nil && n >= 120
			fmt.Fprintf(os.Stderr, "ui_wr_r15_soak: pixel %-32s text_px=%d (want >=120) ok=%v\n", c.desc, n, ok)
		}
		result[c.desc] = ok
	}
}
...
func evaluateGolden(snapDir string, rects []rect) (diffPct float64, totalPx int64, firstRun bool) {
	cur := filepath.Join(snapDir, "r15_final.png")
	base := filepath.Join(snapDir, "r15_final_base.png")
