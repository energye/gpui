...
//
// Output: tmp/render_text_transform.png (900x700)
package main
...
func main() {
	...
	cells := [rows][cols]cell{
		...
		},
		{
			...
			{fmt.Sprintf("Rotate(%s/6 = 30%s)", "\u03c0", "\u00b0"), func(dc *render.Context) { dc.Rotate(math.Pi / 6) }},
		},
		{
			{fmt.Sprintf("Rotate(%s/4 = 45%s)", "\u03c0", "\u00b0"), func(dc *render.Context) { dc.Rotate(math.Pi / 4) }},
			{"Shear (faux italic)", func(dc *render.Context) { dc.Shear(-0.3, 0) }},
			{fmt.Sprintf("Scale(2)+Rotate(%s/8)", "\u03c0"), func(dc *render.Context) {
				dc.Scale(2, 2)
				dc.Rotate(math.Pi / 8)
			}},
		},
	...
	if err := dc.SavePNG("tmp/render_text_transform.png"); err != nil {
		log.Fatalf("Failed to save PNG: %v", err)
	}

	log.Printf("Created tmp/render_text_transform.png (%dx%d) using font: %s",
