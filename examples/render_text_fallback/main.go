...
func main() {
	...
	if err := dc.SavePNG("tmp/render_text_fallback.png"); err != nil {
		log.Fatalf("Failed to save PNG: %v", err)
	}

	log.Println("Created tmp/render_text_fallback.png")
}

// drawMultiFaceDemo demonstrates MultiFace with emoji fallback.
func drawMultiFaceDemo(dc *render.Context, mainFace text.Face, mainSource *text.FontSource, emojiFont string) {
	if emojiFont == "" {
...
func drawNoEmojiFallback(dc *render.Context, mainSource *text.FontSource) {
	face16 := mainSource.Face(16)
	dc.SetFont(face16)
