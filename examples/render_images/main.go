...
func main() {
	...
	if err := dc.SavePNG("tmp/render_images.png"); err != nil {
		log.Fatalf("Failed to save image: %v", err)
	}

	fmt.Println("Image drawing example completed successfully!")
	fmt.Println("Output saved to: tmp/render_images.png")
}
...
func drawExamples(dc *render.Context, img *render.ImageBuf) {
	...
	// Example 9: Bicubic (2x) — GPU 4x4 convolution (I.03). Same size as the
	// nearest example (4) so sharpness is directly comparable.
	drawText(dc, "9. Scaled - Bicubic", 550, 400)
	dc.DrawImageEx(img, render.DrawImageOptions{
		X:             550,
		Y:             430,
		DstWidth:      150,
...
func drawText(dc *render.Context, _ string, x, y float64) {
