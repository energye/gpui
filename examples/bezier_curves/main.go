	...
	win.OnInit(func(ctrl *ui.TGPUControl) {
		...
		ctrl.SetOnRender(func(ctx *render.Context) {
			...
			for _, c := range curves {
				...
				ctx.DrawCircle(c.x2, c.y2, 4)
				ctx.DrawCircle(c.x3, c.y3, 4)
				ctx.Fill()
				ctx.SetRGBA(0, 0.5, 0, 1)
				ctx.DrawCircle(c.x1, c.y1, 3)
				ctx.DrawCircle(c.x4, c.y4, 3)
				ctx.Fill()
				ctx.SetRGBA(0, 0, 0, 1)
				ctx.DrawString(fmt.Sprintf("%s: (%.0f,%.0f)-(%.0f,%.0f)", c.name, c.x1, c.y1, c.x4, c.y4), c.x1, c.y1-20)
			}
			ctx.SavePNG(filepath.Join(exec.CurrentDir, "examples/bezier_curves/gpu_bezier_curves.png"))
		})
	})
	app.AddWindow(win)
	app.Run()
