// Command game_anim is the 4.3 skeleton independent real window.
//
// Only --case=sk is served here (walk/run/jump on true art, no drift):
// 4.2 timeline (--case=tl) and 4.4 state machine (--case=fsm) each get
// their own case later; no combo window stands in for a single ability.
//
...
func main() {
	caseFlag := flag.String("case", "sk", "anim case: only sk (walk/run/jump bones)")
	...
	flag.Parse()
	if *caseFlag != "sk" {
		fmt.Fprintf(os.Stderr, "FAIL: --case=%q want sk (tl/fsm get their own cases later, no combo)\n", *caseFlag)
		os.Exit(2)
	}
