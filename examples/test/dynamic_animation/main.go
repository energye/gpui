...
func main() {
	...
	win.OnInit(func(ctrl *gpui.TGPUControl) {
		ctrl.SetOnRender(func(ctx *gg.Context) {
			...
			ctx.DrawCircle(400,300,80); ctx.Fill()
			ctx.SetStrokeBrush(gg.Solid(gg.RGBA{R:1,G:0,B:0,A:1})); ctx.SetLineWidth(3)
			ctx.MoveTo(400,300); ctx.LineTo(400+120*math.Cos(angle),300+120*math.Sin(angle)); ctx.Stroke()
			ctx.SetFillBrush(gg.Solid(gg.RGBA{R:1,G:0.8,B:0,A:1}))
			ctx.DrawCircle(400+80*math.Cos(angle*2),300+80*math.Sin(angle*2),10); ctx.Fill()
			pulse := 30 + 20*math.Sin(angle)
			ctx.SetFillBrush(gg.Solid(gg.RGBA{R:0.2,G:0.8,B:0.2,A:0.5}))
			ctx.DrawCircle(400,300,pulse); ctx.Fill()
			...
			ctx.SavePNG("gpu_dynamic_animation.png")
		})
		ctrl.StartAnimation()
	})
	app.AddWindow(win)
