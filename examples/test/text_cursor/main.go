...
func main() {
	...
	win.OnInit(func(ctrl *gpui.TGPUControl) {
		ctrl.SetOnRender(func(ctx *gg.Context) {
			...
			ctx.SetRGBA(0,0,0,1); ctx.DrawString("Text with cursor|", 100, 100)
			ctx.SetLineWidth(2); ctx.MoveTo(370,88); ctx.LineTo(370,108); ctx.Stroke()
			ctx.DrawString("Cursor position indicator", 100, 200)
			ctx.SetRGBA(0,0,0,0.5); ctx.MoveTo(420,188); ctx.LineTo(420,208); ctx.Stroke()
			ctx.LoadFontFace("../NotoSansCJK-Regular.ttc", 20)
			ctx.SetRGBA(0,0,0,1); ctx.DrawString("Multiple cursor positions:", 100, 300)
			ctx.SetRGBA(1,0,0,1); ctx.SetLineWidth(2); ctx.MoveTo(100,310); ctx.LineTo(100,328); ctx.Stroke()
			ctx.SetRGBA(0,0,1,1); ctx.MoveTo(200,310); ctx.LineTo(200,328); ctx.Stroke()
			ctx.SetRGBA(0,0.5,0,1); ctx.MoveTo(300,310); ctx.LineTo(300,328); ctx.Stroke()
			ctx.LoadFontFace("../NotoSansCJK-Regular.ttc", 28)
			ctx.SetRGBA(0,0,0,1); ctx.DrawString("Edit this text", 100, 400)
			ctx.SetRGBA(0,0,0,0.7); ctx.SetLineWidth(2); ctx.MoveTo(250,388); ctx.LineTo(250,408); ctx.Stroke()
