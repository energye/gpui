...
func main() {
	...
	win.OnInit(func(ctrl *gpui.TGPUControl) {
		ctrl.SetOnRender(func(ctx *gg.Context) {
			...
			ctx.DrawRectangle(100,100,150,150); ctx.Fill()
			ctx.SetFillBrush(gg.Solid(gg.RGBA{R:0.8,G:0.2,B:0.2,A:0.6}))
			ctx.DrawRectangle(180,130,150,150); ctx.Fill()
			ctx.SetFillBrush(gg.Solid(gg.RGBA{R:0.2,G:0.8,B:0.2,A:0.6}))
			ctx.DrawRectangle(140,170,150,150); ctx.Fill()
			ctx.SetRGBA(0,0,0,1); ctx.DrawString("Alpha Blended Rectangles", 100, 80)

			ctx.SetFillBrush(gg.Solid(gg.RGBA{R:1,G:0,B:0,A:0.3}))
			ctx.DrawCircle(500,150,60); ctx.Fill()
			ctx.SetFillBrush(gg.Solid(gg.RGBA{R:0,G:1,B:0,A:0.3}))
			ctx.DrawCircle(550,150,60); ctx.Fill()
			ctx.SetFillBrush(gg.Solid(gg.RGBA{R:0,G:0,B:1,A:0.3}))
			ctx.DrawCircle(525,120,60); ctx.Fill()
			ctx.SetRGBA(0,0,0,1); ctx.DrawString("Alpha Blended Circles", 400, 80)

			g := gg.NewRadialGradientBrush(500,400,10,100)
			g.AddColorStop(0, gg.RGBA{R:0,G:0,B:0,A:0.8}); g.AddColorStop(1, gg.RGBA{R:0,G:0,B:0,A:0})
			ctx.SetFillBrush(g); ctx.DrawCircle(500,400,100); ctx.Fill()
			ctx.SetRGBA(0,0,0,1); ctx.DrawString("Radial Transparency", 450, 520)
