...
func main() {
	...
	target := render.NewPixmap(width, height)
	...
	if err := target.SavePNG("tmp/render_scene.png"); err != nil {
		log.Fatalf("Failed to save PNG: %v", err)
	}

	fmt.Println("\nSuccessfully created tmp/render_scene.png")
...
func demonstrateIncrementalRender(renderer *scene.Renderer, target *render.Pixmap, sceneGraph *scene.Scene) {
