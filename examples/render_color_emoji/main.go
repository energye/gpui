...
//
// Outputs:
//   - tmp/render_color_emoji_bitmap.png   (36 color emoji from CBDT/CBLC font)
//   - tmp/render_color_emoji_palette.png  (COLR/CPAL color palette visualization)
//   - tmp/render_color_emoji_sample.png   (single emoji at high resolution)
package main
...
func generateBitmapGrid(fontData []byte) bool {
	...
	outPath := "tmp/render_color_emoji_bitmap.png"
...
func generateSampleEmoji(fontData []byte) bool {
	...
	for gid := uint16(1); gid < 1000; gid++ {
		...
		outPath := "tmp/render_color_emoji_sample.png"
		f, err := os.Create(outPath)
		if err != nil {
			return false
		}
...
func generateColorPalette(fontData []byte) bool {
	...
	outPath := "tmp/render_color_emoji_palette.png"
	f, err := os.Create(outPath)
	if err != nil {
		log.Printf("Failed to create %s: %v\n", outPath, err)
		return false
