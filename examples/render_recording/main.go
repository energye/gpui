...
//
//	go get github.com/energye/gpui/render-pdf
//	go get github.com/energye/gpui/render-svg
//
...
import (
	...
	_ "github.com/energye/gpui/render/recording/backends/raster" // Built-in raster backend
	// _ "github.com/energye/gpui/render-pdf" // Uncomment for PDF export
	// _ "github.com/energye/gpui/render-svg" // Uncomment for SVG export
)
...
func main() {
	...
	bgGrad := recording.NewLinearGradientBrush(0, 0, 400, 300).
		AddColorStop(0, render.RGBA{R: 0.2, G: 0.3, B: 0.5, A: 1}).
		AddColorStop(1, render.RGBA{R: 0.4, G: 0.5, B: 0.7, A: 1})
	...
	if recording.IsRegistered("pdf") {
		...
		fmt.Println("PDF backend not available (import github.com/energye/gpui/render-pdf)")
	}
	...
	if recording.IsRegistered("svg") {
		...
		fmt.Println("SVG backend not available (import github.com/energye/gpui/render-svg)")
	}
}
...
func exportRaster(r *recording.Recording) {
	...
	if fb, ok := backend.(recording.FileBackend); ok {
		...
		if err := fb.SaveToFile("tmp/render_recording.png"); err != nil {
			log.Printf("Failed to save PNG: %v", err)
			return
		}
		fmt.Println("Saved: tmp/render_recording.png")
	}
}
