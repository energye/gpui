...
func main() {
	...
	if err := dc.SavePNG("tmp/render_pixel_perfect.png"); err != nil {
		log.Fatalf("Failed to save PNG: %v", err)
	}

	log.Println("Created tmp/render_pixel_perfect.png — compare left (AA) vs right (no-AA)")
}

func drawShapes(dc *render.Context, offsetX float64) {
	// Diagonal line
	dc.SetRGB(1, 0, 0)
	dc.SetLineWidth(1)
