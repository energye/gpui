// Command ui_wr_r12b_debug_repaint is the R12b gate: debug repaint overlay
// on live paints (not cache Replay).
//
// Quality bar (§2.6.2 R12b): debug_repaint=1 时有叠加标志 (magenta flash);
// 人眼见谁在重绘——多 boundary + 部分重绘可见; 染色区域与脏区一致——脏区染色、
// 静区不染; off 路径 (DEBUG_REPAINT=0) 必须 0 overlay。
//
//	export LD_LIBRARY_PATH=$PWD/lib WGPU_NATIVE_PATH=$PWD/lib/libwgpu_native.so
//	RUN_SECONDS=8 go run ./examples/ui_wr_r12b_debug_repaint
//	DEBUG_REPAINT=0 …  # off path must draw 0 overlays
package main

import (
	"fmt"
	"math"
	"os"
	"strconv"
	"sync/atomic"
	"time"

	"github.com/energye/gpui/examples/exhost"
	"github.com/energye/gpui/examples/wrgate"
	"github.com/energye/gpui/examples/wrkit"
	"github.com/energye/gpui/ui/embedder"
	"github.com/energye/gpui/ui/platform"
	"github.com/energye/gpui/ui/rendering"
	"github.com/energye/gpui/ui/scheduler"

	_ "github.com/energye/gpui/render/gpu"
)

const (
	winW, winH   = 1200, 800
	closeSeconds = 8
	hudH         = 72.0
)

func main() {
	secs := runSeconds(closeSeconds)
	if secs < 5 {
		fmt.Fprintln(os.Stderr, "FAIL: RUN_SECONDS must be >= 5 to close R12b (U16)")
		os.Exit(1)
	}
	// Default ON for the close gate; DEBUG_REPAINT=0 exercises off path.
	debugOn := os.Getenv("DEBUG_REPAINT") != "0"
	fmt.Fprintf(os.Stderr, "ui_wr_r12b_debug_repaint: R12b — %ds @ 1200x800 debug=%v\n", secs, debugOn)

	if _, path, err := wrkit.EnsureUIFace(); err != nil {
		fmt.Fprintf(os.Stderr, "ui_wr_r12b_debug_repaint: WARN font: %v — labels may be blank\n", err)
	} else {
		fmt.Fprintf(os.Stderr, "ui_wr_r12b_debug_repaint: font ready (%s)\n", path)
	}

	var proc scheduler.ProcessTracker
	proc.Start()

	win, err := exhost.Open(exhost.Options{
		Width: winW, Height: winH, Title: "gpui ui_wr_r12b_debug_repaint",
	})
	if err != nil {
		fmt.Fprintf(os.Stderr, "FAIL: window open (needs_gpu_window): %v\n", err)
		os.Exit(1)
	}

	sc := buildScene(float64(winW), float64(winH))
	app := embedder.NewPipelineApp(win.Host(), sc.Root, embedder.PipelineOptions{
		ClearR: 0.06, ClearG: 0.07, ClearB: 0.09, ClearA: 1,
		RunFor: time.Duration(secs) * time.Second,
		WarmUp: true,
		OnEvent: func(ev platform.Event) {
			if ev.Type == platform.EventClose {
				fmt.Fprintln(os.Stderr, "ui_wr_r12b_debug_repaint: close")
			}
		},
	})
	app.SetDebugRepaint(debugOn)

	phases := wrkit.NewPhaseClock(1.5, 4.0) // Steady 0–1.5 · Spike 1.5–4 · Recover
	app.Scheduler().Tickers().Add(&tick{on: func(dt float64) {
		ph := phases.Advance(dt)
		sc.onTick(dt, ph)
		proc.Sample()
		if wrkit.HUDEnabled() && sc.hud != nil {
			sc.hud.NoteTick(dt)
			snap := app.Metrics().Snapshot()
			wrkit.MergeBoundaryCache(app, &snap)
			fps := 0.0
			if snap.AvgFrameIntervalMs > 1e-6 {
				fps = 1000.0 / snap.AvgFrameIntervalMs
			}
			policy := snap.PresentPolicy
			if policy == "" {
				policy = scheduler.PresentPolicyFullPaint
			}
			gateOK := fps >= 55 || phases.Elapsed() < 1.5
			if snap.P95FrameIntervalMs > 22 && phases.Elapsed() >= 1.5 {
				gateOK = false
			}
			draws := app.DebugRepaintDraws()
			hotCount := sc.hotDirtyTicks.Load()
			staticClean := sc.staticCleanTicks.Load()
			sc.hud.Update(wrkit.Snap{
				AbilityID:   "R12b",
				Phase:       ph,
				FPS:         fps,
				P95Ms:       snap.P95FrameIntervalMs,
				Policy:      policy,
				PresentMode: snap.PresentMode,
				PaintCount:  snap.PaintCount,
				Presents:    app.PresentCount(),
				Core:        fmt.Sprintf("debug=%v draws=%d hot=%d staticClean=%d", debugOn, draws, hotCount, staticClean),
				GateOK:      gateOK,
				Extra:       "3 hot boundaries flash magenta · static stays clean",
			})
		}
		app.ScheduleFrame()
	}})
	app.Scheduler().SetMode(scheduler.ModePersistent)

	if err := app.Open(); err != nil {
		fmt.Fprintf(os.Stderr, "FAIL: present target open: %v\n", err)
		os.Exit(1)
	}
	t0 := time.Now()
	if err := app.Run(); err != nil {
		fmt.Fprintf(os.Stderr, "FAIL: run: %v\n", err)
		os.Exit(1)
	}
	proc.Stop()
	app.Close()
	win.Close()
	proc.NoteAfterClose()
	proc.Apply(app.Metrics())

	elapsed := time.Since(t0).Seconds()
	draws := app.DebugRepaintDraws()
	snap := app.Metrics().Snapshot()
	wrkit.MergeBoundaryCache(app, &snap)

	hotDirty := sc.hotDirtyTicks.Load()
	staticClean := sc.staticCleanTicks.Load()

	rep := wrgate.BuildReport(wrgate.BuildInput{
		AbilityID:     "R12b",
		Scenario:      "ui_wr_r12b_debug_repaint",
		Snap:          snap,
		PresentCount:  app.PresentCount(),
		ElapsedSec:    elapsed,
		SurfaceAreaPx: int64(winW * winH),
		Warmup:        true,
		Extra: map[string]any{
			"client_px":           "1200x800",
			"run_seconds":         secs,
			"debug_repaint":       debugOn,
			"debug_repaint_draws": draws,
			"hot_boundaries":      3,
			"hot_dirty_ticks":     hotDirty,
			"static_clean_ticks":  staticClean,
			"g_metrics":           "skipped",
			"g_metrics_reason":    "R12b is not R9/R10; text/measure cache not in scope",
			"phase_script":        "Steady→Spike→Recover (hot pulse rate doubles in Spike)",
			"impl_correctness":    "debug=1 时脏区染色 (magenta overlay)、静区不染; off=0 overlay; 染色区域与脏区一致",
			"impl_dirty":          "debug_repaint_draws 累计 live 重绘数 (非 cache Replay); hot MarkNeedsPaint each tick",
			"impl_cache":          "N/A for R12b (BoundaryCache is R3); R12b proves debug overlay visualization, not cache hits",
			"impl_edge":           "3 hot boundaries at different frequencies; static boundary frozen; off path (DEBUG_REPAINT=0) must 0 overlay",
			"impl_fail":           "debug=1 时 draws=0 (overlay 不渲染) / off 时 draws>0 (overlay 泄漏) = FAIL",
			"impl_visible":        "3 hot regions (A/B/C) flash magenta overlay each tick; static boundary and neighbors stay clean (no magenta)",
		},
	})
	b, err := wrgate.Marshal(rep)
	if err != nil {
		fmt.Fprintf(os.Stderr, "FAIL: marshal: %v\n", err)
		os.Exit(1)
	}
	fmt.Fprintln(os.Stderr, "ui_wr_r12b_debug_repaint: metrics JSON follows on stdout")
	fmt.Println(string(b))

	if err := wrgate.EvaluateGates(rep, wrgate.GateOptions{
		MinPresents: 1, RequireFullPaintPolicy: true,
		RequirePersistentFPS: true, MinFPSWall: 55, MinFPSElapsed: 5,
	}); err != nil {
		fmt.Fprintln(os.Stderr, err.Error())
		os.Exit(1)
	}
	// R12b overlay invariants.
	if debugOn {
		if draws < 1 {
			fmt.Fprintf(os.Stderr, "FAIL: debug_repaint_draws=%d want ≥1 when DEBUG_REPAINT on\n", draws)
			os.Exit(1)
		}
		if hotDirty < 10 {
			fmt.Fprintf(os.Stderr, "FAIL: hot_dirty_ticks=%d want ≥10 (3 hot boundaries pulsing)\n", hotDirty)
			os.Exit(1)
		}
		if staticClean < 10 {
			fmt.Fprintf(os.Stderr, "FAIL: static_clean_ticks=%d want ≥10 (static boundary stays clean)\n", staticClean)
			os.Exit(1)
		}
	} else if draws != 0 {
		fmt.Fprintf(os.Stderr, "FAIL: debug_repaint_draws=%d want 0 when DEBUG_REPAINT=0 (off path)\n", draws)
		os.Exit(1)
	}
	fmt.Fprintf(os.Stderr, "ui_wr_r12b_debug_repaint: PASS draws=%d debug=%v hot=%d staticClean=%d fps=%.1f\n",
		draws, debugOn, hotDirty, staticClean, rep.FPSInterval)
}

// --- helpers ---

func runSeconds(def int) int {
	if v := os.Getenv("RUN_SECONDS"); v != "" {
		if n, err := strconv.Atoi(v); err == nil && n > 0 {
			return n
		}
	}
	return def
}

type tick struct{ on func(dt float64) }

func (t *tick) Tick(dt float64) bool {
	if t.on != nil {
		t.on(dt)
	}
	return true
}

// --- scene ---

type scene12b struct {
	Root *rendering.AbsoluteBox
	hud  *wrkit.LiveHUD

	// Three independent hot boundaries, pulsing at different frequencies.
	hotA *rendering.RenderColorBox
	hotB *rendering.RenderColorBox
	hotC *rendering.RenderColorBox

	// Static boundary that must stay clean (no magenta overlay) while hots pulse.
	staticB *rendering.RenderColorBox

	// Counters for R12b overlay invariants.
	staticCleanTicks atomic.Int64
	hotDirtyTicks    atomic.Int64

	phaseA, phaseB, phaseC float64
}

func buildScene(w, h float64) *scene12b {
	s := &scene12b{}
	root := rendering.NewAbsoluteBox(w, h)
	root.Background = &rendering.Color{R: 0.08, G: 0.09, B: 0.11, A: 1}
	s.Root = root

	// --- TopBar (U17 多区域 第 1 区) ---
	top := wrkit.NewPanel(w, 48, 0.12, 0.14, 0.18, 1)
	top.PlaceOn(root, 0, 0)
	top.LabelAt("R12b debug repaint overlay — magenta marks dirty boundaries, static stays clean", 14, 16, 14, 0.88, 0.92, 0.98)

	// --- Legend (U17 第 2 区, ≥8 行色块+文字) ---
	leg := wrkit.NewPanel(240, 540, 0.11, 0.12, 0.15, 1)
	leg.PlaceOn(root, 12, 60)
	leg.LabelAt("LEGEND", 13, 12, 10, 0.55, 0.75, 0.95)
	legendLines := []struct {
		text    string
		r, g, b float64
	}{
		{"DEBUG_REPAINT on (default)", 0.90, 0.30, 0.80},
		{"Magenta overlay on dirty", 0.90, 0.20, 0.70},
		{"Hot A — red, pulses ~4 Hz", 0.90, 0.30, 0.25},
		{"Hot B — green, pulses ~3 Hz", 0.25, 0.75, 0.40},
		{"Hot C — blue, pulses ~5 Hz", 0.30, 0.50, 0.90},
		{"Static boundary — frozen", 0.55, 0.60, 0.70},
		{"染色区域≡脏区 (debug)", 0.65, 0.68, 0.72},
		{"off path: 0 overlay", 0.40, 0.40, 0.40},
		{"PhaseClock: Steady/Spike/Recover", 0.55, 0.75, 0.85},
	}
	for i, ln := range legendLines {
		leg.ColorAt(14, 14, 12, 36+float64(i)*28, ln.r, ln.g, ln.b, 1, false)
		leg.LabelAt(ln.text, 11, 32, 36+float64(i)*28, ln.r, ln.g, ln.b)
	}

	// --- Three hot boundary panels (U17 多区域 第 3/4/5 区) ---
	// Each hot region is a RepaintBoundary color box; debug overlay (magenta)
	// marks them as dirty each tick. Static boundary + neighbors stay clean.

	// Hot A: red, top-left of body.
	hotAX, hotAY := 276.0, 70.0
	hotAW, hotAH := 180.0, 180.0
	s.hotA = rendering.NewRenderColorBox(hotAW, hotAH, 0.90, 0.30, 0.25, 1)
	s.hotA.SetRepaintBoundary(true)
	root.Place(s.hotA, hotAX, hotAY)
	hotALbl := wrkit.NewPanel(hotAW, 22, 0.12, 0.14, 0.18, 0.9)
	hotALbl.PlaceOn(root, hotAX, hotAY-24)
	hotALbl.LabelAt("HOT A (red ~4Hz) — magenta overlay", 11, 6, 14, 0.90, 0.75, 0.60)

	// Hot B: green, top-right of body.
	hotBX, hotBY := 740.0, 70.0
	hotBW, hotBH := 180.0, 180.0
	s.hotB = rendering.NewRenderColorBox(hotBW, hotBH, 0.25, 0.75, 0.40, 1)
	s.hotB.SetRepaintBoundary(true)
	root.Place(s.hotB, hotBX, hotBY)
	hotBLbl := wrkit.NewPanel(hotBW, 22, 0.12, 0.14, 0.18, 0.9)
	hotBLbl.PlaceOn(root, hotBX, hotBY-24)
	hotBLbl.LabelAt("HOT B (green ~3Hz) — magenta overlay", 11, 6, 14, 0.60, 0.85, 0.70)

	// Hot C: blue, bottom-center of body.
	hotCX, hotCY := 508.0, 320.0
	hotCW, hotCH := 180.0, 180.0
	s.hotC = rendering.NewRenderColorBox(hotCW, hotCH, 0.30, 0.50, 0.90, 1)
	s.hotC.SetRepaintBoundary(true)
	root.Place(s.hotC, hotCX, hotCY)
	hotCLbl := wrkit.NewPanel(hotCW, 22, 0.12, 0.14, 0.18, 0.9)
	hotCLbl.PlaceOn(root, hotCX, hotCY-24)
	hotCLbl.LabelAt("HOT C (blue ~5Hz) — magenta overlay", 11, 6, 14, 0.60, 0.75, 0.90)

	// --- Static boundary (U17 多区域 第 6 区) ---
	// Frozen color block with a RepaintBoundary; must stay clean (no magenta)
	// while hot A/B/C pulse each tick.
	staticX, staticY := 276.0, 540.0
	staticW, staticH := 460.0, 80.0
	s.staticB = rendering.NewRenderColorBox(staticW, staticH, 0.15, 0.55, 0.85, 1)
	s.staticB.SetRepaintBoundary(true)
	root.Place(s.staticB, staticX, staticY)
	staticLbl := wrkit.NewPanel(staticW, 22, 0.10, 0.11, 0.13, 0.9)
	staticLbl.PlaceOn(root, staticX, staticY+staticH+6)
	staticLbl.LabelAt("STATIC boundary — frozen, no magenta overlay", 11, 6, 14, 0.70, 0.80, 0.90)

	// --- Dense static text grid (U17 第 7 区, ≥8 labels) ---
	gridX, gridY := 936.0, 320.0
	for row := 0; row < 4; row++ {
		for col := 0; col < 2; col++ {
			cell := wrkit.NewPanel(120, 28, 0.10, 0.11, 0.13, 0.85)
			cell.PlaceOn(root, gridX+float64(col)*124, gridY+float64(row)*32)
			cell.LabelAt(fmt.Sprintf("static %d-%d", row, col), 11, 8, 16, 0.70, 0.75, 0.85)
		}
	}

	// --- LiveHUD (U18 窗内可见指标) ---
	if wrkit.HUDEnabled() {
		s.hud = wrkit.NewLiveHUD(w, hudH)
		root.Place(s.hud.Box, 0, h-hudH)
	}

	return s
}

func (s *scene12b) onTick(dt float64, phase string) {
	if s == nil {
		return
	}
	// PhaseClock drives pulse rate — Spike doubles frequency.
	rateA, rateB, rateC := 4.0, 3.0, 5.0
	switch phase {
	case wrkit.PhaseSpike:
		rateA, rateB, rateC = 8.0, 6.0, 10.0
	case wrkit.PhaseRecover:
		rateA, rateB, rateC = 3.0, 2.0, 4.0
	}
	s.phaseA += dt * rateA
	s.phaseB += dt * rateB
	s.phaseC += dt * rateC

	// Hot A: red pulse.
	ra := 0.55 + 0.35*math.Sin(s.phaseA)
	s.hotA.R, s.hotA.G, s.hotA.B, s.hotA.A = ra, 0.20, 0.15, 1
	s.hotA.MarkNeedsPaint()

	// Hot B: green pulse (different frequency).
	gb := 0.50 + 0.35*math.Sin(s.phaseB+0.7)
	s.hotB.R, s.hotB.G, s.hotB.B, s.hotB.A = 0.15, gb, 0.30, 1
	s.hotB.MarkNeedsPaint()

	// Hot C: blue pulse (different frequency + offset).
	bc := 0.55 + 0.35*math.Sin(s.phaseC+1.4)
	s.hotC.R, s.hotC.G, s.hotC.B, s.hotC.A = 0.20, 0.35, bc, 1
	s.hotC.MarkNeedsPaint()

	// R12b overlay invariant: static boundary must stay clean (no magenta)
	// after all three hots called MarkNeedsPaint.
	if !s.staticB.NeedsPaint() {
		s.staticCleanTicks.Add(1)
	}
	s.hotDirtyTicks.Add(3) // 3 hot boundaries dirty each tick
}
