...
> **状态（组级）：** A/B · Dispose ✅ · **Wave：** — / P1–P2 · 修订：2026-07-28  
...
**已落地 Dispose（FImg-DISPOSE）：**  
- `ImageBuf.Dispose` / `Disposed`（幂等、nil 安全；释放像素存储）  
- `RenderImage.SetImage` **取得所有权**，替换/Clear/SetError 时 `Dispose` 旧缓冲  
- `SetImageShared` 不取得所有权（调用方负责 Dispose）  
- 绘制路径对已 Dispose 缓冲 fail-closed（不画 / 占位）
...
| FImg-DISPOSE | 释放图像 | image.dispose | 防泄漏 | SetImage 所有权 · Clear | **ImageBuf.Dispose** | RenderImage | A | buf.go · image.go · image_dispose_test | 幂等；Shared 例外 | — |
...
| 层 | 路径 |
|----|------|
| RO | `ui/rendering/image.go`（SetImage 所有权 · SetImageShared · Clear · Dispose 接线） |
| IO | `ui/io/decode.go` Pool |
| render | `DrawImage`/`Ex`… · **`ImageBuf.Dispose`/`Disposed`** |
| 窗测 | `examples/ui_render_base_image` ✅（共享源用 SetImageShared） |
| 测 | `image_dispose_test.go` · `internal/image` Dispose 测 · decode 测 |
...
- [x] **ImageBuf Dispose 规范**（`Dispose` + RO 所有权；测绿）  
- [ ] Atlas/GPU tex 层（P2）  
- [ ] 解码耗时进 JSON 字段 |
