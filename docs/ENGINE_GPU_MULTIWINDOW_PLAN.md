...
| 1.3 关闭 | 2026-09-05 · 显存压力降级链 + 永不黑屏：`NewPresentTarget` 按 resolved→low→software 换卡重试（仅 OOM 触发，`IsGPUOutOfMemory` 三短语统一判定；每级重建并释放本级资源；`Fallbacks()` 计数），建纹理 OOM 经 flush→`PurgeEvictables`（image-cache 全逐出 + picture-texture 非 live 逐出，各自重建路径已验、teardown 注销防野指针）→MSAA 降级重试，OOM 日志限 1 条/秒；必需纹理双败置 textureSet 失效位（同尺寸 fail-fast，换尺寸重探一次，设备丢失清除；错误写明角色/尺寸/动作）；embedder 连续 3 帧 OOM 报人话退出（exit 1，禁黑窗禁 SIGSEGV）；JSON 增 `gpu_backend`/`gpu_fallbacks`（经 scheduler.FrameMetrics + wrgate.Report 全窗生效）。验收：单窗回归零变化（accept 58.4fps/核显/fallbacks 0）；压力（R6+m5 各占独显 321MB，accept 要独显）一次降级落核显 `gpu_fallbacks=1`、exit 0、快照六区出字正常、熔断未触发；失效位单测 PASS + 小堆注入显式 SKIP（无注卡无假绿）。字形图集页未进 purge：CPU 侧页，purge 不 relief 显存且断 GPU 重传。 |
