...
> 实现列图例：✅ = 已实现；🔨 = 实现中；⬜ = 占位未实现（语义已定）；⛔ = 平台协议/API 不可做（返回 ErrUnsupported）；⚠️ = 部分支持/依赖外部决策（如 WM 是否执行）。
...
| 外发拖放 | `StartDrag(offer)/StartDragTo(target,offer) error`（`DragOffer{Files,Data}`，空报 `fmt.Errorf` 非 `ErrUnsupported`） | 本窗当拖放源（源端完成后静默清理，无新事件；X11 无指针 grab/取消手势，落下即发） | ✅ To=显式编程落下（Enter/Position/Drop 连发+XdndSelection 供数，Finished 清场）；Start=XQueryPointer 快照找 XdndAware 窗再落下 | ✅ Start=start_drag（上次按钮 serial，无 serial 诚实 ErrUnsupported，合成器按指针路由，send 按 MIME 供数，cancelled 清场）；To=⛔（协议无显式目标） | ⬜ OleDoDragDrop | ⬜ NSDraggingSession |

> `WindowEdge` 枚举（RequestResize 用）：None/Top/Bottom/Left/Right/TopLeft/TopRight/BottomLeft/BottomRight（9 值，对应各平台 resize 方向）。
> `DragOffer` 载荷（外发拖放用）：`Files []string`（本地路径，按 text/uri-list 公告+供数）+ `Data map[string][]byte`（每 MIME 原始字节；显式 `Data["text/uri-list"]` 覆盖 Files 派生；`MIMETypes()` 按 uri-list 优先+其余排序给确定性公告序，空字节载荷不公告）。

**平台能力边界（写死）**：Wayland 协议无客户端定位/焦点/置顶/装饰切换/显式拖放目标 → ErrUnsupported（显隐已真实现，不在此列）。上层调用方 nil-check + 降级。
...
| EventDrop | Files/X/Y/Data | 外部文件落入窗口（位置 + 路径 + 每 MIME 原始字节；uri-list 解析 Files 并保留原文；单类型 1MB、总量 4MB，超限跳过；空拖放静默吞掉） | ✅ XDND TARGETS 协商 + 串行逐转 | ✅ offer 串行逐拉（uri-list 优先） | ⬜ | ⬜ |
| EventDragEnter/Over | X/Y/MIMETypes | 拖放悬停（位置 + offered 类型；高亮用；空类型也报；X11 XDND Enter 首包无坐标恒 0,0，有效坐标自首个 Position/Over 起） | ✅ XDND Enter/Position | ✅ wl_data_device enter/motion 已上报（常报 Enter 空类型也报、motion 无 enter 先合成 Enter、mime 快照随 motion 增长） | ⬜ | ⬜ |
...
A 层：go run ./examples/ui_pf_x11      # X11 全能力真窗（本环境 DISPLAY=:1 19 行 PASS = 18 能力行 + 1 事件观察行，零事件也过）
   ...
   A′ 层：export LD_LIBRARY_PATH=$PWD/lib WGPU_NATIVE_PATH=$PWD/lib/libwgpu_native.so && \
           ...
           go run ./examples/ui_pf_x11_dnd -auto-only     # XDND 自检门禁（4 行 PASS）
           go run ./examples/ui_pf_x11_device -auto-only  # 设备插拔自检门禁（4 行 PASS）
```
   A′ 层为 GUI 交互式：IME 双窗为人打字/Ibus 验证，无 JSON 门禁，能起窗打字即接线完整；DnD/Device 双窗为自检 + 人工二合一，`-auto-only` 只跑自检供门禁，常驻收真事件并改标题，关窗或 `-manual-seconds` 超时后出汇总 + JSON。

   P6 相关改动只跑这三层（平台其余测试/渲染层不在范围内）；完整平台回归（ui/platform 全部文件）留到各 S 阶段验收按文件跑。

---

## 3. 状态与实现（现状 · 2026-09-12）
...
约定：坐标逻辑像素 Y 向下；指针 ID 0=鼠标，≥1=触控槽；✅=已实现；⬜=占位未实现（语义已定，Win32/AppKit 均如此）；⛔=协议天生无；🔨=S6 实现；Kind 只增不改号。
...
|---|---|---|---|---|---|---|
| KindCloseRequested | — | ✅ WM_DELETE 源头拆分 | ✅ | ⬜ WM_CLOSE | ⬜ windowShouldClose: | 可拦截；不调 Close 则存活 |
| KindClose | — | ✅ | ✅ | ⬜ WM_DESTROY | ⬜ windowWillClose: | 已销毁；外部强杀通知 |
| KindResize | Width/Height/Scale | ✅ | ✅ | ⬜ WM_SIZE | ⬜ didResize | 客户区+dpr |
| KindMove | MoveX/MoveY | ✅ FromPlatform + 主循环路由 | ⛔ | ⬜ WM_MOVE | ⬜ didMoveNotification | Wayland 无事件即不支持 |
| KindScale | Scale | ✅ FromPlatform + 主循环路由（RandR + Xft.dpi） | ✅ FromPlatform + 主循环路由（output 监听取 max） | ⬜ WM_DPICHANGED | ⬜ viewDidChangeBackingProperties | 多屏拖动/系统缩放 |
| KindOccluded | Occluded bool | ✅ VisibilityNotify | ✅ suspended 上报 | ⬜ WM_SHOWWINDOW | ⬜ occlusionState | 全遮挡/最小化停渲染 |
| KindHidden | Hidden bool | ✅ 显隐发射 + 外部重映射通知 | ✅ | ⬜ SW_SHOW/HIDE | ⬜ orderFront:/orderOut: | 应用显隐 |
| KindFocus | Focused bool | ✅ FromPlatform + 主循环路由；IsFocused 可查 | ✅ FromPlatform + 主循环路由（activated） | ⬜ WM_SETFOCUS/KILLFOCUS | ⬜ didBecomeKey/didResignKey | 键盘焦点 |
| KindStateChanged | Minimized/Maximized/Fullscreen bool | ✅ FromPlatform + 主循环路由（_NET_WM_STATE/WM_STATE 回读） | ✅ FromPlatform + 主循环路由（configure 推导） | ⬜ SW_MAXIMIZE 状态推导 | ⬜ zoom:/isZoomed 推导 | 非原生事件，由配置回传推导；用户点按钮也被动通知 |
| KindThemeChanged | Dark bool | ⛔ | ⛔ | ⬜ WM_SETTINGCHANGE 深浅色 | ⬜ effectiveAppearance | 深浅色；Linux 走设置守护，P1 polling/portal |
| KindFramePresented | — | ✅ XPresent | ✅ frame 回调 | ⬜ DWM 复合帧 | ⬜ displayLink | pacing 用 |
| KindResizeSync | — | ✅ 统一 Kind + 原始排帧双通 | ⛔ | ⛔ | ⛔ | X11 同步 resize 专用 |
| KindMonitorChanged | Count/PrimaryScale | 🔨 RandR | 🔨 output 增减 | ⬜ WM_DISPLAYCHANGE | ⬜ applicationDidChangeScreenParameters | P2 显示器插拔 |
...
| KindIMEDeleteSurrounding | Start/End | ✅ | ✅ | ⬜ TSF | ⬜ NSTextInputClient | 复用 KindIME + IMEDeleteSurrounding |
| KindLocaleChanged | Language/RTL bool | ⛔ | ⛔ | ⬜ WM_INPUTLANGCHANGE | ⬜ currentLocale | P1 系统语言方向 |
...
| 上层事件 | 携带 | X11 | Wayland | Win32 | AppKit | 备注 |
|---|---|---|---|---|---|---|
| KindTouchDown/Move/Up/Cancel | ID(≥1)/X/Y | ✅ FromPlatform + 主循环路由（XI2 探测 + 解析 + 门控） | ✅ FromPlatform + 主循环路由（wl_touch 绑定 + 解码） | ⬜ WM_TOUCH/POINTER | ⬜ touchesBegan/Moved/Ended: | 复用 KindTouch + TouchEvent.Kind；一槽一竞技场 |
| KindTouchForce | ID/Pressure 0–1 | ⛔多数屏 | ⛔ | ⬜ WM_POINTER 压力 | ⬜ force | 无则零值，不报错 |
...
| 上层事件 | 携带 | X11 | Wayland | Win32 | AppKit | 备注 |
|---|---|---|---|---|---|---|
| KindPinch | ScaleDelta + Phase | ⛔ | 🔨 gesture 协议 | ⬜ WM_GESTURE 缩放 | ⬜ magnifyWithEvent: | 双指缩放 |
| KindRotate | AngleDelta + Phase | ⛔ | 🔨 gesture 协议 | ⬜ WM_GESTURE 旋转 | ⬜ rotateWithEvent: | 旋转 |
| KindSmartMagnify | — | ⛔ | ⛔ | ⛔ | ⬜ smartMagnifyWithEvent: | mac 双击放大；他端恒无 |

G 拖放（对齐 Gio transfer：MIME + 文件）：

| 上层事件 | 携带 | X11 | Wayland | Win32 | AppKit | 备注 |
|---|---|---|---|---|---|---|
| KindDragEnter/Over | X/Y/MIMETypes | ✅ XDND Enter/Position + FromPlatform + 主循环路由 | ✅ wl_data_device enter/motion + FromPlatform + 主循环路由 | ⬜ OleDragEnter/Over | ⬜ draggingEntered/Updated: | 悬停高亮用；两端已接；X11 首包 Enter 无坐标恒 0,0，定位等 Over，仅合成 Enter 带坐标；Wayland 首包常无 mime（异步公告），motion 带增长快照，无 enter 的 motion 先合成 Enter |
| KindDragLeave | — | ✅ XDND Leave + FromPlatform + 主循环路由 | ✅ data-device leave + FromPlatform + 主循环路由 | ⬜ OleDragLeave | ⬜ draggingExited: | 取消高亮；两端已接；野 leave 两端静默 |
| KindDrop | X/Y/Files/MIME/Data | ✅ XDND TARGETS 协商 + 串行逐转 + FromPlatform 深拷贝 + 主循环路由 | ✅ offer 串行逐拉（uri-list 优先）+ FromPlatform 深拷贝 + 主循环路由 | ⬜ OleDrop/CF_HDROP | ⬜ performDragOperation: | 文件 + 每 MIME 原始字节；拿不到任何数据不报 Drop；源端（本窗外发）完成后静默清理，不报新事件 |
| （源端，无新 Kind） | DragOffer（Files/Data，Controller 方法） | ✅ StartDragTo 显式落下 + StartDrag 指针快照落下（XdndSelection 供数，Finished 清场） | ✅ StartDrag 真 start_drag（serial 门控，send 供数，cancelled 清场）；StartDragTo ⛔ | ⬜ OleDoDragDrop | ⬜ NSDraggingSession | 源端协议先实现，端到端双窗对拉验（X11 双窗 6 例、Wayland send/cancelled 单测）；X11 暂无 grab/取消手势 |
...
- v2.18（2026-09-12）：**X11 XDND 拖放三件套 + 文件落下**——源码为准：新增 `EventDragEnter/Over/Leave + MIMETypes` 平台事件（`host.go` 尾部追加，旧值不动），`FromPlatform` 补三映射 + 主循环路由放行；X11 目标端 `x11_dnd_linux.go`（XdndAware=5 公告、Enter 内联3类型/TypeList、Position 根转窗内、Status 仅 uri-list 接受、Leave 清场、Drop 经 XConvertSelection 取 text/uri-list、空拖放静默吞、Finished 回执；测试源角色 `dndSrcData + OwnSelection + SelectionRequest` 应答）；`fromplatform_dnd_test.go` + `x11_dnd_linux_test.go` 双窗対拖（Enter/Over/Leave、双文件 Drop、非 uri-list 无 Drop）。§2.4 Drop 格 ❌→✅ 并补 Enter/Over/Leave 两行，§4.4 G 组 X11 三格翻绿（Wayland 三上报仍 🔨），§3 S6-P1 ⬜→🔨。回归按文件逐个：`ui/input` 三文件 + embedder 路由 + pfkit 恒绿，B 层 X11 S6-P0/窗口/同步/S4 全绿，A 层 `ui_pf_x11` 19/19；Wayland 无显示诚实 Skip。
