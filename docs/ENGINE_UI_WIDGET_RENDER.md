...
| 描边角点AA | **stencil cover 内带（interior band）在尖角处不再 Replace 掉角点楔形像素（render/internal/gpu/tessellate.go，ui_render_graphics/basic 图形六描边三角修复）**：用户报告描边三角三个闭合位置有空像素+刺尖+轻微锯齿。根因=sampleCount=1 的 analytic fringe 内带（Replace blend，cover 后覆写）在**每个边的 ±aa 切向延伸**把相邻两条边内带叠进角点楔形，且长直边/孔洞 V 臂的逐边距离模型对楔形像素给 0.5-0.85 部分覆盖，而 CPU 参考（Skia AAA）对锐角 join 楔形填满覆盖（三角峰顶 50°/底角 65° 合并条带）→ GPU 闭合处空心。修复=`TessellateAA` Pass 2 内带发射加三层约束：①**长直边（>aaInnerMaxSegLen=40px）不发内带**（CPU 对直边 just-inside 给满覆盖；此前沿三角长边出现周期性凹陷）；②**尖角短段（≤4px 且转角在 {40-60° 孔洞V / 85-95° 正交孔洞 / ≥110° 外 miter}）整段抑制内带**（角点装饰段全身都在 CPU 填穿的楔形内，末端侵蚀够不到段中像素）；③**其余尖角段末端按 aa·(1+3/sin(φ/2)) 侵蚀**（楔形重叠区移出 Replace 范围）；内带重建保留原 ±aa 切向延伸（无侵蚀时几何不变）。眼睛尖端（75° 转角）与圆帽弧（22.5°/段）不触发——CPU 参考保留其内部渐变。**验证（2026-08-17）**：活窗 xwd 抓帧（GPU llvmpipe）图形六三闭合处全部满覆盖（峰顶 y=399/401/403/405、两底角 y=513 均 (242,102,140)≈CPU(243,101,141)），区域差异从修复前 250 像素降至 2（tol12）；`render_clipping` SavePNG 与 CPU 0 像素差异（ex1 圆/ex3 星/ex5 眼睛含 y=69 内边 181==191 均保持）；单测更新 `TestTessellateAA_ConvexBandUnaffected`（长直边无内带）、新增四带尖角覆盖；render/internal/gpu 全量测试 PASS；apidoc 绿；`go build ./...` OK（tmp_ref_stroke 他人遗留除外）。 |
