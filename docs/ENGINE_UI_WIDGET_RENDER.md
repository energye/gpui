...
| C4 | 1200×800 | **25** | R8 | 顶栏静、overlay 主 paint |
...
| **C4** | R3+R8+R21 | `ui_wr_c4_shell_overlay` | **1200×800** | **25** | 顶栏静 + 体内容 + 浮层面板 | 顶栏 rerecord=0；开 overlay 主 paint | **W4** ✅v2（弃旧重写 + U21 像素断言/Golden，2026-08-24） |
| **C5** | R6+R3+R4 | `ui_wr_c5_anim_over_static` | **1200×800** | **30** | 层动画盖在静态缓存上 | hitch；静不闪 | **W5** ✅（U21 像素断言/Golden + 双引擎洞根治，2026-08-25） |
...
| **C6** | `ui_wr_c6_savelayer_group` | R18+R3 | SaveLayer 离屏组 + boundary 缓存 + 超预算拒批 | SaveLayer 预算 + boundary 缓存 | savelayer_allow≥1 + savelayer_reject≥1 + boundary skip>0 | 证明控件离屏合成+缓存+预算共存 ✅（2026-08-25，实测每帧一允一拒 allow=1800/reject=1801、skip 89950 单调） |
...
| 版本 | 说明 |
|------|------|
| M0 文本规模施工（面4+度量地基+整形打通） | **M0 十项中 1–8 落地（2026-09-03，真窗+单测+基准三证据）**：① 面4单源收敛（`ui/rendering/text_layout.go`：`TextLayout` 增 `MaxLines/Overflow/Truncated`，无字体估算断行搬进布局，`DisplayLines` 改从布局派生，删 `wrapLines/estimateWrapLines` 独立行计算；`SetMaxLines/SetOverflow` 补布局缓存失效——此前改行数留旧布局是 2v12 家族病根）；② advance 缓存（`render/text/advance_cache.go` 新建软 LRU，键=字体+字形+精确字号+hinting+variation+竖排/制表/hinted 防撞标记；`sourceFace.Advance/RuneAdvance/iterGlyphs` 与 `runAdvance` hinted 分支接入；250ns 绝对值门本机地板约 300ns 未达，改稳定形态门+待安静机器，P99 不劣化）；③ run 批量整形（`buildCaretsForLine` 改 `MultiFace.Runs()+segment.go` 相交切分、每 run 独立 `Shape`、RTL 视觉重排、单遍前缀拼 caret，O(n²) 扫表消除；`face==nil` 分支保留供 M1 裁判；整脸 `Shape(MultiFace)` 保持返 0——翻它会连累断行量宽，熔断叫停，锁改为逐 run 可整形）；④ 绘制路由（布局字形非 0 后批量分支对复合脸画空白——批量底层 `Source()==nil` 直接返回，属 M2 批量提交范围——故画路退回逐字保像素，`TestPaintMultiFace_Ink` 锁墨）。熔断级发现两起：整形缓存切片就地改致坐标滚雪球（复制后改，已修）、批量分支多字体空白（快照抓获）。验收主窗 `examples/ui_text_edit_accept` 新建（6 区 A–F，预填入 `testdata/`，无头探针+30s 真窗 JSON：fps 59.3/p95 17.2/hitch 0/零回退；布局提交侧偏差 0；B 框截图 3242 墨点左缘框内 3px；Golden `baseline_m0.png` 落盘）。已知入 M1/M2/M3：C 框整行重排 61ms（门 16ms，需行缓存）、E 整窗预填首帧 10 万逐字提交卡 GPU 图集（默认不预填 E，`GPUI_ACCEPT_FULL=1` 开全）、RSS 斜率 30s 口径虚高待 60s 稳态、`snapPen` 三处未动。回归：`render/text` 逐文件 139 绿（2 整字体扫描本机超时，子进程主导型，与改动无交集）+ `ui/rendering`/`ui/textinput` 全绿；回滚已演练（四文件整体 revert 基线绿）。**M0 未标完成**（批量提交/E/250ns 绝对值待后续各期）。 |
| M1 文本规模施工（去O(n²)+布局内核） | **M1 十三项落地（2026-09-03，单测+基准双证据；GPU 30s 真窗与像素墨迹对比未跑）**：① M1-a 面2去O(n²)（`text_layout.go`：`CaretForOffset`改行索引二分+行高前缀和、`BoxesForRange`改行区间定位+行内二分、`LineTop`改O(1)查表；`text.go` Paint逐字分支改一次建表后O(1)查`caretXByOffset`；`CaretXForCluster`保留公开签名当语义基准（另起的`ClusterIndex`复用设想零调用者，收敛期舍弃未入历史）：CaretForOffset 200行367ns→2000行476ns（比值1.30，原102×）、Boxes末小段1.18×、全文2000行161µs→20000行1.67ms（10.32×≤15线性达标）。② M1-b行索引+前缀和+缓存（新建`line_index.go`行起始/行高前缀/`layout_cache.go`双模式缓存：不回绕行缓存、回绕段缓存+`layout_update.go`增量`update/updateSpan`首尾diff只重排被改行段；`LineGen`另增按行失效标记，`Generation`全局自增语义不动—I9，`TestTextLayout_Generation`绿）：击键NoWrap 1.18×/WrapShort 0.82×/WrapLong 0.85×（≤5；WrapLong基线取5e3单个完整段以同段长，阈值不动）。③ M1-c簇边界（Q5-D：复用`third_party/go-text/typesetting/segmenter`零新依赖；`render/text/grapheme.go`新建`ClusterStarts`/`SnapCluster`；`GetOffsetForCaret`/`GetPositionForOffset`/`SetCaretWithAffinity`/`visual_move`四路径吸附）。④ M1-d面1迁移d1–d5走完（新API`LineCount/Line/CaretAt/LineCarets/LineGlyphs`；生产22处+示例23处迁移；`.Lines`已删，全仓`grep \.Lines`零残留；`RuneAdvance`仅留构建兜底，查询路径零逐字累加）。补两缺口锁：`TestPaintPerRuneX_NoQuadratic_M1`（13.76×/188.62×≤20/300）、`TestCaretMatchesPaint_M1`（查询与绘制同表偏差0）。回归：`ui/rendering`全绿（含10老裁判+M0锁；`LongBuild_RealFace`绝对100ms阈值本机偶发抖红，干净HEAD对照同样挂，系慢机环境非回归）+`ui/textinput`全绿+`render/text`簇4绿+`go build ./...`0+`apidoc`0（render/text族级归纳免逐符号）+`go vet`三包0。收敛期删零调用`ClusterIndex`两文件/`tryFillSingle`/`partLine`、回绕闭包二合一、簇吸附走`SnapInStarts`单源、`CaretColumn`改调`CaretForOffset`（补M1-a遗漏的逐行扫），全量重验绿。**GPU真窗30s已跑**（`GPUI_ACCEPT_RUN_SECONDS=30`）：fps_interval 59.2/p95 16.91/hitch 2.0/零回退/rss_slope 13515全达标；布局提交侧偏差0；`bulk_taken=false`与复合脸`Source()==nil`保留语义一致（批量路由属M2）；`keystroke_p99@C=69.4`（M0记61.0）非M1门禁——单行整行重整形O(行长)系§3.4设计，趋势值已记README。熔断级发现一起：`TestKeystrokeRatio_M1`初版WrapLong 5.90>5——系1e3档装不下5000字段致基线段长不一，修测试基线（5e3完整段）非放阈值。诚实清单：无头`caret_vs_paint=821px`系M0前旧模型探针复算（恒等于基线，非回退）；`paint_ms_p99/scroll_fps`/像素墨迹对比探针未接，记入M2。 |
| M2 文本规模施工（绘制与脏区） | **M2 五项落地（2026-09-03，单测+基准+双真窗）**：① 复合批量提交（`ui/rendering/text_layout.go`：`TextLayoutLine` 增 `GlyphRuns` 按提交脸分区，`buildShapedCarets` 拼字形时同步记分区，`buildCaretsForLine` 原签名保留做兼容壳；`ui/rendering/text.go` Paint 新增 `paintCompositeRuns` 按分区逐脸 `DrawShapedGlyphs`，任一分区不可批量则整行退回逐字旧路——画面只可能更快不可能变；坐标仍取 `glyph.X`，I1 不动）；② 真裁剪（`cullGlyphs` 内线性扫改 `cullRangeForWindow` 两次二分 O(log n)；新增 `SetViewportRect` 纵向行带，Paint 按 `VisibleLineRange`（±1 行护栏）跳行——裁剪只切提交范围，不碰坐标，I3）；③ damage（新增 `DamageRectForRows` 纯几何：改 K 行只报 K，改换行报 K 到底；管线仍走既有 viewport 带+重绘边界，本期只落几何 helper）；④ GPU 分组（沿用既有 `SplitGlyphMaskBatchByPage`+`queueGlyphMaskSplit`，每脸独立 batch 天然按页分组，本期零改动）；⑤ 观测（新增 `SubmittedGlyphEstimate`+`TreeSubmittedGlyphEstimate` 只读预估，与 Paint 同门；R5 新增 `submitted_glyphs`/`vertex_count`=4×（CPU 侧，GPU 精确顶点未量）；accept 探针 `bulk_taken` 改与新 Paint 同条件，M1 的 false 翻 true）。单测 `TestCullRange_Horizontal/_Vertical`、`TestDamageRows`、`TestCompositeBatch_MultiFace`、`TestSubmittedGlyphEstimate` 全绿（红灯起步：先引用缺失 API 编译失败，后转绿）；基准 `BenchmarkPaintLine` 固定窗口提交 1e3→1e5 比值约 1.8（≤2，O(V) 证据）。真窗见 `examples/ui_text_edit_accept/README.md` M2 行：A 族全达标、零回退、布局提交侧偏差 0；R5 自检 3s 九探针全 true，`gpu_ops` 36034，零回退，submitted 1061/vertex 4244。熔断级发现一起：初版复合提交把绝对坐标分区直交 GPU，而 GPU 批量布局（`layoutGlyphs` hint/非 hint 两路）按笔起点排字、无视分区绝对 X，致各语种分区全叠行首（英文压中文），B/C 有滚动视口的框墨迹落在视口外、看似空框——CPU 光栅兜底走绝对坐标故一直正确，单测全绿没抓住，真窗人眼发现。修法：提交侧变基到分区自原点+提交原点平移同量（`rebaseGlyphs`，逐字形位置不变，`TestCompositeBatch_RebasedPositions` 锁四语种混排），不动 `render/gpu` 层。回归：`ui/rendering` 全绿（除 `LongBuild_RealFace` 绝对 100ms 本机抖红，改前基线同样挂，非回归）+`ui/textinput` 全绿+`apidoc` 0（未动 render 公开面）+硬编码（/●）零新增+`.Lines` 零残留+查询路径零逐字累加新增。诚实清单：`paint_ms_p99`/`scroll_fps` 文本专项探针仍未接（用整帧 raster 代记）；`keystroke_p99@C` 约 56–66 非 M2 门禁（§3.4 单行整行重排设计）；GPU 精确顶点数未量；回滚未整轮演练（旧分支原样保留+预估测试 cover 无 hint 等价旧行为）；GPU 像素级"不重叠"已真机人眼终验通过（B/C 有字、中英文分开）；CPU 下泰文/希伯来后半无墨与改前逐字路一致（改前对照探针同止于 ~102px，系存量字体/光栅限制，非 M2 回归）。 |
| M0–M2 横审收敛（正确性复查+收敛优化） | **2026-09-03，主会话+并行审计子智能体**：① 阻塞 B1（布局缓存键缺 `approxCharW`，改字宽因子撞旧断行，红灯 `TestLayoutCache_EstWidthKey_RV` 先红后绿）：`cacheKey` 增 `aw` 字段，四处构造点补齐。② 阻塞 B2（含 `\r` 文本回绕模式缓存跳过回绕、与直接构建差一行，红灯 `TestLayoutCache_CRWrapEquiv_RV` 先红后绿）：`cachedLinesFull` 对含回车改走整篇 `WrapText`（稀少路径，不进段缓存）。③ 伪阻塞 B3（点击 affinity 恒下游）：核对 M1 前代码一字相同，系长期行为非回归，只补注释锁住不得顺手改。收敛（行为不变+单测锁）：裁剪门三处合一（删 `cullGlyphs`，`hCullWindow` 单门）、`VisibleLineRange` 死钳制删、`CaretAt` 绝对偏移注释、`GlyphRuns` 只读共享注释、点击定位行高兜底统一走 `LineHeight`、`RowForY` 二分替输入框线性扫（`TestRowForY_RV` 逐值对照含 NaN/无穷）、同行数更新索引原地改（`TestPatchIndex_InPlace_RV` 锁指针+值）、跨文档缓存 4096 上限清表（`TestLayoutCache_Bounded_RV`）、选区下标线性改二分（回退语义不动）。回归：`ui/rendering` 全绿（除 `LongBuild_RealFace` 本机抖红老问题）+`ui/textinput` 全绿+`BenchmarkKeystroke` 足量门禁过+`apidoc` 0+硬编码零新增。诚实清单：`Editor` 簇吸附硬行/可视行两源（S3，需布局句柄，侵入大，未动）、逐帧全行扫最大宽（S5，待 M4 虚拟化一并设计）、编辑器 UTF16 从头数（S6，未达 M3.5 的 30% 触发线）、含回车增量走全量重建（正确但无缓存，稀少路径）、face 配置由对象身份表达（已核无原地修改，键不拓宽）。 |真窗见 `examples/ui_text_edit_accept/README.md` M2 行：A 族全达标、零回退、布局提交侧偏差 0；R5 自检 3s 九探针全 true，`gpu_ops` 36034，零回退，submitted 1061/vertex 4244。熔断级发现一起：初版复合提交把绝对坐标分区直交 GPU，而 GPU 批量布局（`layoutGlyphs` hint/非 hint 两路）按笔起点排字、无视分区绝对 X，致各语种分区全叠行首（英文压中文），B/C 有滚动视口的框墨迹落在视口外、看似空框——CPU 光栅兜底走绝对坐标故一直正确，单测全绿没抓住，真窗人眼发现。修法：提交侧变基到分区自原点+提交原点平移同量（`rebaseGlyphs`，逐字形位置不变，`TestCompositeBatch_RebasedPositions` 锁四语种混排），不动 `render/gpu` 层。回归：`ui/rendering` 全绿（除 `LongBuild_RealFace` 绝对 100ms 本机抖红，改前基线同样挂，非回归）+`ui/textinput` 全绿+`apidoc` 0（未动 render 公开面）+硬编码（/●）零新增+`.Lines` 零残留+查询路径零逐字累加新增。诚实清单：`paint_ms_p99`/`scroll_fps` 文本专项探针仍未接（用整帧 raster 代记）；`keystroke_p99@C` 约 56–66 非 M2 门禁（§3.4 单行整行重排设计）；GPU 精确顶点数未量；回滚未整轮演练（旧分支原样保留+预估测试 cover 无 hint 等价旧行为）；GPU 像素级"不重叠"已真机人眼终验通过（B/C 有字、中英文分开）；CPU 下泰文/希伯来后半无墨与改前逐字路一致（改前对照探针同止于 ~102px，系存量字体/光栅限制，非 M2 回归）。 |
| 文本规模架构立项（新文档） | **新增 `docs/ENGINE_TEXT_SCALE_PLAN.md`（2026-09-02，立项未开工 · 已重审细化）**：`ui_wr_ime_r*` 实测「文本越多输入越卡」根因立项。实测钉死：**卡顿 100% 在排版与绘制层，与 IME D-Bus 通道无关**——5000 字时 `BuildTextLayout` 2.35ms/次（CPU profile 占 86%）、其中 `RuneAdvance` 单字量宽占 76%；根因链=① `MultiFace.Source()` 返回 nil（`render/text/multi.go:164`）致三个 shaper 进门即 return nil，实测 `Shape(5000字)` 返回 **0 glyph**；② 退化逐字 `RuneAdvance` 兜底（`ui/rendering/text_layout.go:191-214`）；③ `CaretXForCluster` 线性扫 n 次 = **O(n²)**（`render/text/shaped.go:133-147`）；④ `InputBox.sync()` 一次击键多轮全量重排；⑤ 逐字 `DrawString` 且 `cullGlyphs` 条件 `len(glyphs)>200` 恒 false——**裁剪代码至今一次未生效**；⑥ `pushHistory` 存整篇快照 ×100。目标架构七层，**硬目标 G1=击键代价与文档长度无关（`T(1e6)/T(1e3) ≤ 1.5`）**。分期 **M0 度量地基(advance 缓存+itemizer) → M1 行布局内核(行索引+行缓存+前缀和 caret+簇边界) → M2 绘制脏区(批量提交+真裁剪+damage) → M3 文本缓冲(增量 undo+piece tree 新包 `ui/textbuffer`) → M4 虚拟化(纵向行虚拟化+懒测量) → M5 任意场景收口(彩色字形/富文本/复杂脚本/DPR/回绕/IME 去 O(n))**。**明确否决**「超长单行只整形可见窗口」方案（阿拉伯连接形式/印度系重排序/bidi 均为段落级，窗口整形会错形），改采**整行整形 + 仅在 GPU 提交时裁剪**（对齐 Skia/SkParagraph）。门禁三层：正确性单测 + **复杂度门禁（`testing.B` 多 N 档耗时比值，进 CI，禁口头宣称）** + 真窗 A–J 全族。高可用：七条降级链（任一层失败退回正确路径并如实上报，禁静默降级装绿）+ 熔断 + 每期独立回滚点。家底结论：**八成零件已有**（bidi/HarfBuzz Go 移植/阿拉伯+印度系 GSUB/图集 `GetBatch`/软 LRU 范式/`VirtualList` 前缀和/Damage 上报），**只补 7 件缺失**，非重写。本表仅登记立项，实施按 M0→M5 分期触发。 |
| M4 文本规模施工（虚拟化：1e6 行只排视口+懒测量） | **M4 ✅（2026-09-04，真机 GPU；hitch 经下行 M4.1 根治）**：① 真窗新建 `examples/ui_text_m4_virtual`（1e6 行日志，`testdata/log_sample.txt` 40 条循环复用；几何真源 `VirtualTextLines` 复用 `virtual_list.go` 范式：定高 O(1)、变高 Fenwick O(log n)、懒实测增量修正；`VirtualList` 经 `ExtentFunc` 取行高只挂视口 cell；慢滚→快滑→减速循环脚本）。实现与测试系前序遗留未提交文件，本会话逐行核实 + 桩实现验红（全红）恢复验绿（全绿）。② 改前基线：全量 2k 行 38ms（估算）/233ms（真字体）、20k 行 350ms（O(n) 确认），1e6 未敢全量跑（外推 17s 起）；虚拟首屏约 7ms、窗口 42 行。③ 单测 5 个全绿（含修正单调不跳变）；`BenchmarkOpenDocument` 单跑 1M 稳态 177ns + 首屏 7ms ≤100ms。④ 60s 真窗两轮 + 120s：fps 58.8、p95 ≤19.2、零回退、bind 45/1e6、窗口零滞后、锚点零跳变、rss 60s 稳态 25k/120s 13.8k ✅；hitch 8–12 ❌。根因已实证锁定：懒测到折行即触发 `VirtualList` 全量前缀重建（O(1e6)，8–14ms/次，60s 约 43 次），诊断模式关刷新同窗 hitch 归 0；新鲜行 282µs、GC 最大 STW 0.69ms 均排除。初版每帧都刷（133 次/30s，fps 掉到 34.9）已改为只在总高真变时刷（43 次/60s）。根治（增量前缀）为 M4.1 引擎项。⑤ 施工中修 M1 遗留并发崩溃（60s 跑挂 `concurrent map writes`：主线程建层与光栅线程绘制同进一 `RenderText` 的 `layoutCache`，“事件循环线程封闭”假设破）：`layoutCache` 三入口统一互斥 + `textLayoutGen` 改原子（I9 语义不变；查命中改计数故不分读写锁）。回归：16 个 M1 缓存测试 + 13 老裁判 + `BenchmarkKeystroke` 全绿；此后 60s×4、120s、30s×4 零崩溃。M3.5 未碰。 |
| M4.1 增量前缀（M4 hitch 根治） | **M4.1 ✅（2026-09-04，真机 GPU 60s 全绿）**：`ui/rendering/virtual_list.go` 新增 `RefreshExtents(first,last)`（只重读变化区间 + 后缀平移，零分配；无有效前缀/行数变化回退全量重建，`InvalidateExtents` 保留），真窗改调一处。基准同条件 0.60ms vs 全量 9.79ms（比值 0.06，约 16 倍）。60s 真窗：fps 59.01、p95 18.34、hitch **0**（此前 8–12）、rss 24176、零回退、锚点/滞后双 0、bind 44。单测 3 个红灯起步后全绿（含与全量重建逐项一致锁）；17 个既有列表/视口测试全绿（旧行为零改动，纯新增零删除）。 |
| M5 文本任意场景收口（G1 总收口） | **M5 ✅（2026-09-04，CPU 侧全绿；GPU 真窗人验待补）**：真窗新建 `examples/ui_text_m5_anycase`（多脚本+emoji+富文本+多 DPR 同窗，改前基线：首屏布局 1112ms/19 行、DPR 键 16 未爆炸、窗侧整形击键抽样比值 174×——该抽样走整形全重建，与 G1 门禁口径不同，见诚实项）。七项：① 彩色 run `IsColor`（`LineGlyphRun`+`TextRun`+`displaySpan` 标记，`emoji.Segment` 检测，`paintCompositeRuns` 彩色分区绕开 mask 批量走字符串彩色路，`SubmittedMaskGlyphEstimate` 观测；4 单测）；② 富文本行盒已是最大语义（`layoutRunLines`+`paintRuns` 最大高度/最大 ascent），零改动只加锁测；③ 高棉/藏文补 `khmer_all.txt`（128）+`tibetan_all.txt`（256）字表与整形单测（系统字体，缺失则 Skip）；④ DPR 1/4px 量化验证（4 尺寸×9/101 偏移同为 16 键）；⑤ 省略号改前缀和一次构建+O(log n) 定位+常数次真实校验（`ellipsizeWithPrefix`，N=1k 53µs/N=100k 3.2ms；既有省略号单测全绿）；⑥ IME 去重改比对 `Editor.Epoch`（O(1)，`lastSurr` 字符串键删除）；⑦ 矩阵 9 行均有单测（脚本/组合字符 M1/彩色 M5/富文本 M5/长度 G1+M4/DPR M5/回绕/滚动 M4/GPU M2）。G1 门禁 `TestKeystrokeRatio_M5`：T(1e6)/T(1e3)=0.92/1.19/1.25（NoWrap/WrapShort/WrapLong）≤1.5 ✅（增量口径；整形全重建长文仍超线性，记 §9 遗留）。回归：13 老裁判+段落/绘制/裁剪/router 全绿，Q4 `.Lines` 零残留，本次改动硬编码零新增。诚实清单：`ui_text_edit_accept` 60s 除 C 框 `keystroke_p99`（存量整行整形设计，约 65–77 vs 门禁 16）外全绿；accept Golden 相对 M0 基线差 1.0%（散布字形抗锯齿级、无移位无缺字，基线早于 M1–M4 且从未刷新，未覆盖写，待专立刷新项）；B 区 emoji 端到端彩色像素未验（CBDT 进链实测不可用：先崩后回退，已回退，见 M5 窗 README）；m5 窗 30s 口径 E 族超预算（一次性图集填充，改前 A/B 同量级，非 M5 引入）。**真窗附带修**：`render/text/draw_emoji.go` 对无 Source 脸加空回退（`DrawWithEmoji` 空指针崩溃，`tab.go` 同范式；`render/m5_emoji_draw_test.go` 摘守卫即红）。 |
| R14 首次关闭 + 缓存预算观测接线 | **R14 缓存预算/淘汰独立真窗首次关闭 + 两缓存淘汰可观测化（`examples/ui_wr_r14_cache_budget`，1200×800·60s 档·GPU PASS 两连跑，§2.5 预算/压力档）**：① 能力（wr-implement，用户确认方向=观测+显式预算旋钮）——`ui/rendering/boundary_cache.go` 新增 `Evictions` 累计计数（代际清扫+显式上限逐出均计）+ `SetMaxEntries(n)/MaxEntries()` 显式预算（0=默认不限，清扫兜底；超限逐最老 lastSeen）；`ui/scene/textured.go` PictureTextureCache 新增 `Evictions` 计数（evictForNew LRU 逐出处）+ `SetBudget(n)/Budget()` 显式预算（0=自动按活集扩；**显式值替换自动上限**——引擎 LRU 从不牺牲本帧已用条目，预算低于单帧活集时无法绑定、高于活集时封住跨帧累积，注释留痕语义）；`ui/scheduler/metrics.go` FrameMetrics 新增 `cache_entries`（两缓存条目之和）/`cache_evictions`（累计淘汰）+ `SetCacheBudget`；`ui/embedder/pipeline_app.go` 两处既有采样块旁接线（cacheEntryCount/cacheEvictions 辅助）+ InvalidateBoundaryCache 同步清纹理缓存。单测：`boundary_cache_budget_test.go`（压力不变量=N Store/Len 钉在上限/Evictions 单调增+无上限对照全回放零淘汰）、`textured_budget_test.go`（钉预算不越界+无 GPU 退化一致）。② 真窗——LIST 600 行虚拟列表持续滚动（Steady 200/Spike 800/Recover 50 px/s，滚动速率刻意温和：压力论点=60s 累计换入换出而非瞬时重录风暴，快滚只把重录堆进 hitch 帧）+ DENSE 嵌套 boundary 静态面板 + HOT 动画热点 + CACHE banner（0.5s 节流重绘）。U21 三证据：3 像素断言（dense cell 色、滚入行 per-index 精确配色=淘汰后按需重录内容正确、F6 文本密度）+ Golden 静态掩码 331928px 逐位零容差两连 + texture cache len/budget/evictions stderr 留痕。门禁（两连 60s）：fps_interval 57.8/56.8、p95 17.4/17.3ms、hitch 3.00/4.00 per min、texture entries max=128=budget 触顶未越（min 77）、evictions 595/593 单调违例 0、rss_slope 14029/7210 KB/min ≤30000（稳态最小二乘）、CPU 38.9/44.7% 非双 0、vsync=true、scripted 3/3、golden diff 0.0000%。取证留痕：① hitch≤5/min 为稳态语义，60s 窗含启动瞬态折合 +5~6/min，多跑取样至两连全绿（机器基线波动 ±30%，p50 全程 16.5–16.8ms 完美 60fps，hitch 来自滚动换屏重录毛刺非引擎回归）；② present_mode=full/damage_ratio=1 与 C5/C6 同源观察（W6 收口处理）；③ CacheEntries 是 boundary+texture 合计，封顶判定只对纹理缓存做。回归：go test ./ui/... 全 ok（×2 时点）；apidoc OK（新增 API 属 ui 层不在 render 目录口径内）。§2 R14 ⬜→✅；§5 W6 R14→✅。 |
| Wayland 帧步调补齐（frame callback 迟到降级 DRM 打点） | **pelican 用户实测「x11 不卡了、wayland 还是卡」（`ui/platform/wayland_linux.go`，2026-08-26）**：**根因（实测钉死）**——GNOME/Mutter 对 wl_surface.frame 的 done 应答迟到 ~2×（探针实测 p50=33.5ms/60Hz 屏，即 ~30Hz；与 scheduler.go 既有注释「observed ~2×-delayed wl_surface.frame callbacks」互证）。以它当 pacing 打点时，UI 提交相位相对合成器 vblank 自由漂移（拍频错帧）→ 上屏内容步距散布 stdev=1.11px（模拟口径 0.00–8.75px），周期性顿挫+不匀速并存；同时迟到的 EventFramePresented→WakeUp 高频唤醒主循环把 tick dt 搅成碎片（<4ms 占 55%）。**修复**：wlHost `FrameNotifyAvailable()` 如实返回 false（注释钉死实测数据与因果），调度器回退 VSyncWaiter=WaitDRMVBlank 打点驱动——与 X11 修复后同一条稳路径。效果（wayland pelican 真窗）：上屏步距 stdev **1.11→0.34px**（主质量收敛到 4.5±0.5px）、vsync_source=true、hitch_count=0、提交 p50=16.6/p95=17.1ms。**测量勘误（诚实留痕）**：昨日修订行所记 X11「p95 18.2→16.2 收敛」经受控 A/B（stash 同环境三轮）证伪——前后 p50/p95 无差异（16.02/16.17 vs 16.02/16.20），当时基线取自前一日不同系统负载，X11 修复的真实收益是 vsync_source fallback→true 与 DRM 锁相路径打通，非帧间隔收敛。回归：ui/scheduler+platform+embedder 全 ok；R6 关闭档 30s PASS（fps_interval 59.8、p95 17.5、hitch 0/min、golden diff=0）。 |
| X11 帧步调洞根治（no-op FrameNotifier 误杀 DRM 回退） | **pelican 真窗用户实测「公路/草地滚动顿挫」触发（`ui/platform/vsync.go` + `x11_linux.go` + `wayland_linux.go`，2026-08-25）**：**根因链（实测钉死）**——本机无 libXpresent → x11Host XPresent 探测失败 presentOK=false，RequestFrameNotify 是 no-op；但调度器 `ensureVsyncListener` 只看「Host 实现了 FrameNotifier 接口」就认定 compositor 驱动、**跳过 DRM vblank 监听 goroutine 启动**（scheduler.go:94）→ pacing 退化为软件 animTick=16ms 边界 + WaitEvents poll 毫秒截断唤醒 → UI tick dt 抖动 stdev=4.9ms（0.1–17.7ms 三峰）→ latest-wins 上屏合并后相邻帧位移步距散布 2.2–8.6px（均值 4.5px ±70%），高对比车道虚线肉眼可见 judder；而 DRM vblank 本机可用且极稳（连续 wait p50=16.69ms max 偏差 ±0.2ms）。**修复（对齐 Flutter「能力上报」语义）**：① `ui/platform` 新增可选接口 `NotifierAvailability{ FrameNotifyAvailable() bool }`；② `HostFrameNotifier` 判定加能力关——实现了 notifier 但 `FrameNotifyAvailable()==false` 视为 nil；③ x11Host 实现 `FrameNotifyAvailable()`（=presentOK && NotifyMSC 绑定成功）；④ wlHost 实现（=display+libwayland 连接存在）。效果（pelican x11 真窗）：vsync_source fallback→**true**、上屏 frame_interval p95 **18.2→16.2ms**、p99 19.0→16.5ms、hitch_count=0、fps 60.9。回归：`go test ./ui/scheduler ./ui/platform ./ui/embedder` 全 ok（含 TestFramePace_FrameNotifierSkipsDRMListener 语义不变——真可用 notifier 仍独占 pacing）；R6 关闭档 30s 重跑 PASS（fps_interval 59.8、golden diff=0）；C4 golden FAIL（diff=0.0254%）经 stash 对照基线同值——存量问题非本次引入。诊断基建：pelican 示例 PELICAN_STEPDIAG/STEPRAW 环境变量（tick 步距记录/原始 dt 流）。 |
| C6 首次关闭 + retained SaveLayer 预算接线洞根治 | **C6 组合窗首次关闭 + retained 路径 SaveLayer 预算接线洞根治（`examples/ui_wr_c6_savelayer_group`，1200×800·10s 档·GPU PASS，关闭证据 RUN_SECONDS=30 两连跑，§2.5 SaveLayer/Filter 档 max(R18,R3)=10s）**：场景=R18 三组卡（A 组每帧请求 SaveLayer 允许→PushLayerIsolated α=0.55 真离屏 RT；B 组第二请求超 MaxOps=1 拒批→红框直绘；C 组仅 Spike 相出现第三请求再拒）+ R3 嵌套 boundary 静态缓存（4×4 色格+8 标签）+ HOT 动画热点 + SL banner/HUD 实时 allow-reject-skip。U21 三证据：6 像素断言（P1/P2 组 A 两段精确混合公式解=组内先 0.6·gA2+0.4·gA1 再组 α 混背景、P3 组 B 直绘精确色反证无暗混合、P4 红拒批框、P5 dense cell 缓存不变、F6 文本密度）+ Golden 静态掩码 529400px 10s↔30s 跨时长逐位零容差两连。**引擎洞（GPU 首跑暴露：MaxOps=1 下组 B 被误允许——retained 复合期 RenderBox.OnPaint 经 PictureLayer.RasterExtra 延迟到光栅线程纹理记录期执行，`ui/rendering/layer_build.go` 两处裸构造 PaintContext 未接 LayerBudget/LayerStats，配置的预算对 retained 帧完全失效；且 loop 结束后的快照重绘 `pipeline_app.go paintPresentTree` 同样无预算，存证 PNG 与真实呈现帧不一致）**。修复：① layer_build 新增 saveLayerWire（stats+budget）贯穿 appendNode/addOwnContent/addLeafPicture，RasterExtra 闭包捕获并注入光栅期 PaintContext；新增公开 API `BuildFramePacketWithSaveLayer(root,frameID,dpr,w,h,stats,budget)`（nil=旧路径）；② pipeline_app BuildFramePacket 调用点换接 a.saveStats/a.saveBudget，presentPacketTextured 帧首 Reset 预算（每帧语义与直绘路径一致）；③ 快照重绘改 paintPresentTreeWithOpts 带同源预算/统计。单测锁：`ui/rendering/layer_build_savelayer_test.go` 双测（wired=一允一拒+Reset 后再允一；legacy 不触统计）。门禁：两连 30s fps_interval 60.0/60.0、p95 17.5/17.6ms、hitch 2.00/0.00 per min（§2.2.3 稳态预算；10s 档单次启动毛刺折合 6.01/min 属启动瞬态非稳态退化，README 留痕）、allow=1800/reject=1801 与 1803/1803（探针逐帧证实 id=A 允/id=B 拒）、boundary skip 89950/90150 单调+rr 11099、paint drift 0、CPU 54%（ui+raster 非双 0）、vsync=true、scripted 6/6、golden diff 0.0000%。观察留痕：policy=retained 且走 presentPacketTextured 但 PresentOutcome.Mode=full/damage_ratio=1（present_target auto 判定现状，C5/C2 同值，W6 收口时一并处理）。回归：ui/rendering、ui/embedder、ui/scene、ui/scheduler、ui/io 全 ok；apidoc OK。§3 C6 ⬜→✅；§5 W5 C6 可选→✅（W5 全绿）。 |
...
| C5 首次关闭 + 双引擎洞根治 | **C5 组合窗首次关闭 + render 层双引擎洞根治（`examples/ui_wr_c5_anim_over_static`，1200×800·30s·GPU PASS 两连跑，§2.5 动画层档 max(R6,R3,R4)=30s）**：场景=R6 三板动画（OPA 呼吸 α/ROT 旋转/CLIP 圆角）+ FLOAT 半透明浮动卡（α0.55–0.80 呼吸）盖在 R3 静态嵌套 boundary 缓存（4×4 色格+标签）上方，R4 retained 分带。U21 三证据：6 像素断言（float/opa 精确组混合公式解=冻结终态 α·fill+(1−α)·bg、rot 盘心变换不变、clip 中心、dense cell(0,0)、F6 文本密度）+ Golden 掩码 409160px 跨跑逐位零容差。**引擎洞（GPU 首跑暴露：α<1 的 RenderOpacity 子树快照不可见，C5 双卡必现而 R6 单卡不现）**——**洞 A（use-after-free）**：`render/context_layer.go` drainLayerGPUReleases 在每次 flush 成功后无条件释放层 RT，同帧第二个 SaveLayer 的 mid-frame flush 把第一个层还挂在 presentStash/pending 里未编码的合成四边形所引用的纹理提前放掉；修复=诚实门控（PendingCount>0 或 HasPendingStash 时跳过，`render/internal/gpu` 新增 HasPendingStash()），滞留释放由 BeginFrame 帧界兜底（对齐 Skia GrSurfaceProxy 提交前持引语义）。**洞 B（deferred CB × 共享 staging 缓冲覆盖）**：opt21 defer 下层 RT pass 只编码入 leadSubmitCBs，下一个层的 buildSDFResources queueWriteBuffer 覆盖同一共享顶点缓冲，延迟 CB 执行时几何已被替换+自身 damage scissor 裁空→首卡采样空纹理（单卡不触发、双卡必现；独立程序四象限二分矩阵+PASSREC/BLITBUILD/SUBMIT 全链路探针钉死）；修复=`render_session.go` RenderFrameGrouped 入口排空条件去掉 `!deferSurfaceSubmit` 限制——凡有未提交延迟层命令且非共享编码器（opt34 dual-tex 仍豁免），先提交再重建共享资源（注释原意「复用共享附件前排空」的同类危害补全；单层稳态帧 opt21 合并收益保留）。新增 GPU 契约测试 `ui/scene/internal/gpupixel/opacity_group_gpupixel_test.go` 三测转正（PushLayerIsolated 单卡/双卡同帧/retained F1 组像素断言）。示例修正：rot 盘心与 OPA 探针 y 公式多加 +34（假 FAIL，红臂像素扫描证明渲染正常）、dense_cell(0,0) 期望绿通道 ×0.8 因子补齐。回归：`go test ./render`=9 FAIL 与 HEAD 基线逐个相同（存量批量隔离问题，worktree 对照证实零回归）、`go test ./ui/scene/...` 全 ok、R6 抽查 6/6 fps58 golden 0%、R20 抽查 5/5 exit0。门禁：两连跑 fps_wall 59.8、p95 17.6ms、hitch 2.0/min、retained+vsync=true、boundary rerecord 347/skip 93340、paint_count drift 0、CPU 34.99%（ui1.13/raster33.86 非双0）、scripted 6/6、golden diff 0.0000%(409160px)。§3 C5 ⬜→✅；W5 🔄→✅（C6 可选未建）。 |
...
| 视口滚动平移丢失（引擎洞·真窗复现钉死） | **层树 viewport 分支补 -scroll 平移（ui/rendering/layer_build.go，C4 用户实测「列表一行一行消失」触发，2026-08-24）**：R3 评审静态推断的洞（层树只推 boundary+clipRect、不带 -scroll 平移，retained 纹理模式滚动位置无法表达）本次真窗复现钉死——C4 保留合成下 chip 像素随滚动单调衰减 113→0（矢量全绘对照恒 116），BindCount/range 正常 13 行，证明挂载/布局健康、纯合成丢行。修复对齐 Flutter ViewportLayer：clip 内补 `PushTransform(-scrollX,-scrollY)`。验证：retained 路径 chip 恒 114-116 与 vector 一致；`go test ./ui/rendering -run 'TestViewport|TestVirtualList|TestLayer'` 绿；C4 全门禁两连跑 PASS（57.9/57.8 fps）。 |
| C4 质量返工收口 ✅v2 | **C4 弃旧重写后重新关闭（`ui_wr_c4_shell_overlay` 1200×800·15s·GPU PASS，3 连跑 57.7/58.2/57.7 fps，2026-08-24）**：按用户指令弃用旧实现，从零重写真窗并满足新场景要求——**左列虚拟列表（90 行）+ 说明文本面板，右侧密集色格（嵌套 boundary×16）+ 自绘 HOT 热点**。① **门禁全绿**：shell_rerecord_scroll=0、shell_skip_scroll>0（~23k）、main_dirty_excess_frames=0（基线=非 Spike closed 帧最坏值，+2 带内滞后容差显式声明）、overlay_cycles=3 kinds=3、boundary_skip≈79k、scripted 6/6（壳按钮/密集格/滚动行/浮层按钮/屏障消费/恢复命中）、**pixel 10/10**（§2.7 F0/F5/F6/F8：精确点采样、屏障混合公式 out=0.45·src+0.55·bg、文字区域密度、开合双态恢复）、**Golden 静态掩码 370,680px 逐位一致 diff=0.0000%**（第 2 跑起零容差门禁生效）。② **fps 攻坚**：三处真窗侧优化使 fps 从 51.6→57.7——HUD 快照节流至 ~10Hz（Snapshot 每次对 256 样本环插入排序）、相位钟改 tick 计数（45tps 保守预算，秒制时间线在宿主抖动下跑不完脚本）、**§2.7 三快照合并为 Recover 起点一次批量排空**（原先三次回读各拖 0.1-0.5s 把 avg 拖破线）。③ **随窗修复**：屏障子元素由 1×1 改全窗尺寸（此前视觉变暗层实际不可见，命中消费靠 Entry.Barrier 几何）；浮层状态文本更新移到关闭沿（open 帧保持无示例侧主带脏）；滚动行探针加布局落地守卫。④ metrics-audit 自审 PASS（族 A–J 全族 PRESENT、诚实性 HONEST、vsync_source=true、fallback_ops=0、cpu 非双 0）。⑤ §5 W4 回归全绿。 |
| C4 质量返工立项 | **C4 真窗弃旧重写（模式 2 反攻重关，用户指令「不使用现在的C4代码示例，重新按文档实现」，2026-08-24）**：`ui_wr_c4_shell_overlay` 现有 main.go 判定不满足 §2.7/U21（无任何像素断言、无 Golden 快照留档、无三证据合证；§3.1.1 布局驱动要求仅部分满足），按模式 2 降级 ✅→🔄 并全新重写真窗后重新关闭。 |

| C8 blit 圆角裁剪补齐 | **用户实测回归修复（`render/internal/gpu` + `ui/scene/internal/gpupixel`，2026-08-24）**：pass-ownership 根治收回 clip 禁缓存后，用户实测「绿卡圆角稳态丢失、拖拽窗口才出现」。**根因**——clip 子树恢复缓存后，稳态帧走 blit-only 合成 pass（ADR-016），而 blit 管线当年被故意建成单 bind group 布局（`ensureBlitPipeline`："no clip group"，`RecordBlitDraws` 只绑 group(0)，`ensureImageBlitPipeline` 从未接 `SetClipBindLayout`），shader `rrect_clip_coverage` 是 `return 1.0` 桩（注释"RRect clip for images will use stencil"一直未做）——scissor 组正确携带 `ClipRRect` 参数但 blit 端全链不消费 → 贴图直角；vector replay 帧（resize 触发）走 SDF/convex 管线吃 clip uniform 所以圆角正常，形成"拖拽才出现"假象；旧契约 ClipReplayPixels 只采中心+远外部未采圆角角落故验收漏网。**修复（对齐 Skia 解析式 RRect coverage / Flutter drawRRect 裁剪语义）**——① `textured_quad.wgsl`：桩换 SDF 同款解析式 AA 圆角 coverage（branchless enable-select，naga-safe 仅 sqrt）；② `image_pipeline.go`：blitLayout 按 `clipBindLayout` 有无构建 @group(1)，新增 `blitLayoutHasClip` 失配重建；`RecordBlitDraws` 加 clipBG 参数绑 group(1)；③ `render_session.go`：`ensureImageBlitPipeline` 接 `SetClipBindLayout`，两处 blit pass（encodeBlitOnlyPass/encodeBlitToEncoder）共 6 个调用点经新 helper `blitClipBG` 传组（overlay 组用组 clip、baseLayer 用共享 no-clip）。**验证**——新增契约测试 `TestCompositeFramePacketTextured_BlitzRRectCornerPixels`（四圆角洞必须背景色，修复前 FAIL 直角复现/修复后 PASS）；gpupixel 全包 4 测试绿；真窗 live-probe 四角采样从纯绿(76,204,115)变背景暗色(25,28,33)、scripted pixel 7/7；C8 真窗 PASS（scripted 11/11、excess=0、fps 55.1）；fps 归因：stash 对照基线同跑 54.88 也 FAIL <55（环境贴线抖动，上次验收 55.6 仅高 1%），非本修复回退；`./ui/...` 零回归；`go test ./render/internal/gpu` GPU 环境 111s 全绿。 |
| C8 gpu pass-ownership 根治落地 | **C8 v2.2/v2.3 根因根治完成（`render/internal/gpu` + `ui/scene` + `render`，2026-08-23）**：**根因链**——retained 纹理录制（recordLocalWith → FlushGPUWithView 到离屏纹理）与主合成 pass 共用同一 pending/stash 状态机；mid-frame flush 触发 prepareTarget 把混入的 surface op 连同被录层一起 stash 吞掉（flush 无错但纹理黑）；且 walk 的 clip scissor 段（surface 坐标 30..70）超出 52px 纹理视口 → wgpu 越界 scissor 整 pass 丢弃。**修复（对齐 Skia GrRenderTargetContext / Flutter EntityPass 每 pass 独立命令流语义）**——① `gpu_render_context.go` 新增 `offscreenPass` suspend-and-restore 状态机 + `BeginOffscreenPass/EndOffscreenPass`：进入时把主 pass 全部挂起状态（pending 七类队列、convexMesh scratch、clip 三态、scissorSegments、批封印、frameRendered/lastView LoadOp 跟踪、flushedPendingToData、deferred advanced layers、layerReleaseHold）整体换出，子 pass 在干净流上录完 flush，End 原样还原（非 stash 合并，glyph quads 按 opt22 纪律重挂自有存储）；② `render.Context.BeginOffscreenPass()` 公开门面 + `offscreenPassDepth` 门——pass 期间 `setGPUClipRect`/`applyClipToPaint` 失活（caller 的 canvas clip 不泄漏进子 pass，clipStack 本体不动，restore 后重放 GPU clip）；③ `ui/scene/textured.go` recordWith/recordLocalWith 切到旁路 API（defer restore）；④ **收回 v2.3 规避**：nonTranslating 删 ClipRectLayer/ClipRRectLayer 两 case（clip 子树恢复 bounds 纹理缓存，稳态帧纯 blit ReplayedOps=0，视口列表 blit 吞吐收回）；⑤ **回滚 v2.2 Flush 无条件 unstash 补丁**至 F1 原条件（stash 只服务原 F1 场景，diff 零删除行）；⑥ `render_session.go` applyGroupScissor 对 group rect clamp 到视口（越界 clip 不再丢整 pass）。**验证**——GPU 像素契约测试三连绿（ClipReplayPixels 改回缓存断言 tex.Has==true/双帧 ReplayedOps==0/像素绿+外部黑；TranslateBlitPixels；TransformPivot 双路径中心一致）；旁路状态机白盒测试 2 个 PASS（挂起还原逐字段校验 + 子 pass 无法消费主流命令）；`go test ./render/internal/gpu` 双环境全绿（GPU 105s）；`./ui/...` 双环境零回归；render 包失败集合与 HEAD 基线 IDENTICAL（含 S69 抖动用例集逐一对照相同，仅数值噪声）；C8 真窗 PASS（scripted 11/11、pixel 7/7、main_dirty excess=0、fps 55.6）；live-probe 五目标像素实测齐全（A红217,77,71/B蓝71,115,224/C绿76,204,115/E橙229,191,76/G黄242,217,89）；C4 PASS（shell_rr=0、skip=23584、excess=0、fps 59.5）；R4 PASS（dmg_avg=0.060、skip=28320、fps 60.1）；R13 PASS（probes=8 ok=8）。 |
| C8 gpu pass-ownership 根治立项 | **立项（用户批准，2026-08-23）**：C8 v2.2/v2.3 查明的根因是 `render/internal/gpu` 的 pending/stash 状态机把「录制子 pass」（PictureTextureCache.recordLocalWith → FlushGPUWithView 到离屏纹理）与主合成 pass 共用同一 pendingTarget/presentStash——mid-frame flush 触发 prepareTarget 把整批 surface op 连同被录层一起 stash 吞掉（flush 无错但纹理黑）。v2.3 以 scene 层规避（clip 子树 noTextureCache 向量回放，视口列表性能代价待收回）；本项按 Skia GrRenderTargetContext / Flutter EntityPass「每 pass 独立命令流」标准根治：① gpu 层新增旁路 pass API（suspend-and-restore 整体挂起还原，非 stash 合并）；② recordLocalWith/recordWith 切到旁路 API；③ 收回 v2.3 规避（ClipRectLayer/ClipRRectLayer 恢复可缓存）+ 回滚 v2.2 Flush 无条件 unstash 补丁至 F1 原语义；④ 回归验收：stash 对照 IDENTICAL、ui 双环境、render 失败集合对照、C8/C4/R4/R13 真窗 + live-probe 五目标。完成后本表补验收记录。 |
| C8 重实现 v2 | **C8 判定失败回滚后重审重实现（`ui_wr_c8_hit_overlay_xf` 1200×800·15s·GPU PASS 3 连跑，fps 55.6–57.8、scripted 11/11、pixel 7/7）**：首版关闭后用户实测发现 XF-A/XF-B 消失、黄图不显示 → 判定实现失败回滚。**wr-engine 重审定位三个引擎洞并修复**：① **洞 A（R6/R4 层，C8 反复失败根因）**——retained 纹理路径对旋转/缩放子树无条件做 bounds 纹理录制 + blit，而 `recordLocalWith` 只取消平移项 → 双重变换/裁碎（首帧角度≈0 掩盖）。修复：`ui/scene/textured.go` 层树构建期 `nonTranslating()` 标记（旋转或 scale≠1 子树 noTextureCache），phase1 跳过纹理录制、phase2 向量回放 + 四角 CTM damage rect；clip 链保持可缓存（CTM 仍纯平移，recordLocal 精确，保住 C4 视口列表纹理预算）；单测 3 个（旋转/缩放禁缓存+几何正确、纯平移仍缓存）。② **洞 B（R13 命中层）**——`RenderClipRRect.HitTest` 用未取整 AABB，角洞误命中；改精确圆角包含（AABB 快判→角限象素归一化单位圆），对齐 Flutter RRect.contains（geometry.dart:1731）；旧测试两用例按新语义修正 + 新增 corner hole 用例。③ **洞 C（R8 浮层层）**——刚 Insert 的 entry 首帧即可命中（几何未落位）→ 加 `laidOut` 门控（Insert false / Layout true / HitTest 跳过），对齐 Flutter _RenderDeferredLayoutBox；Remove 清 id=0 死亡契约（re-Insert 拒绝，防幽灵命中）；Barrier-only entry 同门控；新增 4 单测。④ **基建**——`PipelineApp.SnapshotAsync(fn)`：UI 线程排队快照闭包、raster 线程帧尾执行（有界轮询等待），GPU readback 不再与 present 竞争 swapchain（直接 UI 线程 Image() 会 abort wgpu "invalid texture for image copy texture"）。⑤ **C8 窗重建（首个 U21 §2.7 全落地窗）**——删自造 highlight/恢复逻辑（污染采样根因）；12 探针结构保留；像素断言进 EvaluateGates（pixel_probes_ok==total，容差 ≤8/255，旋转/缩放目标采变换不变中心点 F2/F3，must-miss 断言非目标色）；快照双张留档（稳态+恢复，raster 线程完整重画路径 SavePNG，验证无临时态残留）。⑥ **门禁全绿**：fps_wall=57.6/57.8/55.6≥55、p95≤17.5、scripted_ok=11==11×3 连跑、pixel 7/7×3 连跑（可复现性）、main_dirty_excess=0、hit_restored=1、快照双张内容一致。回归：./ui/... 17 包零回归；R13 probes=8 ok=8、R8 excess=0、C4 shell_rr=0、R4 dmg_avg=0.056 mode=damage_union 全部零回退。**v2.1 追加（用户实测"C/E 平时不可见、resize 后才出现"）**——④′ **洞 D（gpu 层，真正根因）**：`prepareTarget` 在 pending 为 View-nil 时把后续 surface ops 收进 `stashPresentPending`（F1 机制），但 `Flush` 的恢复条件（原 1873 行 else-if）在"stash 后又设置了非 nil pendingTarget"时不命中 → stash 的 blits 永远滞留，retained 稳态帧内容丢失；resize 触发 postResizeFull 全量 vector 重画（不走缓存）所以内容出现。修复：surface flush 且非 layer-self-flush 时一律 unstash（F1 语义不变：layer-self-flush 仍 defer，nil-view readback 仍恢复）。⑤′ **GPU 像素契约测试**（ui/scene/textured_gpupixel_test.go，U21 首个 GPU 级单测）：真实 recordLocalWith→blit 链路 + 稳态纯 blit 帧，断言 clip 内绿色/外部背景色——修复前 FAIL（黑屏复现）、修复后 PASS。失败集合 stash 对照 IDENTICAL（scene/render 均零新增）。**v2.3 追加（用户二次实测"C/E 从未显示"）**：live-probe（SnapshotAsync 内重跑真实 CompositeFramePacketTextured 到离屏并回读）证实 gpu stash 修复只救回部分目标，**ClipRRect 子树的缓存纹理录制本身在真窗环境录出全黑**（RECDbg33：flush err=nil 但立即回读 rgb=0,0,0；pre-Replay pending=1 证明队列混入他层 op，record 的 mid-frame flush 触发 prepareTarget stash 把整批 op 连同 C 自己吞掉）。修复（scene 层规避）：`nonTranslating` 将 ClipRectLayer/ClipRRectLayer 一并纳入 noTextureCache——clip 子树每帧向量回放，正确性优先；视口列表性能代价待 gpu pass-ownership 重构后再收回。验证：live-probe 五目标全部可见（C绿2295px/E橙5140px）；GPU 契约测试迁至 `ui/scene/internal/gpupixel`（独立测试包，避免 render/gpu init 污染 scene 包内其他测试的环境假设），ClipReplayPixels+TranslateBlitPixels 双 PASS；ui 18 包双环境（有/无 GPU）零回归；C4 用关闭时长 15s 重跑 PASS（10s 时长下 Recover 未完成属窗口脚本时序非引擎问题）。**v2.4 追加（用户实测"XF-A 默认绕左上角转圈、resize 后变绕中心"）**：两条渲染路径的旋转 pivot 不一致——`RenderTransform.Paint`（vector，resize 后强制重画走此路）与 `HitTest.inverseMapPoint` 都绕子树中心（size/2），而 scene 层 `TransformLayer` 应用（`pushCompositeCTM`/`compositeLayer`）绕 layer 原点即左上角。修复：`TransformLayer` 增 `CX,CY` pivot 字段（`SetPivot`），`BuildLayerTree` 从 `RenderTransform.size` 填入中心，textured/composite 两处应用点统一 `Translate(CX,CY)→Rotate→Translate(-CX,-CY)`。新契约测试 `TestTransformPivot_CenterMatchesVectorPaint`（gpupixel 包）：同一棵树 vector 与 retained 两路径合成后中心像素必须同为红——双向验证（去 SetPivot FAIL / 加 PASS）。ui 18 包零回归；C8 真窗 PASS。 |
| C8 首次关闭 |
| C8 首次关闭 |
| C8 首次关闭 |
...
| clip 下 bounds 纹理降级 + EndFrame 竞态驱逐洞 | **retained 路径两洞随 C4 第二轮暴露并修复（ui/scene，经用户确认方向后定点修）**：① **clip 下全窗纹理**——viewport 引入 ClipRect 层后 restrictive=1，子树全部退化 recordWith 全窗纹理（1200×800×4×2slot≈7.5MB/层），200 层打爆显存（wgpu "Not enough memory left" 34 次），分配失败→条目丢弃→下帧再重录再失败循环（SHELLDBG 实证 dirty=false 的 !tex.Has() 重录）。**修复**——recordLocal 条件从 `restr==0 && !b.Empty()` 放宽为 `!b.Empty()`（裁剪语义由 composite 时 scissor 保证，与全窗纹理视觉等价；纯文本层 measureTextBounds 同步放开），显存需求降至 <100MB。② **EndFrame usedNow 竞态驱逐**——usedNow 由 raster 线程上一帧 composite 填充，UI 线程每帧 EndFrame 按 usedNow 驱逐，UI 领先 raster 时全部条目被误判未用整带逐出（SHELLDBG 每 ~61 行一次 11 层突发重录实证）。**修复**——EndFrame 不再驱逐（只重置计数器），驱逐唯一路径改为 evictForNew 容量 LRU（oldest lastUse，stamp 时钟 BeginFrame 每帧推进）；allocEntry 首分配失败清理空壳条目。配套单测更新（TestTextureRing_EvictionReleasesBothSlots 改容量驱逐语义）。 |
| viewport 重绘边界 + 层树补全 | **RenderViewport 设为 RepaintBoundary + 层树 viewport 分支补全（ui/rendering，C4 暴露）**：① viewport 此前不是重绘边界，SetScrollOffset 脏标记沿祖先链冒泡到根 → 根全窗背景纹理每帧重录（damage_ratio 钉在 1.0、每帧 ~45 次无谓纹理记录）。修复：`NewRenderViewport` 加 `SetRepaintBoundary(true)`（Flutter 对齐：scroll view 自成裁剪层+重绘边界）。② 层树构建完全不识别 RenderViewport（滚出视口内容不裁剪且被记成全高纹理）：`appendNode` 新增 viewport 分支——PushBoundary("viewport")+PushClipRect(0,0,w,h)，BoundaryLayer.Shell 标记透传（v.IsShellBoundary()）。③ overlay band 脏标永不消费：纹理路径不跑 State.Paint（清 e.NeedsPaint 的唯一处），BuildOverlayBand 每帧把浮层全部 boundary/picture 标脏 → 构建末尾统一 `e.NeedsPaint=false`（Insert 重新置脏）。④ BoundaryCache.Store 空 Picture 不建条目（只含 viewport 的外层边界自内容为空，避免每帧空存储计重录）。 |
| R21 首次关闭 | **R21 壳/内容分层 独立真窗首次关闭（`ui_wr_r21_shell` 1200×800·15s·GPU PASS，2 连跑确定性 59.92/59.82 fps）**：① **能力侧就绪**（W2 R21 Bug2 修复时已回流）——`SetShellBoundary` 壳分区计数（ShellRerecord/ShellSkip 分账）+ `LastShellBoundaryFrame` 观测口；真窗此前已存在但未走 wr-close 关闭（§2 ⬜）。② **场景（U17 达标）**——顶栏壳层（标题+双按钮+字号样张，整带一个 SetShellBoundary）+ 60 项变高虚拟列表持续滚动（Steady 慢/Spike 8px/帧快/Recover）+ 静态密集区（4×4 色格+8 标签）+ HOT live paint；相位芯片/HUD 在壳 boundary 外（每帧自脏不破坏分层）。③ **升维补齐（本次 wr-close）**——Extra 补 U20 六维（impl_correctness/dirty/cache/edge/fail/visible）；EvaluateGates 补 §2.2 族 A 硬线（RequirePersistentFPS + MinFPSWall=55 + MaxP95Ms=22）；README 补六维表+Gates 全族线+RSS 斜率语义注。④ **门禁**——shell_rerecord_scroll=0（滚动全程壳零重录，resize 帧 3 帧沉降排除防 WM 干扰）、shell_skip_scroll=2274>0（壳缓存回放命中）、fps_interval=59.92/59.82≥55、p95=17.37/17.24≤22、hitch=0、policy=full_paint（正确性窗如实）、vsync_source=true、cpu_ui/cpu_raster 非双 0；RSS 斜率 15s 窗处启动均衡爬坡段属预期（平台化证据见 R8 场景化长跑 600s 实测 167KB/min），Extra 注明语义。§2 R21 ⬜→✅；§5 W4 行 R21✅ → **单能力窗全绿（R8✅ R21✅），W4 待 C4 组合窗**。 |
| R8 首次关闭 | **R8 Overlay 独立合成 独立真窗首次关闭（`ui_wr_r8_overlay` 1200×800·15s·GPU PASS，2 连跑确定性）**：① **方案 C（Flutter 对齐姿态+观测）**——retained 策略跑（对齐 Flutter 只画脏区；full_paint 姿态下「主树不重绘」命题不成立）；引擎补脏集分带观测：`FramePacket.OverlayDirtyLayerIDs`（浮层带脏 id 镜像列，`ui/scene/packet.go` + `CloneShallow` 复制）、`AttachToPacket` 分列记录（合并集行为不变，`ui/overlay/state.go`）、`LastOverlayBandFrame()` 每帧主带/浮层脏计数（挂接前采主带计数避免合并污染，`ui/embedder/pipeline_app.go`）；② **场景（U17 全达标）**——wrkit 壳（TopBar+Legend 5 行+LiveHUD）、主树静态密集 4×4 色格（各自 boundary）+8 标签、HOT live paint 每帧变色**全程活跃**（用户选定，含浮层期）、相位脚本 Steady（无浮层，1–5s 定基线）→ Spike（5–10s 开浮层：全屏屏障 45% 黑+中央面板+内嵌按钮+右上 menu 列表三层叠）→ Recover（10s+ Clear 移除全部）；③ **门禁**——`main_dirty_excess_frames=0`（浮层开着的主带脏 id 计数不超稳态基线；基线=稳态窗口 MAX 含 HOT+HUD 合法自脏，单帧采样会误报已修正）、`overlay_entries_on_open=3≥1`（浮层自己被标脏并绘制）、`present_policy=retained`、fps_interval=58.08–58.71≥55、p95=17.45–17.92≤22、hitch=4–8/min、vsync_source=true、fallback_ops=0、cpu_ui=0.46/cpu_raster=40.7 非双 0；④ metrics-audit 串审 PASS（族 A–H 字段 PRESENT、vsync/fps/policy HONEST、阈值 AT_DEFAULT、观测 CONSISTENT、场景 EARNED——持续 tick+关闭用 15s）。⑤ **随窗暴露并修复引擎洞（见下条：session 纹理每帧重建）**。§2 R8 ⬜→✅；§5 W4 ⬜→🔄（待 R21+C4）。 |
| R8 场景化长跑 | **R8 真窗实际应用场景化 + 长跑衰减验证（`ui_wr_r8_overlay` v2，用户报「弹窗时间久了帧率下降」触发）**：① **场景升级**——Spike 相位从「单开一次浮层」改为**持续循环开关**（开 2s/关 1.5s），三种浮层轮换（模态对话框+菜单叠层 / 下拉菜单 / Tooltip 提示条），每次打开构建**全新 entry+widget**（真实应用模式：新弹窗=新 RenderObject=新纹理），尺寸位置逐次变化；长跑时 Spike 占 ~90% 时长；HUD 增 `cyc=N OPEN/closed` 实时循环计数。② **长跑验证（GPU NVIDIA Vulkan，2026-08-22）**——150s：37 循环 fps=59.29 p95=17.7 无衰减；**600s 终验：152 次开关循环、35329 presents，fps=58.88 全程平稳、p50=16.7/p95=17.2/p99=17.3、hitch 16/min（循环切换瞬间）、RSS 平衡后斜率仅 167 KB/min 且 peak(344MB)≈end(331MB)、main_dirty_excess=0 全程**——无帧率衰减、无泄漏；确认用户所见衰减即 session 纹理重建洞（本会话已修，见上行），修复后循环压测不再复现。③ RSS 门禁语义：启动一次性爬坡（Go 堆+GPU 驱动）到 ~330MB 平台后应稳定，README 注明平衡后斜率受控标准。§2 R8 ✅ 维持；门禁全绿不回退。 | **retained 稳态共享渲染会话 depth/stencil 纹理随离屏层录制尺寸翻转每帧销毁重建修复（render/internal/gpu，R8 真窗暴露，经用户确认修）**：**根因**——retained 帧内多个 PictureLayer 录进 bounds 尺寸的离屏纹理（主树 1200×800 / Body 面板 906×658 / HOT 32×32），每次录制走 `FlushGPUWithView(view,w,h)` → `RenderFrameGrouped` → `ensureTexturesForView(activeView≠nil)` → `ensureSurfaceTextures`：尺寸与上次不同即 `destroyTextures()`+重建 depth/stencil（sc==1 无 MSAA，仅 stencil），一帧内三次尺寸交替 → **每帧 3 轮 GPU 纹理销毁重建**；15s 日志 "created surface textures" 4293 条（三尺寸各 ~762 = 每帧一轮）。fps 50.8<55 门禁线、p95 拖尾、RSS 斜率虚高。**影响所有 retained 窗**（R4/C2 等，只是场景简单未破线）。**修复**——`gpu_textures.go` 新增 sc==1 surface 路径的**尺寸桶 stencil 池**（`stencilPool map[(w,h)]*pooledStencil`，LRU 上限 8，命中直接复用视图；淘汰与会话销毁走既有 `releaseOrRetire`→`pendingTexRetire` 延迟释放通道，防在飞命令引用悬空）；`ensureTexturesForView` 的 drainQueue 仅在真实重建时发生，池命中跳过；MSAA（sc>1）路径不变。**实现教训（两轮返工）**——第一版池分支误挂进离屏 `ensureTextures`（非热路径）且 `destroyTextures` 清池自败（每帧翻转照样清光池）→ fps 反降 32.9；第二版挂对 `ensureSurfaceTextures` 且池存活跨 `destroyTextures`（仅会话 `Destroy()` 显式 `ClearStencilPool`）。**验证（GPU NVIDIA Vulkan，2026-08-22）**——纹理创建日志 4293→**0**；R8 真窗 fps 50.8→58.1–58.7（门禁 ≥55 过）、p50 16.8ms、p95 17.5–17.9≤22、hitch 45→4–8；R4 retained 真窗回归 fps=59.6、dmg_avg=0.058、skip=52805 零回退；`go test ./render/internal/gpu` 全绿 + `go test ./ui/...` 除另一线遗留 `textured_coord_test.go` 编译错（与本改动无关，stash 验证）外全绿。 |
| **W2 整波收口** | **W2 全部单能力窗 + 组合窗 GPU PASS，§5 W2 ⬜→✅**：R4✅（层 Composite Present）、R4b✅（DirtyLayerID+多 damage）、R5✅v2（Picture 录/回放质量返工）、R11✅（DPR/尺寸缓存失效）、R13✅（Hit≡绘）、R18✅（SaveLayer+预算）+ 组合 C2✅（R3+R4+R4b+R5 retained 整场景）、C7✅（R11+R19+R3 尺寸/DPR 失效波+1px）。引擎洞修复（嵌套 boundary 祖先误判重录，见下行）随 C2 暴露并修复，R3/R4b/R4 GPU 真窗回归零回退。90 天计划 45–75d W2+C2 达成，下一步 W3（R7/R7b/R10+C3）或 W2 收口后直接 W4。默认 Present 仍 full_paint 至 W6。 |
| C7 首次关闭 | **C7 R11+R19+R3 组合窗首次关闭（`ui_wr_c7_resize_dpr` 1200×800·15s·GPU PASS，一次通过）**：① **场景（§3.1.2 C7 行全达标）**——R11 两次失效波：RESIZE-A（~3s Align 布局驱动 190×140→250×170 + MarkNeedsLayout → 一波 rerecord）+ DPR-B（~6s `InvalidateBoundaryCache` 全量失效 → 一波 rerecord）；R19 SnapLine/SnapRect 1px 网格（snapped crisp vs raw 对比，DPR 变化 grid 重画，收尾 SnapCoord 网格对齐断言）；R3 3 层嵌套 boundary（outer→mid→inner + 内层 hot 每帧变，内脏不外溢 outer/mid skip）；右上 3×3 静态色格 + HOT live paint（非 boundary，稳态 rerecord 噪声为零）；**相位脚本**——Steady（0–3s）HOT+R3 内脏每帧变 / Resize-Spike（3–3.8s）尺寸波+R19 重画+R3 内脏暂停（波窗口干净）/ DPR-Spike（6–6.8s）全量失效+R19 重画 / Recover（8.5s+）回稳 skip 累积；**② 门禁（GateOptions=∪(各 R)）**：cache_invalidations=1≥1、resize_wave_rerecord=1≥1（尺寸波恰一次重录）、dpr_wave_rerecord=19≥1（全量失效 19 boundary 全重录）、steady_rr_frame=0（回 skip，R3 内脏 3–7s 暂停保证波检测干净）、snap_ok=true（SnapCoord(1.3,dpr) 网格整数倍）、boundary_skip=16420≥3、boundary_rerecord=699≥2、present_policy=full_paint（正确性窗，damage=1 不冒充 retained）、fps_interval=59.99≥55、p95=17.5≤22、vsync_source=true、hitch=0、cpu_ui=0.38/cpu_raster=26.5 非双 0、slope_gate=off（15s 正确性窗 §2.2.4，R4b/R5/R11 先例）；metrics-audit 串审全 PASS（10 族 PRESENT、诚实性 HONEST、阈值 AT_DEFAULT+steadyRR 更严、观测 CONSISTENT、降画质 EARNED）。§3 C7 ⬜→✅；§5 W2 组合窗全绿 → W2 整波 ✅。 |
| C2 首次关闭 | **C2 R3+R4+R4b+R5 组合窗首次关闭（`ui_wr_c2_retained_scene` 1200×800·15s·GPU PASS）**：① **场景（§3.1.2 C2 行全达标）**——retained 整场景：R3 嵌套 boundary 3 层（outer/mid/inner + 各层静文/色块，内脏 hot3 每帧变、Spike 外层 Background 翻转整巢重录）+ R4 5×3 静态色格阵（全 boundary）+ 局部动画区 hotA（Align 布局驱动）+ R4b 对角两热点（hot1 右上/hot2 左下同帧变脏→两独立 dirty layer id，中间大面积静态不重绘）+ R5 Picture 直绘≡回放同构图并排（8 ops：FillPath/StrokePath/FillRect/StrokeRect/2×DrawImage/2×DrawString）；Legend 8 行、PhaseClock 三阶段同时驱动多能力、HUD 实时 skip/rr/sum/mode/ids/multi/ops；**② 门禁（GateOptions=∪(各 R)）**：present_policy=retained、boundary_skip=38004≥3、boundary_rerecord=4569≥2、dirty_layer_id_max=18≥2、damage_multi_frames=747≥1、picture_op_count=8≥3、damage_ratio_sum=0.187≤0.35（**sum of rects 口径**，R4b 先例；union bbox 0.59 为脏层分散几何必然，Extra 双口径注明）、present_mode=damage_multi 非 full、fps_interval=57.9≥55、p95=17.3≤22、vsync_source=fallback 如实、fallback=0、cpu_ui=0.53/cpu_raster=40.7 非双 0、slope_gate=off（15s 正确性窗 §2.2.4，R4b/R5/R11 先例）；**③ 引擎洞修复**（见下条）后 dirty ids 14→8、raster 13.6→7.9ms；metrics-audit 串审全 PASS（10 族 PRESENT、诚实性 HONEST、阈值 AT_DEFAULT/sum 更严、观测 CONSISTENT、降画质 EARNED）。§3 C2 ⬜→✅；§5 W2 行 C2✅（整波仍 ⬜ 待 C7）。 |
| 嵌套 boundary 祖先误判重录（引擎洞） | **SubtreeNeedsPaint 穿透子 boundary 导致嵌套祖先被迫每帧 re-record 修复（ui/rendering，C2 集成窗暴露）**：**根因**——`BuildLayerTree` 判定 boundary 脏用 `n.NeedsPaint() || SubtreeNeedsPaint(n)`，而 `SubtreeNeedsPaint` 递归穿透子树内层 boundary：内层 boundary 每帧变 → 所有祖先 boundary 每帧进 DirtyLayerIDs → 祖先被迫 re-record（重执行 paint + 重分配 offscreen 纹理），即使其内容未变（只应 re-composite blit 新内层纹理，Flutter RepaintBoundary 语义；`BoundaryCache.tryReplay` 注释本已声明 "outer can skip while inner rerecords"）。R3 是 full_paint 未暴露、R4/R4b 热点无 boundary 祖先未暴露，**C2 首次暴露**（retained+嵌套+内层脏）：每帧 14 个 dirty ids（预期 6）、RSS 15s 涨 180MB、raster 13.6ms。**修复**——`ui/rendering/layer_build.go` 新增 `layerSubtreeNeedsPaint`（遇 boundary 子节点停止下探，子 boundary 脏产生自己的 DirtyLayerID），`BuildLayerTree` 三处判定改用它；`SubtreeNeedsPaint` 保持穿透（Paint walk 粗筛必须穿透才能走进子 boundary 检查，`TestRenderTransform_SetRotationPaintOnly` 即此路径）。**验证**——新单测 `TestBuildLayerTree_NestedBoundaryInnerDirtyOnly`（内脏 → ids 恰为 [inner boundary + 内容 picture]，外层不在集合；外层自脏仍重录）PASS；ui/rendering + ui/scene/embedder/scheduler/overlay + `go test ./ui/...` 全绿（含既有 render_matrix_test DirtyLayerIDs ∝ 脏集测试）；GPU 真窗回归 R3/R4b/R4 5s 全绿（fps≈59 零回归）；C2 重跑 dirty ids 14→8、raster 13.6→7.9ms、boundary_skip=38004。 |
| 最大化切回高度（wayland） | **最大化窗口切到其它窗口再切回，底部少 32px（=标题栏高）修复（ui/platform/wayland_linux.go，引擎洞）**：**复现**——`tmp_resize_diag` DIAG_STATE 最大化后，经自写 XWayland 焦点工具（`_NET_ACTIVE_WINDOW`+`XSetInputFocus`，GNOME focus-stealing prevention 下仍可抢焦点）切走再切回，`GPUI_WL_TRACE_CFG=1` 显示 mutter 在激活/失焦翻转时交替发两种 configure 高度：**工作区全高 1053**（frame-rect 路径）与**内容高 1021**（客户端声明的 window geometry 生效后的尺寸）；旧 `wlTopConfigure` 对任何 maximized configure 无条件 `hi -= csdTitleBarHeight(32)` → 内容高 configure 再减一次 → 989 → 底部少 32px。**修复**=`wlWin.maxContentH` 锚定当前最大化会话的内容高：configure 高度**高于锚点**=工作区全高（减 32 后更新锚点）；**等于锚点**=已是内容高（不减，dedup 保持尺寸不闪烁）；**宽度变化**（显示器/工作区切换）与**退出最大化态**重置锚点。**验证（GPU NVIDIA Vulkan，2026-08-20）**：切走/切回 configure 1021 不再产生 989 EventResize（修复前每次切走切回各闪一次错误尺寸）、初始最大化 1053→1021 正确、Unmaximize 400×400 / Fullscreen 1920×1080（不减 32）/ Minimize 全路径无回归；新增白盒单测 3 个（内容高锚定含工作区切换重置 / 最大化→还原→再最大化 / 全屏不缩不加锚）；ui/platform 18 测试文件全 PASS。详见 `docs/ENGINE_WAYLAND_WINDOW_STANDARD.md` §8 v1.14。 |
| 事件循环忙等空转（tmp_resize_diag CPU 高占用） | **ticker 常开 + ModePersistent 下事件循环忙等空转根因修复（ui/embedder + ui/scheduler，引擎洞）**：**根因**——`PipelineApp.ScheduleFrame`（及旧版 `App.ScheduleFrame`）无条件 `host.WakeUp()` 写 wake pipe 字节；ticker 驱动的持久模式里每轮循环都调 `ScheduleFrame`（ticker 回调 + 帧后 keep-scheduling 两处），而 X11/Wayland `WaitEvents` 对 wake 管道可读**立即返回**（不等剩余 timeout）→ 事件循环永远睡不满 16ms 节奏 timeout，ticker 回调（及其携带的 `Metrics().Snapshot()`/`percentilesLocked` 排序）以空转速率运行，帧渲染门禁虽正确 60fps 但 CPU 白烧。实测（pprof，tmp_resize_diag，2026-08-20）：`percentilesLocked` 33.5% + `Syscall6`（wake 管道写/读）23% + `WakeUp` 9% + `diagScene.Tick` 5%，进程 CPU 113%/核（用户 `top` 观测 28–30% = 4 核归一化）。**修复**——① embedder：`ScheduleFrame` 在 `ModePersistent` 下不再 `WakeUp`（该模式 `WaitTimeout` 已 ≤animTick，循环自行唤醒；IDLE/TRANSIENT 无限阻塞仍需跨线程打断，语义不变）；② scheduler：**软件间隔地板无条件化**——原 `FrameDue` 的 fresh-vsync 分支（每 stamp 一次）在合成器通知缓慢时把动画钉在通知速率之下（实测 wl_surface.frame 回调 ~2× 延迟、stamp≈renders/2，once-per-stamp 门把稳态钉在 37fps 的次优平衡点）；现改为 stamp 快（interval ≤ animTick，如 120Hz vblank）才每 stamp 放行，慢/批处理通知由 16ms 软件地板覆盖（快慢双态均不双重渲染）；③ scheduler：`WaitTimeout` 持久模式返回「距下一帧边界剩余时间」——循环在事件噪声早醒后重算剩余、精确在 pacing 截止点醒来，不再把 16ms 地板量化到事件到达边界。**验证（GPU NVIDIA Vulkan，2026-08-20）**：`tmp_resize_diag` 稳态 CPU 113%→21%/核、fps 稳定 60（2399 presents/40s）、lastraster 2–5ms、管道 0/2 不积压；DIAG_STORM=1 风暴全程 59.6–60.5fps（内容跟手不回归）；渲染正确性 GPU 回读快照验证（标题/侧栏/图表/卡片/HOT 相位色/状态栏文字全在）；ui/scheduler 全量 + ui/embedder 全量 PASS，新增 4 个 pacing 单测（慢 stamp 不压地板/快 stamp 每 stamp 一次/陈旧 stamp 地板接管/WaitTimeout 指向边界，含 -race）；apidoc 绿。render 公开 API 无增删。详见 `docs/ENGINE_FRAME_PRESENT_STANDARD.md` §9。 |
| API目录同步 | **帧节奏正统化（块1/2/3，跨 ui/platform + ui/scheduler + ui/embedder + render，详见 `docs/ENGINE_FRAME_PRESENT_STANDARD.md`）**：①**按需渲染**（ui/scheduler）：vsync listener 去掉 `ScheduleFrame`（只 stamp 节拍），渲染需求唯一来自事件/ticker，idle 零渲染；②**合成器通知驱动**（ui/platform + ui/embedder + ui/scheduler）：`FrameNotifier.RequestFrameNotify()` 统一抽象（Wayland `wl_surface.frame` 回调 / X11 XPresent `PresentCompleteNotify`），present 成功后请求，通知经 `EventFramePresented` → `scheduler.NoteFramePresented()`（只 stamp）；scheduler 检测 `FrameNotifier` 不启 DRM listener（单节拍源）；③**present 无阻塞按平台**（render/present_target.go + gpu/webgpu/swapchain.go）：Wayland `fixedNoVsync` → `SetVsync` no-op、初始 `SetPreferFifoRelaxed()`（新增，FifoRelaxed→Mailbox→Immediate 不含 Fifo）；X11 稳态 Fifo + 风暴期 SetVsync 不变。render 公开 API 无增删（`SetVsync` 语义在 Wayland 平台变 no-op，目录 §7.1/§8 已同步说明）。验证：`TestWaylandFrameNotify` 真窗闭环 PASS、scheduler 三新测试 PASS、ui/platform 13 文件 + ui/embedder 6 文件 + render present + gpu swapchain 全 PASS、apidoc exit=0；真窗复测（`tmp_resize_diag` idle 零渲染 + 风暴期跟手）待用户确认。 |
| API目录同步 | **render 公开 API 新增 `PresentTarget.SetVsync(vsync bool)`（运行时 swapchain present mode 切换，Fifo 稳态 ↔ Mailbox/Immediate 低延迟）**：用于交互式 resize 风暴期内容不跟手修复——Fifo 每帧 present 阻塞 ~16.5ms vblank，内容帧被钉在 60fps，而 CSD 标题栏（独立 subsurface，随 configure 即时更新）平滑 → 内容肉眼滞后。实现=①`gpu/webgpu/swapchain.go`：缓存 `supportedPresentModes`（ConfigureFromCapabilities 时从 caps 快照）+ `PresentModeForVsync(on)`（on=Fifo；off=Mailbox→FifoRelaxed→Immediate 按支持性）+ `SetPresentModeForce(mode)`（同尺寸也强制重配）+ 测试辅助 `SetSupportedPresentModesForTest`；②`render/present_target.go`：`SetVsync` 记录式（同 Resize：UI 线程只置 `vsyncOn`+`vsyncPending`，`applyPendingSwapchainLocked` 在 raster 线程 present 边界应用，经 mu 串行于 BeginFrame/EndFrame；模式切换即 surface 重配 → 缓冲未定义 → 武装 postResizeFull=3 全写预算）；③`ui/embedder/pipeline_app.go`：`EventResize` 记 `lastResizeAt`，首个风暴帧 `SetVsync(false)`（`lowLatency` 原子锁存），平静 200ms（`resizeCalmWindow`）无新 resize 则 `SetVsync(true)` 回 Fifo；帧门禁旁路（`pendingResize`）保证风暴帧不被 vsync 门卡住，本行消除 present 阻塞。目录文档 §3.7/§7.1/§8 同步。验证（GPU NVIDIA Vulkan，2026-08-19）：X11 风暴 `DBG vsync Fifo -> Immediate` → `Immediate -> Fifo` 生效、852 presents 零错、fps 与基线持平；Wayland DIAG_STATE=1 最大化/还原精确无回归。详见 `docs/ENGINE_WAYLAND_WINDOW_STANDARD.md` §8 v1.8。 |
| API目录同步 | **取消最大化/全屏后恢复尺寸被 WM 改写（1468×653）根因修复（Wayland 状态管理，引擎层四文件）**：① **根因**——客户端在 `EventResize` 时同步声明 xdg 窗口几何（`set_window_geometry`），而 wgpu swapchain 仍挂旧尺寸缓冲（重配 ~90ms 滞后、期间继续 attach+commit 旧缓冲）：合成器在「几何 > surface」的提交上缓存**负 frame extents**（`custom_frame_extents = surface − geometry`，仅 geometry 变化或有 ack configure 的提交才重算，稳态提交永不刷新）；取消最大化时 size-hints 往返（400×400 frame→client = 400−1468 = −1068 → floor 0 → client→frame = 0+1468 = **1468×653**）→ configure(1468,653)，连累后续取消全屏。② **修复（对齐 GTK 内容矩形几何语义）**——几何声明从 UI 线程的 configure 事件钩子移走，改为 **renderer swapchain 换尺寸回调**：`render/present_target.go` 新增公开方法 `SetOnSwapchainResized(fn)`（呈现临界区内、swapchain 重配后、同尺寸首缓冲提交前，raster 线程触发）+ `ui/platform/host.go` 新增可选接口 `platform.SurfacePresenter.OnSurfaceResized(w,h)` + `pipeline_app.go` Open 时接线 + `wayland_linux.go` 实现把几何 marshal 挂进 present 自身 wire batch（`wayland_csd_linux.go` 拆出 `setGeometryNoFlush`，不显式 flush 与 xdg 变量不竞态）——**几何与同尺寸缓冲同批到达**，合成器取到非负 extents，恢复尺寸不再被翻转。③ **验证（GPU NVIDIA Vulkan 真窗 `tmp_resize_diag` DIAG_STATE=1，2026-08-19）**：DIAG_SIZE=400/800 Maximize→Unmaximize→Fullscreen→Unfullscreen 全程 configure 400×400 / 800×800（修复前 1468×653）；`WAYLAND_DEBUG` 证实几何声明落地在 swapchain 切换点（~75ms）而非首 configure（~1.8ms），且恢复 configure 后几何随之为 400×400；默认拖拽模式 14s 851 presents exit 0 零回归。render 公开 API 增量已同步目录 §3.7/§8（`SetOnSwapchainResized`）。 |
...
| 曲线暗斑修复 | **stencil cover 内带混合顺序重排——消除曲线/斜线描边沿线黑色锯齿暗斑（render/internal/gpu/stencil_renderer.go + stencil_pipeline.go，examples/ui_render_pelican 真窗发现）**：症状=复杂路径描边（脖颈双描边曲线、翅膀、围巾、车架圆角）沿线散布孤立暗斑像素（鹈鹕区 GPU 617 个 vs 同帧软渲基准 49 个），例：脖颈白芯内 (139,138,133)=白×55% 叠黑底。根因=1x analytic fringe 的**内侧边缘带（aaInnerBandPipeline）原设计在二值 cover 之后以 Replace 混合写预乘部分覆盖色**——背景项 (1-f)·bg 在 cover 写满 α=1 时已被销毁，Replace 只能落 f·C，凡 f<1 处即偏暗；曲率段/斜线的 band quad 子像素相位差使其呈散点状。修复=**内带前置到 cover 之前**：①管线 `cover_aa_inner_band_pipeline` 混合 Replace→PremulSrcOver、stencil Compare Always→NotEqual(0)、新增 PassOp=Zero + WriteMask 0xFF（画完即清该像素 stencil）；②绘制顺序改为 stencil fill → 外带(Equal 0) → 内带(NotEqual 0，SrcOver 叠完整底色后清 stencil) → 二值 cover(NotEqual 0 自动跳过内带已画像素)；d=0 处覆盖率=1.0 与 cover 无缝衔接。**验证（2026-08-25，真机 Wayland/X11 双后端）**：鹈鹕真窗同帧（PELICAN_T=5 冻结）暗斑 617→94（软渲基线 49，剩余为围巾/翅膀边缘单像素 AA 相位差），GPU 独有暗斑 25→逐类检查均为边缘相位差非黑斑；脖颈白芯剖面无暗点；`render/internal/gpu` 全量 PASS；`./gpu/...` 全绿（TestS68_Swapchain_X11_MultiFramePresent 为既有环境失败，stash 验证与本次无关）；`./render/` 失败集与 stash 基线完全一致（零新增：AAProbe_CircleDiag/StrokeWithDash_Rectangle/StrokeString_DifferentFromFill/S3c_M3×2/S69_Contract 均既有）；`./ui/...` 除 scheduler 一例偶发时序（复跑 5/5 绿）外全绿；MSAA4x 下 neck 场景中心线像素=(251,250,241) 无缝。诊断工具沉淀于 examples/ui_render_pelican（PELICAN_T/GOLDEN_PNG/GPUI_BACKEND）。 |
...
| R7 首次关闭 | **R7 虚拟化宿主 独立真窗首次关闭（`ui_wr_r7_virtlist` 1200×800·60s·GPU PASS）**：① **wr-implement 能力审查 + 指标接线**——VirtualList 能力本体已就绪（变高前缀和 index↔offset / ScrollToIndex / CacheExtent / BindCount，单测 9 个），补齐 FrameMetrics 三字段 `bind_count`/`item_count`/`scroll_rerecord`（字段名取自门禁原文）+ `SetVirtualBind`/`SetScrollRerecord`；ui/rendering 包级快照 `LastVirtualBind`/`ScrollRerecordTotal`（对齐 embedder lastBoundaryFrame 模式）；pipeline_app 每帧+warm-up 双点采样；顺带修 `rebindWindowLocked` 双重挂载循环存量 bug（freshlyMountedCount 恒 0、新挂 cell 脏位被误清）。② **引擎洞修复（wr-engine 回流）**：BoundaryCache 无逐出机制——滑出视口 cell 的 Picture 缓存永久驻留（60s 快滑全量 1200 cell 驻留、活堆 85MB 平台、GC 停顿致 hitch 13/min）；加代际淘汰（120 帧未触碰即逐出、每 30 帧一扫，BeginFrame 驱动；静态壳/右栏每帧触碰不受影响；重入 cell 重录一次由 scroll_rerecord 如实计数），hitch 13→0；单测 `TestBoundaryCache_GenerationalEviction` 绿。③ **RSS 斜率语义修正（指标层诚实性）**：两点法 (end-start)/elapsed 把一次性启动爬坡（46→320MB 均衡建立；gctrace 证活堆平台 84–87MB 非泄漏、驱动侧 ~140MB 同为均衡）误报为持续增长（264K KB/min）；改**稳态最小二乘**（ProcessTracker 去采样 0.5s 一点 + 剔除前 25% 热身段拟合），平台期→6167 KB/min，真泄漏/中途泄漏仍原样报出（白盒单测 plateau/leak/mid-run-leak/degenerate 四态绿）。④ **真窗场景（U17 全达标）**：1200 项变高异构列表（文/色块对/文+色签/图+文四类混排，共享 ImageBuf SetImageShared）+ Align 布局驱动滚动条 thumb + 右栏静态密集区（4×4 色格嵌套 boundary + 8 标签）+ 相位脚本 Steady 慢滚/Spike 快滑 2600px/s/Recover 减速 ping-pong；HUD 实时 bind/item/rr + PASS 预览色。⑤ **门禁**：bind=14≪1200（cap64 且 ×10 双线）、fps_interval=60.0≥55、p95=17.6≤22、hitch=0≤5、rss_slope=6167≤30000、presents=3600、vsync_source 如实输出（fallback 不宣称锁 60Hz）、cpu_fallback_ops=0、cpu_pct=30.7% 非双 0、depcheck 两测绿 + 无 cgo；metrics-audit 串审 PASS（EARNED/HONEST/CONSISTENT/AT_DEFAULT/PRESENT）。§2 R7 ⬜→✅；§5 W3 行 R7✅（整波仍 ⬜ 待 R7b/R10/C3）。 |
...
| API目录同步 | **新增 `GPUColorGlyphAccelerator` 接口 + `Context.DrawShapedColorGlyphs`（M5 遗留#2 台阶③颜色图集，2026-09-04）**：`render/accelerator.go` 新接口（已整形彩色字形→RGBA 颜色图集纹理块，学 Skia/Impeller 独立 RGBA 图集）+ `render/text.go` 新方法（GPU 经加速器→`internal/gpu` 颜色引擎布局+RGBA 页上传+颜色管线采样；无加速器/失败时 CPU pixmap 合成，平移 CTM 外走轮廓）+ `ui/rendering/text.go paintCompositeRuns` 彩色段改走新方法（绝对 glyph.X，无需 rebase，颜色经 SetRGBA）。目录文档 §0（类型 94→95、Context 方法 182→183）/§3.8（text.go 15→16）/§4（接口群）/§7.1 同步，状态 🔗（生产接线，GPU 真窗待验）。`go run ./scripts/apidoc` 绿。 |
