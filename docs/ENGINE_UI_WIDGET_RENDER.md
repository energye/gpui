...
| 策略 | 含义 | 默认 |
|------|------|------|
| `full_paint` | 每帧全树 paint | **能力窗显式 opt-in**（§3.27 起）；测试 full_paint 路径的能力窗显式声明 |
| `retained` | 稳态 CompositeOnly + PresentWithAuto damage | **全局默认**（§3.27 起，W6 翻转达成；防 Clear 丢静态靠 LoadOpLoad + damage） |
| `hybrid` | 可选 | — |
...
| 版本 | 说明 |
|------|------|
| **3.30** | **审计 23 项修复闭环（metrics-audit 审出）——P3 组合窗门禁 + P4 架构差距文档化**：**P3**（5/5，全部真窗复跑）：C0 场景增 hotA（Steady/Recover 冻结=R0 静态存活证明，Spike 随 hotB 脉冲）→ PhaseClock 驱动多能力，`policy_note` 诚实声明 C0 集成窗跑 retained（C 不代替 R，R0/R16 full_paint 由 solo 证明）；C1 GateOptions `MinBoundaryMaxDepth` 2→3 与实测嵌套链一致，隔离不变式保持独立 if-FAIL（并集补充非代替）；C2 impl_fail 移除 `skip=0` FAIL 并注明 retained 下 skip=0 是正确语义（静 RB 靠 LoadOpLoad 保像素，与 gate `MinBoundarySkip:0` 自洽）；**C3 关键发现**——加 `RequireFullPaintPolicy` 后真窗 FAIL，暴露 C3 实际跑 **retained**（present_mode=damage_multi、dmg≈0.6、skip=4296 全程正常、无像素擦除），旧注释「FullPaint 安全策略、retained unsafe on LoadOpClear」是过时误导，gate 改 `RequireRetainedPolicy: true` + impl_correctness/impl_cache/impl_dirty/impl_edge/integration_note 五处注释按实测 retained 诚实改写 + 补 covers=["R4","R7","R7b","R10"]；C7 独立 FAIL（tex_rasterize≥1/invalidation≥1/失效后 rerecord/static_cells≥8）加注释说明为 C7 特有集成不变式（GateOptions 并集补充）。**P4**（4/4 架构差距文档化，不修代码）：**F01/F02** Layer COW 未实现——Layer 树每帧重建，静态复用落在 Picture 层（BoundaryCache R3/R3b）；**F05** 滚动 offset 未进 Offset/Transform Layer——VirtualList.Paint 期 `pc.WithOrigin` origin 平移代替（virtual_list.go:460-530），不产生 OffsetLayer 节点，等价于 transform 平移但无层树语义；**F09** 动画 paint-only（全量重绘路径）非 compositor-only（compositor-only 仅 retained 组合窗 R4/R4b/C2 场景生效）；**F15** 遮挡停帧未实现，属 W4+ 分期。逐窗 PASS 证据：C0（6s presents=360 policy=retained fps=59.9）·C1（8s skip=1560 rr=785 cnt=6 depth=3 inner_static_clean=479 outer_side_clean=479）·C2（8s skip=0 dmg=0.2095 ops=8 retained fps=59.9）·C3（8s bind=14/1000 rr=0/frame dirty/img=1 dmg=0.60 fps=59.9）·C7（8s inval=2 skip=7542 rr=36 cnt=19 cells=16）。修复账本 `docs/ENGINE_WR_AUDIT_FIX_PLAN.md` 19/23→23/23。 |
| **3.29** | **B1 GPU picture 缓存推广完成（wr-engine，15 窗全绿）**：按用户批准顺序 C3 → C2 → 剩余 full_paint 窗。**C3 推广中修关键引擎洞**（render/·高）：**录制期 clip 主坐标溢出**——C3 cell 带 viewport clip（R3 色块无 clip 故不炸），录制 replay 时 `setGPUClipRect` 用主 surface 坐标（1200×800）scissor 落到 904×100 录制纹理 → wgpu `Queue.Submit` validation 失败（`submit failed: wgpu: Queue.Submit` 全帧报错·tex=0）；修复：`BeginPictureCache(w,h,ox,oy)` 携带录制原点，`setGPUClipRect` 录制期把 clip bounds 平移 (-ox,-oy) 并与 [0,w]×[0,h] 求交后设局部 scissor；`setGPUClipPath` 录制期禁用深度裁剪；UI 层非 rect clip 拒绝纹理化（`ClipIsRectOnly()` 守卫·path/RRect 内容回退命令级 Replay 语义不变）。附带修复 `BeginPictureCache` scratch pixmap 尺寸（主屏尺寸→录制尺寸）与 `gpuRenderTarget()` 录制期返回录制 view 尺寸（MSAA/text 路径提交一致性·R3 纯 blit 路径曾容忍不匹配故延迟暴露）。**C3 PASS** `RUN_SECONDS=15`：tex_rasterize=23·tex_hit=7911/7941（99.6% skip 走纹理 blit）·skip=7941 保留·fps=59.93·p95=18.36<22·hitch=0·bind_peak=14·cpu_raster 100%→22.9%（blit 代重放命令）。**逐窗接线**：13 个 full_paint 窗（R0/R2/R3b/R5/R7/R7b/R11/R12b/R13/R16/R18/C1/C7）加 `SetPictureTextureCache(true)` + Extra `tex_rasterize/tex_hit/tex_impl` + gate `tex_rasterize≥1`；全部 GPU PASS（RUN_SECONDS=5-18 按窗·tex_rasterize=4-36·tex_hit 占 skip 96-99%·fps≥59.9·原门禁含 fling/bind_peak/rr_max 全保持）；R7 fling=2·bind_peak=21、R7b flings=20·rr_max=1/frame、C7 含 DPR resize 波 34/36、R11 含 invalidation 波 inval=1 全绿。**无触发面窗（与 R4/C2 同理·不接线·留 B1 note 注释）**：C2（retained·skip=0 静态从不重放·用户确认跳过）、R9（每帧强制 MarkNeedsLayout·rr=1196/skip=0）、R10（场景无 RepaintBoundary 重放节点·skip=0）、R19（hairline 无静区·skip=0）——基线对照 git stash 验证接线前后 skip 恒 0，B1 未破坏任何 skip 语义。**回归**：`go build ./...` 绿·`go test ./ui/...` 14 包全绿（G1 depcheck 保持）·R3 复验 3/353/0 err 无回归。文档 §25 同步（见下序 13）。范围：全推广完成，B1 关窗即默认全 off。 |
| **3.28** | **B1 GPU picture 缓存试点闭环（wr-engine，对齐 Flutter RasterCache）**：**四层闭环**（ui/rendering 录纹理 → ui/embedder 开关接线 → render 录制 API → gpu 复用现成 blit 管线**零改动**）。**render 层**（context_picture_cache.go 新增·高）：`BeginPictureCache(w,h)`（flush 主目标防串色 + CPU 回退画 scratch pixmap + GPU 画录制纹理）、`EndPictureCache() *TextureView`（FlushGPUWithView 落纹理）、`TextureView` 包装类型（藏 gpu/context 依赖，保 G1：ui→render→gpu，杜绝 ui→gpu）；`Context.DrawCachedTexture`（context_image.go，经 DrawGPUTextureWithOpacityUV 复用 `GPUTextureDrawCommand` 管线）。**ui/rendering**（boundary_cache.go·低）：`texEnabled` + `tex map[uint64]*texEntry{view,w,h}`、`SetTextureCacheEnabled/TextureCacheEnabled/TextureCacheStats/releaseTextures`、tryReplay 纹理分支（命中→blit；未命中→Begin→Translate(-ox,-oy) replay→End→同帧 blit 无空帧→失败回退命令级 Replay）；Invalidate/Clear 同步 `Release` 纹理；`paint_context.go` **修复 `WithOrigin` 漏拷 `UsePictureTextureCache`**（AbsoluteBox 用子 pc 调 tryReplay 致纹理分支永不进入——bug 定位经插桩实锤 texEnabled=true/use=false 双 walk 后修复）。**ui/embedder**（pipeline_app.go·低）：`SetPictureTextureCache` + `PictureTextureCacheStats`。**试点窗 R3**（full_paint 每帧重放静态 boundary 是正确试版面；R4 retained 静态不重放 skip=0 语义无收益·已注释说明）：GPU PASS `RUN_SECONDS=10`·tex_rasterize=3（3 静态边界各录 1 次）·tex_hit=610+（后续帧全 blit）·skip=613 保留·rr=1178·fps=59.9·全门禁绿；`go test ./ui/...` 全绿（含 G1 depcheck 回归修复）；C0/R9 默认关抽查 PASS 零影响。**范围**：默认关（opt-in per window），未推广 C2/C3；推广与逐窗门禁待用户决策。 |
| **3.27** | **底层收敛 A 组 + B2 retained 默认翻转（wr-engine 对齐 Skia/Flutter）**：**A1 死路径收敛**（render/internal/gpu·高）：`RenderFrame`（render_session.go:847）从 ~95 行完整实现改为 **RenderFrameGrouped 薄包装**（单 ScissorGroup 无 scissor），生产路径单一实现；连带删除两条死实现 `encodeSubmitReadback` 与 `encodeSubmitSurface`（仅被 RenderFrame 与 1 个负向测试调用；Grouped 变体 `encodeSubmitReadbackGrouped`/`encodeSubmitSurfaceGrouped` 已完全覆盖语义）；冗余负向测试 TestReadback_NilTexturesReturnsError 删除（Grouped 同名负向测试已覆盖）。**A2 双实现合并**：`encodeBlitOnlyPass`（自建 encoder+submit）与 `encodeBlitToEncoder`（外部共享 encoder）~120 行逐行重复 → 提取共享核心 **`recordBlitPass`**（loadOp 判定/ADRR-028 damage scissor/overlay scissor/End/frameRendered 追踪唯一实现），两入口只留 encoder 生命周期差异。**B2 retained 全局默认**（ui/embedder/pipeline_app.go·低）：`NewPipelineApp` 默认 `useRetained=true` + metrics `present_policy=retained`（原 full_paint）；每帧兜底同步；`SetPresentPolicy` 注释更新。**语义处理**（示例层声明意图非绕洞）：15 个 W0/W1 语境能力窗（R0/R2/R3/R3b/R5/R7/R7b/R9/R10/R11/R12b/R13/R16/R18/R19/C1/C7）显式 `SetPresentPolicy(full_paint)` 保持其能力路径语境与 gate 阈值不变；C0 冒烟窗跟随引擎新默认（gate `RequireFullPaintPolicy`→`RequireRetainedPolicy`·tick gateOK 同步）；R12 schema 窗随默认。**验证**：24 真窗全部 PASS（RUN_SECONDS=18·59.2-59.7fps·p95<22·0 hitch·policy 字段如实）；retained 组 C0/C2/C3/R4/R4b/R12 全绿（C3 60s 场景 retained 下 bind_peak=14·skip=2818+ 达标）；full_paint 显式组 18 窗全绿（boundary skip/damage 阈值语境不变）；R4 拖动风暴 58.94fps·p95=17.98·hitch=5 与 r4v33 基线一致不倒退；`go test ./ui/... ./render/scene ./render/text ./gpu/webgpu` 全绿（rwgpu SIGSEGV·standardtest 缺模块为既有）。§25.4 诚实点#1 同步更新（默认 compositor=retained）。R 全部维持 ✅v2 |
| **3.26** | **R4/R4b/C2 resize 平滑度与闪烁修复（wr-engine 跨层 + wr-debug 全流程）**：报告「调整窗口大小期间卡顿 + 闪烁」→ 强制复现（xresize 风暴 + 用户手动拖动）→ 分层定位三个缺陷 + 一个拖动期瓶颈。**缺陷 #1（ui/embedder/pipeline_app.go·中风险）**：EventResize 无尺寸检查——position-only / 重复 ConfigureNotify（WM 拖动 ack）触发 `target.Resize + FlushLayout(force) + MarkNeedsPaint + forceFullPresent` 每事件全帧重渲（闪烁 + 丢帧）→ **coalesce**：`lastResizeW/H/Scale/Handled` 同尺寸事件 `break`。**缺陷 #2（gpu/webgpu/swapchain.go·最高风险·用户确认）**：`Swapchain.Resize` 立即 `Configure()` 绕过自身 500ms 节流（S14 soak 崩溃根因的节流补丁 1491813 被 bypass）→ 交互式拖动时 Configure 风暴（每次 surface 重建 15-100ms + 黑帧）→ **Flutter 式三保证**：Resize 只记 `pendingReconfigure` + 尺寸，`BeginFrame` 帧边界执行 `configureIfNeeded()`（幂等·每帧至多一次·suboptHandled==size 时跳过·DiscardTexture 前置），删除 500ms 定时器与 `lastReconfig`。**HUD RB（examples/wrkit/hud.go）**：非 RB HUD 烘焙进根 picture → 每 10Hz HUD 刷新强制根全量重录 → `SetRepaintBoundary(true)`。**cache.Clear 收敛（pipeline_app.go）**：纯尺寸变化不再 Clear（`tryReplay` size 检查自失效·boundary_cache.go:142-146），仅 DPR 变化 Clear——旧 Clear 每事件诱发第二次全量重录波。**拖动期尺寸降采样（pipeline_app.go·本轮新增）**：WM 交互式拖动 = 60Hz 连续不同尺寸流，每尺寸变化一次全帧（40 draws 全量重渲 10-20ms GPU·HD 520 实测）→ 连续流下帧率塌到 ~50fps（hitches=72/20s·p95=33ms）。`pendingResize*` 停放最新尺寸 + `resizeCoalesceWindow=100ms` 应用窗口（事件静默收尾必补最终尺寸·合成器缩放旧帧兜底·Flutter/Chromium 同类），全帧率从 ~5.5/s 压到 ~3/s。**验证**（R4DIAG4/5 临时插桩拿 breakdown 后移除·同机）：稳态 59.4-59.6fps·p95≈17-19<22·hitch=0·**0 全帧**（retained damage 完美）；稀疏风暴 20×80ms 59.6fps·0 hitch（修复前 47-51fps·70-89 hitch）；密集拖动 60×30ms **58.9fps·p95=21.8·hitch=8**（修复前 49.7fps·p95=33.2·hitch=72）；r4v14 风暴 err=<nil>·空闲帧正确·真实尺寸变化=1 全帧（冻结修复保持）。全帧 encMs=16-69ms 中大头为 GPU 全量重渲提交阻塞（paintMs≈0·endFrameMs≈0 已证），非 CPU/编码。**TestS68 修正**：原断言同尺寸 Resize 后 reconfig≥1（stash A/B 证为 pre-existing fail）→ 改 Resize(161,121) 不同尺寸断言·PASS。**回归**：`go test ./ui/embedder ./ui/rendering ./gpu/webgpu` 全绿（rwgpu SIGSEGV·standardtest 缺模块为既有非本修复）。R4/R4b/C2 维持 ✅v2（bug 修复类不降级） |
| **3.25** | **C3 RSS 渐进泄漏修复（wr-debug 全流程）**：报告「C3 bug 修复 RSS 占用 500+M 应几十 M」→ 强制复现 `RUN_SECONDS=60`（RSS 每 5s +40~50MB 线性爬升无平台期·rss_peak=615780KB·rss_slope=601750 KB/min·after_close==peak·性能指标全绿仅内存异常）→ 分层定位（`/proc` RssAnon 88→523MB 非文件映射·`GODEBUG=gctrace` Go live 堆 6→275MB 证 Go 侧对象泄漏非 wgpu C 堆·pprof heap diff +80MB 集中 `render/text.layoutGlyphsUncached`·链路 `LiveHUD.paint→drawHUDText→Context.DrawString→tryGPUGlyphMaskText→GlyphMaskEngine.LayoutText`）→ 缓存插桩 shapeResultCache（entries 329→3128 线性·hit≈12 总·单条 maxlen=1088 glyphs≈89KB 异常大）→ **WR_HUD=0 对照实验 RSS 平线 132-134MB·entries 稳 34 → HUD 为唯一泄漏驱动**→ 最长字符串抓取破案：**`examples/ui_wr_c3_list_scroll/main.go:132` HUD core 字符串 `fmt.Sprintf("bind=%d/%1000 ...")` 非法格式动词 `%1000`**——fmt 按「宽度 1000」处理输出 `%! (int=1000)`+1000 空格 padding，每帧生成 ~1088B 唯一超长字符串，数字占比被空格稀释<20% 致 `IsHighChurnLabel` 高抖动判定失效 → shapeResultCache 每帧全 miss 新增 ~89KB 条目（60s 3103 条≈276MB 与 RSS 增量吻合，4096 上限 60s 内未触发故线性无平台期）。**修复**：`%1000`→`%d`（示例自身 bug 非引擎洞·未改引擎层）。**复跑验证 PASS**：RSS 129→133MB 平线（5s→55s）·peak=133944KB<200MB·slope=123840 KB/min<300000·**凭真实阈值 PASS 不再依赖 after_close==peak 豁免**·fps=59.93·p95=18.48<22·hitch=0·bind_peak=14≪60·rr_max=10≤10·dirty/img=1·skip=34173·cpu_avg=28%·同机 R7/R7b/R10 基线 ~130MB 一致。§3.24 原「cold-start 非渐进泄漏」裁定纠正：当时 C3 实测 609MB/594229 KB/min 实为 `%1000` bug 掩盖的渐进泄漏（被 after_close==peak 豁免放行），非 GPU 驱动延迟回收。门禁逻辑保持（peak>200MB + slope>300000 + after_close!=peak 真泄漏仍 FAIL）·Extra rss_baseline_note 与门禁注释同步回写为诚实记录。**C3 维持 ✅v2**（bug 修复类不降级） |
