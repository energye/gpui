...
| **M5** | **CFF2 可变字体**：blend/vsindex 支持（cf2 语义顺带），可变 CFF 字体实例化 | 62 字 × 4 点位 blend 对照 go-text 逐点一致（cff2_gate_test.go）；**全字形含 vsindex 指令扫描（SourceSans3VF 694 字形 × wght 0/0.5/1/-1）对照 go-text 逐点一致（cff2_vsindex_gate_test.go）** | ✅ 完成（2026-08-08） |
...
| M5 CFF2 可变（blend） | ✅ 完成（2026-08-08） | **两个对照窗全绿**：① TestCFF2VarBlendMatchGT（62 字 × wght 0/0.5/1/-1，vt 层对齐 go-text 逐点 ±1/64）；② TestCFF2VSIndexBlendMatchGT（**SourceSans3VF 全字形扫描：694/2478 字形含 vsindex 指令，各 × 4 wght 对照 go-text 逐点一致**）；修复：cffcs.go lineTo/closeContour 浮点比较改 q64 量化（blend 微差）、gt 闭合点判定量化（vsindex 字形 g4 末控制点上多 1 点，cff2_gate_test.go expandGTSegs）；注：`extractCFF2Outline` 已有 fvar/avar/blend（cff_outline.go），M5 为 cf2 语义融合 |
