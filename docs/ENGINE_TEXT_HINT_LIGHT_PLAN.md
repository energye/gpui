...
| **M5** | **CFF2 可变字体**：blend/vsindex 支持（cf2 语义顺带），可变 CFF 字体实例化 | 62 字 × 4 点位 blend 对照 go-text 逐点一致（cff2_gate_test.go）；**全字形含 vsindex 指令扫描（SourceSans3VF 694 字形 × wght 0/0.5/1/-1）对照 go-text 逐点一致（cff2_vsindex_gate_test.go）**；**CJK VF 实例化：Noto Sans SC VF 3000 常用字 × 4 wght 对照 go-text 逐点一致（cff2_cjk_vf_gate_test.go）** | ✅ 完成（2026-08-08） |
...
| 阶段 | 内容 | 验证线 |
|---|---|---|
| **M6** | **CJK 竖排**：vhea/vmtx 解析 + 竖排度量（vertical advance/bearing）+ vert/vrt2 特性（gpui 目前无任何 vhea/vmtx 支持） | 中文/日文竖排样本行排对照；CFF 竖排字形走 cf2 语义 | ✅ 完成（2026-08-11，见 §7 M6 行） |
| **M7** | **系统字体发现（FontMgr 等价）**：fontconfig/系统字体目录枚举 + 家族匹配 + MultiFace 自动回退链 | 系统字体列表与回退选择对照（fc-match 等价） | ✅ 完成（2026-08-11，见 §7 M7 行） |
| **S1** | **shaping 复杂脚本 HarfBuzz 语义 audit**：OwnShaper 已有 GSUB/GPOS 全类型 + arabic_joining + indic_reorder；对照 HarfBuzz 验证覆盖率（泰文重排/孟加拉 matra/emoji ZWJ 序列/默认 feature 集） | 各复杂脚本样本 shaping 结果对照 HarfBuzz（go-text 可作参考实现） | ✅ 完成（2026-08-11，见 §7 S1 行——以 HbShaper/go-text 为标准，OwnShaper 降级） |
...
| M5 CFF2 可变（blend） | ✅ 完成（2026-08-08） | **三个对照窗全绿**：① TestCFF2VarBlendMatchGT（62 字 × wght 0/0.5/1/-1，vt 层对齐 go-text 逐点 ±1/64）；② TestCFF2VSIndexBlendMatchGT（**SourceSans3VF 全字形扫描：694/2478 字形含 vsindex 指令，各 × 4 wght 对照 go-text 逐点一致**）；③ TestCFF2CJKVFBlendMatchGT（**Noto Sans SC VF（NotoSansSC-VF.otf，testdata 内置，CFF2 wght 100–900 单轴）：31036 字形含 vsindex 8675 个，3000 常用字（testdata/cjk3000.txt）× wght 0/0.5/1/-1 对照 go-text 逐点一致**）；修复：cffcs.go lineTo/closeContour 浮点比较改 q64 量化（blend 微差）、gt 闭合点判定量化（vsindex 字形 g4 末控制点上多 1 点，cff2_gate_test.go expandGTSegs）、**gt 零长直线段按 FT 语义归一（CJK default 实例的「回笔」零长 lineto：FT cf2_glyphpath_lineTo 忽略、go-text 保留，段内量化相等剔除，见 expandGTSegs 注释）**；注：`extractCFF2Outline` 已有 fvar/avar/blend（cff_outline.go），M5 为 cf2 语义融合 |
| M6 CJK 竖排（vhea/vmtx/vrt2） | ✅ 完成（2026-08-11） | **vhea/vmtx 解析 + 竖排度量 + vert/vrt2 全链路**：① vhea/vmtx 表解析（font_metrics.go parseVheaTable/parseVmtx，与 hmtx 同构）接入 ttGlyphLoader 与 ownParsedFont；② FontMetrics/Metrics 新增 VerticalAscent/Descent/LineGap（vhea，无表 0）；GlyphVerticalAdvance（vmtx 优先、无 vmtx 回退 OS/2 公式 = FT TT_Get_VMetrics 语义）；TT phantom 竖排点改用 vmtx 真值（原 ascent-descent 兜底保留给无 vmtx 字体）；③ vert/vrt2 特性：collectDesiredFeatures 加 vertical 参数，TTB/BTT 方向激活 vertical alternates（HarfBuzz 同语义），ftexp 新增 vadv 模式（FT_Get_Advance+VERTICAL_LAYOUT 对照基准）；④ 验证窗 zz_m6_vertical_test.go 6 测试全绿：vmtx vs ftexp 逐字对照（wqy/Noto CJK）、无 vmtx 回退公式（FreeSans/DejaVu）、Metrics 竖排字段、TTB 标点 vert 替换 gid（（）、、「」等 6 标点全替换、普通汉字不变）、own/hb 双后端方向行为。**连带修复：hbDirection TTB/BTT 映射反了**（DirectionTTB→hb.BottomToTop 实为 TTB→TopToBottom，已修）。已知限制：go-text 后端对个别 CJK 标点（如（ ）的 vrt2 系 lookup 不替换，own 后端正确——记录不修 |
| M7 系统字体发现（FontMgr 等价） | ✅ 完成（2026-08-11） | **家族匹配 + rune 回退对照 fc-match**：① 复用已有 go-text fontscan 索引（fontscan_fallback.go），新增公共 API `SystemFontForFamily`/`SystemFontsByFamily`（NormalizeFamily 归一、aspect 打分选 regular，增量不破坏既有 rune 回退）；② **rune 回退改用 FontMap（SetScript+ResolveFace）**——go-text 完整匹配语义（family 替换抄自 fontconfig、按 script 选回退），替代自写 bestLocation；复用同一持久索引 + FontMap LRU（4096）+ 外层 pathCache，**性能/内存零新增**；③ 对照窗 zz_m7_fcmatch_test.go：8 个代表家族 fc-match 命中**严格一致**；script 对照（泰 ก→Loma、天城 क→Lohit、阿拉伯 ف→DejaVu）与 fc-match **一致**（FontMap 切换后消除原 FreeSerif/Gargi 偏好差异）；拉丁 A 例外——本机 fontconfig 用户字体（NotoSansCJKsc）优先级极高连 lang=en 都选 CJK，go-text 选 DejaVu 才是正确拉丁选择，该差异是 fc-match 本机配置怪癖不判。依赖 fc-match/fc-scan 环境，缺失 t.Skipf |
| S1 shaping 复杂脚本 HarfBuzz audit | ✅ 完成（2026-08-11） | **生产路径 HbShaper（= go-text/HarfBuzz 移植）验证收口（方案 B）**：① **修复接入 bug——hbFeatures 曾把 OwnShaper 默认 feature 集（含阿拉伯 isol/init/medi/fina）传给 HarfBuzz，干扰 complex shaper staging 致阿拉伯全错（presentation form 而非字体 GSUB 形位），改为只传用户 feature**；② 硬断言族（Thai/Hebrew/Lao/Bengali × Loma/FreeSans/Phetsarath/Mukti）与原生 libharfbuzz（系统 2.7.4）骨架 gid/cluster **完全一致**——接入正确性铁证；③ 版本差异族（Arabic/Myanmar/Deva/Tamil：2.7.4 vs go-text 新版 rlig 合成/重排更积极）按覆盖性+产出自洽判（gid≠0、cluster 方向单调），差异留档；④ 基建：新增 hbshape 原生对照工具（testdata，purego 绑 libharfbuzz，$HBSHAPE_HB_LIB 可选版本）；⑤ 用户下载的 libharfbuzz 8.3.0 因 GLIBC 2.38>2.35 无法在本机运行（Ubuntu 22.04），基准以 2.7.4 + go-text 自身语义为准 |
...
| 切换决策 | ✅ 完成（2026-08-08） | **GPU 生产路径已正式启用 HintingVertical**：`selectGlyphMaskHinting`（glyph_mask_engine.go）解除 R21 时期的 None 硬编码（当时自研 Vertical/Full 结构差 40% 墨量），改为轴对齐 ≤48px → HintingVertical（=FT light，M0–M5 全绿链）、旋转/倾斜/大字号/CJK HiDPI≥2x → None；回退开关 `GOGPU_TEXT_NO_HINT`；三处守卫测试更新（glyph_mask_hinting_test / glyph_mask_engine_test / gpu_text_cjk_test） |
