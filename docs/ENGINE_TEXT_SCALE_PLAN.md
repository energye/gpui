...
**状态**: **施工中（M0/M1/M2/M4/M5 已落地，M3 明确不标完成，M3.5 仅代码合入，M3.8 已验收）** — 本文档为设计与施工总纲，实施按 `M0 → M1 → M2 → M3 → M4 → M5` 分期触发，每期独立验收、独立可回滚。`M3.5` 为条件触发（非必经，buffer 已合入，piece tree 仍按条件触发），`M6` 为贯穿全程的验收主窗（非施工期）。过程见 `docs/ENGINE_TEXT_SCALE_CHANGELOG.md`，落地汇总见 `docs/ENGINE_UI_WIDGET_RENDER.md:553-560`。
...
> **🔴 开工前必读（否则会卡住）**
> - **M0 的第一件事不是改代码，是 M0-pre**：验收主窗 `ui_text_edit_accept` **现已存在（2026-09-04 起）**，M0–M5 每期都要跑它。立项时该窗不存在，先建再跑基线，见 **M0 第 ② 项**。
...
- **🔴 前置任务 M0-pre：先建验收主窗**（**必做，不得跳过**）
  ...
  > **为什么必须先建**：M0–M5 **每一期**的验收清单都要求跑 `ui_text_edit_accept`（§5.6）记录趋势值，立项时该窗不存在（`examples/` 下无此目录），现已存在。M0 收口前必须先建好并跑出基线，否则发散基线再也取不到。
...
| **`ui_text_edit_accept`**（§5.6 M6） | ✅ **已存在（2026-09-04 起）** | **M0-pre 已完成** | **M0–M5 每期都要跑**，记录趋势曲线 |
| `ui_text_m4_virtual` | ✅ 已存在 | **M4 已落地** | M4 |
| `ui_text_m5_anycase` | ✅ 已存在 | **M5 已落地** | M5 |

> **🔴 最关键的一条**：`ui_text_edit_accept` 被 **M0 到 M5 每一期**的验收清单引用，立项时尚未创建，现已存在（2026-09-04 起）。M0-pre 已完成，后续沿用该窗跑基线与趋势。
...
| **C 框单行整形 O(L)（M5 遗留，归 M1；已定位，需引擎活）** | 单行整形复用 | **核验结论（2026-09-04，源码+实测）**：生产增量路径单行 5 万字中位数 **71ms**（`ui/rendering/m5_singleline_test.go` 真字体常驻锁，红灯复现后转 200ms 防退化绊线；先前 72µs 系 nil face 估算口径，已纠正）。根因链（逐行核对）：`input_box.go:467` 走 `SetTextSpan` → `layout_update.go:75 updateSpan`（`checkSpan` 仅 32 字节抽查，O(1)）→ `applyPatch`/`patchRows` 粒度是整行 → 单行文档整行重整 → 整形缓存键是整串 `textHash`（`shape_result_cache.go:23`）必然 miss → O(L) 整形（§3.4 整行整形设计的直接后果）。**修法**：行内整形复用（只重整变更 run + 拼接，`ui/rendering` 内部，中）或前后缀复用缓存；M3.5 不碰（代价是整形非拷贝）。改门禁数值本身需审批，禁单方面。**已落地（2026-09-04，feat/ime-x11）**：行内复用（`ui/rendering/row_reuse.go`，只重整变更run+拼接；另补`effectiveFace`记忆化，否则增量因face身份抖动永不生效）。实测单行5万字击键中位数72→24ms，单次Shape分发50000→1；余量约24ms为整行`SegmentText`（UAX#9在`render/text`层，另立项）。200ms绊线与13裁判全绿。**已落地（2026-09-04，feat/ime-x11，LEGACY_02）**：整行分段增量前后缀复用——`render/text/segment_reuse.go` 新增 `SegmentReuse`（LTR快径：前后缀整段复用、窗口重分、弱字符扩窗含强上下文、同质行强分段内切分、缝位强弱校验+任一可疑回退全量；约千组等价扫描与全量逐位一致）+ `ui/rendering` 经 `layoutCache` 单行分段缓存（仅≥4KB单行）与 `itemizeRunsWithSegs` 接入行内复用（`tryReuseShapedRow` 原签名保留，短行/多行/RTL路径逐字节不变）。真机单行5万字击键中位数24→约14ms（13.6/15.2ms两跑）；200ms绊线与13裁判全绿，`BenchmarkKeystroke`单跑无退化；真窗 `ui_text_m5_anycase` 两跑HUD外0px（B区emoji彩色呈现差为存量颜色管线/环境差，C/D与基线一致）、R21顶栏零重录。门禁数值未动（新门禁待核定）。残留：击键仍有O(L)小常数遍（前后缀平移/拼接拷贝，约10ms）与弱字符邻接编辑回退全量，后续立项。 |
| **完整彩色图集 GPU 通道（M5 遗留，归 M5 后续；已建 2026-09-04，feat/ime-x11）** | emoji 画质/性能 | M5 只做到 mask 旁路 + 字符串彩色路。**CBDT 实测（2026-09-04，真机）**：NotoColorEmoji 进链先崩（`DrawWithEmoji` 对无 Source 脸无 guard，已修 + 回归锁），修完仍整段回退（19 万 fallback、hitch、CPU 127%）且空白——CPU 逐帧兜底此路不通，已回退。**层位**：`render/`（CPU 位图小字号无像素）+ `gpu/`（mask 管线无位图感知、无显式拒绝）。**家底盘点（逐文件核对）**：COLR 解析器（`emoji/colr.go`）+ CBDT 提取器（`emoji/cbdt_extractor.go`，含 StrikeBestFit）+ emoji 检测分段 + `IsColor` 标记旁路 + 空 guard 均存在；**但 `ColorFont` 接口（`render/text/color_font.go:12`）零实现者**——解析器与绘制之间缺适配器，`DrawWithEmoji` 永走 outline 回退（`DetectGlyphType` 恒返 outline），此为 CBDT 空白的直接根因（非 strike 选择问题）。`gpu/` 内无 CBDT/CBLC/strike 任一分支（mask 管线纯轮廓）；`scripts/cap_compare_golden.py` 系 capability_matrix 的 RMSE/SSIM 门，与 U21 逐位口径无关，不可复用。**缺口与三级台阶**：① 补 `ParsedFont`→`ColorFont` 适配器（绑 CBDT 提取 + COLR 解析，小–中，接口与解析器都已存在）；② `gpu` 层 mask 管线加显式拒绝（有色/位图走干净回退，学 `resolveGlyphMaskParams:310` 的 nil-Source 模式，小）；③ 完整 GPU 颜色图集（大，动 `render/` 公开面 + apidoc）。禁一步到位，先走①②。**已落地①（2026-09-04，feat/ime-x11）**：`ownParsedFont` 实现 `ColorFont`（`render/text/color_font_adapter.go`，CBDT 提取 + COLR 解析懒加载，无新公开符号，apidoc 通过），`DetectGlyphType` 不再恒返 outline；红灯单测 `color_font_adapter_test.go` 4 项 + emoji/绘制/M5 色跑回归绿。**已落地②（2026-09-04，feat/ime-x11）**：mask 管线显式拒绝含彩色字形的 run（`render/internal/gpu/glyph_mask_engine.go rejectColorRun`，学 nil-Source 错误契约，上游已映射 `ErrFallbackToCPU`，只拒含彩色字形的 run、纯描边 run 不受影响），红灯单测 `glyph_mask_color_reject_test.go` 3 项 + 遮罩引擎 20 项回归绿。**已落地③（2026-09-04，feat/ime-x11，提交 f358085）**：独立 RGBA 颜色图集（`render/internal/gpu/glyph_color_atlas.go` shelf 打包）+ 颜色引擎（`glyph_color_engine.go` 布局/RGBA 页上传）+ 颜色管线（`glyph_color.wgsl`，复用遮罩顶点布局）+ 公开面（`GPUColorGlyphAccelerator`/`Context.DrawShapedColorGlyphs`/`SplitColorGlyphs`，apidoc 绿）+ `ui/rendering` 彩色段路由（`LineHasColorRun` 避让批量口）+ 真窗 B 区 MultiFace 接表情字体。**收敛（同提交）**：遮罩热路径零分配快路（`colorFontOf` 先验颜色表）、切分纯描边零分配、`submitColorGlyphs` 去重、Context 复用颜色缓存、光栅锁收窄+单项淘汰。**真窗证据（10s 同版现跑）**：回退 0、CPU 约44%、B 区窄条差异 0.2835（真彩）、C 区差异 0（无重叠），基线对照内存无新泄漏（证据 `/tmp/m5_verify/`）。 |
| **M5 GPU 真窗已补，余一小项（M1–M4 欠账，M5 代记）** | 收口完整性 | **已补（2026-09-04，真机）**：m5 窗三跑 fps≈58.8/p95≈17.4/hitch 0/零回退，Golden 两跑 0px，F6 四区有墨、F0 一致；accept 60s 除 C 框 `keystroke_p99`（存量设计）外全绿。**已落地（2026-09-04，feat/ime-x11，提交 f358085）**：② B 区 emoji 端到端彩色像素（颜色图集通道打通，真窗 10s 回退 0、B 区窄条差异 0.2835、C 区 0，证据 `/tmp/m5_verify/`）。**剩余**：① accept Golden 相对 M0 基线差约 1.4%（散布字形抗锯齿级，基线过期）——**已刷新归档（2026-09-04，LEGACY_03）**：本会话重跑 accept 取新截图（`/tmp/accept_m5_rerun.png`），逐位对照旧基线（全幅 >8 差 1.48%，字形行 25–31 个短段/行、游程≤20px，底部动态条约 0.3%；无移位无缺字），独立评审确认抗锯齿级通过，已归档为 `examples/ui_text_edit_accept/golden/baseline_m5.png` 并删除旧基线，窗 README 已改指新基线（差异图 `/tmp/accept_diffmap_rerun.png`） |

| **G1 回绕击键超线性（M5 新遗留，待修，需重新确认，2026-09-16）** | G1 门禁 `TestKeystrokeRatio_M5` 贴门晃动 | **现象（2026-09-16，5 连跑）**：原采样比值 1.4–2.4 且坏子项轮换（NoWrap/WrapShort/WrapLong 轮流超 1.5）；交错 15 轮（预热丢掉 + 奇偶换序）取中位数后 5 遍 1.26–1.54，仍贴门两边晃；正确性断言（50 次全 span 增量、零回退）一次没挂。**差分（200 次均值）**：NoWrap N=1k 928ns→N=1e6 967ns（平）；WrapShort N=1k 922ns→N=1e6 2834ns（约 3 倍）；WrapLong N=5k 29269ns→N=1e6 34470ns；三模式 hits+=200、blds+=0、miss+=0。**画像（300 万次击键 CPU）**：NoWrap `patchRows` 占约 47%（`updateSpan` 70%，`rowLinesParts` 28%）；WrapShort `patchRows` 占约 62%（`partLines` 37%，`estSpan` 路，`growslice` 挂栈顶）；调用链 `ensureLayout→updateSpan→applyPatch→patchRows→estSpan/rowSpan→rowLinesParts/partLines`。**根因**：回绕两模式每次击键把变化所在硬段整段重估（`ui/rendering/layout_update.go:patchRows→estSpan`，`ui/rendering/layout_cache.go:partLines`），段缓存命中但段内仍整段重走 + 切片反复长大；单行复用（`ui/rendering/row_reuse.go`）只盖不回绕单行，回绕无段内增量。NoWrap 亚微秒抖动是量具问题，回绕是真超线性。**修法（另排期，禁在 T 线顺手改）**：段内增量估算或变化段缓存（`ui/rendering` 生产，影响 13 裁判 + 文本全回归，先出影响面评估）；量具可用交错 15 轮中位数（门禁 1.5 不动）但盖不住真慢。**重确认要求（硬）**：动手前重跑 `TestKeystrokeRatio_M5` + 上述差分探针 + 画像，确认比值与热点仍在；改门禁数值需审批，禁单方面。测试文件已恢复原样，树干净。 |
...
| 单行分段增量（遗留#2） | 2026-09-04 · LEGACY_02：`render/text` 增 `SegmentReuse`（增量前后缀复用+缝位回退）+ `ui/rendering` 单行分段缓存接入行内复用；单行5万字击键中位数24→约14ms，200ms绊线/13裁判/`BenchmarkKeystroke`单跑全绿，真窗m5两跑0px、R21零重录；门禁数值未动。 |
| 单行分段收敛优化 | 2026-09-04 · LEGACY_02 收敛遍：`SegmentReuse` 拆阶段helper（校验/定界/放窗/切分/重分/拼接）+拼接缝检合并并成一遍（4遍O(L)→2遍）；分段缓存加256KB上限（超限回退legacy全量，`TestReuseCachedRow_Cap`锁）；14裁判全绿，击键中位数约14ms（机器抖动±3ms内无可测变化）；`TestKeystrokeRatio_M5` 在亚微秒量级本机偶发抖动（同代码连跑pass/pass/fail、失败模式跳动，改动路径与该三档多行臂字节一致，属环境噪声非退化，未动门禁）。 |
| accept Golden 基线刷新归档（LEGACY_03） | 2026-09-04 · 以 M5 截图为新基线归档，accept 重跑对照归零口径（静态区约 0.07% 光标相位差）；独立评审确认约 1.4% 确为抗锯齿级（逐位对照记录在案，差异图 `/tmp/accept_diffmap_rerun.png`）；`golden/baseline_m5.png` 新增、M0 时期旧基线删除，窗 README 已改指新基线，§9 该行同步。 |
| G1 回绕击键超线性落位（待修，需重新确认） | 2026-09-16 · `TestKeystrokeRatio_M5` 贴门晃动（交错 15 轮中位数 5 遍 1.26–1.54，正确性断言全绿）定位为回绕两模式真超线性：每次击键整段重估（`patchRows→estSpan→partLines`，`partLines` 占约 37%，`growslice` 挂栈顶；NoWrap 亚微秒抖动另算量具问题），差分与画像数据见 §9 新行；修法（段内增量估算/变化段缓存）另排期，动手前重跑门禁 + 差分 + 画像三件套，改门禁数值需审批；测试文件已恢复原样。 |
