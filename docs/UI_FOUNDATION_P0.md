...
| 默认 GPU Present | ❌ 仍在 `examples/exboot`，未上收 `ui/app` |
| AnchoredPopup flip/shift / outside dismiss | ⚠️ 极简 / 无统一策略 |
| Trigger `DelayMs` | ❌ 字段预留未实现 |
| CapClipboard | ❌ 仅 Caps 枚举 |
| Editable 完整选区 / 多行 caret | ⚠️ 基础 caret + IME preedit API |
| Sticky 真语义 | ⚠️ paint 偏移简化；Table 头为 Column 假 sticky |
...
| 按需帧入口 | `ui/app/app.go` |
| Present 现状（待上收） | `examples/exboot/demand_ui.go` |
| IME Caps | `ui/platform/ime.go` · `caps.go` |
| 合成带说明 | `ui/core/doc.go` · MAP §4.1 |
...
| P0.1 契约 | ⬜ 未开工 | |
| P0.2 浮层 | ⬜ 未开工 | |
| P0.3 滚动/Sticky | ⬜ 未开工 | C3 待决策 |
| P0.4 编辑/剪贴板 | ⬜ 未开工 | |
| P0.5 默认 Present | ⬜ 未开工 | |
| P0 关门 | ⬜ | 满足 §5 全表 |

更新本表时以 PR / 提交为准；完成后把版本改为 1.x 并改页眉状态为 **已落地** 或 **部分落地**。
