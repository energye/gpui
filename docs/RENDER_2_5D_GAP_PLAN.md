...
看法一句话：箭头 `A -> B` 的意思是 A 没过，B 别开工。没箭头的可以并行。

```text
P0 纯算数（先 core，再全并行）
  0.0 core（vec/color/time/rand/asset/result/version）——P0总前置
    -> step/fixed, step/pool, step/time
    -> camera（只做位置缩放角度震动限位平滑，不碰渲染）
    -> anim/easing
    -> input/action, input/buffer
    -> physics/body
    -> audio（位置音、音乐混音全独立）
    -> save/save（只做存盘读盘槽位迁移，不做画质档）

P1a render先行（P0绿后开，四项串完才进P1b）
  R1 m4梯形CPU（1.4）→ R2 vertices CPU渐变（9.2）→ R3采样按图开关（3.2底）
  → R4 图集扩展AtlasSprite加rot/flip/pivot/tint（2.2底，新分支不碰老路）
  → R5 ysort深度分支（1.2底，隔离新分支）

P1b 画出来（P1a全绿后开，可并行）
  sprite/ysort（2.3，只靠core和R5接口）＋ sprite/flipbook（2.4，只靠core）
    -> sprite/batch（2.1，靠core＋R3开关，不等1.4/9.2）
    -> sprite/atlas（2.2，靠core＋R4）
    -> camera/project（1.1）＋ camera/parallax（1.3画出部分，靠1.1）
    -> anim/timeline（4.2，靠4.1）
    -> tex/mipmap开关（3.2，靠R3）
    -> ysort深度扩展（1.2，靠R5）

P2 世界立起来（靠 P1b）
  长链（串行，不许并行）：tex/compressed（3.1）→ asset/asset（12.1）
    → anim/skeleton（4.3，另靠2.2画）→ anim/statemachine（4.4，另靠4.2＋2.4）
  世界线（可并行）：world/entity（10.1，靠step时间）
    -> world/prefab, world/scene（10.2/10.3，靠entity加camera视口）
  地图线（可并行）：tilemap/tilemap, tilemap/iso（7.1，只靠core）
    -> physics/platform（14.2，靠14.1-P0部分加tilemap碰撞面）
    -> tilemap/chunk（7.2，靠7.1加camera视口）
      -> tilemap/lod（7.3，靠chunk加camera距离）
  碰撞画出：physics/body画出部分（14.1b，靠P0-body加7.1）
  资产支线：asset/atlaspack, asset/hotreload（12.2/12.3，靠12.1）
    -> tex/stream（3.3，靠12.1）
  场景更新：动态局部更新（8.3，靠core＋2.1）

P3 有气氛（靠 P2）
  particle/emitter, particle/cpu（5.1，靠sprite画）
    -> particle/gpu（5.2，要新管线，隔离做）
    -> particle/trail（5.2，靠gpu）
  fx（5.3/5.4/5.5，靠 sprite/world 画完再盖，要新中间图，custom 只碰游戏层）
    -> light/light, light/normal, light/shadow（6.1/6.2/6.3，盖在 fx 之后）

P4 养得住（靠 P3）
  debug/stats先行（17.2，看所有人的数）
    -> save/quality（16.2，高中低跟档走，靠17.2出数）
    -> tools/levelprev, tools/particleprev（17.1）
  长跑、显存、崩溃、热重载全量回归
...
| 期 | 装啥 | 谁能并行 | 关门线 |
|---|---|---|---|
| P0 | core先绿，再时间、镜头纯算、变速、输入、碰撞体、音频、存档基础 | core串行，其余全可并行 | 单测过，无窗口要求 |
| P1a | render五小修串行：梯形、渐变、采样开关、图集扩展、深度分支 | 串行，一个红后面全停 | 五项单测过，老路金图不红 |
| P1b | 合批排序序列帧、视差投影、时间轴、防闪开关、图集、深度扩展 | P1a绿后并行 | 每个能力有单测，显卡和 CPU 画出来只差抗锯齿 |
| P2 | 世界实体预制场景、瓦片加斜45度、平台斜坡、分区远近切换、长链压缩资产骨头状态机、后台加载、动态更新 | 长链串行，世界线地图线并行 | 每个主能力有独立真窗，自动加人工两步过 |
| P3 | 粒子拖尾、后期、灯光 | 粒子和灯光可并行，灯光等后期 | 真窗看得出气氛，帧率显存有数 |
| P4 | 看病先行、画质档、工具预览、长跑回归 | 看病先绿，画质工具随后 | 长时间跑显存不涨，崩了留日志 |
