...
| 2.3 | 按高低排序加层号分层 | game/sprite/ysort.go | P1b | S10/W1 | 0.0 | YSort(feetY)＋Layer{world,fx,ui} | game新包只算数 | 测ysort_test；窗game_sprite--case=ysort | A 脚底高度加层号排对、界面层不被盖，B 同高稳定不闪，C 两边齐，D 百个排序耗时有数，E 长跑不乱，F 真窗看前后盖特效不飘界面 | 未开工 |
