...
- 范围：本线只做 2.5D 游戏引擎 `game/` 侧。V1 算对→V2 做像→V3 做大→V4 玩法机制→V5 量产工具。平台层（窗口、显卡后端、声音后端、打包签名、跨端重测）归别线，本线只定接口与数的口子（见“平台口子”一节）。玩法以 Godot 4 为唯一实现参考（G01–G35＋D01–D20），底层只用自家 `render/`，只抄做法不搬代码。
- 做成算完：W0–W22 共 23 波全绿；S00–S105 共 106 号（S80 作废，实做 105 项）每项走完五步，带日期＋测试文件＋真窗名；真窗每窗自动＋人工双 JSON（人工事件非 0），纯算数留离屏对比、音频加人工听；帧门、量门、像素门、长跑门、换机门、坏路门全见§5（阈值数只在§5出现一次，这里不重写）。
- 非目标：人和网（权威服、视野同步、服存档、社交、反作弊）单独立项；联机回滚逻辑 V6 以后；音频效果器除 S103 三样外其余以后；平台层实现与跨端重测归别线；具体示例画法、美术资源、关卡设计；窗口、输入法、文字排版（别线管）。
...
| §2 禁空壳 | 留 |  |
| §3 开工拿号 | 改 | 号段 W0–W22 / S00–S105，拿活按 W 块 |
...
| V2 不含 N1–N8 | 改 | 去向更新：N1→S87，N2→S100，N3→S101，N4→S84/S85，N5→S98，N6→S89＋S103，N7→S104，N8金属→S99、联机→V6 |
| V2–V5 能力行 / 门禁挂波 | 留 |  |
| V3 / V4 / V5 整节 | 留 | 做法卡已满 |
...
- 自家 `render/` 缺功能就补：“不搬代码”指的是不搬 Godot 的 C++，不是不许动自家库；2.5D 要的功能自家库没有、不够用，就加（照 R1–R5 的样子：新分支新文件、老路不动、两边齐、坏路不崩，主路红了先回滚游戏分支）。
...
看法一句话：上一波没绿，下一波别开。P 是记忆分类（只帮助记忆，不挂开工不挂门禁，能力表里看到 P 只当身份证看）；W 是开工波次（W0→W22，谁卡谁，开工拿活只看 W 块）；S 是单会话序号（一次只做一个，按 S00→S105 拿号，新会话不被干扰）。同一波里标【可并行】的可以一起开也可以一个一个来，标【串行】的必须一个做完再做下一个。标【波内部分串行】的按写明的先后做，其余可并行。W11 的 S 号不连续（S55–S61＋S74–S79），拿活按 W11 块拿，别按 S 号顺序拿。S78 躲避小怪的前置写死等 W11 关门（含 S79 在内全绿），故归 W13。拿活按 W 块拿。
...
W10 总指标：三项全正式包，三遍全贴取最差判（帧门见§5，双金全 0 差），换机一遍照样过，文档回写三处齐。少一项 W11 不开。
...
W11 总指标：12项全建（有目录有main.go才算建，无目录算未建），每窗自动＋人工双 JSON（人工事件0算空转不算过），金图全进 testdata容差写死（静图0容差，金图按窗大小），正式包＋三遍最差＋换机口径，（帧门见§5，双金0差）全过。少一项 W12 不开。
...
#### V5 真窗（新分支5处＋tools/editor新窗：game_fx--case=refr、game_anim--case=hero、game_physics--case=joints、game_input--case=touch、tools/editor；其余复用）
