...
1. **回归**：`go test ./render/text/` 全绿（含 autofit 矩阵、golden、shape 相关测试；Rust_FullParity 已于 2026-08-05 修复：测试改用 glyph 级 script metrics + scaleWithUPM，与生产路径/skrifa 一致）。
...
- ~~Rust_FullParity 既存失败清理~~（2026-08-05 完成：测试 metrics 选择修正为 glyph 级，引擎无 bug；skrifa 0.31.1 源码对照见 /tmp/opencode/skrifa_probe）
