...
# 工作目录
> 所有需要提权自动确认，工作目录：gpui, 可写当前工作区：gpui/tmp
> 避免提权，如果是示例运行程序，可直接在目标程序目录go run .
...
**S6.0–S6.9 已关闭**。**M 内存生命周期档已落地**（`docs/MEM_LEAK_TEST_PLAN.md` §11 更新）。**F1 核心已收口**。**当前主线**：**稳 60+ 维持 + CPU 正向**（[`PERF_CPU_FORWARD_OPT_PLAN.md`](./PERF_CPU_FORWARD_OPT_PLAN.md) R8 opt43d/opt44 Keep）；下一刀 R8.5 mesh/convex 或 WriteBuffer residual；mem 护栏绿。控件层 W 仍后置。可选 S6.10 不阻塞。
