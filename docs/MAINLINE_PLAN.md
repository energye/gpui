...
**下一执行刀（建议）**：[`CAPABILITY_MATRIX_WINDOW.md`](./CAPABILITY_MATRIX_WINDOW.md) **P7 L5 画布 100% 宣告**（P6/L4 已关：`/tmp/cap_p6_final`）；控件层仅在 `S5_WIDGET_ENTRY` 条件满足后。**不要默认**再开 N3/N5 优化。
