...
11. **示例矩阵**：P0 按 §6.8 逐例对照表（7 例主路径：基本/图标/参数/分隔符/下拉/独立分隔符/Debug Routes），余下 P1 一次做完（`style-class` 语义深度、`component-token`）。
...
> 本章把 antd **Breadcrumb** 补成 **可开发、可测试、可验收** 的产品规格。  
...
**逐例 P0/P1 对照表**（§2.4 全量；P0+P1=§6.8 非debug 7+2 例一次做完）：
...
| `dropdownProps` 全量透传 | P1 必做 |
| 动画像素级 / 链接色 transition | P1 必做 |
| 浏览器-only API 或桌面无等价项 | 单测 Skip 写清平台原因 |
| component-token / 官网截图相似（99.99%） | P1 必做 |
| ConfigProvider 全局 breadcrumb 默认 | P1 必做 |

### 6.9 验收用例表（可测）

> 测试名建议：`TestBreadcrumb_PRD_<ID>` 或 gallery 场景 ID。  
> **P0+P1 相关用例全部通过** 才可宣称 Breadcrumb 完成 1:1。
...
| BC-20 | L3 | 关键态 golden 截图 | 与showcase基线一致（容差内）+官网并排验收通过 |
| BC-21 | L4 | 与 ant.design 并排 | 官网并排验收记录（必验） |
...
6. `coverage.go` Notes：P0+P1 已对齐 `docs/antd/breadcrumb.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` Breadcrumb 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围定义（无裁剪）。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
