...
1. **配置面**：`type`/`shape`/`icon`/`content`/`tooltip`（字符串）/`disabled`/`loading`；Group `trigger`/`open`/`placement`/`closeIcon`；BackTop/draggable/badge 按 P1 一次做完。
...
5. **示例矩阵**：P0 按 §6.8（9 例）、余下 P1 一次做完；与 §4 例数打架以 §6.8 为准。
...
> 本章把 antd **FloatButton** 补成 **可开发、可测试、可验收** 的产品规格。  
...
**15 例→P0/P1 范围对应表**（§2.4 全量；P0+P1=§6.8 非debug 9+4 例一次做完，2 例 debug 不验收（仅参考））：

| 示例 | 范围 | 原因 |
| --- | --- | --- |
| 基本/类型/形状/描述/气泡卡片/浮动按钮组/菜单模式/受控模式/弹出方向 | P0 | 主路径行为 + gallery 必备（弹出方向仅四向行为，动画 P1） |
| 可拖拽 | P1 | 宿主拖拽映射 P1必做 |
| 回到顶部 | P1 | BackTop 滚动宿主 P1必做 |
| 徽标数 | P1 | badge 叠加 P1必做 |
...
> **P0+P1 相关用例全部通过** 才可宣称 FloatButton 完成 1:1。
...
| FB-27 | L3 | 关键态 golden 截图 | 与showcase基线一致（容差内）+官网并排验收通过 |
| FB-28 | L4 | 与 ant.design 并排 | 官网并排验收记录（必验） |
...
6. `coverage.go` Notes：P0+P1 已对齐 `docs/antd/float-button.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` FloatButton 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围定义（无裁剪）。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
