...
5. **示例矩阵**：P0 按 §6.8 逐例对照表（3 例主路径），余下仅 `debug.tsx` #17678，不验收。
...
> 本章把 antd **Affix** 补成 **可开发、可测试、可验收** 的产品规格。  
...
| 真 `position: fixed` 出流 / 跨层 Portal 钉住 | P1 P1 必做（P0+P1 为 scroll 内 paint/hit 钉） | P1 |
...
**逐例 P0/P1 对照表**（§2.4 全量；P0+P1=§6.8 非debug 3+1 例一次做完）：
...
| ConfigProvider 全局 affix 默认 | P1 必做 |
| 真 `position: fixed` 出流 / 跨层 Portal 钉住 | P1 必做（P0 为 scroll 内 paint/hit 钉） |
| debug 示例 | 不验收（仅参考） |

### 6.9 验收用例表（可测）

> 测试名建议：`TestAffix_PRD_<ID>` 或 gallery 场景 ID。  
> **P0+P1 相关用例全部通过** 才可宣称 Affix 完成 1:1。
...
| AFX-14 | L3 | 关键态 golden 截图 | 与showcase基线一致（容差内）+官网并排验收通过 |
| AFX-15 | L4 | 与 ant.design 并排 | 官网并排验收记录（必验） |
...
6. `coverage.go` Notes：P0+P1 已对齐 `docs/antd/affix.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` Affix 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围定义（无裁剪）。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
