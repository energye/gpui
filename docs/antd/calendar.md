...
1. **配置面**：覆盖 §6.8 P0+P1 字段（value/defaultValue/mode/fullscreen/showWeek/disabledDate/validRange/cellRender/fullCellRender/headerRender/onChange/onSelect/onPanelChange）；`classNames/styles/locale` P1。
...
> 本章把 antd **Calendar** 补成 **可开发、可测试、可验收** 的产品规格。  
...
| 完整 locale 文案 / 周起始可配 | P1 必做 |
| 内置农历算法 | 业务注入即可 |
| 动画像素级 | P1 必做 |
| ConfigProvider 全局 calendar 默认 | P1 必做 |
| debug 示例 | 不验收（仅参考） |

### 6.9 验收用例表（可测）

> 测试名建议：`TestCalendar_PRD_<ID>` 或 gallery 场景 ID。  
> **P0+P1 相关用例全部通过** 才可宣称 Calendar 完成 1:1。  
...
| CAL-20 | L3 | 关键态 golden 截图 | 与showcase基线一致（容差内）+官网并排验收通过 |
| CAL-21 | L4 | 与 ant.design 并排 | 官网并排验收记录（必验） |
...
6. `coverage.go` Notes：P0+P1 已对齐 `docs/antd/calendar.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` Calendar 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围定义（无裁剪）。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
