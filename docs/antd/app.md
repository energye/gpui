...
> 本章把 antd **App** 补成 **可开发、可测试、可验收** 的产品规格。  
...
> **P0+P1 相关用例全部通过** 才可宣称 App 完成 1:1。
...
| APP-12 | L3 | 关键态 golden 截图 | 与showcase基线一致（容差内）+官网并排验收通过 |
| APP-13 | L4 | 与 ant.design 并排 | 官网并排验收记录（必验） |
...
6. `coverage.go` Notes：P0+P1 已对齐 `docs/antd/app.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` App 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围定义（无裁剪）。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
