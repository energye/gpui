...
> 本章把 antd **Cascader** 补成 **可开发、可测试、可验收** 的产品规格。  
...
| 级联搜索 `showSearch`（布尔路径过滤 + 空态 `notFoundContent`） | **对等**布尔；对象形态 filter/limit/sort P1必做 | P0 布尔 / P1 对象 |
| 远程 `loadData`（完成回填下钻 / 失败标错重试，与 `showSearch` 互斥） | **对等**完成分支；失败标错重试 | P0 |
| `fieldNames`/`optionRender`/`tagRender`/`popupRender` 定制 | P1 必做 | P1 |
...
| `showSearch` 对象形态（filter/limit/sort/render/matchInputWidth） | P1 必做 |
| `tagRender` / `optionRender` / `popupRender` / `fieldNames` | P1 必做 |
| `maxTagCount=responsive` 像素级 / 虚拟长列 | P1 必做 |
| 动画像素级 / 复杂虚拟列表 | P1 必做 |
| 浏览器-only API 或桌面无等价项 | 单测 Skip 写清平台原因 |
| debug 示例 | 不验收（仅参考） |

**20 例→P0/P1 范围对应表**（§2.2 全量；P0+P1=§6.8 非debug 8+12 例一次做完）：
...
> **P0+P1 相关用例全部通过** 才可宣称 Cascader 完成 1:1。
...
| CAS-22 | L3 | 关键态 golden 截图 | 与showcase基线一致（容差内）+官网并排验收通过 |
| CAS-23 | L4 | 与 ant.design 并排 | 官网并排验收记录（必验） |
...
6. `coverage.go` Notes：P0+P1 已对齐 `docs/antd/cascader.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` Cascader 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围定义（无裁剪）。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
