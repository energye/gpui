...
有图显示图，无图按 `icon > children` 显示图标或文字（长串按 `gap` 缩小），`src` 失败时同规则回退。
...
1. **配置面**：覆盖 §6.8 P0+P1 字段（size/shape/icon/children+gap/src+onError/alt/loading/disabled/Style/Group max+响应式）；`srcSet/crossOrigin` P1。
...
> 本章把 antd **Avatar** 补成 **可开发、可测试、可验收** 的产品规格。  
...
| 配置 / 能力 | 说明 |
| --- | --- |
| Group `max.popover` 展开隐藏头像 | P1 必做 |
| 真 HTTP 解码 `src` / `srcSet` / `crossOrigin` | P1 必做 |
| `draggable` 桌面拖拽 | P1 必做 |
| semantic classNames/styles 深度 | P1 必做 |
| ConfigProvider 全局 avatar 默认 | P1 必做 |
| 动画像素级（静态终态） / debug 示例 | P1 必做（静态终态与官网一致；debug 仅参考不验收） |

### 6.9 验收用例表（可测）

> 测试名建议：`TestAvatar_PRD_<ID>` 或 gallery 场景 ID。  
> **P0+P1 相关用例全部通过** 才可宣称 Avatar 完成 1:1。
...
| AV-19 | L3 | 关键态 golden 截图 | 与showcase基线一致（容差内）+官网并排验收通过 |
| AV-20 | L4 | 与 ant.design 并排 | 官网并排验收记录（必验） |
...
6. `coverage.go` Notes：P0+P1 已对齐 `docs/antd/avatar.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` Avatar 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围定义（无裁剪）。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
