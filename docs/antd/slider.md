...
- **依赖等级 L2**：表单件（tooltip 就地气泡，非 Portal 浮层）；轨/柄自绘 host；无组内前置依赖。
- **Form**：数值 `value`（单值 / range 双值）直绑。
- **ConfigProvider**：`tooltip` 全局默认随 ConfigProvider（P1）。
- **文件归属**：`ui/kit/slider/`（轨 + 柄 + 刻度 + 就地 tooltip）。
---
## 3. 配置（API）
...
11. **示例矩阵**：§6.8 P0 主路径 8 例（基本、带输入框的滑块、带 icon 的滑块、自定义提示、事件、带标签的滑块、垂直、控制 ToolTip 的显示）；余下（反向、范围可拖拽、多点组合、动态增减节点、禁用指定滑块、语义结构）P1，Notes 显式列出。
...
> 本章把 antd **Slider** 补成 **可开发、可测试、可验收** 的产品规格。  
...
| 配置 / 能力 | 说明 |
| --- | --- |
| `reverse` 反向坐标轴 | P1 必做（官方 reverse 示例） |
| `range.draggableTrack` 范围轨拖拽 | P1 必做 |
| 多点 / editable / maxCount | P1 必做（multiple / editable 示例） |
| range 单柄 `disabled=[bool,bool]` | P1 必做 |
| semantic classNames/styles 深度 | P1 必做 |
| 动画像素级 handle hover 过渡 | P1 必做 |
...
> **P0+P1 相关用例全部通过** 才可宣称 Slider 完成 1:1。
...
| SLD-24 | L3 | 关键态 golden 截图 | 与showcase基线一致（容差内）+官网并排验收通过 |
| SLD-25 | L4 | 与 ant.design 并排 | 官网并排验收记录（必验） |
...
6. `coverage.go` Notes：P0+P1 已对齐 `docs/antd/slider.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` Slider 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围定义（无裁剪）。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
