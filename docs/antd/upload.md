...
11. **示例矩阵**：§6.8 P0 主路径 8 例（点击上传、用户头像、已上传的文件列表、照片墙、圆形照片墙、完全控制的上传列表、拖拽上传、粘贴上传）；余下 13 例逐例去向见 §6.8 范围对应表，Notes 显式列出。
...
> 本章把 antd **Upload** 补成 **可开发、可测试、可验收** 的产品规格。  
...
| 图片预览（`preview-file.tsx`：`previewFile` / `isImageUrl` / `thumbUrl`） | **P1 必做**：缩略图绘制管线 | P1 |
...
| 自定义渲染（`iconRender` / `itemRender` / `progress` 全量 / `showUploadList` 对象） | **P1 必做**：P0 仅 `percent` 线宽 2 | P1 |
...
**21 例→P0/P1 范围对应表**（§2.2 全量；P0+P1=§6.8 非debug 8+13 例一次做完）：

| 示例 | 范围 | 原因 |
| --- | --- | --- |
| 点击上传/用户头像/已上传列表/照片墙/圆形照片墙/完全控制列表/拖拽/粘贴 | P0 | 主路径行为 + gallery 必备 |
| 文件夹上传 | P1 | 宿主目录能力 |
| 手动上传 | P1 | `beforeUpload` 手动触发变体 |
| 只上传 png/图片列表样式/自定义预览/上传前转换 | P1 | `accept/previewFile/transform` 深度 |
| 限制数量 | P1 页面（`maxCount` 行为 P0） | 能力已验，整页后补 |
| 阿里云 OSS | P1 | 业务 `customRequest` 实例 |
| 自定义交互图标/自定义进度条/拖拽排序/裁切图片 | P1 | 自定义渲染/业务层 |
| 自定义语义结构 | P1 | semantic 深度 |

> Picker 三实现方：host 真实现（生产选文件 + 业务 `customRequest` 真传）/ gallery-fake（无注入时 Ticker 模拟进度到 done，仅 demo）/ 业务 `customRequest`（OSS/自传，覆盖默认）。

### 6.9 验收用例表（可测）

> 测试名建议：`TestUpload_PRD_<ID>` 或 gallery 场景 ID。  
> **P0+P1 相关用例全部通过** 才可宣称 Upload 完成 1:1。
...
| UPL-24 | L3 | 关键态 golden 截图 | 与showcase基线一致（容差内）+官网并排验收通过 |
| UPL-25 | L4 | 与 ant.design 并排 | 官网并排验收记录（必验） |
...
6. `coverage.go` Notes：P0+P1 已对齐 `docs/antd/upload.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` Upload 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围定义（无裁剪）。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
