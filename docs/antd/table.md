...
**固定列同步滚动**：横滚时固定列与表体共用同一横向偏移源；表头、表体、总结栏三区同帧同步，不各自记偏移。

**虚拟行定高**：`virtual` 开启后每行定高（取首行实测或 `scroll.y` 均分），滚动只挂载可视窗内行；不定高内容（换行/展开行）不得进虚拟模式。

**筛选浮层挂载点**：筛选菜单默认挂表体宿主内就地浮层；`getPopupContainer` 有值时改挂指定容器，关闭时随表销毁。
