...
> 本章把 antd **Transfer** 补成 **可开发、可测试、可验收** 的产品规格。  
...
| `selectionsIcon` / 顶栏下拉「全选/反选/清空」菜单 | P1 必做（P0 仅 checkbox 全选） |
| `selectAllLabels` 自定义 | P1 必做 |
| 树穿梭框（tree-transfer） | P1 必做 |
| 自定义状态完整 demo 页以外深度 / style-class | P1 必做（status 边框 P0 已覆盖） |
| 动画像素级 / 超大虚拟列表 | P1 必做（分页 P0 可承载 large-data 主路径） |
...
> **P0+P1 相关用例全部通过** 才可宣称 Transfer 完成 1:1。
...
| TF-21 | L3 | 关键态 golden 截图 | 与showcase基线一致（容差内）+官网并排验收通过 |
| TF-22 | L4 | 与 ant.design 并排 | 官网并排验收记录（必验） |
...
6. `coverage.go` Notes：P0+P1 已对齐 `docs/antd/transfer.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` Transfer 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围定义（无裁剪）。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
