...
| G4 日期引擎 | `ui/kit/date_engine_*.go` + `examples/kit/date-engine/`（DatePicker/Calendar 接入待 F3/F5） | 🟩已绿 | 本次提交 | 闰年周起始全对+多格式首个展示+佛历+disabled矩阵+Range排序+多选+预设函数+受控面板+可换实现；真窗第二跑3/3+Golden 0+resize回1200x800+待人工看 |
