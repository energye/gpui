...
1. **配置面**：覆盖 §6.8 P0+P1 字段（src/alt/宽高/fallback+onError/placeholder+percent/preview/open+onOpenChange/preview.src/scaleStep+工具栏/actionsRender/Group items-current-onChange）；`imageRender/mask.blur/cover` P1。
...
> 本章把 antd **Image** 补成 **可开发、可测试、可验收** 的产品规格。  
...
| Semantic classNames/styles 函数形态 | P1 必做 | P1 |
...
| 配置 / 能力 | 说明 |
| --- | --- |
| semantic classNames/styles 深度 / 函数形态 | P1 必做 |
| `imageRender` / 自定义预览内容 | P1 必做（示例 imageRender） |
| `mask` 高级（`blur`） | P1 必做——需 GPU `backdrop blur`，无能力时只能降级纯色 `colorBgMask`，主路径先保 `enabled/closable` |
| `cover` / `CoverConfig`（`coverNode/placement top/bottom/center`） | P1 必做——缩略图 hover 遮罩定位 + 自定义节点，主路径先保默认 hover cover（黑底 0.3+预览文案） |
| 嵌套 Modal 内预览 / nested | P1 必做 |
| 真 HTTP 解码 src、crossOrigin、srcSet | P1 必做 |
| 大图拖拽 `movable`（仅大于视口可拖）+ 滚轮缩放像素级 | P1 必做——需视口裁剪 + 手势/滚轮宿主，主路径先保工具栏 zoom/rotate/flip/reset（`movable` 默认 true 的语义见下节状态机） |
| 动画像素级 / reduced-motion 细控 | P1 必做 |
| ConfigProvider 全局 Image 默认 | P1 必做 |
| debug 示例 | 不验收（仅参考） |
...
> **P0+P1 相关用例全部通过** 才可宣称 Image 完成 1:1。
...
| IMG-20 | L3 | 关键态 golden 截图 | 与showcase基线一致（容差内）+官网并排验收通过— 本阶段可后置 |
| IMG-21 | L4 | 与 ant.design 并排 | 官网并排验收记录（必验） — 本阶段可后置 |
...
6. `coverage.go` Notes：P0+P1 已对齐 `docs/antd/image.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` Image 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围定义（无裁剪）。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
