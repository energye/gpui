...
> **P0 相关用例（无 P1 标记）全部通过** 才可宣称 QRCode 完成 1:1 主路径。

| ID | 级别 | 步骤 | 期望 |
| --- | --- | --- | --- |
| QR-01 | L1 | NewQRCode 默认创建 | 不崩溃；默认值符合 §6.10 / antd |
| QR-02 | L1 | value 非空 | 绘出矩阵不崩 |
| QR-03 | L1 | size=200 | 边长 200 |
| QR-04 | L1 | status=expired | 遮罩 |
| QR-05 | L1 | onRefresh | 点击刷新回调 |
| QR-06 | L1 | icon | 中心图标 |
| QR-07 | L1 | errorLevel | 可生成 |
| QR-08 | L1 | 默认 size | 160 |
| QR-09 | L2 | 读取 §6.2 关键尺寸/间距 | 与表内数字一致（±0.5px，或文档写明容差） |
| QR-10 | L2 | 默认皮颜色 | 无硬编码品牌色；走 Theme Token |
| QR-11 | L2 | disabled 外观（适用者） | 禁用色；无 hover 高亮 |
| QR-12 | L1 | 键盘/焦点主路径（适用者） | 可聚焦者 Focus ring 可见；激活键有效 |
| QR-13 | L3 | 关键态 golden 截图 | 与仓库基线一致（AA 容差） |
| QR-14 | L4 | 与 ant.design 并排 | 人眼签字记录 |
| QR-15 | P1 | §6.8 P1 任一能力（若做） | 单独用例；Notes 标明 |
