...
- **依赖等级**：L1（加载指示，反被多控件复用）。
- **等谁**：无上游等待；反被 Button（`loading`）、Modal（`confirmLoading`）、Drawer/Skeleton 复用。
- **文件归属**：`ui/kit/spin/`。
- **组合**：嵌套模式 children 常驻树中；`percent=auto` 模拟进度走 Ticker。
---
## 3. 配置（API）
...
> 本章把 antd **Spin** 补成 **可开发、可测试、可验收** 的产品规格。  
...
| semantic classNames/styles 函数式/深度 | P1 必做 |
| 动画像素级（4-dot keyframes / 405°） | P1 必做 |
| ConfigProvider 全局 spin | P1 必做 |
| debug / `_semantic.tsx` / 官网截图相似（99.99%） | 不验收（仅参考） |

### 6.9 验收用例表（可测）

> 测试名建议：`TestSpin_PRD_<ID>` 或 gallery 场景 ID。  
> **P0 相关用例（无 P1 / N/A / L3 / L4 标记）全部通过** 才可宣称 Spin 完成 1:1。
...
6. `coverage.go` Notes：P0+P1 已对齐 `docs/antd/spin.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` Spin 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围定义（无裁剪）。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
