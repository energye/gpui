...
> 本章把 antd **Watermark** 补成 **可开发、可测试、可验收** 的产品规格。  
...
> 源码：`/home/yanghy/app/projects/ant-design/components/watermark/`（`index.zh-CN.md` + `style/` + 组件实现）。

### 6.1 对齐级别定义（Watermark）
...
| alternate clip 与官网 canvas 逐像素 | P1 必做 |
| debug 示例 | 不验收（仅参考） |
| ConfigProvider 全局 watermark 默认 | P1 必做 |

### 6.9 验收用例表（可测）

> 测试名建议：`TestWatermark_PRD_<ID>` 或 gallery 场景 ID。  
> **P0 相关 L1/L2 适用用例全部通过** 才可宣称 Watermark 完成 1:1。  
...
6. `coverage.go` Notes：P0+P1 已对齐 `docs/antd/watermark.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` Watermark 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围定义（无裁剪）。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
