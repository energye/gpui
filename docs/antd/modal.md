...
> 本章把 antd **Modal** 补成 **可开发、可测试、可验收** 的产品规格。  
...
| `getContainer=false`/挂载当前位置、`forceRender`、`scrollLock` 宿主级 | **映射**或 P1必做 | P1 |
| `modalRender`/鼠标位置/嵌套像素动效、`mask.blur` 真模糊 | **近似**或 P1必做 | P1 |
...
| `modalRender` / 自定义鼠标位置 / 嵌套 Modal 像素动效 | P1 必做 |
| 嵌套弹框（`nested.tsx`，debug） | P1：多层遮罩 + zIndex 栈 + 焦点返还栈，待 ModalHost 层叠管理前置 |
| `getContainer` / `forceRender` / `scrollLock` 宿主级 | P1 必做 |
| `okButtonProps` / `cancelButtonProps` 全量透传 | P1 必做（P0 仅文案/类型/loading） |
| `mask.blur` 真实模糊 | P1 必做（P0 可近似 dim / 无模糊） |
| ConfigProvider 全局默认 / locale 注入 | P1 必做 |
| 动画像素级 open/close | P1 必做（P0 瞬时） |
| debug 示例 | 不验收（仅参考） |
...
> **P0+P1 相关用例全部通过** 才可宣称 Modal 完成 1:1。
...
| MDL-26 | L3 | 关键态 golden 截图 | 与showcase基线一致（容差内）+官网并排验收通过 |
| MDL-27 | L4 | 与 ant.design 并排 | 官网并排验收记录（必验） |
...
6. `coverage.go` Notes：P0+P1 已对齐 `docs/antd/modal.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` Modal 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围定义（无裁剪）。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
