...
11. **示例矩阵**：P0 按 §6.8 逐例对照表（8 例主路径），余下 P1 一次做完（位置全矩阵/card 完整页/editable 完整页/自定义触发器/页签头/拖拽；3 个 debug 仅参考不验收）。
...
> 本章把 antd **Tabs** 补成 **可开发、可测试、可验收** 的产品规格。  
...
| `more` 折叠菜单/溢出 Dropdown | P1 必做（`more.icon/trigger` + Dropdown 底座） | P1 |
| ink/tabPane 动画（`animated` 默认 ink 开/pane 关） | P0 瞬时或短滑动，像素级 P1；reduced-motion 下瞬时 | P0 L1/P1 |
| `renderTabBar`/可拖拽标签 | P1 必做 | P1 |
...
**逐例 P0/P1 对照表**（§2.4 全量；P0+P1=§6.8 非debug 8+11 例一次做完）：
...
| 自定义新增页签触发器（`custom-add-trigger.tsx`）/ 自定义页签头（`custom-tab-bar.tsx`）/ 可拖拽标签（`custom-tab-bar-node.tsx`） | P1 | `renderTabBar`/拖拽 P1必做 |
...
| 动画像素级 ink / tabPane | P1 必做（P0 ink 可瞬时或短滑动） |
| `more` 折叠菜单 / 溢出 Dropdown | P1 P1 必做；P0+P1 溢出一律横向滚动代替下拉菜单，不出 `more` 按钮 |
| `renderTabBar` / 可拖拽标签 | P1 必做（3 个自定义页，见逐例表） |
...
> **P0+P1 相关用例全部通过** 才可宣称 Tabs 完成 1:1。
...
| TAB-24 | L3 | 关键态 golden 截图 | 与showcase基线一致（容差内）+官网并排验收通过 |
| TAB-25 | L4 | 与 ant.design 并排 | 官网并排验收记录（必验） |
...
6. `coverage.go` Notes：P0+P1 已对齐 `docs/antd/tabs.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` Tabs 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围定义（无裁剪）。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
