...
> 本章把 antd **Tree** 补成 **可开发、可测试、可裁剪** 的产品规格。  
...
> **P0 相关用例（无 P1 标记）全部通过** 才可宣称 Tree 完成 1:1 主路径。
...
6. `coverage.go` Notes：P0 已对齐 `docs/antd/tree.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` Tree 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围裁剪。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
