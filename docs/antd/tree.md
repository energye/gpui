...
1. **配置面**：覆盖 §6.8 P0+P1 字段（treeData/展开/选中/勾选/loadData/showLine/showIcon/draggable/blockNode/directory/searchValue）；P1 必须实现且命名与官网一致。
...
> 本章把 antd **Tree** 补成 **可开发、可测试、可验收** 的产品规格。  
...
> 源码：`/home/yanghy/app/projects/ant-design/components/tree/`（`index.zh-CN.md` + `style/` + 组件实现）。
...
| 动画像素级 / `height` 虚拟滚动像素级 | P1 必做 |
| `allowDrop` 细粒度 / 拖拽 icon 关闭 / dropPosition 全矩阵 | P1 必做 |
| `fieldNames` 自定义字段 / `treeDataSimpleMode` | P1 必做 |
| 自定义 switcherIcon 节点级复杂渲染 / 多行 title ReactNode | P1 必做 |
| 浏览器-only API 或桌面无等价项 | 单测 Skip 写清平台原因 |
| ConfigProvider 全局默认 | P1 必做 |
| debug 示例 | 不验收（仅参考；`directory-debug/drag-debug/big-data/multiple-line/line-debug/component-token` 不验收） |

### 6.9 验收用例表（可测）

> 测试名建议：`TestTree_PRD_<ID>` 或 gallery 场景 ID。  
> **P0+P1 相关用例全部通过** 才可宣称 Tree 完成 1:1。

| ID | 级别 | 步骤 | 期望 |
| --- | --- | --- | --- |
| TRE-01 | L1 | NewTree 默认创建 | 不崩溃；默认值符合 §6.10 / antd |
| TRE-02 | L1 | 展开节点 | 子可见；onExpand |
| TRE-03 | L1 | 选中 | selectedKeys |
| TRE-04 | L1 | checkable 勾选 | checkedKeys |
| TRE-05 | L1 | 父子联动（默认） | 勾父影响子 |
| TRE-06 | L1 | disabled 节点 | 不可交互 |
| TRE-07 | L1 | loadData | 异步出现子节点 |
| TRE-08 | L1 | 受控 expandedKeys | 外部优先 |
| TRE-09 | L1 | showLine | 连接线可见 |
| TRE-10 | L1 | multiple 选 | 多 selected |
| TRE-11 | L1 | 搜索（若接） | 过滤 |
| TRE-12 | L1 | 复现官方示例「基本」（`basic.tsx`） | 交互与主视觉符合文档；无控制台级错误 |
| TRE-13 | L1 | 复现官方示例「受控操作示例」（`basic-controlled.tsx`） | 交互与主视觉符合文档；无控制台级错误 |
| TRE-14 | L1 | 复现官方示例「拖动示例」（`draggable.tsx`） | 交互与主视觉符合文档；无控制台级错误 |
| TRE-15 | L1 | 复现官方示例「异步数据加载」（`dynamic.tsx`） | 交互与主视觉符合文档；无控制台级错误 |
| TRE-16 | L1 | 复现官方示例「可搜索」（`search.tsx`） | 交互与主视觉符合文档；无控制台级错误 |
...
| TRE-24 | L3 | 关键态 golden 截图 | 与showcase基线一致（容差内）+官网并排验收通过 |
| TRE-25 | L4 | 与 ant.design 并排 | 官网并排验收记录（必验） |
...
6. `coverage.go` Notes：P0+P1 已对齐 `docs/antd/tree.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` Tree 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围定义（无裁剪）。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
