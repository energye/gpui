...
7. **示例矩阵**：P0 按 §6.8（8 例）、余下 P1 一次做完；与 §4 例数打架以 §6.8 为准。
...
> 本章把 antd **Space** 补成 **可开发、可测试、可验收** 的产品规格。  
...
**15 例→P0/P1 范围对应表**（§2.4 全量；P0+P1=§6.8 非debug 8+2 例一次做完，5 例 debug 不验收（仅参考））：
...
| 紧凑布局嵌套 | 不计 | 官方 debug 示例（`compact-nested.tsx` 带 debug，不验收（仅参考）） |
...
> **P0+P1 相关用例全部通过** 才可宣称 Space 完成 1:1。
...
| SPC-17b | — | 挂 `compact-nested.tsx`（Compact 嵌套，debug 不验收（仅参考）） | 嵌套 Compact 叠边正常，无 2px 双边框；无控制台级错误（debug，仅参考不验收） |
...
| SPC-22 | L3 | 关键态 golden 截图 | 与showcase基线一致（容差内）+官网并排验收通过 |
| SPC-23 | L4 | 与 ant.design 并排 | 官网并排验收记录（必验） |
...
6. `coverage.go` Notes：P0+P1 已对齐 `docs/antd/space.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` Space 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围定义（无裁剪）。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
