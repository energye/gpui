...
> 本章把 antd **Input** 补成 **可开发、可测试、可裁剪** 的产品规格。  
...
> **P0 相关用例（无 P1 标记）全部通过** 才可宣称 Input 完成 1:1 主路径。
...
| INP-26 | L3 | 关键态 golden 截图 | 与仓库基线一致（AA 容差） |
| INP-27 | L4 | 与 ant.design 并排 | 人眼签字记录 |
...
6. `coverage.go` Notes：P0 已对齐 `docs/antd/input.md` §6；P1 显式列出。  

---

**本章用法**：实现 `ui/kit` Input 时以 **§6 为需求与验收**；§1–§3 为 antd 能力全集；§6.8 为范围裁剪。细度样板见 [Button §6](./button.md#6-11-产品需求增量gpui-验收规格)。
