...
> 状态一共五种：**未开工** → **首批中** → **首批完**（新 DoD 八条全勾：P0+P1 全合 + showcase 大图 + 官网并排 + 独立真窗 + 人测签字）→ **全完**（官方非 debug 全覆盖）；另有 **重写中**（推翻重写专用，旧状态作废待重做）。
...
5. 门禁分工：`ui/kit/acceptance_test.go` 查 P0（用例号/Layout/脏局部/testdata/焦点主题/纪律）；P0+P1 特性全量靠 `examples/kit/<名>/ -auto-only` 逐功能验效果；四处全绿才算数。
