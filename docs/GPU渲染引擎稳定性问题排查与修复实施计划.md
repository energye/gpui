...
- 所有资源 Destroy 后强制 handle 置空，杜绝野句柄调用
- 新增进程全局 sticky device-lost 熔断标记，一旦失效永久拦截底层调用
- 所有 purego Call 前强制句柄有效性校验
- 增加全局渲染锁，杜绝多协程并发操作 Device/Surface/Swapchain
- 重构错误体系，结构化区分：设备丢失、无效句柄、表面失效、显存溢出
2. gpu/webgpu 层（核心规范层 P0）
- 注册 WGPU 原生 DeviceLost 异步回调，主动捕获设备失效
- Swapchain 强制帧配对校验：一次 BeginFrame 严格对应一次 Present
- BeginFrame 前置三重校验：熔断标记、设备状态、Surface 有效状态
- 提供 Resize Configure 标准接口（调用时机交由应用层）
- 实现全局 GPU 资源统一注册、销毁管理，预防缓慢泄漏
3. render 渲染调度层（通用稳定层 P1）
- 纯渲染逻辑，不耦合窗口、事件、循环、重试逻辑
- 每帧临时资源自动回收，避免显存堆积
- 保证渲染提交线程安全、状态干净
五、应用层必须实现的容错与压测逻辑
- 兼容 GNOME Iconify 最小化场景：无 UnmapNotify 也可主动降帧节流
- 窗口 Unmap/关闭 资源主动释放逻辑
- 渲染循环错误分级：临时重试 / 设备致命失效重建
- 设备丢失后自动销毁全套GPU资源、重新初始化上下文
- Soak 压测日志：RSS、帧计数、Present计数、设备状态、窗口状态
- 多场景串行浸泡测试：前台常驻、后台最小化、窗口解绑、频繁切换
六、标准化验证测试用例（固定不变）
  所有用例均需要 mem_anim_window、particle_kitchen_sink 双示例全部通过
  用例1：基础功能验证
- 程序正常启动、窗口正常显示
- 粒子动画、场景渲染完全正常，无花屏、黑屏、卡顿
  用例2：短时最小化浸泡（5min）
  窗口最小化后台驻留5分钟，无崩溃、无闪退、恢复正常渲染
  用例3：中时最小化浸泡（8min）
  后台驻留8分钟，无 SIGABRT、无卡死、内存无持续上涨
  用例4：长时最小化浸泡（10min）
  极限后台浸泡，验证隐性设备延迟丢失、缓慢泄漏问题
  用例5：高频窗口抖动测试
  反复最小化/还原50次以上，无随机崩溃、无渲染异常
  用例6：前台长时稳定性（600s）
  前台持续渲染，监控内存、帧率、资源稳定性
七、结项验收标准（全部满足才算完成）
1. 内核层彻底杜绝：SIGABRT 设备丢失崩溃、野句柄调用崩溃
2. 无线程竞态、无并发安全问题、无静默失败
3. 长时间 Soak 测试无单向内存/显存泄漏
4. GNOME 最小化特殊场景完全兼容，无后台异常渲染堆积
5. 示例所有测试用例全部稳定通过, 如果示例使用有问题，或者修复问题后示例需要跟着修改需要把示例程序改为正确使用
6. 渲染内核保持纯粹通用，无窗口业务逻辑侵入
7. 修复只能正向优化，gpu/cpu/内存
八、迭代规则
   每出现一次异常，必须：
- 定位根因内核/应用层归属
- 修复当前问题 + 修复整条关联隐患
