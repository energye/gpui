...
| 脏旗 `needsPaint` | 界面独占 | 界面消费，光栅不动 |
| 脏旗 `NeedsRaster` | 光栅独占（D2/D12 单写者） | 界面建包只读，清零只在光栅任务；读写收进一处 |
...
| 2026-09-18 | R2 封印机制+删死件 | 🔄 修理中（用户定范围：只做封印机制+删死件，活闭包另立项） | 两段封印（主带封印+浮层尾巴只贴一次，二次贴回 false）+ 光栅入口拒收未封印包（`RejectedUnsealed` 旗）+ 裸建包口统一盖戳；删 `RegenerateFrom`（G2）与 hops 表（D10，零生产调用）；D3 注释按现状订正。活闭包（R2-4/R2-6，20 组件逐个快照）另立项。 |
| 2026-09-18 | R3 首刀：金图恢复+计数器 | 🔄 修理中（用户定第一刀） | R3-0 订正（2026-09-18 复核）：`be161b5` 覆盖的是每次跑重写的输出图（open/recover/steady），比对基线（`*_base`，`db37853` 入库）从未离开——逐字节验过，原“基线先没”指控不成立，收回。11.1026% 是代码对真基线的真信号（25s 重跑，仅此一项红）。新 hygiene 项：C4 每跑必脏 worktree 输出图，归入 §18 新项。R3-5：解码池三处计数改诚实（裸任务记完成、无回调记完成、大图按成功排队记）+ 3 新单测。R3-1：真路载体补上（包级释放撞合成 `-race` 单测，红绿验过；通用自定义活闭包仍属另立项的 R2-4）。按钮 140 门禁改诚实（hex 带 alpha、开关走真 wiring、活窗重跑删除、路由无头单测、JSON 自带计数，窗绿）。T 窗杂物清理（删重复 Open 空分支、常量落地使用处、大图并行生成 887ms）。剩口径放宽 4 项（D8 见次刀行）。 |
| 2026-09-18 | R3 次刀：D8/口径 | 🔄 修理中 | D8：`Resize` 记录语义无头单测（待画必挂号、同尺寸不清挂号、关窗拒收；X11 真卡两项照旧跳过）。R3-6：闲帧年龄已加（`present_mode_age_s`+`LastPresentModeAgeSec`，只加不改值）；C5 缺基线红+ACCEPT 入库流（基线已入库，二次跑 0.0000%）；R10 峰值总量门试过撤回（总量 707，主因 HOT 动画按设计逐帧重录，全局计数器分不出归因，豁免成立有数；真修需按格归因的引擎计数，另议）；render 整包 9 红 259 跳核过（7 个单拎全绿是负载抖，真红仅 S69 两性能门，逐文件跑才是门）。R3 关账。 |
...
| 2026-09-19 | #1 活闭包：Tag/Alert/Statistic 快照化收口 | ✅ 三家已关（§18.1 #1） | `5d0409d`：三家各 `snapshot.go` 冻结绘画输入（按钮范式），mark*Dirty 双刷新点，paint/geometry 只读快照；hidden/closable/focused 原子活读事件态保留；收敛 Tag EffectiveChrome 4 调用合一、Alert geometry 4 处活读归快照；测试四件套齐（race 测试对齐 PhaseRacesPaint 并发读快照串行画），三包 -race 绿 |
| 2026-09-19 | #1 活闭包：Popover/Tooltip 快照化收口 | ✅ 两家已关（§18.1 #1，`5d4a6ce` 提交） | 两家各 `snapshot.go` 冻结绘画输入（按钮范式），paintTrigger/paintPanel/paintArrow/paintText 只读快照，disabled/hovered/pressed/focused 原子活读事件态保留；修两处机制——tooltip `relayout()` 漏斗补 refreshSnapshot（SetTitle/SetTitleNode/SetTriggerLabel/SetTriggerNode 过户）、tooltip `paintPanel` 去 `hasContent()` 活读改判快照（光栅零活字段读）；测试四件套齐，两包 -race 绿 + 全量绿，`ui_polish_gallery` 编译绿 |
| 2026-09-19 | #1 活闭包：Progress/Spin 快照化收口 | ✅ 两家已关（§18.1 #1，`5d4a6ce` 提交） | 两家各 `snapshot.go` 冻结绘画输入（按钮范式）。Progress——ProgressSnap 冻结 ptype/steps/stroke/rail/渐变/stepColors/reduceMotion/status 色系，percent/successPercent/phase 保持原子活读，markPaint 单漏斗收口 setter（Tick 相位步进直标 track 不刷新快照），paintTrack/paintLine/paintGradientFill/paintStepsLine/paintActiveSweep/paintRing/paintGradientArc/paintRingSteps 全走快照。Spin——SpinSnap 冻结描述/尺寸/颜色/百分比/指示器/textFace，可见性（spinning+delay+display）+ WantMask 于 refresh 时解析冻结，phase/indSize 原子活读，15 个配置 setter + Tick 两刷新点（delay 翻转、auto 百分比步进），paint/paintSection/paintDots/paintRing/paintCustom/paintDescription 只读快照。四件套齐（IgnoresLivePoke/FrozenDeterministic/ConcurrentRace/Purity），两包 -race 绿，`ui_polish_gallery` 编译绿 |
| 2026-09-19 | #1 活闭包：Skeleton 全家 / Splitter 快照化收口 | ✅ 两家已关（§18.1 #1，`5d4a6ce` 提交） | Skeleton——全家 6 件（Skeleton/Avatar/Button/Input/Image/Node）各冻结绘画输入：SkeletonSnap 冻结结构旗标/rows/round/radius/avatarShape/尺寸/宽度配置（切片按值复制），宽高画时按盒尺寸纯算；其余 5 件各冻结 shape/fill/HasChild；active/loading/phase 原子活读；Skeleton dirty/dirtyPaint 双漏斗 + SetReduceMotion 补刷新，其余 5 件 config setter 逐一点刷新（Tick 相位直标防逐帧重建）；6 处 paint 全只读快照。Splitter——SplitterSnap 冻结方向解析/bar 尺寸/五色/拖柄与折叠图标有无/按条 Resizable+CollapsibleLeft/Right 布尔表；hovered/dragging/barActive/focusedBar 原子活读；rebuild/markLayout/markPaint 三漏斗 + Layout 收口 refreshSnapshot（Layout 兜底直抠面板配置 pokes）；paintBar/paintCollapseMarks 只读快照。两包各四件套齐，-race 绿 + 全包逐文件回归绿，`ui_polish_gallery` 编译绿 |
| 2026-09-19 | #16 推送完成 | ✅ 已关（§18.3 #16） | `feat/ime-x11` 20 笔本地提交一次性推送（含 R0–R6、T4–T5、§18 关账、六家快照 `5d4a6ce`、R10 按格归因立门 `c49c3a9`），origin 领先归零；别线游戏金图改动留在工作区不碰 |
...
## 18. 未关账总账（2026-09-18，R0–R5 修完盘点，修完一项勾一项）

> R0–R5 关账状态：R0 会崩 7 项修完；R6 胖死 5 项修完（R6-1 133 张全屏纹理归零：按钮窗峰值 32MB，压测窗 228MB）；R1 竞态修完（R1-4 验明无罪不改）；R2 封印机制+删死件修完（活闭包另立项）；R3 门禁修完（R10 总量门试过撤回）；R4 文字死码修完；R5 省 IO 修完；D7/D15 图片命修完。下面是没搞定的，按“另议 / 存量红 / 杂项”归位。
>
> 修序（先死后伤，先窗后数）：P0 = #2（C4 门堵着）→ #1（唯一敞口 race）→ P1 = #3（R10 门）→ #2b/#10（顺手）→ P2 = #4–#9（逐个重跑确认）→ #13/#14/#15（值守项）→ P3 = #11/#12（有绕行）→ #16（用户一句话）。
...
| # | 是什么 | 现状与证据 | 下一步 |
|---|--------|------------|--------|
| 1 | 活闭包 20 组件（R2-4/R2-6） | 光栅读组件肚子里的活字段；按钮/图标/文字已快照。已修：画中回写消除（Spin 指示尺寸、Popover 触发尺寸，原子值）；相位原子化（Timeline 快照+原子相位、Skeleton×6、Spin、悬浮钮、光束、进度条）；事件开关原子化（Tooltip/Popover/Splitter/Tag/Alert/Sider 悬停按压聚焦选中折叠隐藏，Statistic loading 收尾，SpaceAddon 紧凑边旗 2026-09-19，edge 位包 1 原子 + addon_atomic_test -race 绿）；进度条百分比原子化；PhaseRacesPaint 两测改并发读原子态、串行画（Tick vs Node().Paint 会误伤 rendering 脏旗，非组件错）。剩文案配置类：watermark 已全快照化收口（2026-09-19，用户定全快照按钮范式：`snapshot.go` WatermarkSnap + dirtyMark/Layout 双刷新点，paintMarks/paintTextTileSnap 只读快照，SnapshotRace -race 绿 + SnapshotPurity 无活读断言 + 全量回归绿）。Tag/Alert/Statistic 已快照化收口（2026-09-19，`5d0409d`，按钮范式：三家各 `snapshot.go` 冻结绘画输入 + mark*Dirty 双刷新点 + paint/geometry 只读快照；hidden/closable/focused 保持原子活读事件态；收敛 Tag EffectiveChrome 4 调用合一、Alert geometry 4 处活读归快照；测试四件套齐，race 测试对齐 PhaseRacesPaint 并发读快照串行画，三包 -race 绿）。Popover/Tooltip 已快照化收口（2026-09-19，`5d4a6ce` 提交：各 `snapshot.go` 冻结绘画输入，paintTrigger/paintPanel/paintArrow/paintText 全走快照，disabled/hovered/pressed/focused 事件态保原子活读；修两处机制：tooltip `relayout()` 漏斗补 refreshSnapshot（SetTitle/SetTitleNode/SetTriggerLabel/SetTriggerNode 四 setter 过户），tooltip `paintPanel` 去掉 `hasContent()` 活读改判快照 Title/TitleNode（光栅不再读活字段）；测试四件套齐，两包 -race 绿 + 全量绿）。其余组件配置类（labels/颜色/尺寸/节点）同范式逐个啃。剩余名单已盘点（2026-09-19 只读扫描）：【需快照化 6 家】popover — ✅ 已收口、tooltip — ✅ 已收口、progress — ✅ 已收口（2026-09-19，`5d4a6ce` 提交：snapshot.go ProgressSnap 冻结 ptype/steps/stroke/rail/渐变/stepColors/reduceMotion/status 色系，percent/successPercent/phase 原子活读，markPaint 单漏斗 + Tick 直标 phase-only 防浪费刷新，paint 七函数只读快照，四件套 -race 绿）、spin — ✅ 已收口（2026-09-19，`5d4a6ce` 提交：snapshot.go SpinSnap 冻结描述/尺寸/颜色/百分比/指示器/textFace/可见性（spinning+delay+display 于 refresh 解析，host 交 overlay 的 WantMask 一并冻结），phase/indSize 原子活读，15 配置 setter + Tick 双刷新点，paint 六函数只读快照，四件套 -race 绿，`ui_polish_gallery` 编译绿）、skeleton（6 处 paint 同型读 hasAvatar/hasTitle/avatarSizePx/shape）— ✅ 已收口（2026-09-19，`5d4a6ce` 提交：skeleton 全家 6 件各冻结绘画输入——SkeletonSnap 冻结结构旗标/rows/round/radius/avatarShape/尺寸/宽度配置（切片按值复制）/fill/高度，宽高在画时按盒尺寸纯算；Avatar/Button/Input/Image/Node 各冻结 shape/fill/HasChild；active/loading/phase 原子活读；Skeleton dirty/dirtyPaint 双漏斗 + SetReduceMotion 补刷新，其余 5 件 config setter 逐一点刷新（Tick 相位直标防逐帧重建）；6 处 paint 全只读快照，四件套齐，-race 绿，`ui_polish_gallery` 编译绿）、layout-Sider（paint 读 bg/siderTheme/reverseArrow，Header/Footer/Content 已快照化，Sider 唯一漏网）——✅ 已收口（2026-09-19，`110733a`：sider_snapshot.go 冻结 bg/triggerBg/箭头色/焦环色+trigger 几何方向，paint 三函数只读快照，markPaint 单漏斗收口 20 处裸 MarkNeedsPaint，测试四件套齐，layout 全包 -race 绿）、splitter（paintBar/paintCollapseMarks 读 vertical/barColors/draggerIcon/panels/collapse 图标）— ✅ 已收口（2026-09-19，`5d4a6ce` 提交：snapshot.go SplitterSnap 冻结方向（orientation+vertical 解析成 Vertical 布尔）/bar 尺寸/五色（bar/hover/active/handle/focus）/拖柄与折叠图标有无/按条 Resizable+CollapsibleLeft+CollapsibleRight 布尔表（`barResizable`+`snapShow` 冻结版）；hovered/dragging/barActive/focusedBar 原子活读事件态保留；rebuild/markLayout/markPaint 三漏斗 + Layout 收口 refreshSnapshot（Layout 兜底直抠 SplitterPanel 配置 pokes）；paintBar/paintCollapseMarks 只读快照；四件套齐 + 全包逐文件回归绿）；【干净】flex/grid/masonry/space/timeline（无自定义 paint 或只读快照+原子态）；【W0 scaffold 无实现 48 家】实现落地时按范式同步做快照 | ✅ 全项关闭（2026-09-19，六家全部 snapshot 收口 + 事件/相位/画中回写三组全清零；【干净】家复核无可画输入；W0 未实现 48 家按约定在实现落地时同步做快照，不占当前账） |
...
| 3 | 按格归因计数（R10） | ✅ 已关（2026-09-19 真窗验证）：plumbing 已落（`ui/scene/textured.go` `rerecordBy` key→cause→累计数，cause=full/full-extra/local/local-extra 挂进 recordWith/recordLocalWith，`RerecordByKeySnapshot()` 拷贝口，单测 2 项 + race + `ui/scene` 全量绿）。窗侧按格断言已接线（`2029f0f`）：`ui_wr_r10_async_image` 逐批 settle 做键级差分 `changedKeys ≤ applied+1`（同线总量门），违例即 FAIL。真窗实测（2026-09-19 30s GPU 判批干净跑）：rrV=0 / keyV=0 / maxRRΔ=0 / maxKeyΔ=0，loaded=20/20，keys==dRR 每批同线（键级与 boundary 全局计数逐批一致，无按格缺失）；WR_R10_DBG 探针佐证增量键为 HOT(1x 逐帧)+格子+静态混入、无全局重绘（增量键≪boundary 总数 39）；判定违规只在负载批次同现（hitch 7.5 / rss 69k 同批），非能力回归。RSS slope 本次 30420 仍处 R10 已知抖带（21947–44414，见 §18.2 #4） | 已关（键级总量门与 R10 原「出图后仅一格」同语义，按格归因使总量可分键证伪） |
...
| 8 | r22 头晕（基线绿）/c6 卡顿/c8 帧率/r20 卡顿/c10/r15 短版时长 | T5 窗单原样，基线同红或同类红 |
| 9 | 8s 跑 X8 最小化偶发跳过 | 离 7.2s 最小化点太近的时序抖，14s 稳绿 |
