...
| # | 是什么 | 现状与证据 | 下一步 |
|---|--------|------------|--------|
| 1 | 活闭包 20 组件（R2-4/R2-6） | 光栅读组件肚子里的活字段；按钮/图标/文字已快照。已修：画中回写消除（Spin 指示尺寸、Popover 触发尺寸，原子值）；相位原子化（Timeline 快照+原子相位、Skeleton×6、Spin、悬浮钮、光束、进度条）；事件开关原子化（Tooltip/Popover/Splitter/Tag/Alert/Sider 悬停按压聚焦选中折叠隐藏，Statistic loading 收尾，SpaceAddon 紧凑边旗 2026-09-19，edge 位包 1 原子 + addon_atomic_test -race 绿）；进度条百分比原子化；PhaseRacesPaint 两测改并发读原子态、串行画（Tick vs Node().Paint 会误伤 rendering 脏旗，非组件错）。剩文案配置类：watermark 已全快照化收口（2026-09-19，用户定全快照按钮范式：`snapshot.go` WatermarkSnap + dirtyMark/Layout 双刷新点，paintMarks/paintTextTileSnap 只读快照，SnapshotRace -race 绿 + SnapshotPurity 无活读断言 + 全量回归绿）。Tag/Alert/Statistic 已快照化收口（2026-09-19，`5d0409d`，按钮范式：三家各 `snapshot.go` 冻结绘画输入 + mark*Dirty 双刷新点 + paint/geometry 只读快照；hidden/closable/focused 保持原子活读事件态；收敛 Tag EffectiveChrome 4 调用合一、Alert geometry 4 处活读归快照；测试四件套齐，race 测试对齐 PhaseRacesPaint 并发读快照串行画，三包 -race 绿）。其余组件配置类（labels/颜色/尺寸/节点）同范式逐个啃。剩余名单已盘点（2026-09-19 只读扫描）：【需快照化 6 家】popover（paintTrigger/paintPanel 读 triggerLabel/title/content/arrow）、tooltip（paintTrigger/paintPanel 读 triggerLabel/title/arrow/colorKey）、progress（paintTrack 链读 ptype/strokeColor/steps/gradFrom/gradTo/stepColors/reduceMotion）、spin（paintSection 读 description/tip/hasPercent/indicator/textFace）、skeleton（6 处 paint 同型读 hasAvatar/hasTitle/avatarSizePx/shape）、layout-Sider（paint 读 bg/siderTheme/reverseArrow，Header/Footer/Content 已快照化，Sider 唯一漏网）——✅ 已收口（2026-09-19，`110733a`：sider_snapshot.go 冻结 bg/triggerBg/箭头色/焦环色+trigger 几何方向，paint 三函数只读快照，markPaint 单漏斗收口 20 处裸 MarkNeedsPaint，测试四件套齐，layout 全包 -race 绿）、splitter（paintBar/paintCollapseMarks 读 vertical/barColors/draggerIcon/panels/collapse 图标）；【干净】flex/grid/masonry/space/timeline（无自定义 paint 或只读快照+原子态）；【W0 scaffold 无实现 48 家】实现落地时按范式同步做快照 | 事件组已清零；剩余配置组按按钮范式逐个快照化（下刀顺序建议：Sider → popover/tooltip → progress/spin → skeleton → splitter） |
