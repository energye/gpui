...
| **v2.5** | **点击定位修复：命中路径祖先链分发（2026-08-26，用户实测反馈「点文本光标不切换」）**：根因——`InputRouter.routePointer` 只把指针事件投给**命中的最深叶子节点**；示例的点击处理函数挂在包裹容器（inputBox）上，而命中返回的是其内部的 RenderText 叶子 → 处理器永远收不到事件，SetCaret 从未执行（探针实证：target=*rendering.RenderText，box.OnPointer 零调用）。修（对齐 Flutter 命中路径分发语义）：routePointer 沿 `Parent()` 祖先链逐级分发，路径上所有 PointerHandler 依次收到事件（叶子优先）——任何容器包裹的可编辑控件从此都能收到落在其后代上的点击。验收新增 `click-live` 场景走完整生产链（platform.Event → FromPlatform → hit-test → 分发 → SetCaret）：两次点击光标分别落 byte 13（row-t|wo 前）与 byte 1（r|ow-one 前），与字体度量离线复算完全一致；最终帧条位于第二次点击处。回归：embedder 全绿、全 ui 包零失败、SELFTEST pass:true。 |
| **v2.6** | **纯英文光标条压字根治（2026-08-26，用户实测「mmmm… 从第三个起条完全在字母上」）**：深度审计（字体度量+离屏渲染+真窗像素三重取证）——① 笔位零漂移：CaretColumn 对 36 字符 m 串逐边界对照理论笔位，drift=0.000，几何定位本身精确；② 真因＝DejaVu 'm' 字间空隙仅 1.70px < 旧条宽 2px → 条从第 1 个间隙起必然两侧触字，视觉上「压在字母上」（首个光标在串首无左邻所以显得正确，与用户描述的「第 3 个开始越到后面」现象吻合——实际是所有非首边界都触）；③ 垂直几何同步标准化：废弃 fs×0.28 魔法系数，条盒=字体墨迹带（baseline−ascent .. baseline+descent），高度随 Metrics 走、随行槽钳制。修：条宽 2→1px（成熟编辑器对窄字距的标准处理；1px 条落点列与两侧墨迹间实测各余 ≥0.74px 自由列，全边界零接触）。验证：latin-caret 新场景（生产点击链多点扫描 byte4→36 全部精确命中）+ 放大像素图条两侧零 '#' 接触；composing-cjk 复查中文侧同样干净；全 ui 回归绿；SELFTEST pass:true。 |
