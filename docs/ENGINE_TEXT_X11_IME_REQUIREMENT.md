...
|---|---|---|---|---|
| **S1 总线** | 依赖与总线 + **三层地基/A5** + 地址解析（Q2） | `godbus/dbus/v5` 纯 Go。`ibus` 私有总线（`x11ResolveIbusAddress` 读 `~/.config/ibus/bus/*` → `IBUS_ADDRESS` 私有 `unix:abstract`，验 `IBUS_DAEMON_PID` 存活，跳过含 `fcitx` 的冒名，`dialIbusPrivate` 单例复用）与 `fcitx5` 会话总线（`SessionBus` 单例）分流；`AddMatch` 分别订阅；`FlagNoAutoStart` 阻断未拥有名字的 `StartServiceByName`；无守护 `Window.IME()==nil` 降级；`GPUI_IME_DEBUG` 打 `dial/hello/match`；**只按 `GTK/QT/XMOD` 单选一条总线，不双探**；**`fcitx5` 为基线，`ibus` 另线，期间不改 `x11_ime_fcitx.go`** | `go.mod` + 新建 `ui/platform/x11_ime_proto.go`（第1层协议）+ `x11_ime_ibus.go`（第2层私有总线+生命周期）+ `x11_ime_fcitx.go`（第2层会话总线，S1 期间不改动）+ `x11_dbus_ime_linux.go` 瘦身为第3层统一调度 + `x11_linux.go:imeForX11` 去桩 | 建窗不阻塞；`ibus` 私有与 `fcitx` 会话各单例，多窗口连接数恒为1；有/无守护 `go vet 0`；`P8` 前 `nil` 降级；**污染文件被跳过**（`fcitx_random_string` 死 PID 旧文件不被选中）；`TestX11ResolveIbusAddress_SkipPolluted` 无头可测 |
