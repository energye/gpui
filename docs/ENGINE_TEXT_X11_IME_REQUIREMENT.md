# 文本编辑 + IME X11 生产级实现需求文档（复用 Wayland 版 v3.5 · X11 完整版 v2.4 · 完全对齐 Flutter）
...
> **变更说明 v2.4（2026-09-01）**：于 `v2.3` 之上，落实三层架构（A5，Q1-A 协议层共用）：`x11_dbus_ime_linux.go` 瘦身为统一层、`x11_ime_ibus/fcitx.go` 引擎层、`x11_ime_proto.go` 协议层；明确 `fcitx5` 复用 `ibus` 协议（` /org/freedesktop/IBus` 兼容节点）；Q2 单选（只探 `GTK/QT/XMOD` 指定的一家，`FlagNoAutoStart` 防 D-Bus 激活）；S1 私有总线解析（`IBUS_ADDRESS` 验 `PID`、跳过 `fcitx` 污染，单例复用）。框架不改系统输入法，只用用户已配置的一家。
> **变更说明 v3.20（2026-09-02）**：修复「`fcitx5→ibus` 正常、反向 `ibus→fcitx5` 失败」的方向不对称（**B24，P0，已完整落地**，见 §15）。根因是**观察者随私有总线一起消失**：程序跑在真 ibus 上时握着它那条 `unix:abstract` 私有总线，守护一退该总线连同挂在其上的 `NameOwnerChanged` 观察者**一起没了**，此后**无人**能通知程序输入法已换 —— 而 `objectPath` 仍非空、脏标记无人置位，懒重探直接早退，只能重启进程。反向不受影响是因为 fcitx5 把服务开在会话总线（由 `dbus-daemon` 提供、与输入法无关），守护换人它还在。修复：新增 `sessionUsable()`，在「无脏标记但有 IC」时补判窗口连接是否仍通向活服务方（连接活着＝观察者还在，信号有效，零 D-Bus 往返；连接断了＝补查会话总线，fcitx5 只出现在那里），必要时自行触发重探，不再依赖收不到的信号。同批修复三处同源缺陷：① `ensureReprobe` 抢先把探测连接写回 `im.conn`，导致旧 IC 被发到无关总线销毁、且多窗口传播匹配错对象（`TestX11MultiWindowConnPropagation` 由 FAIL 转 PASS）；② `imeForX11` 自建取总线分支与引擎层重复，使引擎层回落对建窗路径不生效；③ 私有地址文件被写成空值（本机 `fcitx5` 跑 `ibusfrontend` 时实测）时 `ibusEngine.Bus` 应回落到会话总线。另修 `im.conn` / `imeDirty` 的未加锁读取（`-race` 实测报出）。新增 `x11_ime_switch_watch_test.go`（真拓扑复刻 + 空地址文件回落）。**回退验证**：撤掉 `sessionUsable` 调用后 `TestX11SwitchRealIBusToFcitx5Frontend` 立刻 FAIL。**注（2026-09-12 源码复核）**：`sessionUsable()` 本体已在（`x11_dbus_ime_linux.go:387`），但上述 `x11_ime_switch_watch_test.go` 等开关测试文件在库内缺失，测试证据待补，B24 本体修复不受影响。回归：`ui/platform` 全部测试文件逐个跑全绿 + 14 例 `-race` 无竞态 + `ui/input`/`ui/textinput`/`ui/embedder`/`ui/application` 全绿 + 真机端到端确认（`GTK_IM_MODULE=ibus`、实际守护 fcitx5、地址文件空值下仍能建出 `InputContext`）。
...
### A5 X11 IME 三层架构（Q1-A 协议层共用）

> **结论：1639 行 `x11_dbus_ime_linux.go` 实扫，仅 12 处需拆，其余信号处理、事件推送、坐标转换、截断、修饰键判断两边一致不动。**

**绞合的 12 处：**
*   9 个“二选一”函数（`if eng=="ibus"{...}else{...}`）：`callFocusIn / callFocusOut / callSetCursorLocation / callSetSurroundingText / callSetContentType / ProcessKeyEvent / asyncProbe / ensureReprobe / x11MarkDirtyForOwnerChange`
*   3 个 fcitx 专属：`tryCreateFcitx5 / setCapabilitiesFcitx / destroyObject`

**三层落位（Q1 选 A：协议层共用）：**
*   **第 3 层 统一层** `x11_dbus_ime_linux.go`（瘦身后）：对外仍是 `IME` 接口，上层无感，只做 `GTK_IM_MODULE` 单选调度。
*   **第 2 层 引擎层** `x11_ime_ibus.go`（私有总线+生命周期）与 `x11_ime_fcitx.go`（会话总线+生命周期），各自实现同一方法集，内部只管“连哪条总线+何时建/销毁 IC”。
*   **第 1 层 协议层** `x11_ime_proto.go`：`ibus` 方法封装 + 信号解码 + `AttrList` 解析，两引擎共用。

**为何 fcitx 不单写协议：** 实测 `fcitx5` 挂了 `/org/freedesktop/IBus` 兼容节点，实现标准 `ibus` 接口（`CreateInputContext(s)→o` 直接返回 `/org/freedesktop/IBus/InputContext_31`），说的就是 `ibus` 话，故第1层两边共用。

**两引擎惟一差异——连哪条总线：**

|  | ibus | fcitx5 |
|---|---|---|
| 总线地址 | 读 `~/.config/ibus/bus/*` 的 `IBUS_ADDRESS` 私有 `unix:abstract`，验 `IBUS_DAEMON_PID` 存活，跳过含 `fcitx` 的冒名条目 | 直接 `DBUS_SESSION_BUS_ADDRESS` 会话 `unix:path=/run/user/1000/bus` |
| 对象路径 | `/org/freedesktop/IBus`（私有） | `/org/freedesktop/IBus`（会话上的兼容节点） |
| 协议/信号 | ibus 标准 | 同一套 |

框架不修改系统输入法，只按用户已配置的 `GTK_IM_MODULE/QT_IM_MODULE/XMODIFIERS` 单选一条总线，用 `FlagNoAutoStart` 阻断 D-Bus 激活。

**框架热切身份模型（B23 · 硬）：** 判断「能不能继续用现有 IC」必须比对**归属者身份**，不得只看「还有没有人活着」。建 IC 时记下当时 `org.freedesktop.IBus` / `org.fcitx.Fcitx5` / `org.fcitx.Fcitx` 的**唯一归属名**（`:1.2066` 这类），后续按此比对：
*   归属者没变 → 只是抖了一下，**复用 IC**（B22，防重建导致 fcitx5 回落 `keyboard-us`）；
*   归属者从 A 变成 B 且都非空 → **换了框架**，销毁旧 IC 重建；
*   当前无人归属 → 守护暂时消失，**保住 IC** 不清空（B22）。
理由：只看存活会把「fcitx5 让位给 ibus」误判成「还在」，从而复用一个挂在已消失服务方上的死 IC。环境变量（`GTK/QT/XMOD`）说的是用户**声明**要哪家，唯一归属名说的是总线上**实际**是谁在干活——本机实测两者不一致（声明 `ibus`、实际 `fcitx5` 占着 IBus 名），故切换时必须以后者为准。声明的那家在总线上已无名字时，探测目标**单选**校正到实测活着的框架（仍只探一家，不双探、不跨家兜底）。

**连接模型（B23b · 硬）：换了框架往往连总线地址也换了，识别出来还得连得上。**

实测两种守护的总线拓扑不同，这是「换输入法必须重启程序」的根因：

| 守护 | IBus 服务开在哪 | 地址来源 |
|---|---|---|
| fcitx5（ibusfrontend） | **会话总线** `unix:path=/run/user/1000/bus` | `DBUS_SESSION_BUS_ADDRESS`，切换前后不变 |
| 真 ibus-daemon | **私有总线** `unix:abstract=...` | 自己另建一条，新地址写进 `~/.config/ibus/bus/` |

因此「识别出换了框架」只是第一步，第二步必须**重新解析地址并重新拨号**——沿用在启动时缓存的那条连接，新守护上根本建不出输入上下文（实测报错：对象在路径 `/org/freedesktop/IBus` 处不存在）。

实现约束（高可用）：
*   `dialIbusPrivate(force bool)`：**可失效缓存 + 单飞**（`sync.Cond`）。平时严格单连接复用（S1 多窗口连接数恒为 1 不变），仅 `force=true`（确认换框架）时作废缓存重拨；并发触发只拨一条，不因多窗口同时重探而连接数爆炸。
*   引擎差异由 `Bus(force bool)` 承载：`ibus` 强制重拨，`fcitx5` 复用会话总线（守护换人不改会话总线地址，只需重建 IC，不重拨）。
*   重探两轮：先用现有连接试（代价最低），失败或已确认换框架再强制重拨。
*   **先立后破**：新连接上建出 IC 并验证可用后才写回 `im.conn`、才销毁旧 IC；重拨失败只节流，不清空现有会话，避免退化成完全不能输入。
*   旧 IC 必须用**旧连接**销毁（销毁发到新总线无效会泄漏）；新连接通过 `x11PropagateConn` 传播给同进程其他窗口，避免「只有当前窗口能用」。
*   `icOwnerSame` 的「无人应答」分支须再判 `conn.Connected()`：连接还活着＝守护抖动（B22 保 IC，防 fcitx5 回落 `keyboard-us`）；连接已断＝旧守护连同它那条总线一起没了，必须重探，否则永远卡在死连接上。

> **纪律（`fcitx5` 为基线）：** `fcitx5` 会话总线路径现已可用为基线，`ibus` 私有总线实现期间保持两引擎代码、连接、单测完全隔离——禁止在 `x11_ime_fcitx.go` / 会话总线 / `fcitx` 信号路径上改动，`ibus` 另线在 `x11_ime_ibus.go` / 私有总线内单走，直至两引擎各自单跑 `P12（双引擎均过 P1-P9）` 通过后再合。期间任一 `fcitx` 回归失败即停。
...
- **依赖与总线（无 cgo · 见 A5）**：`godbus/dbus/v5` 纯 Go。`ibus` 私有（`~/.config/ibus/bus/*` → `IBUS_ADDRESS`，验 `PID`、跳 `fcitx` 污染，`dialIbusPrivate` 单例）vs `fcitx5` 会话（`SessionBus` 单例），`AddMatch` 分别订阅，`FlagNoAutoStart` 防激活，无守护降级 `nil`，`GPUI_IME_DEBUG` 打点。详见 A5 总线表。
- **打开与探测（Q2 单选 · 官方签名）**：`BusObject.Call` 同步单探（`GTK/QT/XMOD` 含 `ibus`→只探 `ibus` 私有、`fcitx`→只探 `fcitx5` 会话、空值单探 `ibus`，`FlagNoAutoStart` 防激活，`500ms` 超时，`GPUI_IME_DEBUG` 记退化）——
  - **ibus（官方 `src/ibusbus.c:ibus_bus_create_input_context`，三层第1层 `x11_ime_proto.go` + 第2层 `x11_ime_ibus.go` 私有总线）**：`service org.freedesktop.IBus`，`object /org/freedesktop/IBus`，`interface org.freedesktop.IBus`，`CreateInputContext(s client_name) → o`（`g_variant_new("(s)", client_name)`，`client_name="gpui:<进程名>"`，官方面 `s` 单参；老版双参 `ss` 仅作兼容探测），成功得如 `/org/freedesktop/IBus/InputContext_7`，接口 `org.freedesktop.IBus.InputContext`；
  - **fcitx5（官方 `fcitx/fcitx5-dbusfrontend/src/dbus/dbusfrontend.cpp:DBusFrontend::createInputContext`，三层第2层 `x11_ime_fcitx.go` 会话总线，复用同一协议层）**：`service org.fcitx.Fcitx5`（兼容 `org.fcitx.Fcitx` 老名），`object /org/fcitx/Fcitx5/InputMethod`（老路径 `/org/fcitx/Fcitx/InputMethod`），`interface org.fcitx.Fcitx5.InputMethod`，`CreateInputContext(ss appname, s appid) → o`（`appname=argv[0]/"gpui"`, `appid="gpui"`；`godbus Call(FlagNoAutoStart, "CreateInputContext", appname, appid)`），成功得如 `/org/fcitx/Fcitx5/InputContext_2`，接口 `org.fcitx.Fcitx5.InputContext`；
  - 单探失败→ `nil IME`（不 fallback 另一家，严格按用户配置）。每 `Window` 一 `InputContext`（非全局单例），`Window.Close` 时 `Destroy`（`ibus: proxy Destroy` / `fcitx5: DestroyIC`）并 `RemoveMatch`，避免泄漏。
...
- **守护重启与热切（godbus NameOwnerChanged · 懒重连：下次输入按 Q2 单选重探）**：`godbus` 同时订阅 `org.freedesktop.IBus` 与 `org.fcitx.Fcitx5`（兼容 `org.fcitx.Fcitx`）的 `NameOwnerChanged`（`sender='org.freedesktop.DBus',member='NameOwnerChanged'`，`ibus/src/ibusbus.c:_connection_dbus_signal_cb` 同源），**不立即重建**——三态分列（B22/B23）：`owner` 由有变空→ 标记 `imeDirty=true` 并**保留 IC**（不 Destroy、不 FocusOut，防重建导致 fcitx5 回落 `keyboard-us`，见 B22；但若**连接本身已断**则判为守护连同它那条总线一起没了，须重探），由空变有→ 仅标记 `imeDirty=true`；**由 A 变 B（old/new 均非空）→ 过户，标记 `imeDirty=true` 并强制下次重建**（这是「用户换了输入法框架」的信号形态，旧 IC 已挂在消失的服务方上，见 B23）。重建时按 B23b **重新解析地址并重拨总线**（换框架往往连总线地址也变了），新连接验证可用后才写回并传播给多窗口，**先立后破**：新 IC 建好后才销毁旧 IC，重拨失败只节流、不清空现有会话。**真正重建在下次输入时懒触发**：`EnableIME/FocusIn`、`UpdateCursorRect`、`ProcessKeyEvent`、`SetSurroundingText` 任一入口先判 `imeDirty||IME==nil`，则**按当前 Q2 单选顺序单探**（`GTK/QT/XMOD` 含 `ibus`→只探 `ibus` 私有总线 `s→o`，含 `fcitx`→只探 `fcitx5` 会话总线 `ss→o`，空值单探 `ibus`，`FlagNoAutoStart`，`500ms` 超时），成功则清 `imeDirty` 并补 `FocusIn+SetCapabilities/SetCursorRect+SetSurroundingText`，失败保持 `nil` 仍直通。指数退避 `200ms→2s` 仅用于 `Bus` 断开（`conn.Signals` 关闭/`read error`）后的重拨，`P13` 守护重启与 **ibus↔fcitx5 热切**（按单选重探，框架不改系统输入法）均走此“脏标记→下次输入重探”路径，满足“切换后下次输入自动用当前单选的输入法”。
...
| 版本 | 说明 |
|---|---|
| **v3.23 2026-09-13** | 修 `S3` 单测 `-race` 数据竞态（**B26，P2，已完整落地**，见 §15）。根因：`x11Host.wake` 通道两处懒创建（`WakeUp` 任意线程 + `WaitEvents` 事件线程）裸写无锁；S3 假 host 连上本机真 fcitx5 后，后台信号协程与测试主线程同时进 `WakeUp` 即撞（干净树同红，与 B25 无关）。修法：新增叶锁 `wakeMu`，`wake` 通道经 `wakeChan()` 统一建取、自管道 `wakeW` 读写同锁（三处调用点全收敛，无行为变更）。回归：`TestX11S3SessionAnchor -race` 连跑 3 次全绿 + B25 新 5 例 `-race` 绿 + S1/S2/预编辑/router 20 例全绿 + `go build ./ui/...` 通过。 |
| **v3.22 2026-09-13** | 修复键盘切输入法（中/拼/en）小框不跟光标（**B25，P1，已完整落地**，见 §15）。修法（用户已确认方向：不动 F-D3 平时门禁，只在切换那一下补刷一次）：新增 `refreshCursorOnce()`（推预热缓存里的最新锚点，仍要求 `focused+hasRect`，限 press 只刷一次），在同步/异步两条 `ProcessKeyEvent` 路径的“输入法吃掉按键（`handled`）且非组合期”分支调用（修饰键 `Shift` 切中/en 与触发键 `Ctrl+Space` 两条都覆盖，4 处判据收敛为共用的 `needSwitchBurst`，行为不变；组合期不补由 preedit 变更上报；导航键直通分支不补）。F-D3 与 S3 门禁原文不动（`TestX11S3SessionAnchor` 原样全绿）。新增 `ui/platform/x11_ime_switch_refresh_test.go` 5 例（吃掉-press 补刷 1 次且矩形即缓存值/组合期静默/release 与未吃掉静默/失焦静默/异步同样只刷 1 次）。**回退验证**：藏起修复后补刷 2 例立刻 FAIL（`cursor sends = 0, want 1` / `async switch burst never sent`），其余 3 例“静默”锁死防多报。真窗取证（`ui_text_edit_accept` A 框，`GPUI_IME_DEBUG=1`）：① `Ctrl+Space→handled=true` 时 `refreshCursorOnce` 带当前光标实发（`{361.21,124.4}`，与切换前最后一次实发 `{305,…,w=38.9}` 差 56px）；② 纯英文打字期 0 实发（F-D3 保持）；③ 未修复版对照轮小框中心偏光标约 20px（钉在老位置），修复版约 2–3px（贴住光标），小框上沿距光标下沿 2px。回归：`go build ./ui/...` 通过 + 新单测 `-race` 干净 + S3/预编辑/S1-S2/引擎单选/router 20 例全绿。诚实注记：`Shift` 单键在本机 ibus 兼容节点下回 `handled=false`（不触发补刷，一并实测确认）；抓屏处合成器坐标与 X 坐标有整体平移，本轮像素结论全部取自“小框—光标”相对距离（与整体平移无关）。 |
| **v3.20 2026-09-02** | 修复「`fcitx5→ibus` 正常、反向 `ibus→fcitx5` 失败」的方向不对称（**B24，P0，已完整落地**，见 §15）。根因是**观察者随私有总线一起消失**：程序跑在真 ibus 上时握着它那条 `unix:abstract` 私有总线，守护一退该总线连同挂在其上的 `NameOwnerChanged` 观察者**一起没了**，此后**无人**能通知程序输入法已换；而 `objectPath` 仍非空、脏标记无人置位，懒重探直接早退，只能重启进程。反向不受影响：fcitx5 把服务开在会话总线（由 `dbus-daemon` 提供、与输入法无关），守护换人它还在，观察者也就还在。修复：新增 `sessionUsable()`，在「无脏标记但有 IC」时补判窗口连接是否仍通向活服务方（连接活着＝观察者还在、信号有效，零 D-Bus 往返；连接断了＝补查会话总线，因为 fcitx5 只出现在那里），必要时自行触发重探，不再依赖收不到的信号。同批修三处同源缺陷：① `ensureReprobe` 抢先把探测连接写回 `im.conn`，导致旧 IC 被发到无关总线销毁、多窗口传播匹配错对象（`TestX11MultiWindowConnPropagation` FAIL→PASS）；② `imeForX11` 自建取总线分支与引擎层重复，使引擎层回落对建窗路径不生效；③ 私有地址文件被写成空值时 `ibusEngine.Bus` 应回落到会话总线（本机 fcitx5 跑 ibusfrontend 实测会写空条目）。另修 `im.conn` / `imeDirty` 的未加锁读取（`-race` 报出）。新增 `x11_ime_switch_watch_test.go`（真拓扑复刻 + 空地址文件回落 2 例）。**回退验证**：撤掉 `sessionUsable` 调用后 `TestX11SwitchRealIBusToFcitx5Frontend` 立刻 FAIL。回归：`ui/platform` 全部测试文件逐个跑全绿 + 14 例 `-race` 无竞态 + `ui/input`/`ui/textinput`/`ui/embedder`/`ui/application` 全绿 + 真机端到端确认（`GTK_IM_MODULE=ibus`、实际守护 fcitx5、地址文件空值下仍能建出 `InputContext`）。**测试卫生（新增硬纪律）**：`x11_ime_switch_*.go` 一律把 `DBUS_SESSION_BUS_ADDRESS` 切到 `startBareBus` 起的私有总线后再建 IME；假守护**禁止**在用户真实会话总线上抢 `org.freedesktop.IBus`（实测会把名字从用户正在用的 fcitx5 手里抢走，导致其重写出空地址文件，直接破坏用户当下输入能力）。 |
| **v3.21 2026-09-02** | 本轮改动的**收敛优化**（无功能变更，纯整理）。① 删 A5 分层遗留的死代码：`tryCreateIbus/tryCreateFcitx5/setCapabilitiesIbus/setCapabilitiesFcitx`（已被第2层引擎方法取代）、`daemonAlive()`（职责已由判据更严的 `icOwnerSame()` 接管）、`sharedDBusConn()` 与 `x11Shared*` 会话总线单例（与 `sharedFcitxConn()`/`x11Fcitx*` 完全重复，却各起一对观察者循环＝双重订阅）。② 收敛 `imeForX11`：抽 `tryBusFor(engine)` 统一「取总线 + 验名字可见」，消除 ibus/fcitx 两支重复；目标校正改为**先按声明试**——声明的那家可用就不连会话总线，避免为纯 ibus 用户凭空多出一条会话总线连接（**实测该连接自带的观察者会让「fcitx5 上线」变成信号可达，把真正需要兜底的「信号收不到」场景掩盖成测试通过**——这一条是本轮最有价值的发现）。③ 收敛 `ensureReprobe`：把「加锁→解锁→再加锁」的锁 dance 改为**一次持锁取快照**，消除中途状态被并发改写的窗口。④ 时序常量集中为 `probeTimeout/probeThrottleInterval/ownerCheckInterval`，消除散落各处的 `500*time.Millisecond` 字面量。⑤ 观察者循环陈旧判定抽为 `x11ConnIsCurrent(conn)`（原两循环各写一遍三单例比对）；`x11BusWatchLoop` 重连改为按「这条连接属于哪条总线」分派（`reconnectBusFor`），修掉旧代码把 ibus 私有总线重连结果写回会话总线单例的错配。净减 79 行。回归：逐文件全绿 + 19 例 `-race` 无竞态 + 关联包全绿 + 真机可用；并复验 `sessionUsable` 仍为承载性修复（撤回即 FAIL）。 |
| **v3.19 2026-09-02** | 根治「换输入法后必须重启程序」（**B23b，P0，真机实测取证，已完整落地**，见 §15）。真机复现：用户从 fcitx5 切到真 ibus-daemon 后，新守护监听的**总线地址变了**（fcitx5 开在会话总线、真 ibus-daemon 另开 `unix:abstract` 私有总线并改写 `~/.config/ibus/bus/`），而 `dialIbusPrivate` 用 `sync.Once` 永久缓存连接，重探仍走旧连接 → 建不出上下文 → 只能重启进程。修复：`sync.Once` 改为「可失效缓存 + 单飞（`sync.Cond`）」，引擎接口加 `Bus(force bool)`（ibus 强制重拨 / fcitx5 复用会话总线），重探两轮（先用现有连接、失败或确认换框架则强制重拨），新连接验证后再写回并传播给多窗口，旧 IC 先立后破。`icOwnerSame` 补 `conn.Connected()` 判定：连接活着＝B22 抖动保 IC，连接断了＝旧守护连同总线一起没了必须重探。新增 `x11_ime_switch_real_test.go`（6 例，复刻「旧守护在 A 总线、新守护在 B 总线且地址文件被改写」的真实现场）。**回退验证**：恢复旧 `sync.Once` 语义后 2 个核心用例立刻 FAIL，证明修复承载性。真机冒烟：fcitx5（pid 2457668）下建 IC 正常、同守护抖动后复用不重建（B22 防护有效）。回归：X11 IME 全部测试文件逐个跑全绿 + 25 例 `-race` 无竞态 + `ui/input`/`ui/textinput`/`ui/embedder`/`ui/rendering`/`ui/application` 全绿。 |
| **v3.18 2026-09-02** | 修复用户在系统里切换输入法框架（ibus↔fcitx5）后程序感知不到、新输入法接不上（**B23，P0，私有真总线端到端取证，已完整落地**，见 §15）。三处根因：过户分支被 `else` 吞掉、`daemonAlive()` 只看存活不知身份、IC 无归属者记录。修法：过户显式置脏 + `icOwnerSame()` 按唯一归属名（`:1.x`）比对身份 + `icOwner` 建 IC 时快照 + `x11ProbeOrderForOwner()` 把目标单选校正到实测活着的框架。同时保留 B22 防护（同人抖动仍复用 IC，不回落英文）。新增 `ui/platform/x11_ime_switch_test.go`（身份判据/目标校正纯函数 + 真机用例）与 `x11_ime_switch_bus_test.go`（起私有 `dbus-daemon`、两个假输入法服务做真 `NameOwnerChanged` 过户，断言旧 IC 销毁、新 IC 来自新服务方），两处修复均经「回退即 FAIL」验证。回归：X11 IME 全部测试文件逐个跑全绿 + `-race` 无竞态 + `go build ./ui/...` 通过。**未动 B21 ②③**（事件层 `platform.Event.IMESegments` 与绘制层 `InputBox.syncPreeditStyle` 仍未实现，画面仍无 preedit 下划线/选中背景，B21 保持「部分修复」不得标 ✅）。 |
...
| **v3.16 2026-09-01** | X11 IME 三层架构落文档（A5）：1639 行 12 处绞合拆为统一层/引擎层/协议层；`fcitx5` 复用 `ibus` 协议（兼容节点）；Q2 单选（`GTK/QT/XMOD` 只探一家，`FlagNoAutoStart` 防激活）；S1 私有总线解析（读 `~/.config/ibus/bus/*` 验 `PID`、跳过 `fcitx` 污染，`dialIbusPrivate` 单例）与 S2 单探落表；框架不改系统输入法 |
| **v3.17 2026-09-02** | 修复 fcitx5 不能切换/不能输入（**B18-B20，真机实测取证，已完整落地**，见 §15）；**B21 部分修复（仅解析层，画面仍无下划线）**：`ime_format.go` 按线上 `(sa{sv}av)` 结构解析 AttrList 已落地并单测锁死，但**事件层（`platform.Event.IMESegments` / `input.Event.Preedit`）与绘制层（`InputBox.syncPreeditStyle`）均未实现**，segments 解析后仅存于 `x11Ime.lastSegs` 无消费者，故用户画面仍无下划线/选中背景，B21 不得标 ✅；**B22 已完整修复**（守护「有→空」时保留 IC 不销毁重建，`ensureReprobe` 加 `daemonAlive()` 判定复用；实测 Destroy 后重建会让 fcitx5 回落 `keyboard-us`，只 FocusOut/In 则状态保持）。回归：`x11_ime_preedit_state_test.go` 4 例全绿 + `x11_ime_compat_test.go` 5 例全绿 + `go test ./ui/platform ./ui/input ./ui/textinput` 全绿 + `-race` 无竞态；真机端到端实测 `Ctrl+Space→handled=true`、`nihao→preedit n/ni/nih/niha/o`、空格→`CommitText 文本="为"`；真机 B20 根因复现：`uuu` 被接受、`uuuub` 报 `Invalid arguments`。**注**：原 v3.17 行把 B21 三段误记为全部打通，本行已更正为部分修复 |：B18 私有总线解析按「含 `fcitx`」字样过滤，误杀 `fcitx5(ibusfrontend)` 自写的合法条目→候选归零→`IME=nil`（改按 PID 存活判定，删 `x11FileContainsFcitx`）；B19 `fcitx5` 原生 `/org/fcitx/Fcitx5/InputMethod` 未导出时无回落→IC 建不出（新增回落到 fcitx5 自提供的 `/org/freedesktop/IBus` 兼容节点，`fcitxEngine.compat` 标记）；B20 `ProcessKeyEventAsync` 拆分时漏收编、仍硬编码 fcitx `uuuub` 签名，在 IBus 接口上 `Invalid arguments`→按键判为未消费→`Ctrl+Space` 切换失效（改走 `engineImpl.ProcessKeyEvent` 与同步路径同源）。回归：`x11_ime_compat_test.go` 5 例全绿 + `go test ./ui/...` 全绿；真机端到端实测 `Ctrl+Space→handled=true`、`nihao→preedit n/ni/nih/niha/o`、空格→`CommitText 文本="为"` |
...
|---|---|---|---|---|
| **S1 总线** | 依赖与总线 + **三层地基/A5** + 地址解析（Q2）+ **可失效缓存与单飞（B23b）** | `godbus/dbus/v5` 纯 Go。`ibus` 私有总线（`x11ResolveIbusAddress` 读 `~/.config/ibus/bus/*` → `IBUS_ADDRESS` 私有 `unix:abstract`，验 `IBUS_DAEMON_PID` 存活、按 PID 判失效，`dialIbusPrivate(force)` **可失效缓存 + 单飞**复用）与 `fcitx5` 会话总线（`SessionBus` 单例）分流；`AddMatch` 分别订阅；`FlagNoAutoStart` 阻断未拥有名字的 `StartServiceByName`；无守护 `Window.IME()==nil` 降级；`GPUI_IME_DEBUG` 打 `dial/hello/match`；**只按 `GTK/QT/XMOD` 单选一条总线，不双探**；**`fcitx5` 为基线，`ibus` 另线，期间不改 `x11_ime_fcitx.go`**。**缓存纪律**：平时严格单连接复用（多窗口连接数恒为 1），仅在「确认换了输入法框架」时 `force=true` 作废缓存重新解析地址并拨号——换框架往往连总线地址也变了，不重拨则新守护上建不出 IC（B23b） | `go.mod` + 新建 `ui/platform/x11_ime_proto.go`（第1层协议）+ `x11_ime_ibus.go`（第2层私有总线+生命周期）+ `x11_ime_fcitx.go`（第2层会话总线，S1 期间不改动）+ `x11_dbus_ime_linux.go` 瘦身为第3层统一调度 + `x11_linux.go:imeForX11` 去桩 | 建窗不阻塞；`ibus` 私有与 `fcitx` 会话各单例，多窗口连接数恒为1；有/无守护 `go vet 0`；`P8` 前 `nil` 降级；**污染文件被跳过**（`fcitx_random_string` 死 PID 旧文件不被选中）；`TestX11ResolveIbusAddress_SkipPolluted` 无头可测 |
| **S2 上下文** | 打开与探测 + 能力声明 + **Q2 单选 & 异步化**（S8/S10） | **只认环境变量单探**（`GTK/QT/XMOD` 含 `ibus`→只探 `ibus`，含 `fcitx`→只探 `fcitx5`，空值单探 `ibus`，**绝不 fallback 双探**，`FlagNoAutoStart` 防激活），各 `500ms` 超时；`ibus CreateInputContext(s)→o`（`src/ibusbus.c`，走私有总线 `x11_ime_ibus.go`）与 `fcitx5 CreateInputContext(ss)→o`（`DBusFrontend.cpp`，走会话总线 `x11_ime_fcitx.go`）共用协议层 `x11_ime_proto.go`；成功缓存 `engine` 并 `SetCapabilities`（`ibus 41` / `fcitx5 PREEDIT\|SURROUNDING`）；**全异步化**，完成前 `IME==nil` 直通不卡建窗；每窗口一 `InputContext`，`Close` 时 `Destroy+RemoveMatch`；**期间不改 `fcitx` 路径** | `x11_dbus_ime_linux.go:S2`（瘦身后仅调度）+ `x11_ime_proto.go` + `x11_ime_ibus/fcitx.go`（`fcitx` 侧 S2 期间不改动） | `P12 双引擎` 各按 `GTK=ibus/fcitx` 单走一条且均过 `P1-P9`；空值单探 `ibus` 不激活 `fcitx5`；双窗路径不同；`Close` 不泄漏；`TestX11ProbeOrderStrict` 无头可测 |
...
| **S2 上下文** | 探测顺序 `GTK_IM_MODULE/QT_IM_MODULE/XMODIFIERS` 单选（Q2） | `GTK_IM_MODULE=ibus` / `=fcitx` / 空 各跑一次，点框获焦 | `GTK=ibus→只探 ibus`，`GTK=fcitx→只探 fcitx5`，空值单探 `ibus`（不双探），符合单选且不激活另一家 | 日志 `probe order: ibus / fcitx5` 单条 + `CreateInputContext(s)→o / (ss)→o` 500ms 超时（`FlagNoAutoStart` 阻断激活）|
...
| **S6 稳定与分发** | `NameOwnerChanged` 三态（有→空 / 空→有 / **A→B 过户**）置 `imeDirty=true` 懒重探 + **换框架重拨总线（B23b）** | 运行中 `killall ibus-daemon; fcitx5 &` 热切 `ibus↔fcitx5`，下次再打字 **不重启程序** | 有→空**保留 IC**（不 Destroy/不 FocusOut，B22 防回落英文；但若连接本身已断则判为守护连同总线一起没了，须重探）；空→有仅脏标记；**A→B 过户判为换框架，下次输入按实测活着的框架重建**。重建时先试现有连接、必要时**强制重拨总线**（新守护可能监听在另一条总线上），新连接验证可用后才写回并传播给多窗口（先立后破：新 IC 建好才销毁旧的），补 `FocusIn+SetCapabilities+SetCursorRect+Surrounding` | 日志 `NameOwnerChanged owner有→空/空→有/A→B handover imeDirty=true` + `icOwner: framework switched` / `no owner and conn dead` + `conn rotated` + `conn propagated to N window(s)` + `lazy reprobe success/clear dirty`；`P13` PASS |
...
## 15. 已复核缺陷清单（2026-09-01 真源 · B1-B25）
...
| # | 问题 | 复核结论 | 根因定位 | 优先级 | 修复标记 |
| **B26** | **`S3 单测加 `-race` 报 `x11Host.WakeUp` 数据竞态** | ✅ 真实（`TestX11S3SessionAnchor -race` 稳定复现，干净树同红，与 B25 无关） | **`h.wake` 懒创建无锁 + 单测连上真总线**。`WakeUp()` 与 `WaitEvents()` 两处 `if h.wake == nil { h.wake = make(...) }` 裸写；S3 的假 host 调 `EnableIME` 触发重探连上本机真 fcitx5（建真会话、起收信号后台协程），真信号一到后台协程与测试主线程同时进 `WakeUp` 即撞。普通模式时序错开故绿。线上同函数首调亦有理论竞态 | **P2** | ✅ 已修复（v3.23）：见 §13 v3.23 行 |
| **B25** | **键盘切换输入法（中/拼/en）时弹出的小框没跟住光标，钉在老位置** | ✅ 真实（2026-09-13 用户真窗目击 + 本轮真窗复现取证，`examples/ui_text_edit_accept` A 框） | **非组合期锚点不实发，fcitx 切换小框只能按老矩形摆位**。`x11_dbus_ime_linux.go UpdateCursorRect` 仅 `composing==true` 才调 `SetCursorLocation/SetCursorRect`（F-D3），平时打字/点框移动光标只走预热（`lastRect` 缓存已是新的，但 D-Bus 侧仍是 `EnableIME`/上次组合期的老值）。fcitx5 classicui 的切换小框按最后收到的光标矩形摆位，故错位。Wayland 侧 `UpdateCursorRect` 每次都报，无此问题（跨端不对称）。另：框架级切换（`NameOwnerChanged`）与焦点切换（`EnableIME`）本来就会重发，只有**框架内切中/拼/en**（切换键被输入法吃掉、`handled=true`）这条路是漏的 | **P1** | ✅ 已修复（v3.22）：见 §13 v3.22 行 |

| # | 问题 | 复核结论 | 根因定位 | 优先级 | 修复标记 |
|---|---|---|---|---|---|
| **B24** | **`ibus→fcitx5` 切不过去（反向 `fcitx5→ibus` 却正常）** | ✅ 真实（真机拓扑复刻取证） | **观察者随私有总线一起消失**。程序跑在真 ibus 上时握着它那条 `unix:abstract` 私有总线，`NameOwnerChanged` 观察者就挂在这条总线上；守护一退，该总线连同观察者**一起没了**，此后**没有任何人**能给程序报「输入法换了」。而 `ensureReprobe` 的触发条件是 `imeDirty || objectPath==""` —— 脏标记没人置、`objectPath` 仍是旧 IC（非空），于是 `need=false` 直接早退，程序抱着死连接和过期 IC 一动不动，只能重启进程。**反向不受影响**：fcitx5 把服务开在会话总线（由 `dbus-daemon` 提供，与输入法无关、长命），守护换人它还在，观察者也就还在，信号正常送达 —— 这就是「fcitx→ibus 正常、ibus→fcitx 失败」这条不对称的全部由来。另：`B23b` 的 `icOwnerSame()` 只在 `imeDirty` 为真时才跑到，覆盖不到这条「信号根本收不到」的路径 | **P0** | ✅ 已修复（v3.20）：新增 `sessionUsable()`，`ensureReprobe` 在「无脏标记但有 IC」时补判当前连接是否仍通向活服务方——连接活着（零 D-Bus 往返，且保住 B22 抖动不重建）判为可用；连接断了则补查会话总线（fcitx5 只出现在那里，只盯手里的死连接永远看不见它），确实有人就自行触发重探。另修三处同源缺陷：① `ensureReprobe` 抢先把探测连接写回 `im.conn`，使后续 `oldConn` 取到被顶替值 → 旧 IC 被发到无关总线销毁，且 `x11PropagateConn` 匹配错对象导致其他窗口不换（`TestX11MultiWindowConnPropagation` FAIL→PASS）；② `imeForX11` 自建取总线分支与引擎层 `Bus()` 重复，使引擎层回落对建窗路径不生效（`TestX11ImeForX11BothModuleConfigs/GTK=ibus` FAIL→PASS）；③ 私有地址文件被写成空值时（本机 fcitx5 跑 ibusfrontend 实测会写 `IBUS_ADDRESS=`/`IBUS_DAEMON_PID=`）`ibusEngine.Bus` 应回落到会话总线。另修 `im.conn`/`imeDirty` 的未加锁读取（`-race` 报出）。证据：`x11_ime_switch_watch_test.go` 的 `TestX11SwitchRealIBusToFcitx5Frontend`（按真机拓扑复刻「真 ibus 私有总线 → fcitx5 兼容节点在会话总线」，**不手工置脏**，只靠真实路径，断言切换后连接确实通向活服务方）+ `TestX11IBusFallsBackToSessionWhenAddrFileEmpty`。**回退验证**：撤掉 `sessionUsable` 调用后核心用例立刻 FAIL（报「连接不通向活着的服务方」） |
| **B1** | X11 DeleteSurrounding 多删字符 | ✅ 真实 | `x11_dbus_ime_linux.go:740/828` 推 `Start=offset, End=n`（IBus 原生“从 caret+offset 起删 n 个”），`composition.go:34` 却按 Wayland `Start=-before, End=+after` 当 `before=-Start, after=End` → `DeleteSurrounding(-before, before+after)`，`offset=-1 n=1` 误算 `(-1,2)` 多删 `\|offset\|` 个。`editor.go:670 DeleteSurrounding(offset,count)` 本身以 `offset+count` 为右界是对的，错只在转接层 | **P0** | ✅ 已修复 `95c5ee3` `pushDeleteSurrounding` 转 `before/after` byte |
| **B2 弱化** | 守护消失后每键建一个 InputContext 泄漏 | ✅ 真实（弱化版） | 原“卡 2.5s”错：D-Bus 对“服务不存在”即时回错（实测 236~620µs）。弱化版成立：`x11_dbus_ime_linux.go:403-407 ensureReprobe` 失败仅 `imeDirty=false, objectPath=""`，下次 `need=true` 又全量双探且 `tryCreateIbus` 每次 `CreateInputContext` 成功即产新 `InputContext_xxx`，旧路径 `x11MarkDirty:130` 直接 `objectPath=""` 未 Destroy | P1 | ✅ 已修复 `95c5ee3` 500ms 节流+真 Destroy |
| **B3** | 总线重连后 IME 永久失效 | ✅ 真实 | `imeForX11:340 conn:conn` 唯一赋值，`x11BusWatchLoop:227-233` 重拨仅回写 `x11SharedConn`，不回写各 `im.conn`，懒重探仍用旧断连 | P0 | ✅ 已修复 `95c5ee3` 回写 `im.conn` |
| **B4** | 重连循环 goroutine 泄漏 | ✅ 真实 | `sharedDBusConn:176-178` 起一对 `GlobalNameOwnerLoop+BusWatchLoop`，`BusWatchLoop:240-242` 重连成功 `go 新global+go 新watch` 后 `return`，旧 `globalLoop:189 range ch` 无 `RemoveSignal/close` 永阻塞，每次重连多留 1 个 | P0 | ✅ 已修复 `95c5ee3` stale guard+RemoveSignal |
| **B5** | x11UnregisterIme 数据竞争+清理不全 | ✅ 真实（-race 可报） | `x11UnregisterIme:102` 裸读 `x11SharedConn`，写侧 `sharedDBusConn:174`/`BusWatchLoop:231` 持 `x11SharedMu`；清理 `dbusMatchRules[:2]` 仅 2 条，漏 `org.fcitx.Fcitx` 与 `NameOwnerChanged` | P1 | ✅ 已修复 `95c5ee3` 持锁读+清 4 条 |
| **B6** | 无 RandR 多显修正 | ✅ 真实 | 全仓 `RandR/XRR` 零命中，`x11_linux.go:222 XTranslateCoordinates` 仅根坐标，无 RandR 输出偏移 | P2 | ✅ 已修复 `c84af2a` 新增 `x11_randr_linux.go` 绑 `libXrandr` 并在 `translateRect` 挂钩 |
| **B7** | Xutf8LookupString 绑定未用 | ✅ 真实 | `x11_linux.go:186/211` 绑定，零调用，死键走 `XLookupString:1278` | P2 | ✅ 已修复 移除死绑定（XIM v2.0 已删，无需） |
| **B8** | fcitx time 用 time.Now 造假 | ✅ 真实 | `x11_dbus_ime_linux.go:1348 time.Now()`，`xevKeyTimeOff=56` 零引用，本应取 `XKeyEvent.time` | P2 | ✅ 已修复 `95c5ee3` 取 `XKeyEvent.time` 透传 |
| **B9** | ime_format.go 死代码 | ✅ 真实 | `ime_format.go: ImeSegment` 与 `DecodeIBusVariant/parseFcitxPreedit` 算出 `segs` 仅 `x11ImeDebug`，未进 `pushPreedit` 的 `Segment` 通道，`input.Segment` 零消费者 | P2 | ✅ 已修复 `c84af2a` `pushPreedit` 透传并存 `lastSegs` |
| **B10** | 三份截断重复 | ✅ 真实 | `textinput/delta.go:142 TruncateSurrounding` 单源，`wayland_textinput_linux.go:351` 内联一份，`x11_dbus_ime_linux.go:1375 x11TruncateSurrounding` 三参版，各自维护 | P2 | ✅ 已修复 `c84af2a` X11 侧边界回退+`TestX11TruncateSyncB10` 单测卡同源 |
| **B11** | SetComposing 注释方向反 | ✅ 真实 | `platform/ime.go:89` 像 IME→App，实为 App→IME 推环绕，`x11_dbus_ime_linux.go:947` 自注释“reports surrounding text” 承认拧巴 | P2 | ✅ 已修复 `95c5ee3` `ime.go` 明确 App→IME/IME→App 方向 |
| **B12** | Commit 两平台语义互斥 | ✅ 真实，影响有限 | `x11_dbus_ime_linux.go:973 Commit` 与 Wayland 同名但语义不同，引擎内零调用，仅 example/测试在调 | P2 | ✅ 已修复 统一为推 `IMEKind Commit`（Wayland `Commit` 现也 `pushIME`） |
| **B13** | ContentPurpose 恒 Normal | ✅ 真实 | `input_box.go:299 / viewport_input.go:229 / base_editable.go:24` 全硬编码 `PurposeNormal`，`editor.go:279 purposeFromInputType` 未被控件透传 | P2 | ✅ 已修复 `c84af2a` `BaseEditable` 可配 `SetContentType` 并委托 |
| **B14** | 测试不断言 handled | ✅ 真实 | `x11_dbus_ime_linux_test.go` 与 `x11_s4_test.go` 未断言 `ProcessKeyEvent handled`，空焦点恒 false 也 PASS | P2 | ✅ 已修复 `x11_s4_test.go` 补 `empty==false`/修饰键不拦/`IsComposing` 断言 |
| **B15** | 真窗无 Golden/像素断言 | ✅ 真实 | `examples/ui_wr_ime_r*/` 无 `golden/`，未按 `UI_PIXEL_ASSERTION_STANDARD.md F0-F9` 落地 | P2 | ✅ 已修复 骨架 `golden/README.md+.gitkeep` 已建，真图待 GPU 跑产 |
| **B16** | 旧 embedder.App 丢 IME | ⚠️ 部分成立 | `host.go` 旧 `App.Run switch` 只显式 `Resize/Expose`，但前置 `if a.opts.OnEvent != nil { a.opts.OnEvent(ev) }` 已透出所有事件（含 IME），不丢失，只是“不做自动路由，PipelineApp 才做” | P3 | 💬 已澄清 文档修正，无代码改 |
| **B17** | AttachIME/SetFocus 重复注册观察者 | ✅ 真实（新增） | `focus/manager.go:108 AddFocusObserver` 明确不去重，`input_router.go:107 SetFocus` 与 `164 AttachIME` 各注册一次，先 SetFocus 再 AttachIME 重复两次，焦点一切换 `syncSession` 跑两遍 | P1 | ✅ 已修复 `95c5ee3` `ensureIMEFocusObserver` 去重 |
| **B18** | 私有总线解析按「含 `fcitx`」误杀合法条目 | ✅ 真实（真机实测） | `x11_ime_ibus.go` `x11ResolveIbusAddressFromDir` 两条过滤：地址含 `fcitx` 跳过 + `x11FileContainsFcitx` 内容含 `fcitx` 跳过。但 `fcitx5` 跑 `ibusfrontend` 时**自己**会往 `~/.config/ibus/bus/` 写条目（`IBUS_ADDRESS=unix:path=/run/user/1000/bus,fcitx_random_string=<hex>`、`IBUS_DAEMON_PID=<fcitx5 自身 PID，存活>`），两条规则全中→候选归零→`dialIbusPrivate` 失败→`imeForX11` 返回 `nil`。实测该地址**可连通且 `CreateInputContext` 成功**，属误杀非防污染 | **P0** | ✅ 已修复：改为按 PID 存活判定（真正失效判据），删除 `x11FileContainsFcitx`；`TestX11IbusBusResolveKeepsFcitxFrontendEntry` 锁死 |
| **B19** | `fcitx5` 原生 InputMethod 缺失时无回落 | ✅ 真实（真机实测） | `x11_ime_fcitx.go` `CreateInputContext` 只试 `/org/fcitx/Fcitx5/InputMethod` 与老 `/org/fcitx/Fcitx/InputMethod`。本机 `fcitx5` 只装 `ibusfrontend` 未装 `dbusfrontend`，**两个原生对象都不存在**（实测 `Unknown object`），探测必然失败→IC 建不出。`GTK_IM_MODULE=fcitx` 时表现为完全无 IME | **P0** | ✅ 已修复：原生不可用时回落到 `fcitx5` 自提供的 `/org/freedesktop/IBus` 兼容节点（仍是 fcitx5 自家入口，不跨家兜底），`fcitxEngine.compat` 标记后续调用改按 ibus 协议；`TestX11FcitxFallsBackToIBusCompatNode` 锁死 |
| **B20** | `ProcessKeyEventAsync` 漏收编、硬编码 fcitx `uuuub` 签名 | ✅ 真实（真机实测） | 窗口实际走 `x11_linux.go:1145 → ProcessKeyEventAsync`（异步路径），但 `40d9b26` 收 13 处 `if eng` 时**漏了它**，仍内联 `if eng=="ibus"{uuu} else {uuuub}`。兼容节点下 `uuuub` 实测报 `Invalid arguments 'uuuub' ... expecting 'uuu'` → `err!=nil` → 兜底当未消费放行 → **`Ctrl+Space` 切换失效**。同步版 `ProcessKeyEvent` 已走 `engineImpl`，两条路径签名漂移 | **P0** | ✅ 已修复：异步路径改调 `engImpl.ProcessKeyEvent`，与同步路径共用同一协议选择；`TestX11FcitxCompatUsesIBusKeySignature` 锁死 `uuu` 被接受 / `uuuub` 被拒 |
| **B21** | preedit 无下划线/选中背景（样式段被丢） | ✅ 真实（真机实测） | 三处断裂：① `ime_format.go parseIBusText` 先试 `dbus.Store` 到 `ibusTextPayload`，其 `Attrs` 是 `[]ibusAttr`，而线上格式是 `dbus.Variant`（`(sa{sv}av)`）→ 类型不匹配落到 `ibusFullPayload` 分支，该分支**直接丢弃 Attrs** 返回 `singleSegment`（实测输出 `segs=[{0,2,attr=1}]`，真机本应 4 条属性）；② `platform.Event` 无承载 segments 的字段；③ `BaseEditable.DrawPreedit` 签名无 segments 参数，且全仓只有测试在调、生产控件未接（ composing 高亮走 `syncSelectionHighlight`） | **P1** | ⚠️ **部分修复（1/3）**：仅①解析层落地，②③未动，**画面仍无下划线/选中背景，不得标 ✅**。① 已修：新增 `parseIBusAttrList`/`decodeIBusAttr` 按线上结构体（`godbus` 解为具名 struct 而非 `[]interface{}`）正确解析下划线(type=1)/前景(type=2)/背景(type=3)，并补 `ImeAttrUnderline/Double/Selected/Highlight` 常量，`TestX11PreeditAttrsParsed`/`TestX11PreeditNoAttrsFallsBack`/`TestX11FcitxPreeditUnderline` 锁死；② **未修**：`platform.Event` 无 `IMESegments` 字段，`fromIME()`（`ui/input/fromplatform.go`）仅搬 `Text/Start/End`，不传 segments；③ **未修**：`InputBox` 无 `SetPreeditSegments`/`syncPreeditStyle`，`InputRouter` 无 `applyPreeditStyle`。**当前数据流**：`DecodeIBusVariant` 解析出 segments → 存入 `im.lastSegs` → **无消费者终止**。修复②③后须跑 `ui_wr_ime_r3_channel` 真窗肉眼确认下划线与反显真实绘制（P16）方可标 ✅ |
| **B23b** | **换输入法后「必须重启程序」才能用新输入法**（总线地址也变了，旧连接连不上新守护） | ✅ 真实（真机实测取证） | **根因不在「识别」，在「连接」**：用户切到真 ibus-daemon 后，新守护监听的**总线地址本身变了**——实测 fcitx5 把 IBus 服务开在会话总线（`unix:path=/run/user/1000/bus`），而真 ibus-daemon 另开一条 `unix:abstract=...` 私有总线，并把新地址写进 `~/.config/ibus/bus/`。实测证据：切换后新起进程拿旧地址建上下文报错「对象在路径 `/org/freedesktop/IBus` 处不存在」。而 `dialIbusPrivate` 用 `sync.Once`，**成败都永久缓存**，`ensureReprobe` 重探时仍用 `im.conn` 那条启动时建立的旧连接 → 永远连不上新守护，只能重启进程重置 `Once` | **P0** | ✅ 已修复：① `dialIbusPrivate(force bool)` 改为「可失效缓存 + 单飞（sync.Cond）」，`force=true` 作废缓存重新解析地址并拨号，平时仍严格单连接复用（S1 纪律不变）；② 引擎接口新增 `Bus(force bool)`，ibus 走私有总线强制重拨、fcitx5 走会话总线无需重拨（守护换人不改会话总线地址）；③ `ensureReprobe` 改为「先试现有连接 → 失败或确认换框架则强制重拨」两轮，覆盖三种触发（归属者变了 / 当前连接已无人应答 / IC 建不出来）；④ 新连接验证可用后才写回 `im.conn`，旧 IC 用**旧连接**销毁，新 IC 建好后才销毁旧的（先立后破）；⑤ `x11PropagateConn` 把新连接传播给同进程其他窗口，避免「只有当前窗口能用」；⑥ `icOwnerSame` 增加 `conn.Connected()` 判定：连接还活着只是名字暂时没人 → B22 保住 IC；**连接本身已断 → 判为旧守护连同它那条总线一起没了，必须重探**（修前此处恒返回 true，导致死连接上永远卡住）。证据：`TestX11SwitchToDaemonOnDifferentBus`（复刻真机：旧守护在 A 总线、新守护在 B 总线且地址文件被改写，断言连接轮换 + 新连接可用）+ `TestX11IbusBusRefetchOnForce` + `TestX11MultiWindowConnPropagation` + `TestX11DialSingleFlight`（8 并发只建 1 条）+ `TestX11ReprobeKeepsOldConnOnFailure`（重拨失败保住旧会话）+ `TestX11AddressResolutionPicksNewest`。**回退验证**：完全恢复旧 `sync.Once` 语义后，`TestX11SwitchToDaemonOnDifferentBus` 与 `TestX11IbusBusRefetchOnForce` 立刻 FAIL（报「连接未切换：仍用旧总线连接」），证明修复是承载性的而非偶然通过 |
| **B23** | 用户切输入法框架（ibus↔fcitx5）程序感知不到、新输入法接不上 | ✅ 真实（私有真总线端到端取证） | 三处叠加：① `x11MarkDirtyForOwnerChange` 把 `NameOwnerChanged` 的**过户形态**（old 非空→new 非空）落在 `else` 空分支里什么都不做，切框架被当成「无变化」忽略，脏标记永不置位；② `ensureReprobe` 用 `daemonAlive()`（只问「还有没有人活着」）判断能否复用 IC，换框架后总线上依然有人，故判为「没变」继续复用挂在已消失服务方上的旧 IC；③ IC 创建时**未记录归属者**（`:1.2066` 这类唯一名），无从区分「同名抖动」与「真换框架」。另 `imeForX11` 在声明的那家没名字时直接降级 `nil`，用户切完框架后**新建的窗口**永远英文直通 | **P0** | ✅ 已修复：① 过户分支显式置 `imeDirty`（与「守护消失/上线」三态分列）；② 新增 `icOwnerSame()` 按归属者身份比对（同人复用保 B22、换人强制重建），`daemonAlive()` 降级为仅判定存活；③ `x11Ime.icOwner` 在建 IC 时记 `snapshotOwner`（IBus/Fcitx5/Fcitx 唯一归属名 + IBus PID）；④ 新增 `x11ProbeOrderForOwner()`，声明的那家没名字时把目标**单选**校正到实测活着的框架（仍只探一家，不双探不跨家兜底），`imeForX11` 建窗路径同步校正；⑤ 顺带收 `ProcessKeyEventAsync` 里 `if eng=="ibus"` 的引擎专属分支（G5）。证据：`TestX11FrameworkHandoverEndToEnd`（私有 `dbus-daemon` + 两个假服务真过户，断言旧 IC 销毁、新 IC 来自 B、目标校正为 ibus）+ `TestX11FrameworkSwitchForcesRebuild` + `TestX11IcOwnerSameKeepsIcOnSameDaemon`（锁 B22 不回退）+ 纯函数 `TestX11OwnerChangedDetectsFrameworkSwitch`/`TestX11ProbeOrderForOwnerCorrectsTarget`；两处修复均已「回退验证」：撤掉过户分支或撤掉身份比对，对应用例立刻 FAIL |
| **B22** | 切回程序任务栏显示英文（能输五笔但状态回落） | ✅ 真实（真机实测） | `x11MarkDirtyForOwnerChange` 在守护「有→空」时清空 `objectPath` 并 Destroy IC，下次输入 `ensureReprobe` 重建。实测三场景对照：A 只 FocusOut/In（不销毁）→ `State=2 CurrentIM=pinyin` **状态保持**；B Destroy 后重建 → `State=1 CurrentIM=keyboard-us` **回落英文**；C 只 Destroy → `State=0`。即「销毁重建 IC」是状态回落英文的成因 | **P1** | ✅ 已修复：守护消失时**保留 IC**（不 Destroy、不清空 objectPath、不发 FocusOut）；`ensureReprobe` 在「有对象+脏标记」时先 `daemonAlive()` 判定，守护仍在则复用 IC 清脏返回，不重建。守护真死时旧 IC 仍在 `Close`/重探成功替换时销毁，不泄漏。`TestX11OwnerChangeKeepsInputContext` 锁死 |

> 修复顺序：P0 B1 → P0 B3+B4 → P1 B17 → P1 B5 → P1 B2弱化退避 → P2 其余规范与门禁补齐 → **P0 B18 → P0 B19 → P0 B20**（fcitx5 不能切换/不能输入，2026-09-02 真机实测取证后修复，已完整落地）→ **P1 B22**（切回后任务栏显示英文，已完整落地）→ **P1 B21 ②③**（事件层 `platform.Event.IMESegments`/`input.Event.Preedit` + 绘制层 `InputBox.syncPreeditStyle`，解析层①已落地，②③待补；补完须跑 `ui_wr_ime_r3_channel` 真窗 P16 像素确认后才可标 ✅）。
