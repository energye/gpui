...
- **VR2d-3 显示参数与附加信息**⬜待办：还差——显示参数（VUI）只记了“有”没往下读，宽高比、颜色范围、帧率都拿不到；附加信息（SEI，第 0 包里 682 字节）没解析。要做——宽高比/范围/时序读出来透给上层，用户数据透传不丢，不认识的纸条跳过不崩。门禁——`vui_sei_test.go`（以头追踪为真值）。
- **VR2d-4 隔行**⬜待办：还差——手里没有隔行片，不清楚真片长啥样。倾向——先认出来（逐行还是隔行分得清），确属罕见制式就走三条件：报出是哪个工具、不崩不卡、进容错用例留档。
- **VR2d-5 五档与真窗**⬜待办：还差——480p/720p 有现成片，1080p 要现造，1440p/4K 只在本地验（不进仓库）；真窗 `video_vr2_decode`（RUN 15）还没建。要做——480p 的裁边（854 宽不是 16 倍数）先解对，五档逐字节一致后建窗收口。
