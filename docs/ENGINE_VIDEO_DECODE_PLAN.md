...
- **STEP-1 窗口减负（先见效，只动示例层）**：现象——同§6.2 RENDER-SLOW-2，每拍约108毫秒，一秒只画约10张。位置——`examples/video_player/main.go`之`state.tick`（每帧都`refreshTime`+`refreshStatus`重排文字）+ `fastBlitRGBA`整窗大图上传。改法——文字降频刷（数字变了才刷）、视频区脏了才传、不动引擎。归属——示例层。验法——跑窗看一秒画出多少张回到24。状态——方向已确认未动手 2026-09-16。
- **STEP-2 解码提速（治本，走视频本线S1/S2）**：现象——单帧约15.6毫秒，一秒最多解64帧，余量只有约2.4倍；热点是去块滤波约两成五、运动补偿约两成、反变换与色度重建各约一成，每帧约1.8万次小分配。位置——`video/player.go`之`Options.S2Parallel` + `maybeDecodeS2Window`（本片28个关键帧组，吃得到并行，理论约1倍）。改法——先开S2并行，输出必须与串行逐位一致，不一致不开；不够再补加速核。归属——视频本线S1/S2。验法——直解吞吐从64帧每秒往上涨，`h264`全包+`video`根门禁逐文件绿。状态——方向已确认未动手 2026-09-16。
