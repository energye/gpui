...
| S2 | VW5 跟得上 | 多线程（先帧级后片级，复用 `pending` 排序骨架） | 🟨 进行中 | GOP核落地（2026-09-15，见§10 v0.66）：IDR分组各起独立解码器并发解按组序拼回（线程`min(核数+1,16)`，单组留守，纯Go双架构同逻辑），长片200帧/40组逐字节==单线程+重叠5/5+加速约2.18x（ffmpeg同片yuv0.54ms只记差距）；Player接线随后；VR7-D小片单串单组吃不到并行预期仍红；未跑真窗故不标🟩 |
...
> | 17 | VW5 跟得上 | S2 多线程 | §12 S2 行 | 🟨 进行中 | GOP核落地+门禁绿（见S2行与§10 v0.66，长片吞吐约2.18x，Player接线随后；VR7-D小片预期仍红），与 S1 同批文件，一前一后 |
