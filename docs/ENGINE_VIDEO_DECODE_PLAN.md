...
| VR2c 高档 | F1（主档与高档各至少一片解对）、F5高压缩熵编码、F6 8x8 帧内余量、F9 8x8 变换与缩放列表 | `video/h264`：上下文自适应熵解码、高档分支 | `cabac_test.go`（查表对照）+ `vr2_m_main.yuv` 与真片前 N 帧（ffmpeg 原始帧）逐字节一致 | ⬜开工中（CABAC熵半程绿 2026-09-12：`cabac_test.go` 4项绿，主档片双路5帧零差异；8x8帧内/变换I帧绿 2026-09-12：三高档片第0/1/3帧零差异，余P帧第2/4帧；VR2c-1～VR2c-5✅，见 §6.1.3） |
...
- **待办 F6 8x8帧内余量/F9帧间8x8/缩放尾巴：** 第2帧（P帧间8x8首错MB2）与第4帧（P帧错位/解码错MB18）余±1与位流漂移，缩放片第2/4帧大错待查；VR2c主格暂不翻绿。
