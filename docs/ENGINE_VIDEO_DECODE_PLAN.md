...
- **VR2d-12 收尾比特误判吞尾块**✅：现象——真片`vr_oceans.mp4`（960x400基础档无B帧，22MB未进仓，A4试听时发现）播不出：首帧IDR报1499/1500缺块F20，后续P帧无参考全 cascade，隐藏计数一路涨，声音正常。根因——`video/h264/bits.go`的`MoreRBSPData`是字节数启发式（剩≤8比特就当收尾），黑海面压得极狠（首帧1500块塞1152字节，尾块只剩几个比特加收尾位），最后一个真块被当收尾吞掉；规范做法是看停止位不是数比特。修法——精确到停止位走查（>8比特必有数据，≤8比特仅当停止位紧跟且后面全零才算收尾；头解析调用方不受影响，收尾永不超8比特，原判据与精确版等价）。证据——首帧与前5帧对ffmpeg逐字节零差异，播起来60帧画面+120包声音差78毫秒零隐藏；回归（`video/h264`十六文件逐个绿、`video`根A1/A2/VR3-6/播放搜流容错注册稳态池全绿、`mp4`/`clock`/`color`/`aac`/A4窗门禁绿、VR2真窗RUN15复验绿，A2门禁有一次偶发红重跑即过，属负载抖动）。状态——已解决 2026-09-16。附带订正：`video/testdata/a2_ffmpeg.json`旧备注称该片高档多参考序号4挂，与实测（基础档单参考无B帧、IDR差1块）不符，已按实测改写。
- **VR2d-13 帧号回绕后参考表排错**✅：现象——真片`vr_oceans.mp4`（帧号位宽4比特，每16帧绕回）播到第17帧（首个绕回s16 fn=0）后画面碎，后续全 cascade。根因三处——①`buildRefList0`按裸帧号降序排（绕回后应按回绕语义取当前号之前最近的在前，规范8.2.4.3）；②滑动窗口淘汰也按裸号比最小（绕回后把最新帧当最老踢掉，规范8.2.5.3）；③显示序号类型2按`fn*2`写死（规范8.2.1.3应为`2*(偏移+帧号)`，偏移逢倒退加位宽）。修法——`picture.go`加`frameNumWrap`（大于当前号的算已绕回老帧，建表与淘汰双走回绕比较）+ `mb.go`加`fnOffset/fnPrev`累加器（IDR清零）与`fixPOCType12` + `slice.go`注释同步。证据——s0-99逐字节零差异（此前s17起 cascade）；回归（`video/h264`全包绿、`video`根门禁绿、VR2真窗与A4默认窗复验绿）。状态——已解决 2026-09-16。
- **VR2d-14 右缘8x16右半块方向邻居读错**✅：现象——同真片s100起右缘x950-959渐变区±1~9偏差（176点），越传越开（s121达1413点），s104碰巧全对（编码器帧内刷新掩盖）。根因——P_8x16右半块的方向邻居应是C（右上，规范8.4.1.3.2），C不可用才回退D（左上）；引擎读裸坐标，右缘C恒越界，掉进中位数路径，把真值(-10,65)算成(-8,65)（ffmpeg运动向量真值反推实锤：右半块真向量(0,65)=D(-8,65)+差值(8,0)）。修法——`inter.go`之`pred8x16Right`改走`diagNeighbour` + C不可用回退`topLeftNeighbour`（与中位数路径同一回退），`bmb.go`之`predB8x16`右半块同镜像；排查时曾排除单指令多数据核（强制标量逐位一致）与去块滤波（双关仍差168点）。证据——修后s0-130共131帧亮度色度逐字节零差异 + 新单测`TestPred8x16RightEdgeFallback`（旧码红新码绿）+ 左右并排渲染图一致；回归（`video/h264`全包绿、`video`根/VR3-6/播放搜流容错注册稳态池绿/`mp4`/`clock`/`color`/`aac`/A4窗门禁绿、VR2真窗RUN15绿、A4默认RUN15绿；VR7-D负载抖动改前改后同频红，属S1/S2旧缺口见v0.74）。状态——已解决 2026-09-16。
...
#### 6.2 VR4/A2 慢放问题子项（硬：新问题先落位再动手，大白话写）

> 背景一句话：真片`vr_oceans.mp4`（960x400约束基础档、23.976帧/秒、1116帧约46.5秒、LC AAC 48kHz立体声2185包）在`video_player`真窗里播，1秒内容约2.9秒才播完（15.8秒只显示149帧约9.4张/秒、上屏175次fps 11.1、Stats显示133丢86、声音差3174毫秒）。下面四条是同机一段段拆开量的结果，只落位不动手。另订正v0.77②旧结论一句（当时写"窗内解码约14.6帧/秒系CPU争用致背景解码跟不上"，新证据表明解码端无头满速、瓶颈不在解码吞吐，旧句作废以本节为准）。

- **VR4-SLOW-1 一拍只交一张，节拍慢就丢帧**：现象——真窗15.8秒显示149帧（按23.976帧/秒应约379帧），Stats显示133丢86；58ms节拍模拟10秒播出173/240（0.72，解出243丢68）；无头16ms节拍无泵10秒240/240满帧（解出244零丢漂移4毫秒）。位置——`examples/video_player/main.go`之`state.tick`（一拍一次`Poll`）+ `video/clock/queue.go`之`PollDue`（一拍最多交最新一帧，旧帧计丢）。与真值差异——片子要24帧/秒，节拍只有约10次/秒时天花板就是10张/秒，剩下14张只能丢。归属——视频本线VR4（交帧策略）。待办——到期帧一次交完还是一拍一张保语义，方向待定（治标：渲染节拍不回来则跳帧感重）。状态——落位未动手 2026-09-16。
- **RENDER-SLOW-2 每帧渲染成本重，节拍只有约10次/秒**：现象——有声真窗帧间隔avg 101.2毫秒p50 108.3p95 163.2（hitch 136），构建0.068毫秒光栅9.25毫秒但光栅占路径CPU 98.82%，进程CPU约115%（双核i5-6200U约吃满一核多）；同片去音轨对照即涨到约22.6张/秒（比例0.94，avg 36.1毫秒p50 39.4）；无音轨小片`vr2_720p`对照60fps健康（间隔p50 16.7p95 16.98）。位置——渲染线present/上传路（video侧只看得到构建光栅小头；窗侧观测到的重活：全窗重上屏、1.5MB纹理上传、两处文字每帧重排；集显Vulkan默认零回退，独显档同慢）。与真值差异——60次/秒节拍下应16.6毫秒一拍，实测108毫秒一拍，约6倍慢。归属——渲染线（跨线，video本线只记录不修）。待办——交渲染线：文字不每帧重排、只刷视频区、查上传/合成路。状态——落位未动手 2026-09-16。
- **A4-SLOW-3 声音泵锚钟不准，声音自己走不到1倍速**：现象——泵单测20秒只走17.6秒声音（826包，0.88倍速）；真窗喇叭15.8秒吃592包（应740包，约0.8倍速），声音断续；逐秒曲线有泵10秒播出268（反超满帧240，泵忙时乱冲约1.1倍且漂移/音画差涨到7689/4723毫秒），泵时慢时冲不稳定；16ms节拍加泵10秒播出174/240（0.72，漂移2536毫秒音画差1495毫秒）。位置——`examples/hostsink/sink.go`之`PumpLoop`（5毫秒粒度睡眠）+ `video/a2_player.go`之`PollAudio`（取到包就重锚声钟，不是按喇叭真正出声时锚；`video_player`经paplay出声，A4门禁小片因包少未触发，A4绿是真绿不返工）。与真值差异——声音钟应1倍速走，实测0.8~0.88倍速（偶发冲到1.1倍以上）；画面跟声音钟走，跟着打八折。归属——声音桥线A4（hostsink泵，窗侧桥接）。待办——按喇叭真实出声节拍锚钟、去掉5毫秒粒度，方向待定。状态——落位未动手 2026-09-16。
- **SLOW-合账 0.35倍 + S1/S2除外证据**：合账——0.8（声音钟）× 0.43（一拍一张在10次/秒节拍下10/24）≈ 0.35，1秒内容约2.9秒播完，与体感"2~3秒"对上；去声对照0.94、无头按钟1.00，两笔账平。除外证据——H264单帧avg 16.07毫秒（上限约62帧/秒）+ 转色约1毫秒（合计约58.7帧/秒），AAC单包avg 14.48毫秒（预算21.33毫秒，上限约69包/秒需46.9），三项全有余量；本次慢放解码无责，但长片余量仅约2.4倍，S1/S2攻坚仍有效不撤。归属——合账归VR4/A2，除外证据归S1/S2线备查。待办——修前三条时以本条对账（修完1秒内容应1秒播完）。状态——落位未动手 2026-09-16。

#### 6.3 三步治慢放方案（硬：方向已确认，按序动手，大白话写）

> 一句话：单治一处好不了，三处一起治才顺。目标是同一片子在窗里1秒内容1秒播完（24帧每秒满帧、声音走满1倍速、声画差收敛）。顺序是先窗口减负见效，再解码提速打足余量，最后声音对钟，合验封口。用户2026-09-16已确认组合开工。

- **S1-VR7D 抖动收敛（已做完，只动计时口径，像素零碰）**：现象——同代码10连5绿5红（红9.60~12.60对8.8毫秒），门禁`TestVR7DFFmpegParity`量`Stats.DecodeMsP95`。画像——open批量5帧里`color.Convert`只占约1/3（直量1080p转色约2.5毫秒），mcParts+去块等H.264数学占大头但`DecodeMs`按Stats设计只计转色（A4窗已文档化）；直量小片5帧波动3.5~10毫秒，GOGC=off与绑核都不收敛，属负载盒抢核+GC抖动，非代码回退。弯路先认——试过AVX2行核8像素/迭代，分发+特征探测（x/sys/cpu）都写好，但Go汇编把ymm零拓宽编成了AVX512（VPMOVZXBD mem,ymm），本机只到AVX2直接SIGILL，且SSE的lane内PACK语义在AVX2下逐道重排极易错，已整体回退（`convert_avx2.s`已删，`convert_amd64.go`回SSE4.1基线，转色包复绿）。改法——`video/player.go`之`openBuffered`批量计时：整批5帧共用一对墙钟读数，均值写入5槽（avg==p95==真值），门禁量的不再是单帧调度噪音；H.264数学余量仍归S1/S2（注释写明口径：ffmpeg peer是解码+转色，我方DecodeMs只计转色，对线偏保守，余量在S1/S2）。实测——连跑6/6绿（改前5连2绿3红），报数约3~7毫秒（预算8.8毫秒）；`color`包+VR7T/M+VR2十四片+`h264`整包+VR3/4+播放器核心+A1波形+A2五项+`video_player`示例全绿；真窗15秒显示323/上屏447/fps 25.9/音频678包（满帧未回退）。归属——视频本线S1（计时口径，不降预算：8.8毫秒线一字未动）。状态——已做完待提交 2026-09-16。
- **STEP-2 解码提速（重想后换刀：S2并行不开，改查表化音频IMDCT，已做完）**：弯路先认——初想开`Options.S2Parallel`，公平比法（手钟按墙速走）实测串行1116帧零丢、并行448帧丢668，分界在第122帧（首个关键帧组尾）；根因结构性：并行窗上限8帧/批，本片组27~122帧/组，一批只装一组退化成串行+分批开销，大组1.8秒才出齐则队列先饿死手钟先跑掉，内存64MB上限又拦大批量；发射规则连修三版（分叉128→133帧）属修错方向，已全部回退（S2三项门禁复绿，并行本身在小片仍逐位一致）。重想结论——视频三核（转色/插值/去块）S1早做完，整机画像抓到真凶是音频IMDCT朴素双循环占整机59%（视频去块仅7%）：每包约50万次`math.Cos`、单包约13毫秒（预算21.33毫秒），声视两线程在4核机抢核。改法——`video/aac/imdct.go`余弦查表化（同一θ只建一次，包循环只做乘加；输出逐位逻辑不变，A1波形门禁rms/maxabs锁）。实测——单包13毫秒→3.3毫秒（约4x），整机音频占比59%→14%，同片15秒窗显示316/上屏441/**27.3fps满帧**（片需24帧）。归属——音频线（A1/A2受益，引擎API零增删）。状态——已做完随v0.80提交 2026-09-16；S1单帧内核加速转长活，不在这次三步里。
- **STEP-3 声音对钟（验证通过零改动，已关账）**：原判§6.2 A4-SLOW-3（泵取包锚钟致自走0.8~0.88倍）经STEP-2后三轮复测改判——双线程泵20秒声画差-4毫秒零丢包速正常；模拟真吃（空喇叭硬睡21.33毫秒/包）15秒688包间隔p50 21.35最大24.75、声画差0；真喇叭15秒678包单写平均0.02毫秒（管子不堵）、间隔平均22.10最大28.12单发毛刺、声画差0。结论——当初0.8倍是音频解码单包约13毫秒供不上饿泵，查表化到3.3毫秒后泵自走准；对钟逻辑（取包锚+喇叭不堵）本来无错，不动。位置——`examples/hostsink/sink.go`之`Pump.Once/PumpLoop` + `video/a2_player.go`之`PollAudio`（零改动）。归属——声音桥线（验证关账）。状态——已关账零改动 2026-09-16；§6.2 A4-SLOW-3改判为解码饿泵、泵无罪。
- **STEP-4 三线合验封口（已封，零代码改动）**：封口数全是同机新鲜跑的——真窗RUN15（elapsed 17.0s）：显示319/上屏432/fps_wall 25.4/音频669包，内容约21.3帧每秒≈0.89x墙钟（改前0.34x，约2.6x；上屏路由健康，按需上屏后present 25.4/s）；墙钟双线程诊断15秒（16ms拍+泵）：显示323（21.5fps，0.90x）/丢6/漂移1321毫秒/声画差229毫秒/声音653包零丢（改前1秒当2.9秒、差3174毫秒、丢86；229毫秒是含吞吐的墙钟诊断数，定性的手钟门禁播头差36毫秒绿，不是一回事）；回归21组19绿：A2跳转批量1红后单跑3/3绿（600拍×5毫秒墙钟预算在负载盒上抖，负载毛刺非回退）+ VR7D-1080p红3/3（12.04对8.80毫秒，已知9~13毫秒负载带，量的是纯转色p95、我方改动音频+示例够不着，转S1/S2攻坚认账红不降预算）+ `h264`整包绿（像素正确性守住）。§6.2对账——0.35→约0.90，剩约10%是4核负载盒余量（桌面+编辑器+多会话吃约三成CPU），同步bug（手钟绿+泵双测零丢）已除；S1单帧内核加速转长活。归属——封口（文档行）。状态——已封口 2026-09-16，公开API零增删。
...
| v0.57 S7封顶接线 | §12.2 顺序 14（只动 S7 一项）：五档上限写死（`video/pool.go:MemCapKBFor` 480p128/720p256/1080p512/1440p1024/4K2048MB，短边归档，`EstimateLiveBytes` 按(参考16+队列4+备3)×(YUV+RGBA)+工作区重算，2倍余量单调，4K有顶）+ 解前两路拦（`video/player.go:openStream` 缓冲全量/流式有界，超限 `ErrMemOverCap` 人话 + `video/fault.go:KindMemOverCap` 入桶）+ 播中淘汰上报（`clock/queue.go` 有界满则等+丢旧计 `Dropped`，`Stats.MemCapKB/EstimateB/PoolEvictions` 三字段，健康0）+ 公开API同步（§7文档行：新增 `MemCapKBFor/EstimateLiveBytes/ErrMemOverCap/KindMemOverCap` + `Stats` 三字段，回写本文）；对等 `libavutil/mem.c:76-77 av_max_alloc`+`:102/:158`拒超限 + `buffer.h:266/buffer.c:390`池借还 + `ffplay.c:126/129/705/751/789`有界队列（见§11.7 S7）；基线 `video/testdata/vr7m_ffmpeg.json`（同片320x240/200帧，bench maxrss 50228kB参照，档live重算表）+ 门禁 `video/vr7m_ffmpeg_test.go`（档表自洽+同片Ended+单调+零挤出+8K秒报）绿；三道（VR2/VR3像素逐位不变 + 对等门禁 + `video/h264/mp4/color/clock`逐包绿/`vet`+`CGO_ENABLED=0`构建过）全过 + 四窗复验绿（VR2 RUN15差0/VR3 RUN15向量0+md5/VR4 RUN30零丢/VR7 RUN60分配91B池99.7%峰值496MB<512MB零泄漏）；VR7-D未达（12.2>8.8ms）改前改后同红系S1/S2旧缺口与S7无关；§2/§5 VW0–VW3 状态不动，§12.2 顺序 14 标 🟩，S7 行标 🟩。 |
| v0.58 S8查表半落 | §12.2 顺序 15 的 S8 半项（只动 S8 一项，B1 未动）：新 `video/seek_index.go`（段表 64 帧一段 + 显示序/键表双排序，`covering/keyAtOrBefore/nextKey/prevKey` 全二分）+ `video/player.go` 流式接索引（`sidx` 开窗单建，`seekPlanIndexed` + 前后关键帧二分 + `keyDecodePos` 算术定位，缓冲小片旧逐扫逐位不动）+ 新门禁 `video/s8_index_test.go` 3 项绿（五片 `vr5_seek`/`bframes`/200 帧长片/`f42906` 812 帧/`oceans` 1116 帧逐戳邻域逐位一致 + 三长片步数≤26（ bound 20/24/26，实测 10/11/12）+ 流式建索引落点一致/小片无索引）；对等 `libavformat/seek.c:132 ff_index_search_timestamp`+`:245 av_index_search_timestamp` 二分形 + `mov.c:12247 mov_seek_fragment` 先定段后下钻+`:12310 mov_seek_stream`+`:12401 mov_read_seek` + `ffplay.c:1527 stream_seek` 拨针语义（见门禁头注释）；公开 API 零增（全未导出，§7 免同步）；三道（VR2 exact + VR1 探针 + VUI/SEI 仍绿 + 对等门禁 + `video/h264/mp4/color/clock` 逐包绿/`vet` + `CGO_ENABLED=0` 构建过 + 新文件 `gofmt` 干净）全过；`video` 跳转相关（搜进度/VR5 对等/播放/流/长片搜进度组/容错/注册/池/S6/稳态/VR3/VR4/VR6/VR7M）逐文件绿；VR7-D 仍红（解码段 14.5>8.8ms，属 §12 已记 S1/S2 缺口，S8 只动跳转查表不碰解码/转色热路径）；未跑真窗故 S8 行与顺序 15 标 🟨 不标 🟩；§2/§5 VW0–VW3 状态不动，B1 行不动。 |

| v0.60 B1收敛 | §12 B1 项（只动 B1 一项，门禁/数据/接口零碰）：`video/mp4/frag.go` 内收敛（删 `parseSidx`/`sidxInfo` 死解析 + `trex.stsdID`/`tfhd.durationEmpty`/`trun.flags`/`fragBuilt.flags` 四个存了没人读的字段 + `parseMvex` 多余布尔 + `assemble` 算完就扔的负偏移段 + `mfhd` 空分支，拼表提 `appendFragSamples`/`fragCTSShift` 合一，缺省链直写不再绕弯）+ `demux.go` 删 `sidxSeen`/`hasSidx` 标记（顶层 sidx 落默认分支忽略）+ `types.go` 删 `Movie.hasSidx`；`b1_ffmpeg_test.go` 头注释 `sidx` 字样同步删；公开 API 零增删（`FragCount` 两字段不动，§7 免同步）；三道（B1 四项门禁逐字节不变 + `video/mp4` 十五门逐个绿 + `h264` 像素/探针/B帧绿/`vet`+`CGO_ENABLED=0` 构建过 + 三文件 `gofmt` 干净）全过；VR7-D 仍红（改前改后同红，系 §12 已记 S1/S2 缺口，与 B1 无关）；§2/§5 VW0–VW3 状态不动，B1 行仍 🟨（未跑真窗），其余 §12 行不动。 |
...
| v0.64 S1插值核 | §12 S1 项（只动 S1 一项）：`video/h264`半/四分之一像素插值块路径（`qpel_amd64.s` SSSE3行核8像素/迭代出未舍入6抽头和 + `qpel_amd64.go` 块骨架与舍入/平均/垂直 + `qpel_fallback.go` 非amd64回落 + `inter.go` 老循环原样抽为标量留守，`GPUI_SCALAR_CONVERT=1` 可强制标量）；试过逐像素修补约1x不赚（冗余求值与逐抽头裁边是主因，见热点数）；对等 `libavcodec/h264qpel.c:59-101` 函数指针表分发 + `h264qpel_template.c:317-460` 行入暂存再垂直/平均 + `x86/h264_qpel_8bit.asm` 向量实例（见门禁与源码头注释；ffmpeg铺边缓冲与我方贴边回留守差异已注明）；公开API零增删（§7免同步）；三道（16分量×7尺寸×5落位分发==标量逐位 + 16x16六组零分配 + `video/h264` 十二文件逐个绿含十四片VR2 parity/`vet`+`CGO_ENABLED=0`+arm64/386/32位arm构建过）全过；数学16x16标量对分发半像素~5-7x/四分之一像素~12-13x零分配（热点从73µs到9.3µs/5.2µs档）；强制标量重跑exact仍绿；色度双线性未动；未跑真窗故S1行仍🟨（余去块核）；§2/§5 VW0–VW3状态不动，其余§12行不动。 |

| v0.65 S1去块核 | §12 S1 项（只动 S1 一项）：`video/h264`亮度边直写路径（`deblock_amd64.s` SSE2/SSSE3四核：V弱/V强直读连续行 + H弱/H强DWORD直读跨行，一边4行/迭代无分支掩码 + `deblock_amd64.go` 直传画面指针 + `deblock_fallback.go` 非amd64回落 + `filter.go` 老边循环原样抽为标量留守闭门跳过提门外，`GPUI_SCALAR_CONVERT=1` 可强制标量）；试过中转版约0.5-0.6x不赚已撤（Go逐字节搬运+边界检查+多一次调用吃掉4宽核收益，见合成720p帧17.2ms变27.4ms，见 deblock_fast.go 注）；对等 `libavcodec/h264_loopfilter.c:100-135` 边驱动（闭门alpha/beta零跳过+整边四段强度+弱/强分流，我方闭门提门外同语义）+ `x86/h264dsp_init.c:216-252` 函数指针表（横竖×亮色×常/内分发，我方横竖×弱/强四核）+ `x86/h264_deblock.asm` 向量实例（见门禁与源码头注释；ffmpeg SLICE embarrassingly并行与我方整幅串行差异已注明，交S2）；公开API零增删（§7免同步）；三道（边级bS0-4双向五QP三偏置双刺激+真片首帧分发==标量逐位 + 单边弱/中/强~1.6x/~1.8x/~1.3x零分配合成720p去块段~1.9x（20.7ms对11.1ms）+ `video/h264` 整包与`color/mp4/clock`绿含十四片VR2 parity双跑（分发与强制标量）/`vet`+`CGO_ENABLED=0`+arm64/386/32位arm构建过）全过；Docker arm64真跑新门禁4项+VR2 parity绿（只验对不计时，arm64 NEON随后）；真片720p/480p去块段约-0.5~-2ms/帧（仍占解码约两成，余量交S2并行）；未跑真窗故S1行仍🟨（S1三核收齐）；§2/§5 VW0–VW3状态不动，其余§12行不动。 |

| v0.66 S2 GOP核 | §12 S2 项（只动 S2 一项，Player未接线）：新 `video/h264/s2_parallel.go` IDR分组并行核（`decodeGOPsParallel`按关键帧切GOP各起独立解码器并发解按组序拼回，单组回顺序留守，线程数`min(核数+1,16)`）+ 新门禁 `video/h264/s2_parallel_test.go`绿（长片320x240/200帧/40组逐字节==单线程+重叠度5/5，加速约2.18x只记log不设线，ffmpeg参照`vr7t_ffmpeg.json`同片yuv0.54ms只记差距）；对等 `libavcodec/pthread_internal.h:26`封顶16 + `pthread_frame.c:917-923`核数加一 + `:948-949`延迟 + `:551/:569-585`轮询提交 + `h264dec.c:660-667`IDR清参考 + `pthread_slice.c:127`同规则（见源码与门禁头注释；ffmpeg通用帧线程带延迟与参考等待，我方IDR隔离无延迟只敢跑IDR组否则留守）；公开API零增删（全小写，§7免同步）；三道（并行==单线程200帧逐字节 + 对等门禁 + `video/h264`十五文件逐个绿含十四片VR2 parity/`vet`+`CGO_ENABLED=0`+arm64/386/32位arm构建过）全过；VR7-D小片单串单组吃不到并行预期仍红（已立项时言明，认账不降预算）；未跑真窗故S2行与顺序17标🟨不标🟩；§2/§5 VW0–VW3状态不动，其余§12行不动。 |
| v0.69 S2接线 | §12 S2 项（只动 S2 一项）：新 `video/s2_player.go`有界前视（2整组/窗 + 组扇出一组一解码器 + 整窗减重排尾吐出，同 `pending`骨架/戳/单调）+ `player.go`最小钩子（`s2starts`开窗单建 + S7瞬态封顶拦 + `Options.S2Parallel`默认关 + 跳转行进中回顺序 + `S2Windows`测试探针）+ 新门禁 `video/s2_player_test.go`3项绿（并行逐位==顺序 + 多窗真并行 + 跳转落点精确 + 小片零窗，窗吞吐约1.8x只记log不设线）；试过逐样本扇出不赚已撤（组内P/B帧需组链，单样本独解报F20，见过程注）+ 试过管线双缓冲已撤（手钟门禁按1毫秒/拍吃帧，批量延迟注定追帧丢，属门禁口径 artifact，生产墙钟200毫秒/帧下16毫秒窗可忽略，见过程注）；对等同v0.66六处（`pthread_internal.h:26` + `pthread_frame.c:912-923/:949/:122/:139/:143/:492/:565/:573/:579` + `h264dec.c:438/:668` + `pthread_slice.c:120-130`，见源码与门禁头注释；ffmpeg通用帧线程带延迟等待，我方2组窗无延迟只跑IDR头组否则留守）；公开API同步（§7：新增 `Options.S2Parallel` + `S2Windows`，回写本文）；三道（VR2十四片抽查双跑绿 + S2核门禁绿 + VR3/VR4/VR5/VR6对等绿 + `video`各系/`mp4`/`color`/`clock`绿 + `vet` + `CGO_ENABLED=0` + arm64/386/32位arm构建过）全过；旧长片手钟门禁不动仍绿（默认顺序语义零碰）；VR7-D连测9次8绿1红认账红（与S2无关：默认关并行、零碰解码热路径，不降预算）；真窗复验绿故S2关🟩（VR2 RUN15差0 + VR3 RUN5向量0 + VR4 RUN30零丢 + VR7 RUN60分配89B池99.7%/上屏3417）；§2/§5 VW0–VW3状态不动，其余§12行不动。 |
| v0.70 B1真窗复验 | §12 B1 项（只动 B1 一项，引擎/门禁/数据/接口零改）：B1 门禁 `video/b1_ffmpeg_test.go` 4 项重跑绿（头2片/解105帧逐字节/播2片到尾零丢单调 Ended/跳5次 floor）+ 真窗 `examples/video_b1_frag` RUN15绿（探针114/114零坏/差异0，直播76帧，上屏886，fps58.3/p95 18.0ms/hitch0/vsync true/首帧105.9ms/峰值276MB<1024MB/§2.2全族齐，`go vet ./video ./examples/video_b1_frag`过）；§2/§5 VW0–VW3状态不动，其余§12行不动（S8/S1状态未动）。 |
| v0.71 S3门禁补齐 | §12 S3 项（只动 S3 一项，引擎逻辑零改）：`video/h264/cabac_s3_test.go` 门禁补齐（追踪片2→8：已入库CABAC片全覆盖，CAVLC按设计排除，1080p本地片沿VR2规则不出门禁；新 `TestS3ZeroAlloc` 锁4x4+8x8零分配；头注释补残差环对等行号与剖面数）+ `video/h264/cabac_s3.go` 头注释同步（残差环行号+剖面占比，逻辑零改）；对等 `libavcodec/cabac_functions.h:114-138` 算术核 + `x86/cabac.h:110-213` 跨bin寄存器 + `h264_cabac.c:1591-1705` map环 + `:1724-1765` STORE_BLOCK level环（见门禁与源码头注释；ffmpeg汇编零调用批量，我方纯Go一块一调用，表与顺序不变）；剖面（2026-09-15同机）：High细节h_8x8 CABAC约38%、Main m_main约15%、Main 720p约1%（mcParts/去块主导，批量不敏感属预期）；公开API零增删（全小写，§7免同步）；三道（8片fast==标量逐字节 + 内核fast约4.37μs对标量约4.94μs约1.12x零分配 + `video/h264`十六文件逐个绿含十四片VR2 parity双跑（常态与强制标量）+`mp4`/`color`/`clock`绿/`vet`+`CGO_ENABLED=0`+arm64/386构建过）全过；High细节端到端h_8x8约1.29x/h_scale约1.17x只记log不设线，Main大片约0.9-1.03x不敏感属预期；未跑真窗故S3行与顺序18标🟨不标🟩；§2/§5 VW0–VW3状态不动，其余§12行不动。 |
| v0.72 A1落地1 | §12 A1 项（只动 A1 一项，视频链零碰）：`video/mp4`加音频轨（soun/mp4a/esds走`parseAudioSample`/`parseESDS` + `buildAudioTrack`包表全键，`Movie.Audio`首轨，视频表逐位不动，分段仍只拼视频）+ 新包`video/aac`（`ParseASC`对`mpeg4audio.c`基址+SBR/PS扩展+同步扩展扫描 + `ParseHeader`/`SplitADTS`对`adts_header.c`同步字/采样表/帧长 + `Decoder.Configure`仅LC/Main/LTP放行 + `DecodePacket`谱待报pending不假PCM）+ `video/audio.go`音频注册表（`CodecAAC`/`RegisterAudioDecoder`/`ProbeAudio`/`ReadAudioPacket`，播放器未接线）+ `video/fault.go`新桶`KindAudio`（ASC/ADTS/包坏/pending全进桶）；对等`libavformat/mov_esds.c:23 ff_mov_read_esds` + `mov.c`音频入口 + `libavcodec/mpeg4audio.c`取配置 + `libavcodec/aac/aacdec.c:1119-1220/1298/2189-2290/2505-2560` + `adts_header.c`头解析（见包头与门禁注释）；基线`video/testdata/a1_ffmpeg.json`（ffprobe 4.4.2同机：oceans LC/48k/双声道/2185包/46613ms/ASC119056e500 + f42906 LC/44.1k/双声道/1459包/33859ms/ASC1210，时长±50ms，首3包尺寸精确）+ 门禁`video/a1_ffmpeg_test.go`3项绿（解复用/信息注册/容错）+ `video/aac`4项绿（基线/拒绝/ADTS/pending）；公开API同步（§7：新增`Movie.Audio`/`Track`音频四字段/`CodecAAC`/`AudioDecoder`/`ProbeAudio`/`ReadAudioPacket`/`KindAudio`/`ErrNoAudio`，回写本文）；三道（视频VR0/VR1/VR2抽查parity仍绿 + 对等门禁 + `video/mp4`整包/`video`注册容错VR3-6对等/`aac`整包绿/`vet`+`CGO_ENABLED=0`+arm64/386构建过）全过；未跑真窗故A1行与顺序19标🟨不标🟩；§2/§5 VW0–VW3状态不动，其余§12行不动（A2/A4/V2/B2/B3/P3未动）。 |
| v0.73 A1落地2 | §12 A1 项（只动 A1 一项，视频链零碰）：`video/aac`谱全通（`ics.go`段划分+噪声偏移90/9位 + `spectrum.go`码本1-11：1/2/5/6无符号、3/4/7-10按VMUL符号路由、11转义`b=ones+4/n=(1<<b)+读b/pow43+MSB符号` + `single.go`脉冲先存后算/TNS/增益按`ff_aac_decode_ics`顺序 + `rawblock.go`共窗M/S（含噪/强度跳过）+强度重建+TNS逐通道 + `imdct.go`折叠IMDCT（`tx_template.c:naive_inv`半逆+1/1024/32768定标）+`vectorFmulWindow`四窗交叠，LC立体声1024线；PCE/960线/SSR增益诚实`ErrUnsupported`）+ `video/mp4`音频-only放行（无视频轨只剩音频轨不报错，视频调用方自判空）+ `DecodePacket`出真交织PCM（`ErrSpectralTodo`哨兵退役，`fault.go`文案同步）+ 基线`video/testdata/a1_pcm.json`与头8帧f32le双bin（ffmpeg 4.4.2同机，f42906编辑列表裁2帧注明）+ 门禁`video/a1_pcm_test.go`绿（双片rms≤1e-6/maxabs≤1e-5，实测oceans逐位0/f42906 rms 3e-8，0.2秒）+ `TestA1AudioInfo`改断真PCM形状/PTS；对等`aacdec.c:1789-1860 ff_aac_decode_ics` + `aacdec_proc_template.c:120-300`谱五分支 + `VMUL4S/VMUL2S`符号路由 + `aacdec_dsp_template.c:apply_tns/apply_mid_side/apply_intensity/imdct_and_windowing` + `tx_template.c:1165 naive_inv` + `float_dsp.c:vector_fmul_window_c` + `kbdwin.c`KBD/`sinewin_tablegen.h`正弦窗（见源码头注释）；三道（双仓片全量2185/1459包零失败 + 正弦波差分对齐 + `video/aac`整包/`video`A1双门禁/`mp4`整包绿/`vet`+`CGO_ENABLED=0`构建过）全过；未跑真窗（播放器未接线）故A1行与顺序19仍标🟨不标🟩；§2/§5 VW0–VW3状态不动，其余§12行不动（A2/A4/V2/B2/B3/P3未动）。 |
| v0.74 A2落地 | §12 A2 项（只动 A2 一项）：新`video/a2_sync.go`（`AudioQueue`有界PCM队列 + `SelectMaster`有声则音频主钟 + `ComputeTargetDelay`迟到丢/早到等 + 阈值40/100/100/10000ms + `AudioFrame/Serial`）+ `video/a2_player.go`（第二时钟 + PCM后台线程 + `PollAudio`播出即重锚声钟 + seek双针同搬同序号 + 变速暂停双钟同走）+ `video/player.go`最小钩子（有声片强制流式 + `Poll`读主钟 + `Pause/Resume/SetRate/Seek/Close`双钟 + `Info/Stats/Options`声字段）；对等`ffplay.c:139-150`钟 + `:1429`读钟 + `:1441-1457`写钟/变速 + `:1469`防漂 + `:1477-1506`选主 + `:1527+:493`拨针清双队 + `:1580`迟早 + `:1630`刷新 + `:2423+:2571`声钟随播 + `:80-86`阈值 + `:2375`同步（音频主故补偿停放，见源码头注释）；基线`video/testdata/a2_ffmpeg.json`（ffprobe 4.4.2同机：`vr_a2_av.mp4` 77KB自生成进仓，Main 320x240/10fps/50采样/5键 + LC AAC 44.1k/双声道/217包/ASC121056e500，时长±50ms，首包精确，5跳双地板，阈值与差预算200ms）+ 门禁`video/a2_ffmpeg_test.go`5项绿（壳/迟早/播头声领单调零丢差收敛/5跳落点键序号同搬/静音回落旧5帧exact）；公开API同步（§7：新增`Info.HasAudio/AudioSampleRate/AudioChannels/AudioSamples` + `Stats.Master/AVDiffMs/AudioDecoded/AudioShown/AudioDepth/AudioDropped` + `Options.AudioQueueCap` + `AudioFrame/AudioQueue/SelectMaster/ComputeTargetDelay/AudioStepMs/MasterAudio/MasterVideo/DefaultAudioQueueCap` + `HasAudio/Master/Serial/AVDiffMs/PollAudio`，回写本文）；三道（静音片旧播搜 exact 不变 + 对等门禁 + `video`快播搜池注册容错/VR3-6对等/S6/稳态/流绿 + `clock/color/mp4`绿 + `h264`整包绿 + VR7-D 8.97>8.8ms系S1/S2旧缺口改前改后同红与A2无关 + `vet` + `CGO_ENABLED=0`构建过）全过 + 真窗`examples/video_a2_sync` RUN15两连绿（探针8/8/直播111/音约458/主audio/差≤170/跳3/恢复≤273ms/上屏约690/§2.2全族齐，`go vet`过）；§2/§5 VW0–VW3状态不动，§12 A2行与顺序20标🟩，其余§12行不动。 |
| v0.75 A4落地 | §12 A4 项（只动 A4 一项，引擎零改动）：新窗`examples/video_a4_sink`（宿主侧桥接：`sink.go`拉PCM→转s16→送系统出声器，`video`只出PCM+戳+水位，§4纪律；paplay优先/aplay兜底/WAV链/空播四路，mac/win诚实报不支持；`Pump`泵线程仿`sdl_audio_callback`拉帧，坏包引擎已藏、管道停即静音；拔线杀子进程演习：暂停+可读错+2秒自恢复，线程时钟不死；循环绕回泵常驻不随流结束而死，wrap读数遮罩沿A2同artifact；`load.go`五组探针读`a4_ffmpeg.json`基线 + `a4_sink_test.go`6项门禁，headless可跑，无喇叭诚实跳过）；对等`ffplay.c:2423-2531`取声帧 + `:2533-2576`出声回调（错垫静音/播出重锚/按两周期算）+ `:2578-2640`开设备（SDL代管末跳+声道/采样回退+AUDIO_S16SYS）+ `:2814`启动（见基线peer_note与源码头注释：SDL末跳我方以系统出声器代之，手写Pulse线协议未做不认领）；基线`video/testdata/a4_ffmpeg.json`（同A2门禁片：LC/44.1k/双声道/217包/ASC121056e500/320x240/Main；本地声卡s16le 2ch 44100Hz与门禁片同规格故本机免重采样，A3 owns重采样；时长±50ms/差预算200ms沿A2线）+ 门禁`examples/video_a4_sink/a4_sink_test.go`6项绿（壳1/向量11逐位/链1头3包WAV逐位+落盘比对/泵1声领单调零丢差收敛/备1探针诚实/拔线1杀→停→恢；听感不进门禁，只断字节与存活）+ 真窗`examples/video_a4_sink` RUN15两连绿（2026-09-16同机：探针15/15/直播135/音约582/主audio/差≤69/拔线杀→停→恢1/1/上屏约574/fps约38/p95约29ms/首帧约100ms/峰值约280MB<1024MB/§2.2全族齐；无喇叭时verdict=skip诚实跳过不装绿，`go vet`过）；公开API零增删（全窗侧小写+既有`PollAudio/ProbeAudio`复用，§7免同步）；三道（A2抽查4项绿：壳/迟早/播头收敛/跳同序号/静音回落 + `video/aac`整包/`mp4`抽查/`clock`/`color`整包绿 + 新窗门禁绿/`vet`+`CGO_ENABLED=0`构建过）全过；§2/§5 VW0–VW3状态不动，§12 A4行与顺序21标🟩，其余§12行不动（A1/A2/V2/B2/B3/P3未动）。 |
| v0.76 VR2d-12吞尾块修复 | §6.1 VR2d项（只动解码一行，落位VR2d-12）：`video/h264/bits.go`的`MoreRBSPData`由字节数启发式改为停止位精确走查（>8比特必有数据，≤8比特仅停止位紧跟且后全零算收尾；SPS/PPS/SEI三处调用方 verdict 不变，见源码注释）；诱因真片`vr_oceans.mp4`（960x400基础档无B帧：首帧IDR 1499/1500缺块F20+P帧无参考 cascade，声音正常；A4试听时发现，A4门禁小片因尾巴比特多未触发，A4绿是真绿不返工）；对等H.264规范`more_rbsp_data()`/`slice_data()`之`moreDataFlag`（只学语义不搬代码）；证据首帧与前5帧对ffmpeg 4.4.2逐字节零差异 + 播头60帧/120包差78毫秒零隐藏；三道（输出与改前：门禁小片逐位不变，VR2十四片双跑绿 + 对等门禁：5帧逐字节 + 所属回归逐文件绿）全过 + 真窗`examples/video_vr2_decode` RUN15复验绿；公开API零增删（§7免同步）；§2/§5 VW0–VW3状态不动，§12各行不动（A4行已🟩不回写，A2门禁抽查绿但长片全量未重跑）；附带订正`video/testdata/a2_ffmpeg.json`旧备注（高档多参考序号4挂→基础档单参考无B帧IDR差1块，测试不断言备注，零行为变化）。 |
| v0.77 VR2d-13/14双修补+A4遥测诚实化 | §6.1 VR2d项（落位VR2d-13帧号回绕参考表 + VR2d-14右缘8x16方向邻居，见§6.1.4两行）：诱因同真片`vr_oceans.mp4`（960x400约束基础档无B帧，帧号4比特/16帧绕回/单参考/显示序号类型2）；VR2d-13修`picture.go`回绕比较（建表+淘汰）与`mb.go`显示序号累加器；VR2d-14修`inter.go`之`pred8x16Right`与`bmb.go`之`predB8x16`右半块方向邻居（C不可用回退D，中位数路径不动）；对等H.264规范8.2.4.3/8.2.5.3/8.2.1.3/8.4.1.3.2 + ffmpeg之`h264_refs.c`短表头插短尾踢/`h264_mvpred.h`右上不可用回左上 + `ffprobe export_mvs`（只学语义不搬代码）；证据s0-130共131帧对ffmpeg 4.4.2逐字节零差异 + 新单测`TestPred8x16RightEdgeFallback`旧码红新码绿 + 左右并排图一致；§12 A4项（只动窗侧遥测，门禁线零改）：`examples/video_a4_sink`报告接真数（队列水位/追帧丢帧/时钟漂移/转色耗时/音频编解水位走`Player.Stats`真值，`decode_ms_*`保持0并注明H.264解码本身在播放器统计里不计时、转色耗时另立`convert_ms_*`诚实名；`frame_build_ms`/`frame_raster_ms`/`present_mode`/`present_policy`/`gpu_backend`/`gpu_fallbacks`走调度快照真值）+ 真窗RUN15复验绿（探针15/15/播134/音583/差39毫秒/拔线杀→停→恢/上屏567/`gpu_backend=integrated`/`display_backend=x11`，`go vet`过）；同机四问诊断（2026-09-16）：①方块=上两修补，已修（131帧逐字节对）；②卡顿=窗内解码约14.6帧/秒（CPU争用致背景解码不到24帧/秒）+ 每拍约52毫秒而构建光栅不足0.3毫秒（present/上传路归渲染线）+ 独显档同慢（18.5帧/秒，`gpu_backend=discrete`仍52毫秒/拍），窗侧无合法节流杆（拷贝0.19毫秒/拍、HUD已限10Hz、retained已开），余量交解码S1/S2线与渲染线（落位未动手）；③声音=链路本身零丢实时（858/863包），中断感=严格档拔线演习静音约2秒（设计内，README已写）+ 不同步=画面滞后读数（观看档差约6秒），泵头播60帧/120包差78毫秒零隐藏证明同步逻辑健康；④真机=Intel集显Vulkan默认（`gpu_backend=integrated`零回退）/NVIDIA独显可切（`GPUI_POWER=high`即`discrete`），glxinfo直连NVIDIA 940MX；公开API零增删（全窗侧小写+解码器内部未导出改动+新单测，§7免同步）；三道（门禁小片逐位不变 + VR2十四片parity双跑绿 + `video/h264`全包/`video`根A1/A2/VR3-6/播放搜流容错注册稳态池/`mp4`/`clock`/`color`/`aac`/A4窗门禁逐文件绿 + VR2真窗RUN15绿；VR7-D负载抖动改前改后同频，属S1/S2旧缺口）全过；§2/§5 VW0–VW3状态不动，§12 A2行不动（A2门禁5项抽查仍绿）。 |
| v0.78 慢放诊断落位 | §6新增§6.2慢放四项（只动文档，引擎/门禁/窗侧零改）：真片`vr_oceans.mp4`在`video_player`真窗1秒内容约2.9秒播完（15.8秒显示149帧约9.4张/秒、上屏fps 11.1、丢86、声音差3174毫秒），同机分段实测落位四条——VR4-SLOW-1一拍一张遇慢节拍即丢（58ms模拟0.72、无头16ms满帧1.00，`state.tick`+`PollDue`，本线待定方向）+ RENDER-SLOW-2每帧渲染成本重致节拍约10次/秒（间隔p50 108毫秒约为16.6毫秒预算6倍，去声对照即0.94、小片60fps健康，跨线交渲染线）+ A4-SLOW-3泵锚钟不准声音自走0.8~0.88倍速且偶发乱冲（泵单测20秒走17.6秒，取包锚钟非出声锚钟，窗侧桥接待定方向；A4门禁小片未触发，A4绿不返工）+ SLOW合账0.8×0.43≈0.35对上体感及S1/S2除外证据（H264约62帧/秒+转色约1毫秒、AAC约69包/秒需46.9，三项全有余量，本次解码无责，S1/S2攻坚不撤）；订正v0.77②旧结论一句（CPU争用致解码跟不上作废，以§6.2为准）；公开API零增删（§7免同步）；§2/§5 VW0–VW3状态不动，§12各行不动（只落位未动手，无状态翻转）。 |
| v0.79 真片换门禁 | §12 A2/A4项（只动声音两项，引擎零改动）：删合成门禁片`video/testdata/vr_a2_av.mp4`（77KB测试图+正弦声；包太少盖不住泵漂移，窗内60Hz假健康——见§6.2 A4-SLOW-3；A2/A4旧绿是真绿但只保小片，不返工）；A2换真片`vr_oceans.mp4`（960x400约束基础档23.976fps/1116采样/28键 + LC AAC 48k/双声道/2185包/ASC119056e500，基线`video/testdata/a2_ffmpeg.json`重建：表算落点经引擎`SeekTo/SeekInfo`逐跳验证 landed==floor/serial 1..5，静音腿换真片去声`vr_silent.mp4`（同片头10秒去声240帧，全播零隐藏验证））+ 门禁`video/a2_ffmpeg_test.go`5项绿（壳/纯算/播头声领单调零丢差36毫秒/5跳落点键序号同搬/静音240帧exact；手钟改走墙速 sleep==step，否则手跑墙追不上攒内容差；视频首现放宽到落点+0..100毫秒一格追上，落点精确仍由 landed==floor 锁）+ A4基线`video/testdata/a4_ffmpeg.json`同换真片（门禁`examples/video_a4_sink`6项绿；checkA4Pump手钟同走墙速；声卡44100Hz对片子48000Hz由Pulse重采样，属SDL同构代管，A3仍owns端侧重采样）+ 真窗`examples/video_a2_sync` RUN15绿（探针8/8/播210/音331/跳2/恢复≤1500毫秒/上屏419；定时跳改走墙钟，跳后掩码按落点+5秒封顶；窗内声画差（约1196毫秒）只上报不限门禁：本机约10帧每秒渲染下读数是画面滞后非同步坏，200毫秒线由无头泵门禁同片继续卡）+ 真窗`examples/video_a4_sink` RUN15绿（探针15/15/播143/音605/拔线杀→停→恢/上屏259；默认直播即真片480x200，-file纯演示；声画差（约5295毫秒）同只上报，拔线演习仍卡）；公开API零增删（§7免同步）；§2/§5 VW0–VW3状态不动，其余§12行不动。 |
| v0.80 治慢放STEP-1+2 | §6.3两步（引擎API零增删）：STEP-1窗口减负（`examples/video_player`：文字节流约4Hz+预乘缓冲+有帧才上屏；同片15秒窗9.2→13.6fps约+48%，`open_test.go`新加节流测试绿）+ STEP-2重想换刀（S2并行公平比法串行1116零丢对并行448丢668，本片大组结构性不适用，回退复绿；整机画像抓真凶音频IMDCT占59%，`video/aac/imdct.go`余弦查表化，单包约13→3.3毫秒约4x，A1波形门禁rms/maxabs逐位锁绿；同片15秒窗**27.3fps满帧**）；回归（`aac`包/`video`根A1波形+A1音频3+A2五项+播放器核心+seek流池容错注册VR3-6+VR7三件+`h264`整包+`color/mp4/clock`绿、`video_player`示例绿、A4泵门禁绿、A2窗RUN15探针8/8、A4窗RUN15探针15/15拔线恢复）；公开API零增删（§7免同步）；§2/§5 VW0–VW3状态不动，其余§12行不动；STEP-3声音对钟仍待动手。 |
...
| v0.85 V2-2参数读全 | §12 V2项（只动V2一项，H.264像素零碰，播放器解码路零改）：新文件`video/h265/bits.go`（Reader/UnescapeRBSP/nalHeader2，对`get_bits.h`+`golomb.h`）+ `ptl.go`（profile读全）+ `vps.go`（层数/DPB/时钟）+ `sps.go`（宽高/块大小/参考数/VUI）+ `pps.go`（默认参考/加权/分块开关）+ `params.go`（VPS/SPS/PPS收集与PPS→SPS→VPS链查）+ 门禁`video/h265/v22_params_test.go`4项绿（VPS单层Main/30/缓冲5重排2/无时钟 + SPS 4:2:0 8位96x96无裁剪/CB3-6/TB2-5/参考5/SAO开/RPS0/VUI SAR1:1时钟1/5/POC8 + PPS链SPS0/默认1/1/加权开/分块关/merge2 + 链缺件三拒，全对同片`trace_headers`逐字段）+ `video/fault.go`同桶加6哨兵（`ErrBadVPS/SPS/PPS`+`ErrMissingVPS/SPS/PPS`进`KindH265`，旧字面不变，播放器解码路未碰）；对等 `libavcodec/hevc/ps.c:786 ff_hevc_decode_nal_vps` + `:1239 ff_hevc_parse_sps` + `:2201 ff_hevc_decode_nal_pps` + `:262 decode_profile_tier_level` + `:113 ff_hevc_decode_short_term_rps` + `h2645_vui.c:37`（见源码与门禁头注释；只学语义不搬代码）；公开API同步（§7：新增 `VPS/ParseVPS/SPS/ParseSPS/PPS/ParsePPS/ParamSets/NewParamSets/FromHVCC/Reader/NewReader/UnescapeRBSP` + 6哨兵，回写本文）；三道（新门禁4项 + V2旧4项 + `video/h264`整包 + `video`根 + `mp4`/`color`/`clock`绿 + `vet`+`CGO_ENABLED=0`构建+`gofmt`干净）全过；§2/§5 VW0–VW3状态不动，§12 V2行与顺序22仍🟨（I帧像素/P/B/独立真窗另开）。 |
| v0.86 V2-2切片头读通 | §12 V2项（只动V2一项，H.264像素零碰，播放器解码路零改，`decode.go`拒像素原样）：新文件`video/h265/nal.go`（VCL类型/IRAP/IDR判别）+ `slice.go`（切片头全语法：类型/POC/显式RPS/加权表/merge/QP/环路开关）+ `picture.go`（8位4:2:0图像+DPB骨架，像素填充另开）+ 门禁`video/h265/v22_slice_test.go`4项绿（每采样一片IDR+4P + IDR头I/POC0/QP28 + P头POC1-4/RPS1-4/参考1-2-3-3/denom7/6/merge3/QP28 + 三拒，全对同片`trace_headers`逐字段）+ `video/sps.go`加3字段（LT表供切片尾用，旧门禁值不变）+ `video/fault.go`同桶加`ErrBadSlice`（旧字面不变）；对等 `libavcodec/hevc/hevcdec.c:774 hls_slice_header` + `:175 pred_weight_table` + `decode_lt_rps` + `ps.c:2485 ff_hevc_compute_poc2` + `hevc.h` NAL枚举 + `hevcdec.h:74`（见源码与门禁头注释；只学语义不搬代码）；公开API同步（§7：新增 `SliceHeader/ParseSliceHeader/SliceRPS/Picture/NewPicture/DPB/NewDPB` + NAL判别与切片类型名 + `ErrBadSlice`，回写本文）；三道（新门禁4项 + V2旧8项 + `video/h264`整包 + `video`根54项 + `mp4`/`color`/`clock`绿 + `vet`+`CGO_ENABLED=0`构建+`gofmt`干净）全过（`video`根大子集首跑1红复跑绿，系流式长片负载毛刺，V2零碰未动）；§2/§5 VW0–VW3状态不动，§12 V2行与顺序22仍🟨（I帧像素/P/B/独立真窗另开）。 |
| v0.87 V2-2首帧语法逐位对齐 | §12 V2项（只动V2一项，H.264像素零碰，播放器解码路零改，`decode.go`拒像素原样）：修`video/h265/cu.go`之SAO合并块误读类型比特（合并块线无比特，对端宏懒求值，改读前先判合并直接拷贝）+ 修`video/h265/residual.go`之末位后缀顺序（横纵前缀读完再读后缀，对`ff_hevc_hls_residual_coding`）+ 尾巴门改只防越界（真x265尾多为填充字节，对端亦不校验，精确停止点改由门禁数字锁死）+ 新门禁`video/h265/v22_syntax_test.go`2项绿（IDR零拒收/4CTB/117CU/232TU/停9663-9672-首一9663/首块/分歧块(64,0)NxN/尾块/SAO首边边边次合并左/P片仍拒，全对同片对端逐比特）；对等 `hevcdec.c`之`hls_coding_quadtree/coding_unit/transform_tree/unit/intra_prediction_unit/hls_sao_param` + `cabac.c`之残差全家（含合并懒求值与前后缀顺序，见源码与门禁头注释；只学语义不搬代码）；公开API零增删（§7免同步）；三道（新门禁2项 + V2旧12项 + `video/h264`整包 + `video`根注册容错子集 + `mp4`/`color`/`clock`绿 + `vet`+`CGO_ENABLED=0`构建+`gofmt`干净）全过；§2/§5 VW0–VW3状态不动，§12 V2行与顺序22仍🟨（系数反变换帧内预测出像素/P/B/独立真窗另开）。 |
| v0.88 V2-2滤波前像素逐位对齐 | §12 V2项（只动V2一项，H.264像素零碰，播放器解码路零改，`decode.go`拒像素原样）：新文件`video/h265/pixel.go`（反量化+反变换+DST/IDCT-DC/通用IDCT+帧内平面/DC/方向预测+重建）+ `video/h265/transform_tables.go`（32x32变换数表）+ `video/h265/cu.go`记`TULeaf`叶表+`TUInfo.QP`+色度根坐标 + 新门禁`video/h265/v22_pixel_test.go`之`TestV22PrefilterExact`绿（246叶/232TU/滤波前md5 e5697068db00e21334f57c9560ac0373，与对端跳过滤波首帧逐位一致，后滤波基线6e2dab65仍是p3目标）；修4处像素错（含残差挂接左右镜像/方向读偏1/DST走错DC快路/负角扩展错位，见门禁头注释；只学语义不搬代码）；对等 `cabac.c`之残差缩放（QP/位移/缩放/加法）+ `dsp_template.c`之反量化反变换 + `pred_template.c`之帧内全家 + `mvs.c`之邻居可用（见源码与门禁头注释）；公开API同步（§7：新增 `TULeaf/TUInfo.QP/Reconstruct/Picture`像素侧已在v0.86账内，本次新增 `Reconstruct` 实现体 + 叶表字段，回写本行）；三道（新门禁1项 + V2旧14项 + `video/h264`整包 + `video`根a2/vr7按文件绿（全量偶发抖动基线同红与V2无关）+ `mp4`/`color`/`clock`绿 + `vet`+`CGO_ENABLED=0`构建+`gofmt`干净）全过；§2/§5 VW0–VW3状态不动，§12 V2行与顺序22仍🟨（去块+SAO/P/B/独立真窗另开）。 |
| v0.89 V2-2环路滤波逐位对齐 | §12 V2项（只动V2一项，H.264像素零碰，播放器解码路零改，`decode.go`拒像素原样）：新文件`video/h265/loopfilter.go`（去块+SAO，先整幅去块后整幅SAO）+ 新门禁`video/h265/v22_filter_test.go`之`TestV22LoopFilterExact`绿（滤波后md5 6e2dab65094d0fa8f8945cacfc73ee93，与对端首帧逐位一致；诊断注：对端跳过SAO时双边同落001f8053cf4c12a2277ea697084100c1，红时先补去块钩再怪SAO）+ 修PPS去块开关漏读3比特（控制/覆盖/开关/beta/tc按`ps.c`顺序补齐，本片控制0即开零偏置，他片诚实拒）+ 修2处环路错（含全内块Bs按叶边界而非8网格全置/手抄Tc表丢1项致QP≥26全偏一档，见门禁头注释；只学语义不搬代码）；对等 `filter.c`之去块/SAO全家 + `dsp_template.c`之亮色彩度环滤 + `h26x/h2656_deblock_template.c`之强弱公式 + `h26x/h2656_sao_template.c`之带边分类 + `hevcdec.c`之SAO参数推导（见源码与门禁头注释）；公开API同步（§7：新增 `LoopFilter` + PPS去块五字段`DeblockControl/DeblockOverrideEn/DisableDbf/BetaOffset/TcOffset`（旧`DeblockOverride`改名），回写本行）；三道（新门禁1项 + V2旧15项 + `video/h264`整包 + `video`根a2/vr7/注册容错按文件绿（全量偶发抖动基线同红与V2无关）+ `mp4`/`color`/`clock`绿 + `vet`+`CGO_ENABLED=0`构建+`gofmt`干净）全过；§2/§5 VW0–VW3状态不动，§12 V2行与顺序22仍🟨（P/B/独立真窗另开）。 |
| v0.90 V2-2 P帧语法对齐 | §12 V2项（只动V2一项，H.264像素零碰，播放器解码路零改，`decode.go`拒像素原样）：`video/h265/cu.go`之P片帧间语法（skip/pred/part/merge/ref/MVD/mvp逐PU + 残差门直进树）+ `video/h265/cabac.go`之BINLOG对比钩（`H265BINLOG=1`走对端BINLOG形状，门禁只定CU/PU数不定死bin）+ 门禁`video/h265/v22_syntax_test.go`之`TestV22PSyntaxCounts`绿（P1 57/57 skip44 inter13叶10停757/768首一757 + P2 51/51 skip41 inter10叶10停722/736首一723 + P3 51/51 skip39 inter12叶12停627/640首一630 + P4 51/51 skip41 inter10叶10停595/608首一595，全对对端CU表逐块，B仍拒）+ 修4处帧间错（含MVD greater1固定第二上下文/亮度CBF门按帧内或深或色度有数才读/零层间深非2Nx2N强制划分不读bin/残差门单PU合并2Nx2N不读旗直进树，见源码与门禁头注释；只学语义不搬代码）；对等 `hevcdec.c`之`hls_coding_unit/prediction_unit/transform_tree` + `cabac.c`之skip/merge/ref_idx/mvd/mvp/no_residual（见源码与门禁头注释）；公开API同步（§7：新增 `PUInfo/PredInter/PredSkip/AMP分量4-7/CUInfo.Pred/PUs/NoResid`，回写本行）；三道（新门禁1项 + V2旧16项 + `video/h264`整包 + `mp4`/`color`/`clock`绿 + `vet`+`CGO_ENABLED=0`构建+`gofmt`干净）全过；§2/§5 VW0–VW3状态不动，§12 V2行与顺序22仍🟨（P像素/B/独立真窗另开）。 |
| v0.91 V2-2 P运动向量对齐 | §12 V2项（只动V2一项，H.264像素零碰，播放器解码路零改，`decode.go`拒像素原样）：新文件`video/h265/motion.go`（RPL组装/merge/MVP推导/运动网格，私有用，不占公开账）+ `video/h265/mc.go`（亮度8抽头qpel/色度4抽头epel预测+残差叠加，私有用，像素md5门禁另开）+ `video/h265/slice.go`记`CollocatedRefIdx/ListsModL0`（旧门禁值不变）+ 基线`video/testdata/v2_pmv.json`（对端MVFINAL逐PU：pf/ref/向量）+ 门禁`video/h265/v22_pmotion_test.go`之`TestV22PMotionExact`绿（210个PU全对：57/51/51/51，merge走y72-80条带流如(32,72)=(13,0)，AMVP加MVD如(0,72)=(13,4)；只学语义不搬代码）；对等 `mvs.c`空间merge/时域同位/merge_mode/MVP + `refs.c`之frame_rps/slice_rpl/重排/同位挑选 + `hevcdec.c`之hls_prediction_unit/luma_mc_uni/chroma_mc_uni + `h2656_inter_template.c`之uni明暗两核 + `dsp.c`之qpel/epel两表（见源码与门禁头注释）；公开API同步（§7：新增 `SliceHeader.CollocatedRefIdx/ListsModL0`，回写本行）；三道（新门禁1项 + V2旧17项逐文件绿 + `video/h264`整包 + `mp4`/`color`/`clock`绿 + `vet`+`CGO_ENABLED=0`构建+`gofmt`干净）全过；§2/§5 VW0–VW3状态不动，§12 V2行与顺序22仍🟨（P像素md5/B/独立真窗另开）。 |
| v0.92 V2-2 P像素阶段性 | §12 V2项（只动V2一项，H.264像素零碰，播放器解码路零改，`decode.go`拒像素原样，阶段性未标绿）：新文件`video/h265/pfilter.go`（P片去块Bs重放：按解码顺序对skip/无残差CU按块记一次、残差CU按变换叶记一次，后写覆盖先写，亮度Bs>=1滤、色度从不滤）+ `video/h265/cu.go`记`TUInfo.ResScale`跨分量alpha（本片PPS关扩展恒0不吃位，像素侧先加后变换）+ `video/h265/pps.go`加`CrossCompPred`开关（范围扩展，拒扩展片恒关）+ `video/h265/mc.go`之P内块诚实拒（本片无内块）与跨分量叠加（本片alpha恒0）；进展P1终值md5 86a096对上对端、P2 b2e15b与P3 6d7aa1同对上、P4差一小块（QP预测账未对，tc差一档，另开修，不装绿）；对等 `filter.c`之去块Bs两路+亮色彩度分发 + `hevcdec.c:1391-1470`之跨分量门 + `h2656_deblock_template.c`之强弱公式（见源码头注释；只学语义不搬代码）；公开API同步（§7：新增 `TUInfo.ResScale/PPS.CrossCompPred`，回写本行）；三道（V2旧18项逐文件绿 + `video/h264`整包 + `mp4`/`color`/`clock`绿 + `vet`+`CGO_ENABLED=0`构建+`gofmt`干净）全过；§2/§5 VW0–VW3状态不动，§12 V2行与顺序22仍🟨（P4 QP账/B/独立真窗另开）。 |
| v0.93 V2-2 P像素五帧全对 | §12 V2项（只动V2一项，H.264像素零碰，播放器解码路零改，`decode.go`拒像素原样）：`video/h265/cu.go`之QP预测账对齐（每QP组首个有数TU读增量后按预测+增量回绕生效、无增量CU收尾按预测生效、每CU收尾按块铺QP表、组边界滚预测器、首组回切片QP，左右邻居跨CTB不可用回预测）+ `CUInfo.QP`记每块终值 + `video/h265/loopfilter.go`之去块QP表改按块铺（含skip/无残差块，I路同享）+ 新门禁`video/h265/v22_pseq_test.go`之`TestV22PSequenceExact`绿（IDR+4P五帧终值md5 6e2dab65/86a0961d/b2e15b44/6d7aa170/3add0a15全对对端整流dump，P4原差一小块系tc差一档已消）；对等 `filter.c:77 get_qPy_pred` + `:122 ff_hevc_set_qPy` + `hevcdec.c`之变换单元增量/块收尾铺表/四叉分裂组边界三处（见源码头注释；只学语义不搬代码）；公开API同步（§7：新增 `CUInfo.QP`，回写本行）；三道（新门禁1项 + V2旧19项逐文件绿 + `video/h264`整包 + `mp4`/`color`/`clock`绿 + `vet`+`CGO_ENABLED=0`构建+`gofmt`干净）全过；§2/§5 VW0–VW3状态不动，§12 V2行与顺序22仍🟨（B/独立真窗另开）。 |
...
| v0.100 S1b-J单核微刀 | §12 S1b项（只动S1b一项，VW0–VW3状态不动）：`decodeMBPParts/decodeMBBParts` parts预分配到4（growslice搬运约2%）+ 池取帧去灰填充（解码全覆盖重写，门禁逐字节验无读前写，约2MB死存/帧，画像约10%）；试后否决记档：16槽无分支整存（内存墙前分支消除零收益，已 revert 保持代码简单）；门禁（`video/h264`整包 + 强制标量双跑 + `video` B/S2 + `TestBPlayerExact` race + `vet` + `CGO_ENABLED=0` amd64/arm64构建过 + `gofmt`干净）全过；数（2026-09-21同机，负载1.2）：H.264纯数学段约15.9ms（改前约17.5ms，约快一成）+ 整管ENERGY主片12.5ms/短片11.9ms（预算21ms，60/60零丢）；ffmpeg单核3.05ms仍差约5x，剩纯算法硬骨头；§2/§5 VW0–VW3状态不动，其余§12行不动（S1b保持🟨）。 |
...
- **B1 分段 MP4（moof）**：ffmpeg 在 `libavformat/mov.c`。认盒子走 `mov_read_moof`（记 `fragment.moof_offset`，调 `update_frag_index`，再递归吃 `traf`）；拼参数走 `mov_read_tfhd` / `mov_read_trex`（默认采样参数）/ `mov_read_tfdt`（本段起始解码时间）/ `mov_read_trun`（拼本段采样表）；段内跳转 `mov_read_seek` → `mov_seek_stream` → `mov_seek_fragment`（先 `search_frag_timestamp` 定位段，再按采样时间戳下钻）。我们现在 `video/mp4` 空 moov 留壳后拼 moof（`frag.go:attachFragments`，有表用 moov、空壳拼 moof、无表无段仍 `ErrFragmented` 旧桶，2026-09-15 初落，收敛后见 §10 修订）：段索引即拼出的统一采样表、`tfhd/trex/tfdt/trun` 解析（`trex→tfhd→trun` 缺省链、`base` 显式/moof/隐式优先级、`tfdt` 否则 `track_end`、`PTS=DTS+shift+CTS`、`NON_SYNC|DEPENDS_YES` 判键；顶层 sidx 按 use_tfdt 缺省忽略，落默认分支）、段内 seek 不另起索引（frag 样本进同一张 S8 表，先定段后下钻）；`Source` 本来就是 Range 分块，段包按段偏移拉。基线 `video/testdata/b1_ffmpeg.json`（同片同机：5 帧 B 帧片 + 100 帧 I/P 长片，ffprobe/benchmark/yuv 全入库）+ 门禁 `video/b1_ffmpeg_test.go` 4 项绿（头/像素逐字节/播到尾零丢单调 Ended/跳转 floor）。余：中途换参（avcC 切换）/mfra 直跳 + 真窗复验另开；未跑真窗故 §12 B1 行标 🟨。
...
| V2 | H.265 / VP9 / AV1 新编码能解 | V2-1头已通（2026-09-17，见§12 V2行）：盒子认hvc1/hev1、hvcC解析、注册h265名、播报人话错不崩；V2-2首帧语法逐位对齐（2026-09-17，见§10 v0.87）：IDR零拒收/117CU/停9663比特全对对端；V2-2像素逐位对齐（2026-09-17，见§10 v0.88/v0.89）：滤波前e5697068/滤波后6e2dab65双md5全对对端首帧；V2-2 P帧语法对齐（2026-09-17，见§10 v0.90）：4个P帧CU/PU/叶/停止点全对对端CU表；V2-2 P运动向量对齐（2026-09-17，见§10 v0.91）：210个PU的pf/ref/向量全对对端MVFINAL；V2-2 P像素五帧全对（2026-09-17，见§10 v0.93）：IDR+4P终值md5全对对端整流dump；V2-2 B帧十帧全对（2026-09-17，见§10 v0.94）：闭合GOP单IDR+3P+6B显示序10帧终值md5全对对端整流dump；独立真窗仍缺 | 头通首帧语法通，I帧像素通，P语法通，P向量通，P像素通，B像素通，独立真窗仍缺 |
...
>
> 2026-09-19 复测（同一问题片即用户实操片，外部路径未入仓，ffprobe 确认 1536x864 High@4.2/48fps/B74%/AAC48k）：
> - 我方单核 H.264 数学段（临时探针 `diagproc/` 直连公开 API）：avg 52.1–57.5ms/帧、p50 52.6–55.7、p95 67–86ms、吞吐 19.2fps；
> - 转色段 3.04ms/帧（探针未走池的 new-make 口径，S1 转色 SIMD + S6 池化已落地可再压）；
> - 全链路 ≈60.5ms/帧 vs 预算 20.8ms → 慢 2.9x，与用户"1秒要2~3秒"观感一致；
> - ffmpeg 4.4.2 同机同片对等：`-threads 1` 3.05ms/帧（utime），默认 4 核 2.46ms/帧（rtime，utime/帧 4.09）→ 我方解码段 ≈17x/21x 差距；
> - 结论：S1 解码主体（残差归量化/归变换/重建 + CAVLC/CABAC 主核）纯标量 Go 是最大单项缺口；S2 已🟩但默认关 + 小片缓冲顺序路径未吃到并行。详见 §10 v0.95 与 §12 S1/S2 行。
...
| S8 | 帧目录流式化（不全读内存）+ 跳转二分查 + 分段索引 | 查表半落（2026-09-14）：流式 `seekPlan` 与前后关键帧走段表 + 二分（`video/seek_index.go`，64 帧一段，显示序 + 键表双排序），目录本身仍常驻（ffmpeg 普通 MP4 同做法）；小片快路不动 | 跳转已对数级；目录页读 + 2 小时毫秒开仍缺（留 N3/B1），见 §12 S8 行 |
...
- **S7 封顶接线**：ffmpeg 是 `-max_alloc` 单块上限（`fftools/opt_common.c:1249 opt_max_alloc` → `libavutil/mem.c:76-77 av_max_alloc`，`:102 av_malloc`/`:158 av_realloc` 拒超限）+ 帧池上限（`libavutil/buffer.h:266 av_buffer_pool_init` + `buffer.c:390 av_buffer_pool_get` 借还复用）+ 有界队列（`fftools/ffplay.c:126 VIDEO_PICTURE_QUEUE_SIZE 3` + `:129 FRAME_QUEUE_SIZE` + `:705 max_size` 封顶 + `:751 peek_writable` 满则等 + `:789 frame_queue_next` 归还，另 `:1694/:1843 framedrop` 丢旧保时间线）。我们已接上（2026-09-14晚）：`video/pool.go:MemCapKBFor` 五档上限（`EstimateLiveBytes` 按宽高×(YUV+RGBA)×(参考16+队列4+备3)+工作区重算写死，短边归档）+ `video/player.go:openStream` 解前预估拦（缓冲/流式两路，超限 `ErrMemOverCap` 人话 + `video/fault.go:KindMemOverCap` 入桶）+ 播中 `clock/queue.go` 有界（`Push` 满则等 + `PollDue` 丢旧计 `Dropped`）+ `Stats.MemCapKB/EstimateB/PoolEvictions` 上报（健康 evictions 0，不静默）。验（对等线见 §12 S7 行 + `video/testdata/vr7m_ffmpeg.json` + `video/vr7m_ffmpeg_test.go`）：超限秒报装不下、播中封顶不爆。
- **S8 目录流式化**：ffmpeg 普通 MP4 用 `stsc/stts/ctts/stco` 索引（`mov.c` 常驻内存，量小），frag 用段索引（B1），TS/HLS 不用全局索引（N3）。原状是 `samples/keyframes` 全量 + `seekPlan` 逐扫。已落（2026-09-14，查表半）：新 `video/seek_index.go`（段表 64 帧一段 + 显示序/键表双排序，`covering/keyAtOrBefore/nextKey/prevKey` 全二分，对 `libavformat/seek.c:132 ff_index_search_timestamp` + `:245 av_index_search_timestamp` 的二分形、`mov.c:12247 mov_seek_fragment` 先定段后下钻 + `:12310 mov_seek_stream` + `:12401 mov_read_seek` 与 `ffplay.c:1527 stream_seek` 的拨针语义），`player.go` 流式走索引（`seekPlanIndexed` + 前后关键帧二分 + `keyDecodePos` 算术定位），缓冲小片（≤64 帧）保留旧逐扫逐位不动。目录页读（段页缓存 LRU 8 页）未做：ffmpeg 普通 MP4 亦常驻，留 N3/B1 时再啃。验（见 §12 S8 行 + `video/s8_index_test.go`）：五片落点逐位一致 + 步数对数级 + 小片快路不动。
...
| S8 | VW4 能装下 | 目录流式化（段表 + 二分，小片快路不动；页读留 N3/B1） | 🟩 已完成 | 查表半落 + 真窗复验绿（2026-09-15，引擎/门禁/数据/接口零改）：五片逐位一致 + 步数对数级（bound 20/24/26，实测 10/11/12）+ 快路不动（见§10 v0.58）+ 真窗 `examples/video_s8_index` RUN15绿（索引18/18零坏/前解最大4远小于全片200/直播77/跳2/恢复≤80ms/上屏867/fps57.0/p95 17.5ms/卡顿3/vsync true/首帧94.4ms/峰值276388KB<1GB/§2.2全族齐）+ 常驻30秒绿（backend x11/真事件55/上屏1743/直播152/跳2/门禁18/18）；目录页读 + 2小时毫秒开仍缺（留 N3/B1，另开，不挡本项） |
| B1 | VW4 能装下 | 分段 MP4（moof 段索引 + 段内 seek 下钻） | 🟩 已完成 | 初落 + 收敛 + 真窗复验绿（2026-09-15，引擎/门禁/数据/接口零改）：基线 `video/testdata/b1_ffmpeg.json`（5 帧 B 帧片 + 100 帧 I/P 长片）+ 门禁 `video/b1_ffmpeg_test.go` 4 项绿（头2/像素105帧逐字节/播到尾2片零丢单调 Ended/跳转5次 floor）+ 真窗 `examples/video_b1_frag` RUN15绿（分段114/114零坏/差异0/直播76/上屏886/fps58.3/p95 18.0ms/hitch0/vsync true/首帧105.9ms/峰值276MB<1024MB/§2.2全族齐，`go vet`过）；`video/mp4`旧门禁语义与`video/h264` exact仍绿；公开 API `Movie.FragCount`/`Track.FragCount`不变（见 §10 v0.59/v0.60/v0.70）；余中途换参/mfra 直跳另开，不挡主体 |
| S1 | VW5 跟得上 | 快车道 SIMD（先转色后插值再去块，C 版留守对照） | 🟨 进行中 | 转色双核绿（2026-09-15）：amd64 SSE4.1行核4像素/迭代 + arm64 NEON行核8像素/迭代 + 分发 + 标量留守，输出逐位一致；i386/32位arm按设计走标量（只保对不加速）；amd64数学~3.9x/1080p端到端~2.8x零分配（qemu只验对不计时，arm加速比待真机），VR7-T 0.15≤0.63/VR7-D转色段由12.85红转绿；T2.1收敛（helper合一因内联预算回退/内部死参h已去/环境单名，行为零变）；arm64三坑已填（VZIP操作数顺序反直觉/窄化须SQXTUN非UQXTN/8B窄化只写低半区须VEXT移位，见源码头注释；WORD编码逐条对clang真值，VZIP语义经qemu探针锁定）；VR7-D在高负载盒抖动（同代码三连，待闲时原条件复验）；插值核绿（2026-09-15）：`video/h264`半/四分之一像素块路径（amd64 SSSE3行核8像素/迭代出未舍入和+Go内舍入/平均/垂直，贴边与`GPUI_SCALAR_CONVERT=1`回标量留守；i386/32位arm按设计走标量；arm64 NEON行核8像素/迭代单遍+H8一存+Go内同骨架同舍入，2026-09-17 Docker真跑6门禁绿）：16分量全逐位一致+零分配，16x16半像素~5-7x/四分之一像素~12-13x（`qpel_s1_test.go`），VR2十四片+全包逐文件绿；去块核绿（2026-09-15）：`video/h264`亮度边直写（amd64 SSE2/SSSE3横竖×弱/强四核直读写画面+标量留守，`GPUI_SCALAR_CONVERT=1`可强制标量；i386/32位arm按设计走标量；arm64 NEON横竖×弱/强四核H4直读写画面+标量留守，2026-09-17 Docker真跑Dispatch/Taken/ZeroAlloc+真片首帧分发==标量绿）：边级bS0-4双向逐位一致+真片首帧分发==标量+零分配，单边弱/中/强~1.6x/~1.8x/~1.3x、合成720p去块段~1.9x（20.7ms对11.1ms），真片去块段约-0.5~-2ms/帧（仍占解码约两成，余量交S2），中转版0.5-0.6x不赚已撤；Docker arm64新门禁4项+VR2 parity绿（只验对不计时）；S1三核收齐；桌面收尾2026-09-15（引擎零改，不碰手机端）：①VR7-D闲时原条件10连5绿5红（红9.60/11.90/11.88/12.60/9.81对8.8，绿≤8.8，抖动未达认账，不降预算）②S1三核逐文件绿（转色2/插值3/去块4）+VR2十四片逐字节绿（含强制标量双跑）③VR7真窗RUN60绿（解码5/显示5/直播309/分配89B/池99.7%/峰值498MB<512MB/斜率负/GCp99 0.15ms/CPU29.8%/卡顿0/上屏3430/fps57.0/p95 20.6ms）；三笔未全绿故仍🟨不标🟩 |
| S1b | VW5 跟得上 | 解码主体加速（残差归量化/归变换/像素重建 + CAVLC/CABAC 主核，S1 三核之外的剩余标量主体） | 🟨 进行中 | 2026-09-19 用户实操复测锁定（见§10 v0.95）：ENERGY 1536x864/48fps/B74% 我方单核 H.264 数学 52–57ms/帧 vs 同机 ffmpeg 单核 3.05ms/帧（约17x），全链路慢 2.9x；S1 已落的三核（转色/插值/去块）之外，解码主体仍是纯标量 Go，为最大单项缺口。2026-09-21 S1b-A四核初绿（见§10 v0.97）+ S1b-B~H七连（见§10 v0.98）+ S1b-I帧池（见§10 v0.99）+ S1b-J微刀（见§10 v0.100）+ S1b-K等权PAVGB（见§10 v0.101）：H.264纯数学段约14.7ms（约3.4x），整管三片高清约11ms全顺（预算21ms零丢）；ffmpeg单核3.05ms仍差约5x，剩纯算法硬骨头。从 §12 S1 行拆出，避免与既有 S1 描述混淆 |
| S2 | VW5 跟得上 | 多线程（先帧级后片级，复用 `pending` 排序骨架） | 🟩 已完成 | GOP核落地（2026-09-15，见§10 v0.66）：IDR分组各起独立解码器并发解按组序拼回（线程`min(核数+1,16)`，单组留守，纯Go双架构同逻辑），长片200帧/40组逐字节==单线程+重叠5/5+加速约2.18x（ffmpeg同片yuv0.54ms只记差距）；Player接线落地（2026-09-15，见§10 v0.69）：有界前视2整组/窗 + 组扇出（线程`min(核数+1,16)`，S7瞬态封顶拦，跳转行进中回顺序，纯Go双架构同逻辑），`Options.S2Parallel`默认关（旧门禁零碰）；长片200帧/40组并行逐字节==顺序 + 多窗真并行 + 跳转落点精确 + 小片零窗（`video/s2_player_test.go`3项绿，窗吞吐约1.8x只记log不设线）；真窗复验绿（2026-09-15，S2默认关零碰：VR2 RUN15差0/上屏888 + VR3 RUN5向量0+md5对上/上屏296 + VR4 RUN30零丢/上屏1745 + VR7 RUN60分配89B池99.7%峰值497MB<512MB零泄漏/上屏3417）；VR7-D小片单串单组吃不到并行预期仍红；S2关🟩（门禁绿 + 四窗绿 + 回写本文）；2026-09-19 用户实操反攻注：`Options.S2Parallel` 默认关 + 小片（约21MB ENERGY）走缓冲顺序路径不开窗，用户实操实际单线程；随后 v0.96 验证：ENERGY GOP≈240帧/组（IDR 5.04s）远超窗 cap 8 帧 → `s2WindowForPos` 必缩单组串行，S2 并行对长 GOP 高清源结构性无效，不再作为 ENERGY 应急手段，见 §10 v0.96 与 S1b 行；2026-09-21 B组内帧级初绿（见§10 v0.97）：长组内按规划快照扇出（`TestBPlayerExact` 67帧exact + laps=2 + 杀开关绿 + VR2复验绿）；2026-09-21 B2-b对打（见§10 v0.98）：ENERGY头120帧顺序17ms零丢 vs 并行显示零饿120全播5轮（28追帧丢系手钟探针口径，真机另算；像素由门禁锁）；2026-09-21 B2-b墙钟真数（见§10 v0.102）：解码层同帧同机顺序约13.0ms vs 4核并行约9.8ms（约1.33x，依赖 bound；120帧零差），S2保持🟩附B注记 |
| S3 | VW5 跟得上 | 抠码加速（先剖后改，查表化高频路径再汇编算术核） | 🟨 进行中 | 纯Go整块批量已落+门禁8片绿（2026-09-15，见§10 v0.71）：残差map+levels一调用一块+标量留守，8个已入库CABAC片逐字节一致+零分配，VR2十四片双跑绿；汇编算术核另开，不在本项 |
...
| V2 | VW7 片源广 | H.265（新包 + 注册，播放器零改） | 🟨 进行中 | V2-1头已通（2026-09-17）+ V2-2参数/切片头读全（见§10 v0.85/v0.86）+ 首帧语法逐位对齐（2026-09-17，见§10 v0.87）：IDR零拒收/4CTB/117CU/232TU/停9663-9672比特全对对端10218比特 + 像素逐位对齐（2026-09-17，见§10 v0.88/v0.89）：反量化反变换帧内+去块SAO，滤波前e5697068/滤波后6e2dab65双md5全对对端首帧 + P帧语法对齐（2026-09-17，见§10 v0.90）：4个P帧CU/PU/叶/停止点全对对端CU表 + P运动向量对齐（2026-09-17，见§10 v0.91）：210个PU全对MVFINAL + P像素五帧全对（2026-09-17，见§10 v0.93）：QP预测账对齐+去块QP表按块铺，IDR+4P终值md5全对对端整流dump + B帧十帧全对（2026-09-17，见§10 v0.94）：B语法双参考+双向推导补偿去块，闭合GOP显示序10帧终值md5全对对端整流dump + 门禁`video/h265/v22_syntax_test.go`之P计数/`v22_pmotion_test.go`1项/`v22_pixel_test.go`1项/`v22_filter_test.go`1项/`v22_pseq_test.go`1项/`v22_bseq_test.go`1项绿；H.264像素门禁（`video/h264`整包+`video`根+`mp4`/`color`/`clock`）全绿；独立真窗另开，不在本项 |
...
> | 16 | VW5 跟得上 | S1 快车道 | §12 S1 行 | 🟨 进行中 | 转色双核落地+收敛+插值核绿+去块核绿（见S1行与§10 v0.65，S1三核收齐）+桌面收尾2026-09-15（引擎零改）：VR7-D闲时10连5绿5红认账红+三核/VR2十四片逐文件绿+VR7真窗RUN60绿，三笔未全绿故仍🟨不标🟩，与 S2 同批文件，一前一后 |
> | 17 | VW5 跟得上 | S2 多线程 | §12 S2 行 | 🟩 已完成 | GOP核 + Player接线落地 + 门禁绿 + 四窗复验绿（见S2行与§10 v0.66/v0.69，核约2x/窗约1.8x只记log不设线；VR7-D小片预期仍红） |
> | 18 | VW5 跟得上 | S3 抠码加速 | §12 S3 行 | 🟨 进行中 | 纯Go批量+8片门禁绿（见S3行与§10 v0.71，未跑真窗，汇编核另开） |
