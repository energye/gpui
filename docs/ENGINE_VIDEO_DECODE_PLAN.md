...
- **STEP-3 声音对钟（窗侧桥接，不断流）**：现象——同§6.2 A4-SLOW-3，泵取到包就对钟，喇叭真吃慢，钟自走0.8~0.88倍还拽着画面。位置——`examples/hostsink/sink.go`之`Pump.Once/PumpLoop` + `video/a2_player.go`之`PollAudio`（`Start`重锚那一下）。改法——写进喇叭成功那一下再对钟，让钟跟着真吃走，不断流不断线程。归属——声音桥线。验法——20秒声音走满20秒，画面不再被拖。状态——方向已确认未动手 2026-09-16。
- **STEP-4 三线合验封口**：同一片子窗里播满24帧每秒、声画差收敛、`h264`全包+`video`根门禁逐文件绿、§6.2对账（1秒内容1秒播完），文档落位再请确认提交。状态——待三步做完 2026-09-16。
