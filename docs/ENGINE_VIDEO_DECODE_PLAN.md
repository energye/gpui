...
- **VR2d-1 B 宏块解码**⬜待办：还差——B 包一进来就报“要等 VR2d”，一块都解不了。要做——直接模式、只看前向/只看后向/前后一起看三种预测，第二个参考表搭起来，双向的隐式加权，CABAC 那套的 B 块类型。门禁——`bframes_test.go` 5 帧逐字节零差异。
- **VR2d-2 显示排序**⬜待办：还差——现在解一帧吐一帧，B 片一来显示顺序就错。要做——解出来的帧先按显示序号排队，攒够了再按显示顺序吐（参考帧另留一份，不随输出扔掉）。门禁——解码序与显示序双断言都对。
