...
- **STEP-1 窗口减负（已做完，只动示例层）**：位置——`examples/video_player/main.go`之`state.tick` + `fastBlitRGBA`。改法——状态文字数字变了才刷（最多约4Hz）、时间条同阈值节流、显示缓冲换预乘格式（省每帧约150万像素预乘）、无新画面不申请上屏（有帧才刷）。实测——同片15秒窗：改前显示127/上屏143/9.2fps → 改后显示206/上屏277/13.6fps（约+48%），门禁`open_test.go`新加文字节流测试绿。结论——窗口确在偷时间，但单省窗口喂不饱（解码单帧约15.6毫秒+渲染约10毫秒抢核）。归属——示例层。状态——已做完未提交→随v0.80提交 2026-09-16。
