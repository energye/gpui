...
|----|------|----------|---------------------------|------|
| VR2a I帧帧内 | F4（IDR部分）、F5简单熵编码、F6帧内预测、F9 4x4变换与默认缩放、F10滤波开关 | `video/h264`：宏块帧内层、指数之外的残差读数、四分之一精度之外的反变换与帧内、滤波开关 | `intra_test.go`：`vr2_b_intra.yuv`（ffmpeg 原始帧）逐字节一致；切片头追踪值沿用 VR1 单测 | ⬜开工中（地基图像存+切片头已绿 2026-09-12） |
