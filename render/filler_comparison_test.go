...
import (
	...
	_ "github.com/energye/gpui/render/raster" // registers AdaptiveFiller (SparseStrips + TileCompute)
)
...
func TestStrokeSineWave_AnalyticVsSparseStrips(t *testing.T) {
	...
	pmAnalytic := render.NewPixmap(canvasW, canvasH)
	pmAnalytic.Clear(render.White)

	// Save the filler registered by the raster package init(), then clear it.
	savedFiller := render.GetCoverageFiller()
	render.RegisterCoverageFiller(nil)

	rAnalytic := render.NewSoftwareRenderer(canvasW, canvasH)
	...
	pmSparse := render.NewPixmap(canvasW, canvasH)
	pmSparse.Clear(render.White)

	// Restore the AdaptiveFiller (which uses SparseStrips for this path size).
	render.RegisterCoverageFiller(savedFiller)

	rSparse := render.NewSoftwareRenderer(canvasW, canvasH)
...
func buildTestSinePath(numSegments int) *render.Path {
	p := render.NewPath()
...
func makeTestStrokePaint(width float64) *render.Paint {
	p := render.NewPaint()
	p.SetBrush(render.SolidBrush{Color: render.Black})
	p.Stroke = &render.Stroke{
		Width:      width,
		Cap:        render.LineCapButt,
...
func measurePixmap(pm *render.Pixmap, w, h int) measurements {
	var m measurements
	var covSum int64
	data := pm.Data()
...
func measureColumnSpread(pm *render.Pixmap, w, h int) []int {
	spread := make([]int, w)
	data := pm.Data()
...
func comparePixmaps(a, b *render.Pixmap, w, h int) (diffCount, maxDiff int, buckets [5]int) {
	da, db := a.Data(), b.Data()
	for y := 0; y < h; y++ {
		for x := 0; x < w; x++ {
			i := (y*w + x) * 4
