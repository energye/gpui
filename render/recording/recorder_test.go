...
func TestRecorderSetFillStrokeStyle(t *testing.T) {
	...
	// Test SetFillStyle
	fillBrush := NewSolidBrush(render.Red)
	rec.SetFillStyle(fillBrush)
	if rec.fillBrush.(SolidBrush).Color != render.Red {
		t.Error("SetFillStyle did not set fill brush correctly")
	}

	// Test SetStrokeStyle
	strokeBrush := NewSolidBrush(render.Blue)
	rec.SetStrokeStyle(strokeBrush)
	if rec.strokeBrush.(SolidBrush).Color != render.Blue {
...
func TestRecorderFillRule(t *testing.T) {
	...
	rec.SetFillRuleGG(render.FillRuleNonZero)
