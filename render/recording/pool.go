...
func (p *ResourcePool) AddPath(path *render.Path) PathRef {
	if path == nil {
...
func (p *ResourcePool) GetPath(ref PathRef) *render.Path {
...
func (p *ResourcePool) Clone() *ResourcePool {
	clone := &ResourcePool{
		paths:   make([]*render.Path, len(p.paths)),
