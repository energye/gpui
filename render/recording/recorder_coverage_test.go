...
func TestRecorderSetStrokeBrush(t *testing.T) {
	rec := NewRecorder(100, 100)
	rec.SetStrokeBrush(render.Solid(render.Red))
	r := rec.FinishRecording()
	if len(r.Commands()) == 0 {
...
func TestRecorderSetLineCapGG(t *testing.T) {
	rec := NewRecorder(100, 100)
	rec.SetLineCapGG(render.LineCapRound)
	r := rec.FinishRecording()
	if len(r.Commands()) == 0 {
...
func TestRecorderSetLineJoinGG(t *testing.T) {
	rec := NewRecorder(100, 100)
	rec.SetLineJoinGG(render.LineJoinBevel)
	r := rec.FinishRecording()
	if len(r.Commands()) == 0 {
...
func (m *playbackMockBackend) SetClip(_ *render.Path, _ FillRule)                    {}
func (m *playbackMockBackend) ClearClip()                                            {}
func (m *playbackMockBackend) FillPath(_ *render.Path, _ Brush, _ FillRule)          {}
func (m *playbackMockBackend) StrokePath(_ *render.Path, _ Brush, _ Stroke)          {}
