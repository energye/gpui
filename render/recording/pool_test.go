...
func TestResourcePool_AddBrush(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "solid red",
			brush: NewSolidBrush(render.Red),
		},
		{
			name:  "solid with alpha",
			brush: NewSolidBrush(render.RGBA2(1, 0, 0, 0.5)),
		},
		{
...
func TestResourcePool_GetBrush_InvalidRef(t *testing.T) {
	pool := NewResourcePool()
	pool.AddBrush(NewSolidBrush(render.Red))
...
func BenchmarkResourcePool_Clone(b *testing.B) {
	...
	for i := 0; i < 50; i++ {
		pool.AddBrush(NewSolidBrush(render.RGBA{R: float64(i) / 50, G: 0, B: 0, A: 1}))
	}
	for i := 0; i < 10; i++ {
