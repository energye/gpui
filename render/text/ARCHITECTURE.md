...
| 字体形态 | 引擎 | 入口 | light 状态 |
|---|---|---|---|
| CFF / CFF2 | cf2（psaux 移植） | `hint/hint.go` → `hint/psh_light.go` + `hint/cffcs.go` + `hint/cff.go` | ✅ M0–M3 完成（3000 常用字 + 韩 11172 + 泰 87 逐点 bad=0） |
| glyf + fpgm/prep | TT 字节码解释器 | `tt_engine.go`（tt_*.go 家族） | ❌ L0：加 InterpLight 分支（render/ 高风险，改前 question） |
| glyf 无字节码 | autofit（cjk/latin/indic） | `autohint_*.go`（Full 语义） | ❌ M4：改 light 语义；`hint/afcjk.go` / `hint/latin_light.go` 为移植种子；其余脚本透传不 hint |

## 3. light 模式三种引擎各自做什么（对应关系）

| 引擎 | 输入 | light 下的行为 | 状态 |
|---|---|---|---|
| cf2 | CFF 表 + charstring | 蓝区（幽灵区/真实区）→ capture 三态 → hintmap 插入/adjustHints → 全轮廓 cs→ds 映射 | ✅ 完成 |
| tt_engine | fpgm/prep 字节码 | light = 只 Y 向网格调整，X 向指令/IUP 后 X 调整不发 drop-out，强 keeper | ❌ L0 |
| autofit | 原始 glyf 轮廓 | afcjk light（顶横蓝线锚定）/ aflatin light / afindic；阿拉伯/希伯来/泰文等**透传** | ❌ M4 |

## 4. 脚本 → 引擎 → 验证集

| 语言族 | 主字体形态 | 走哪个引擎 | 验证状态 |
|---|---|---|---|
| CJK（中/日/韩） | CFF（Noto CJK） | cf2 | ✅ 3000 字 + 11172 Hangul bad=0 |
| 泰文 | CFF（Noto Sans Thai） | cf2 | ✅ 87 字 bad=0 |
| 拉丁/西里尔/希腊 | glyf（DejaVu/Noto Sans 带字节码） | tt_engine | ❌ L0–L1（255 字集归零） |
| Latin 无字节码样本 | glyf（DejaVu 版/自造） | autofit latin | ❌ M4 |
| 阿拉伯/希伯来等 | glyf/CFF | autofit 透传 或 cf2 | M4 透传 |
