...
func newTestPixmap(width, height int) *render.Pixmap {
	return render.NewPixmap(width, height)
}
...
func newRectPath(x, y, w, h float64) *render.Path {
	p := render.NewPath()
	p.Rectangle(x, y, w, h)
	return p
}
...
func BenchmarkLayerCache_Put(b *testing.B) {
	cache := NewLayerCache(64)
	pixmaps := make([]*render.Pixmap, b.N)
	for i := range pixmaps {
