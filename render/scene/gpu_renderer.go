	...
	for dec.Next() {
		...
		case TagStroke:
			...
			if style != nil {
				...
				dc.SetLineCap(render.LineCap(style.Cap))
				dc.SetLineJoin(render.LineJoin(style.Join))
		...
		case TagBeginClip:
			...
			shape := render.DetectShape(path)
			if shape.Kind == render.ShapeRect {
...
func (r *GPUSceneRenderer) resolveImage(scene *Scene, imageIndex uint32, transform Affine) {
	...
	buf := render.ImageBufFromImage(rgba)
	...
	r.dc.DrawImageEx(buf, render.DrawImageOptions{
...
func CanUseGPU() bool {
	return render.AcceleratorCanRenderDirect()
}
