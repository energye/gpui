...
func BenchmarkScene_Render_HD(b *testing.B) {
	...
	target := render.NewPixmap(1920, 1080)
	...
	for j := 0; j < 100; j++ {
		...
		y := float32(j/10) * 100
		scene.Fill(FillNonZero, IdentityAffine(),
			SolidBrush(render.RGBA{R: float64(j%3) * 0.5, G: float64(j%5) * 0.25, B: float64(j%7) * 0.15, A: 1}),
...
func BenchmarkScene_Render_4K(b *testing.B) {
	...
	target := render.NewPixmap(3840, 2160)
...
func BenchmarkScene_Render_Small(b *testing.B) {
	...
	target := render.NewPixmap(512, 512)
...
func BenchmarkLayerCache_Hit(b *testing.B) {
	...
	// Pre-populate cache
	pixmap := render.NewPixmap(100, 100)
...
func BenchmarkEncoding_Clone(b *testing.B) {
	...
	for j := 0; j < 100; j++ {
		...
		path.Close()
		enc.EncodePath(path)
		enc.EncodeFill(SolidBrush(render.RGBA{R: 1, G: 0, B: 0, A: 1}), FillNonZero)
	}
...
func BenchmarkSceneBuilder_Build_100Shapes(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		for j := 0; j < 100; j++ {
			x := float32(j%10) * 50
			y := float32(j/10) * 50
			builder.FillRect(x, y, 40, 40, SolidBrush(render.RGBA{R: 1, G: 0, B: 0, A: 1}))
		}
		_ = builder.Build()
	}
}
...
func BenchmarkSceneBuilder_WithLayers(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		builder := NewSceneBuilder()
		builder.FillRect(0, 0, 100, 100, SolidBrush(render.RGBA{R: 1, G: 1, B: 1, A: 1}))

		builder.Layer(BlendMultiply, 0.5, nil, func(lb *SceneBuilder) {
			lb.FillCircle(50, 50, 30, SolidBrush(render.RGBA{R: 1, G: 0, B: 0, A: 1}))
		})

		builder.Layer(BlendScreen, 0.75, nil, func(lb *SceneBuilder) {
			lb.FillRect(25, 25, 50, 50, SolidBrush(render.RGBA{R: 0, G: 1, B: 0, A: 1}))
		})
...
func BenchmarkScene_Memory_Large(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		for j := 0; j < 1000; j++ {
			...
			y := float32(j/32) * 30
			scene.Fill(FillNonZero, IdentityAffine(),
				SolidBrush(render.RGBA{R: float64(j%256) / 255, G: float64(j%128) / 127, B: float64(j%64) / 63, A: 1}),
