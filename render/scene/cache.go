...
func (c *LayerCache) Get(hash uint64) (*render.Pixmap, bool) {
	c.mu.RLock()
	_, ok := c.entries[hash]
	c.mu.RUnlock()
...
func (c *LayerCache) Put(hash uint64, pixmap *render.Pixmap, version uint64) {
	if pixmap == nil {
		return
	}
...
func pixmapSize(p *render.Pixmap) int64 {
	if p == nil {
		return 0
	}
