...
func TestRenderer_RenderSimpleScene(t *testing.T) {
	...
	target := render.NewPixmap(200, 200)
	target.Clear(render.RGBA{R: 1, G: 1, B: 1, A: 1}) // White background

	scene := NewScene()
	rect := NewRectShape(50, 50, 100, 100)
	scene.Fill(FillNonZero, IdentityAffine(), SolidBrush(render.RGBA{R: 1, G: 0, B: 0, A: 1}), rect)
...
func TestRenderer_MultipleShapes(t *testing.T) {
	...
	target := render.NewPixmap(400, 400)

	scene := NewScene()

	// Add multiple shapes
	scene.Fill(FillNonZero, IdentityAffine(),
		SolidBrush(render.RGBA{R: 1, G: 0, B: 0, A: 1}),
		NewRectShape(10, 10, 100, 100))

	scene.Fill(FillNonZero, IdentityAffine(),
		SolidBrush(render.RGBA{R: 0, G: 1, B: 0, A: 1}),
		NewCircleShape(200, 200, 50))

	scene.Stroke(DefaultStrokeStyle(), IdentityAffine(),
		SolidBrush(render.RGBA{R: 0, G: 0, B: 1, A: 1}),
...
func TestRenderer_ParallelExecution(t *testing.T) {
	...
	target := render.NewPixmap(1024, 1024)
	...
	for i := 0; i < 100; i++ {
		...
		y := float32(i / 10 * 100)
		scene.Fill(FillNonZero, IdentityAffine(),
			SolidBrush(render.RGBA{R: float64(i%2) * 0.5, G: float64(i%3) * 0.33, B: float64(i%5) * 0.2, A: 1}),
...
func TestRenderer_CircleFill(t *testing.T) {
	...
	target := render.NewPixmap(size, size)
...
func TestRenderer_AlphaCompositing(t *testing.T) {
	...
	s.Fill(FillNonZero, IdentityAffine(),
		SolidBrush(render.RGBA{R: 0, G: 0, B: 1, A: 0.5}),
...
func TestConvertPath(t *testing.T) {
	...
	// Verify verb types
	wantVerbs := []render.PathVerb{render.MoveTo, render.LineTo, render.QuadTo, render.CubicTo, render.Close}
...
func TestConvertFillPaint(t *testing.T) {
	tests := []struct {
		...
		brush    Brush
		style    FillStyle
		wantRule render.FillRule
	}{
		{
			"solid non-zero",
			SolidBrush(render.RGBA{R: 1, A: 1}),
			FillNonZero,
			render.FillRuleNonZero,
		},
		{
			"solid even-odd",
			SolidBrush(render.RGBA{R: 1, A: 1}),
			FillEvenOdd,
			render.FillRuleEvenOdd,
		},
	}
...
func TestConvertStrokePaint(t *testing.T) {
	...
	brush := SolidBrush(render.RGBA{R: 1, G: 0, B: 0, A: 1})
	...
	if s.Cap != render.LineCapRound {
		t.Errorf("Stroke.Cap = %d, want LineCapRound", s.Cap)
	}
	if s.Join != render.LineJoinBevel {
...
func TestConvertStrokePaintNilStyle(t *testing.T) {
	brush := SolidBrush(render.RGBA{R: 1, A: 1})
...
func TestClipBuilderAPI(t *testing.T) {
	...
	builder.
		FillRect(0, 0, float32(size), float32(size), SolidBrush(render.RGBA{R: 0, G: 0, B: 1, A: 1})).
		Clip(NewRectShape(60, 60, 80, 80), func(b *SceneBuilder) {
			b.FillRect(0, 0, float32(size), float32(size), SolidBrush(render.RGBA{R: 1, G: 0, B: 0, A: 1}))
		})
	scene := builder.Build()
...
func TestCompositePixmaps(t *testing.T) {
	dst := render.NewPixmap(1, 1)
	src := render.NewPixmap(1, 1)
...
func TestRenderer_DrawImage_SemiTransparent(t *testing.T) {
	...
	s.Fill(FillNonZero, IdentityAffine(),
		SolidBrush(render.RGBA{R: 1, G: 1, B: 1, A: 1}),
		NewRectShape(0, 0, float32(size), float32(size)))
