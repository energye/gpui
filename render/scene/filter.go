...
type Filter interface {
	...
	Apply(src, dst *render.Pixmap, bounds Rect)
...
func (fc *FilterChain) Apply(src, dst *render.Pixmap, bounds Rect) {
	if len(fc.filters) == 0 {
	...
	temp1 := render.NewPixmap(w, h)
	temp2 := render.NewPixmap(w, h)
...
func copyPixmap(src, dst *render.Pixmap, bounds Rect) {
