...
import (
	...
	"github.com/energye/gpui/render/internal/parallel"
...
func (p *tilePool) getRenderer(w, h int) *render.SoftwareRenderer {
	if v := p.renderers.Get(); v != nil {
		sr := v.(*render.SoftwareRenderer)
		sr.Resize(w, h)
		return sr
	}
	return render.NewSoftwareRenderer(w, h)
}

// putRenderer returns a SoftwareRenderer to the pool for reuse.
func (p *tilePool) putRenderer(sr *render.SoftwareRenderer) {
	p.renderers.Put(sr)
}
...
func (p *tilePool) getPixmap(w, h int) *render.Pixmap {
	...
	return render.NewPixmap(w, h)
}

// putPixmap returns a Pixmap to the pool for reuse.
func (p *tilePool) putPixmap(pm *render.Pixmap) {
	p.pixmaps.Put(pm)
}
...
func (p *tilePool) getFillPaint() *render.Paint {
	if v := p.fillPaints.Get(); v != nil {
		return v.(*render.Paint)
	}
	return render.NewPaint()
}

// putFillPaint returns a Paint to the pool for reuse.
func (p *tilePool) putFillPaint(paint *render.Paint) {
...
func (r *Renderer) Render(target *render.Pixmap, scene *Scene) error {
...
func (r *Renderer) RenderWithContext(ctx context.Context, target *render.Pixmap, scene *Scene) error {
...
func (r *Renderer) RenderDirty(target *render.Pixmap, scene *Scene, dirty *parallel.DirtyRegion) error {
...
func (r *Renderer) RenderWithDamage(target *render.Pixmap, scene *Scene, tracker *DamageTracker) (
...
// The context can be used to cancel long-running renders. When canceled,
// the function returns ctx.Err() and the target may contain partial results.
func (r *Renderer) RenderDirtyWithContext(ctx context.Context, target *render.Pixmap, scene *Scene, dirty *parallel.DirtyRegion) error {
	if target == nil || scene == nil {
		return nil
	}
...
func (r *Renderer) renderTilesWithContext(ctx context.Context, tiles []*parallel.Tile, enc *Encoding, target *render.Pixmap, images []*Image) error {
...
func (r *Renderer) renderTile(tile *parallel.Tile, enc *Encoding, _ *render.Pixmap, images []*Image) {
	if tile == nil || enc == nil {
		return
	}
...
type tileClipState struct {
	...
	savedPM *render.Pixmap // the pixmap we were drawing to before this clip
}
	...
	for dec.Next() {
		...
		case TagBeginClip:
			...
			if pathActive && !currentPath.IsEmpty() {
				...
				convertPathInto(currentPath, tileX, tileY, ggPath)
				resetFillPaint(fillPaint, Brush{Kind: BrushSolid, Color: render.RGBA{R: 1, G: 1, B: 1, A: 1}}, FillNonZero)
...
func extractAlphaMaskInto(pm *render.Pixmap, mask []byte) {
	data := pm.Data()
...
func applyAlphaMask(pm *render.Pixmap, mask []byte) {
	data := pm.Data()
...
//
//nolint:gosec // G115: Integer overflow is not possible - math is bounded to [0,255]
func compositePixmaps(dst, src *render.Pixmap) {
	dstData := dst.Data()
	srcData := src.Data()
...
func (r *Renderer) renderTextOnTile(
	...
	pm *render.Pixmap, sr *render.SoftwareRenderer, fillPaint *render.Paint,
) {
...
func renderFillRoundRect(pm *render.Pixmap, transform Affine, brush Brush, rect Rect, rx, ry float32, tileX, tileY int) {
...
func blitImageToTile(img *Image, transform Affine, tileX, tileY int, pm *render.Pixmap) {
	tileW := pm.Width()
	tileH := pm.Height()
	pmData := pm.Data()
	stride := tileW * 4
...
func resetFillPaint(paint *render.Paint, brush Brush, style FillStyle) {
	if brush.Kind == BrushSolid {
	...
	if style == FillEvenOdd {
		...
		paint.FillRule = render.FillRuleNonZero
	}
	paint.Stroke = nil
}
...
func extractAlphaMask(pm *render.Pixmap) []byte {
...
func convertStrokePaint(brush Brush, style *StrokeStyle) *render.Paint {
	...
	switch style.Cap {
	case LineCapButt:
		s.Cap = render.LineCapButt
	case LineCapRound:
		s.Cap = render.LineCapRound
	case LineCapSquare:
		s.Cap = render.LineCapSquare
	}

	switch style.Join {
	case LineJoinMiter:
		s.Join = render.LineJoinMiter
	case LineJoinRound:
		s.Join = render.LineJoinRound
	case LineJoinBevel:
		s.Join = render.LineJoinBevel
	}
...
func (r *Renderer) compositeTiles(tiles []*parallel.Tile, target *render.Pixmap) {
