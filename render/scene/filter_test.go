...
func (f *mockFilter) Apply(src, dst *render.Pixmap, bounds Rect) {
	f.applyCalled = true
...
func TestCopyPixmap(t *testing.T) {
	...
	for y := 0; y < 5; y++ {
		for x := 0; x < 5; x++ {
			src.SetPixel(x, y, render.RGBA2(float64(x)/5, float64(y)/5, 0.5, 1.0))
		}
	}
...
func colorApproxEqual(a, b render.RGBA, tolerance float64) bool {
