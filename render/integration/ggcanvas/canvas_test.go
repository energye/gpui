...
func (m *mockProvider) SurfaceFormat() types.TextureFormat { return m.format }
...
func captureWarning(fn func()) bool {
	var warned bool
	origHandler := render.Logger().Handler()
	render.SetLogger(slog.New(&warningDetector{warned: &warned, inner: origHandler}))
	defer render.SetLogger(slog.New(origHandler))
	fn()
	return warned
}
...
func TestDraw_ResetsMatrix(t *testing.T) {
	...
	_ = canvas.Draw(func(dc *render.Context) {
		dc.Translate(25, 25)
		dc.Scale(0.5, 0.5)
	})
