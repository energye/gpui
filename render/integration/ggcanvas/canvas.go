...
func warnIfPhysicalDimensions(wp gpucontext.WindowProvider, width, height int, scale float64) {
	...
	if width > int(float64(logicalW)*threshold) || height > int(float64(logicalH)*threshold) {
		render.Logger().Warn("ggcanvas.New: dimensions look like physical pixels, not logical — "+
...
func NewWithScale(provider gpucontext.DeviceProvider, width, height int, scale float64) (*Canvas, error) {
	...
	render.Logger().Info("ggcanvas.NewWithScale",
	...
	if err := render.SetAcceleratorDeviceProvider(provider); err != nil {
		render.Logger().Warn("SetAcceleratorDeviceProvider failed", "err", err)
	}

	var opts []render.ContextOption
	if scale != 1.0 {
		opts = append(opts, render.WithDeviceScale(scale))
	}

	c := &Canvas{
		ctx:      render.NewContext(width, height, opts...),
		provider: provider,
		width:    width,
		height:   height,
...
func (c *Canvas) Context() *render.Context {
	if c.closed {
		return nil
	}
	return c.ctx
...
func (c *Canvas) Flush() (any, error) {
	...
	if err := c.ctx.FlushGPU(); err != nil {
		render.Logger().Warn("FlushGPU error", "err", err)
	}
...
func (c *Canvas) RenderDirect(surfaceView gpucontext.TextureView, width, height uint32) error {
	...
	render.Logger().Debug("ggcanvas.RenderDirect",
...
func (c *Canvas) Render(dc RenderTarget) error {
	...
	sv := dc.SurfaceView()
	if !sv.IsNil() && render.AcceleratorCanRenderDirect() {
