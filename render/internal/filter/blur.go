...
func (f *BlurFilter) Apply(src, dst *render.Pixmap, bounds scene.Rect) {
	if src == nil || dst == nil {
		return
	}
...
func blurHorizontal(src *render.Pixmap, temp []float32, minX, minY, width, height int, kernel []float32) {
...
func blurVertical(temp []float32, dst *render.Pixmap, minX, minY, width, height int, kernel []float32) {
	kernelSize := len(kernel)
	halfKernel := kernelSize / 2
	dstData := dst.Data()
	dstWidth := dst.Width()
...
func copyToTemp(src *render.Pixmap, temp []float32, minX, minY, width, height int) {
	srcData := src.Data()
	srcWidth := src.Width()
...
func copyFromTemp(temp []float32, dst *render.Pixmap, minX, minY, width, height int) {
	dstData := dst.Data()
	dstWidth := dst.Width()
...
func copyPixmapRegion(src, dst *render.Pixmap, bounds scene.Rect) {
	minX := clampInt(int(bounds.MinX), 0, src.Width())
	maxX := clampInt(int(bounds.MaxX), 0, src.Width())
	minY := clampInt(int(bounds.MinY), 0, src.Height())
	maxY := clampInt(int(bounds.MaxY), 0, src.Height())
