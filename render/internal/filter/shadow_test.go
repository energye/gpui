...
func TestDropShadowExpandBounds(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			f := NewDropShadowFilter(tt.offsetX, tt.offsetY, tt.blur, render.Black)
...
func TestDropShadowApplyBasic(t *testing.T) {
	...
	src := render.NewPixmap(20, 20)
	for y := 5; y < 15; y++ {
		for x := 5; x < 15; x++ {
			src.SetPixel(x, y, render.White)
		}
	}

	dst := render.NewPixmap(25, 25) // Larger for shadow expansion

	f := NewDropShadowFilter(3, 3, 1, render.RGBA2(0, 0, 0, 1.0))
...
func TestDropShadowApplyNoBlur(t *testing.T) {
	...
	dst := render.NewPixmap(25, 25)

	f := NewDropShadowFilter(5, 5, 0, render.RGBA2(0, 0, 0, 1.0)) // No blur
	bounds := scene.Rect{MinX: 0, MinY: 0, MaxX: 25, MaxY: 25}
...
func TestDropShadowApplyTransparentColor(t *testing.T) {
	src := render.NewPixmap(10, 10)
	for y := 2; y < 8; y++ {
		for x := 2; x < 8; x++ {
			src.SetPixel(x, y, render.Red)
		}
	}

	dst := render.NewPixmap(15, 15)

	// Fully transparent shadow color - should have no visible effect
	f := NewDropShadowFilter(3, 3, 2, render.RGBA2(0, 0, 0, 0))
...
func TestDropShadowApplyColoredShadow(t *testing.T) {
	...
	dst := render.NewPixmap(20, 20)

	// Blue shadow
	f := NewDropShadowFilter(5, 5, 0, render.RGBA2(0, 0, 1, 1.0))
...
func TestExtractAlpha(t *testing.T) {
	...
	for y := 0; y < 10; y++ {
		for x := 0; x < 10; x++ {
			src.SetPixel(x, y, render.RGBA2(1, 0, 0, float64(x+y)/20.0))
		}
	}
...
func TestExtractAlphaWithOffset(t *testing.T) {
	src := render.NewPixmap(10, 10)
	src.SetPixel(5, 5, render.White) // Only this pixel is white
...
func BenchmarkDropShadowFilter(b *testing.B) {
	...
	for _, size := range sizes {
		b.Run(size.name, func(b *testing.B) {
			src := render.NewPixmap(size.w, size.h)
			...
			dst := render.NewPixmap(size.w+50, size.h+50)
