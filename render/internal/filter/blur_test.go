...
func TestBlurFilterApplyZeroRadius(t *testing.T) {
	src := createTestPixmap(10, 10, render.Red)
	...
	for y := 0; y < 10; y++ {
		for x := 0; x < 10; x++ {
			...
			if !colorApproxEqual(got, render.Red, 0.01) {
...
func TestBlurFilterApplySmallImage(t *testing.T) {
	...
	src := createTestPixmap(5, 5, render.Black)
	src.SetPixel(2, 2, render.White)
...
func TestBlurFilterApplyPreservesAlpha(t *testing.T) {
	...
	for y := 0; y < 10; y++ {
		for x := 0; x < 10; x++ {
			src.SetPixel(x, y, render.RGBA2(1, 0, 0, 0.5))
		}
	}
...
func TestBlurFilterApplyEdgeHandling(t *testing.T) {
	...
	src := createTestPixmap(20, 20, render.White)
	...
	for y := 0; y < 20; y++ {
		for x := 0; x < 20; x++ {
			c := dst.GetPixel(x, y)
			if !colorApproxEqual(c, render.White, 0.01) {
...
func TestBlurFilterApplyOnlyHorizontal(t *testing.T) {
	...
	src := createTestPixmap(20, 20, render.Black)
	for y := 0; y < 20; y++ {
		src.SetPixel(10, y, render.White)
	}
...
func TestBlurFilterApplyOnlyVertical(t *testing.T) {
	...
	src := createTestPixmap(20, 20, render.Black)
	for x := 0; x < 20; x++ {
		src.SetPixel(x, 10, render.White)
	}
