...
func (f *DropShadowFilter) Apply(src, dst *render.Pixmap, bounds scene.Rect) {
	if src == nil || dst == nil {
		return
	}
...
func extractAlpha(src *render.Pixmap, alpha []float32, minX, minY, width, height, offsetX, offsetY int) {
	srcWidth := src.Width()
...
func compositeShadow(src, dst *render.Pixmap, shadowAlpha []float32,
	minX, minY, width, height int, color render.RGBA) {
	srcWidth := src.Width()
	srcHeight := src.Height()
	srcData := src.Data()
	dstWidth := dst.Width()
