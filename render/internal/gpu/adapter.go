...
import (
	...
	"github.com/energye/gpui/render/internal/gpucore"
)
...
type HALAdapter struct {
	...
	buffers          map[gpucore.BufferID]*webgpu.Buffer
	textures         map[gpucore.TextureID]*webgpu.Texture
	shaderModules    map[gpucore.ShaderModuleID]*webgpu.ShaderModule
	computePipelines map[gpucore.ComputePipelineID]*webgpu.ComputePipeline
	bindGroupLayouts map[gpucore.BindGroupLayoutID]*webgpu.BindGroupLayout
	pipelineLayouts  map[gpucore.PipelineLayoutID]*webgpu.PipelineLayout
	bindGroups       map[gpucore.BindGroupID]*webgpu.BindGroup
...
func NewHALAdapter(device *webgpu.Device, queue *webgpu.Queue, limits *types.Limits) *HALAdapter {
	...
	adapter := &HALAdapter{
		...
		buffers:          make(map[gpucore.BufferID]*webgpu.Buffer),
		textures:         make(map[gpucore.TextureID]*webgpu.Texture),
		shaderModules:    make(map[gpucore.ShaderModuleID]*webgpu.ShaderModule),
		computePipelines: make(map[gpucore.ComputePipelineID]*webgpu.ComputePipeline),
		bindGroupLayouts: make(map[gpucore.BindGroupLayoutID]*webgpu.BindGroupLayout),
		pipelineLayouts:  make(map[gpucore.PipelineLayoutID]*webgpu.PipelineLayout),
		bindGroups:       make(map[gpucore.BindGroupID]*webgpu.BindGroup),
	}
...
func (a *HALAdapter) CreateShaderModule(spirv []uint32, label string) (gpucore.ShaderModuleID, error) {
	...
	desc := &webgpu.ShaderModuleDescriptor{
		Label: label,
		SPIRV: spirv,
	}
...
func (a *HALAdapter) ReadBuffer(id gpucore.BufferID, offset, size uint64) ([]byte, error) {
	...
	stagingDesc := &webgpu.BufferDescriptor{
...
func (a *HALAdapter) WriteTexture(id gpucore.TextureID, data []byte) {
	...
	dst := &webgpu.ImageCopyTexture{
		Texture:  texture,
		MipLevel: 0,
		Origin:   webgpu.Origin3D{X: 0, Y: 0, Z: 0},
		Aspect:   types.TextureAspectAll,
	}

	// Placeholder dimensions - real implementation needs texture metadata
	layout := &webgpu.ImageDataLayout{
		Offset:       0,
...
func (a *HALAdapter) CreateBindGroupLayout(desc *gpucore.BindGroupLayoutDesc) (gpucore.BindGroupLayoutID, error) {
	...
	halDesc := &webgpu.BindGroupLayoutDescriptor{
		Label:   desc.Label,
		Entries: halEntries,
	}
...
func (a *HALAdapter) CreatePipelineLayout(layouts []gpucore.BindGroupLayoutID) (gpucore.PipelineLayoutID, error) {
	a.mu.RLock()
	halLayouts := make([]*webgpu.BindGroupLayout, len(layouts))
	...
	halDesc := &webgpu.PipelineLayoutDescriptor{
		Label:            "",
...
func (a *HALAdapter) CreateComputePipeline(desc *gpucore.ComputePipelineDesc) (gpucore.ComputePipelineID, error) {
	...
	halDesc := &webgpu.ComputePipelineDescriptor{
		Label:      desc.Label,
...
func (a *HALAdapter) CreateBindGroup(layout gpucore.BindGroupLayoutID, entries []gpucore.BindGroupEntry) (gpucore.BindGroupID, error) {
	...
	halEntries := make([]webgpu.BindGroupEntry, len(entries))
	...
	halDesc := &webgpu.BindGroupDescriptor{
		Label:   "",
		Layout:  halLayout,
		Entries: halEntries,
	}
...
func (a *HALAdapter) BeginComputePass() gpucore.ComputePassEncoder {
	...
	// Begin compute pass
	halPass, cpErr := a.encoder.BeginComputePass(&webgpu.ComputePassDescriptor{
		Label: "compute",
	})
	if cpErr != nil {
		return &halComputePassEncoder{adapter: a}
...
func convertBindGroupLayoutEntry(entry gpucore.BindGroupLayoutEntry) types.BindGroupLayoutEntry {
	...
	case gpucore.BindingTypeStorageTexture:
		result.StorageTexture = &types.StorageTextureBindingLayout{
...
func (a *HALAdapter) convertBindGroupEntry(entry gpucore.BindGroupEntry) (webgpu.BindGroupEntry, error) {
	result := webgpu.BindGroupEntry{
		Binding: entry.Binding,
	}
...
type halComputePassEncoder struct {
	adapter *HALAdapter
	pass    *webgpu.ComputePassEncoder
}
