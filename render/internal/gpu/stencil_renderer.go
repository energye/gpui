...
// x,y,d triples) for sampleCount==1 stencil covers: the exterior half
...
func (sr *StencilRenderer) updateAACoverBuffers(b *stencilCoverBuffers, bandVerts, innerBandVerts []float32) error {
	...
	if len(bandVerts) == 0 && len(innerBandVerts) == 0 {
		b.bandVertexCount = 0
		b.innerBandVertexCount = 0
		return nil
	}
	if len(bandVerts) > 0 {
...
func (sr *StencilRenderer) RenderPath(target render.GPURenderTarget, path *render.Path, color render.RGBA, fillRule render.FillRule) error {
...
func (sr *StencilRenderer) RecordPath(rp *webgpu.RenderPassEncoder, bufs *stencilCoverBuffers, fillRule render.FillRule, clipBG *webgpu.BindGroup, maskBG *webgpu.BindGroup, blendMode render.BlendMode, depthClipped ...bool) {
	...
	//      fringe over the background.
	//   2) interior band: SrcOver + stencil NotEqual(0), PassOp=Zero —
	//      inside-half fringe blends over the background, then clears those
	//      pixels' stencil so the cover skips them. (The previous post-cover
	//      Replace scheme lost the background term under partial pixels and
	//      left dark speckles along curves/diagonals.)
	//   3) binary cover: fills the remaining full-interior pixels + resets
	//      stencil (unchanged). At d=0 the band coverage reaches 1.0, so band
	...
	// Pixels already drawn (and stencil-cleared) by the interior band are
	// skipped via the NotEqual(0) test.
...
func makeCoverUniform(w, h uint32, color render.RGBA) []byte {
