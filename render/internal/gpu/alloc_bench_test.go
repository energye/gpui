...
func BenchmarkGPURenderContext_QueueShape(b *testing.B) {
	shared := NewGPUShared()
	rc := shared.NewRenderContext()
	target := render.GPURenderTarget{Width: 800, Height: 600, Stride: 3200, Data: make([]byte, 800*600*4)}
	shape := render.DetectedShape{Kind: render.ShapeCircle, CenterX: 100, CenterY: 100, RadiusX: 50}
