...
func TestSDFAccelerator_SetPipelineMode(t *testing.T) {
	...
	a.SetPipelineMode(render.PipelineModeCompute)
	a.SetPipelineMode(render.PipelineModeRenderPass)
	a.SetPipelineMode(render.PipelineModeAuto)
...
func TestSDFAccelerator_Interfaces(t *testing.T) {
	...
	// Core interface.
	var _ render.GPUAccelerator = a

	// Extension interfaces.
	var _ render.GPURenderContextProvider = a
	var _ render.GPUTextAccelerator = a
	var _ render.PipelineModeAware = a
	var _ render.ComputePipelineAware = a
}
...
func TestStrokeRouting_ComputeModeUsesVello(t *testing.T) {
	...
	rc := &GPURenderContext{shared: s, pipelineMode: render.PipelineModeCompute}
	target := makeTestTarget(200, 200)
...
func TestFillShape_RasterAtlas_FallsBackToCPU(t *testing.T) {
	...
	s.deviceReady = true
	s.gpuReady = false
	s.cpuFallback = render.SDFAccelerator{}

	rc := s.NewRenderContext()
	target := makeTestTarget(100, 100)
	shape := render.DetectedShape{
		Kind: render.ShapeCircle, CenterX: 50, CenterY: 50, RadiusX: 30, RadiusY: 30,
	}
	paint := render.NewPaint()
	paint.SetBrush(render.Solid(render.Red))
...
func TestUploadPixmapToView_RasterAtlas(t *testing.T) {
	...
	tex, err := device.CreateTexture(&webgpu.TextureDescriptor{
		...
		Usage:         types.TextureUsageRenderAttachment | types.TextureUsageCopyDst | types.TextureUsageTextureBinding,
	})
	if err != nil {
...
func TestUploadPixmapToView_EmptyData(t *testing.T) {
	...
	target := render.GPURenderTarget{Data: nil, Width: 4, Height: 4}
...
func TestUploadPixmapToView_NilView(t *testing.T) {
	...
	data := make([]byte, 4*4*4)
	target := render.GPURenderTarget{Data: data, Width: 4, Height: 4, Stride: 16}
...
func TestRGBASwizzle(t *testing.T) {
	...
	target := render.GPURenderTarget{Data: data, Width: 1, Height: 1, Stride: 4}
