...
func stageBindGroupLayoutEntries(stage VelloComputeStage) []types.BindGroupLayoutEntry {
	...
	storageRW := func(binding uint32) types.BindGroupLayoutEntry {
		return types.BindGroupLayoutEntry{
			Binding:    binding,
			Visibility: types.ShaderStageCompute,
...
func (d *VelloComputeDispatcher) AllocateBuffers(
	...
	uniformCPU := types.BufferUsageUniform | types.BufferUsageCopyDst
	storageOut := types.BufferUsageStorage | types.BufferUsageCopySrc
