...
func TestRenderPathTriangle(t *testing.T) {
	...
	err := sr.RenderPath(target, path, color, render.FillRuleNonZero)
	if err != nil {
...
func TestRenderPathEmpty(t *testing.T) {
	...
	err := sr.RenderPath(target, nil, render.RGBA{R: 1, G: 0, B: 0, A: 1}, render.FillRuleNonZero)
	if err != nil {
		t.Fatalf("RenderPath with nil elements should return nil, got: %v", err)
	}

	// Empty path should also return nil.
	err = sr.RenderPath(target, render.NewPath(), render.RGBA{R: 1, G: 0, B: 0, A: 1}, render.FillRuleNonZero)
	if err != nil {
...
func TestRenderPathCircle(t *testing.T) {
	...
	color := render.RGBA{R: 0, G: 0.5, B: 1, A: 0.8}
	err := sr.RenderPath(target, path, color, render.FillRuleNonZero)
	if err != nil {
...
func TestRenderPathEvenOdd(t *testing.T) {
	...
	err := sr.RenderPath(target, path, color, render.FillRuleEvenOdd)
	if err != nil {
...
func TestRenderPathBothFillRules(t *testing.T) {
	...
	if err := sr.RenderPath(target, path, color, render.FillRuleNonZero); err != nil {
		t.Fatalf("RenderPath NonZero failed: %v", err)
	}

	// Then render with EvenOdd — both pipelines should coexist.
	if err := sr.RenderPath(target, path, color, render.FillRuleEvenOdd); err != nil {
...
func TestRenderPathResizesTextures(t *testing.T) {
	...
	// Render at 64x64.
	target1 := render.GPURenderTarget{
		Data:   make([]uint8, 64*64*4),
		Width:  64,
		Height: 64,
		Stride: 64 * 4,
	}
	if err := sr.RenderPath(target1, path, color, render.FillRuleNonZero); err != nil {
	...
	target2 := render.GPURenderTarget{
		Data:   make([]uint8, 128*96*4),
		Width:  128,
		Height: 96,
		Stride: 128 * 4,
	}
	if err := sr.RenderPath(target2, path, color, render.FillRuleNonZero); err != nil {
...
func TestMakeCoverUniform(t *testing.T) {
	color := render.RGBA{R: 1.0, G: 0.5, B: 0.25, A: 0.8}
