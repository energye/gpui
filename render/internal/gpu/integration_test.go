...
func TestBackendIntegration(t *testing.T) {
	...
	s.Fill(scene.FillNonZero, scene.IdentityAffine(), scene.SolidBrush(render.Red), rect)

	// Create target pixmap
	pm := render.NewPixmap(100, 100)
...
func TestBackendWithComplexScene(t *testing.T) {
	...
	s.Fill(scene.FillNonZero, scene.IdentityAffine(), scene.SolidBrush(render.White), bg)

	// Circle with transform
	circle := scene.NewCircleShape(100, 100, 50)
	s.Fill(scene.FillEvenOdd, scene.TranslateAffine(10, 10), scene.SolidBrush(render.Blue), circle)

	// Rectangle with different fill rule
	rect := scene.NewRectShape(50, 50, 100, 100)
	s.Fill(scene.FillNonZero, scene.IdentityAffine(), scene.SolidBrush(render.Green), rect)

	// Render
	pm := render.NewPixmap(200, 200)
	err := b.RenderScene(pm, s)
	if err != nil {
...
func TestBackendWithLayers(t *testing.T) {
	...
	s.Fill(scene.FillNonZero, scene.IdentityAffine(), scene.SolidBrush(render.White), rect)
	...
	s.Fill(scene.FillNonZero, scene.IdentityAffine(), scene.SolidBrush(render.Red), circle)
...
func TestBackendWithNestedLayers(t *testing.T) {
	...
	s.Fill(scene.FillNonZero, scene.IdentityAffine(), scene.SolidBrush(render.Red), rect1)

	// Nested layer
	s.PushLayer(scene.BlendMultiply, 0.7, nil)
	circle := scene.NewCircleShape(50, 50, 25)
	s.Fill(scene.FillNonZero, scene.IdentityAffine(), scene.SolidBrush(render.Blue), circle)
	s.PopLayer()
...
func TestFullRenderPipeline(t *testing.T) {
	...
	}{
		{
			...
			setup: func(s *scene.Scene) {
				...
				s.Fill(scene.FillNonZero, scene.IdentityAffine(),
					scene.SolidBrush(render.Red), rect)
			},
		},
		{
			...
			setup: func(s *scene.Scene) {
				bg := scene.NewRectShape(0, 0, 200, 200)
				s.Fill(scene.FillNonZero, scene.IdentityAffine(),
					scene.SolidBrush(render.White), bg)

				s.PushLayer(scene.BlendMultiply, 0.8, nil)
				circle := scene.NewCircleShape(100, 100, 75)
				s.Fill(scene.FillNonZero, scene.IdentityAffine(),
					scene.SolidBrush(render.Blue), circle)
				s.PopLayer()
			},
		},
		{
			...
			setup: func(s *scene.Scene) {
				...
				for i := 0; i < 10; i++ {
					...
					s.Fill(scene.FillNonZero, scene.IdentityAffine(),
						scene.SolidBrush(render.RGB(float64(i)*0.1, 0.4, 0.8)), rect)
				}
			},
		},
	}
	...
	for _, sc := range scenarios {
		t.Run(sc.name, func(t *testing.T) {
			...
			pm := render.NewPixmap(sc.width, sc.height)
			err := b.RenderScene(pm, s)
			if err != nil {
