...
type VelloAccelerator struct {
	...
	pendingTarget *render.GPURenderTarget // target for the pending scene (nil if empty)
...
var _ render.GPUAccelerator = (*VelloAccelerator)(nil)
var _ render.DeviceProviderAware = (*VelloAccelerator)(nil)
var _ render.ComputePipelineAware = (*VelloAccelerator)(nil)
...
func (a *VelloAccelerator) CanAccelerate(op render.AcceleratedOp) bool {
	return op&(render.AccelScene|render.AccelFill|render.AccelStroke|render.AccelCircleSDF|render.AccelRRectSDF) != 0
}
...
func (a *VelloAccelerator) FillPath(target render.GPURenderTarget, path *render.Path, paint *render.Paint) error {
...
func (a *VelloAccelerator) StrokePath(target render.GPURenderTarget, path *render.Path, paint *render.Paint) error {
	if path == nil || path.NumVerbs() == 0 {
		return nil
	}
...
func (a *VelloAccelerator) FillShape(target render.GPURenderTarget, shape render.DetectedShape, paint *render.Paint) error {
	if shape.Kind == render.ShapeUnknown {
		return render.ErrFallbackToCPU
	}
...
func (a *VelloAccelerator) StrokeShape(target render.GPURenderTarget, shape render.DetectedShape, paint *render.Paint) error {
	if shape.Kind == render.ShapeUnknown {
		return render.ErrFallbackToCPU
	}
...
func (a *VelloAccelerator) flushLocked(target render.GPURenderTarget) error {
...
func compositeOver(target render.GPURenderTarget, src *image.RGBA) {
	w := target.Width
	h := target.Height
	if w > src.Bounds().Dx() {
		w = src.Bounds().Dx()
...
func velloSameTarget(a *render.GPURenderTarget, b *render.GPURenderTarget) bool {
