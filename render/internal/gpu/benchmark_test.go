...
func BenchmarkCircle50(b *testing.B) {
	runBackendComparison(b, 800, 600, func(s *scene.Scene) {
		for i := 0; i < 50; i++ {
			...
			color := render.RGBA{R: 0.8, G: 0.2, B: 0.4, A: 0.8}
		...
		for i := 0; i < 10; i++ {
			...
			for j := 0; j < 20; j++ {
				...
				s.Fill(scene.FillNonZero, scene.IdentityAffine(),
					scene.SolidBrush(render.RGBA{R: 0.3, G: 0.6, B: 0.9, A: 0.7}), circle)
			}
		}
	})
}
...
func BenchmarkLayers4(b *testing.B) {
	runBackendComparison(b, 512, 512, func(s *scene.Scene) {
		...
		s.Fill(scene.FillNonZero, scene.IdentityAffine(),
			scene.SolidBrush(render.RGBA{R: 1, G: 0.8, B: 0.2, A: 1}), rect1)
		s.PopLayer()
		...
		s.Fill(scene.FillNonZero, scene.IdentityAffine(),
			scene.SolidBrush(render.RGBA{R: 0.2, G: 0.6, B: 1, A: 1}), circle1)
		s.PopLayer()
		...
		s.Fill(scene.FillNonZero, scene.IdentityAffine(),
			scene.SolidBrush(render.RGBA{R: 0.9, G: 0.3, B: 0.5, A: 1}), rect2)
		s.PopLayer()
		...
		s.Fill(scene.FillNonZero, scene.IdentityAffine(),
			scene.SolidBrush(render.RGBA{R: 0.4, G: 0.8, B: 0.4, A: 1}), circle2)
		s.PopLayer()
	})
}
	...
	for _, m := range modes {
		b.Run(m.name, func(b *testing.B) {
			runBackendComparison(b, 400, 400, func(s *scene.Scene) {
				...
				s.Fill(scene.FillNonZero, scene.IdentityAffine(),
					scene.SolidBrush(render.RGBA{R: 0.5, G: 0.5, B: 0.5, A: 1}), bg)
				...
				for i := 0; i < 20; i++ {
					...
					s.Fill(scene.FillNonZero, scene.IdentityAffine(),
						scene.SolidBrush(render.RGBA{R: 0.9, G: 0.3, B: 0.2, A: 1}), rect)
				}
				s.PopLayer()
			})
		})
...
func runBackendComparison(b *testing.B, width, height int, setup func(s *scene.Scene)) {
	...
	b.Run("GPU", func(b *testing.B) {
		...
		pm := render.NewPixmap(width, height)
