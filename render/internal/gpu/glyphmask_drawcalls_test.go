...
func TestBuildGlyphMaskDrawCalls_QuadOffsetOnNilBindGroup(t *testing.T) {
	...
	batches := []GlyphMaskBatch{
		{Quads: make([]GlyphMaskQuad, 5), Transform: render.Identity(), Color: [4]float32{1, 1, 1, 1}},
		{Quads: make([]GlyphMaskQuad, 3), Transform: render.Identity(), Color: [4]float32{1, 0, 0, 1}},
		{Quads: make([]GlyphMaskQuad, 4), Transform: render.Identity(), Color: [4]float32{0, 1, 0, 1}},
	}
...
func TestBuildGlyphMaskDrawCalls_AllBindGroupsValid(t *testing.T) {
	...
	batches := []GlyphMaskBatch{
		{Quads: make([]GlyphMaskQuad, 2), Transform: render.Identity(), Color: [4]float32{1, 1, 1, 1}},
		{Quads: make([]GlyphMaskQuad, 3), Transform: render.Identity(), Color: [4]float32{1, 1, 1, 1}},
	}
...
func TestBuildGlyphMaskDrawCalls_EmptyBatchSkipped(t *testing.T) {
	...
	batches := []GlyphMaskBatch{
		{Quads: make([]GlyphMaskQuad, 3), Transform: render.Identity(), Color: [4]float32{1, 1, 1, 1}},
		{Quads: nil, Transform: render.Identity(), Color: [4]float32{1, 1, 1, 1}},
		{Quads: make([]GlyphMaskQuad, 2), Transform: render.Identity(), Color: [4]float32{1, 1, 1, 1}},
	}
...
func createMockBindGroup(t *testing.T, device *webgpu.Device, s *GPURenderSession) *webgpu.BindGroup {
	t.Helper()
