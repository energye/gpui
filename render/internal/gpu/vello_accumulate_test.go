...
func TestVelloAccelerator_FillPathRejectsWhenNotReady(t *testing.T) {
	...
	err := a.FillPath(target, path, render.NewPaint())
...
func TestVelloAccelerator_FillPathSkipsEmptyPath(t *testing.T) {
	...
	// Nil path
	err := a.FillPath(target, nil, render.NewPaint())
	if err != nil {
	...
	// Empty path
	err = a.FillPath(target, render.NewPath(), render.NewPaint())
	if err != nil {
...
func TestVelloAccelerator_FillShapeAccumulates(t *testing.T) {
	...
	paint := render.NewPaint()
	paint.SetBrush(render.Solid(render.Green))
...
func TestVelloAccelerator_FillShapeRejectsUnknown(t *testing.T) {
	...
	err := a.FillShape(target, shape, render.NewPaint())
	if !errors.Is(err, render.ErrFallbackToCPU) {
...
func TestVelloAccelerator_StrokePathAccumulates(t *testing.T) {
	...
	paint := render.NewPaint()
	paint.SetBrush(render.Solid(render.Blue))
	s := render.Stroke{Width: 4.0, Cap: render.LineCapButt, Join: render.LineJoinMiter, MiterLimit: 10.0}
	paint.SetStroke(s)
...
func TestVelloAccelerator_StrokeAlwaysUsesEvenOdd(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			path: func() *render.Path { p := render.NewPath(); p.RoundedRectangle(40, 40, 120, 80, 12); return p },
			join: render.LineJoinRound,
		},
		{
			name: "sharp rectangle",
			path: func() *render.Path { p := render.NewPath(); p.Rectangle(40, 40, 120, 80); return p },
			join: render.LineJoinMiter,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			paint := render.NewPaint()
			paint.SetBrush(render.Solid(render.Red))
			paint.SetStroke(render.Stroke{Width: 2.0, Cap: render.LineCapButt, Join: tc.join, MiterLimit: 10.0})
...
func TestVelloAccelerator_StrokeShapeUsesEvenOdd(t *testing.T) {
	...
	shape := render.DetectedShape{Kind: render.ShapeCircle, CenterX: 100, CenterY: 100, RadiusX: 40}
	paint := render.NewPaint()
	paint.SetBrush(render.Solid(render.Red))
	paint.SetStroke(render.Stroke{Width: 2.0, Cap: render.LineCapButt, Join: render.LineJoinRound, MiterLimit: 10.0})
...
func TestVelloAccelerator_FlushClearsPending(t *testing.T) {
	...
	if err := a.FillPath(target, path, render.NewPaint()); err != nil {
...
func TestVelloAccelerator_CanAccelerate(t *testing.T) {
	...
	tests := []struct {
		op   render.AcceleratedOp
		want bool
	}{
		{render.AccelFill, true},
		{render.AccelStroke, true},
		{render.AccelScene, true},
		{render.AccelCircleSDF, true},
		{render.AccelRRectSDF, true},
		{render.AccelText, false},
		{render.AccelImage, false},
		{render.AccelGradient, false},
	}
...
func TestVelloAccelerator_MultiplePathsMixedTypes(t *testing.T) {
	...
	// Fill a circle shape.
	circleShape := render.DetectedShape{
		Kind:    render.ShapeCircle,
		CenterX: 100,
		CenterY: 100,
		RadiusX: 40,
	...
	paint2 := render.NewPaint()
	paint2.SetBrush(render.Solid(render.Blue))
...
func makeTestTarget(w, h int) render.GPURenderTarget {
	stride := w * 4
	return render.GPURenderTarget{
