...
func makeRegularPolygon(cx, cy, radius float64, n int) []render.Point {
	points := make([]render.Point, n)
	for i := 0; i < n; i++ {
		angle := float64(i)*2*math.Pi/float64(n) - math.Pi/2
		points[i] = render.Pt(cx+radius*math.Cos(angle), cy+radius*math.Sin(angle))
	}
	return points
}

// reversePoints returns a copy of points in reverse order (flips winding).
func reversePoints(pts []render.Point) []render.Point {
	n := len(pts)
	out := make([]render.Point, n)
	for i, p := range pts {
		out[n-1-i] = p
	}
	return out
...
func makeStarPoints(cx, cy, outerR, innerR float64, nPoints int) []render.Point {
	pts := make([]render.Point, 0, nPoints*2)
	for i := 0; i < nPoints; i++ {
		outerAngle := float64(i)*2*math.Pi/float64(nPoints) - math.Pi/2
		pts = append(pts, render.Pt(cx+outerR*math.Cos(outerAngle), cy+outerR*math.Sin(outerAngle)))
		innerAngle := outerAngle + math.Pi/float64(nPoints)
		pts = append(pts, render.Pt(cx+innerR*math.Cos(innerAngle), cy+innerR*math.Sin(innerAngle)))
	}
	return pts
}
...
func TestIsConvex(t *testing.T) {
	tests := []struct {
		...
		{
			name: "single point",
			pts:  []render.Point{render.Pt(10, 20)},
			want: false,
		},
		{
			name: "two points",
			pts:  []render.Point{render.Pt(0, 0), render.Pt(100, 100)},
			want: false,
		},
		{
		...
		},
		{
			...
			pts: []render.Point{
				render.Pt(0, 0),
				render.Pt(50, 0), // collinear on top edge
				render.Pt(100, 0),
				render.Pt(100, 100),
				render.Pt(0, 100),
			},
...
func TestAnalyzeConvexity(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "CCW square",
			pts:         []render.Point{render.Pt(0, 0), render.Pt(100, 0), render.Pt(100, 100), render.Pt(0, 100)},
			wantConvex:  true,
			wantWinding: 1,
		},
		{
			name:        "CW square",
			pts:         reversePoints([]render.Point{render.Pt(0, 0), render.Pt(100, 0), render.Pt(100, 100), render.Pt(0, 100)}),
			wantConvex:  true,
			wantWinding: -1,
		},
		{
		...
		{
			name:        "L-shape (concave)",
			pts:         []render.Point{render.Pt(0, 0), render.Pt(100, 0), render.Pt(100, 50), render.Pt(50, 50), render.Pt(50, 100), render.Pt(0, 100)},
			wantConvex:  false,
			wantWinding: 0,
		},
		{
			name:        "degenerate collinear",
			pts:         []render.Point{render.Pt(0, 0), render.Pt(1, 0), render.Pt(2, 0)},
			wantConvex:  false,
			wantWinding: 0,
		},
		{
			name:        "single point",
			pts:         []render.Point{render.Pt(5, 5)},
			wantConvex:  false,
			wantWinding: 0,
		},
		{
			name:        "two points",
			pts:         []render.Point{render.Pt(0, 0), render.Pt(1, 1)},
			wantConvex:  false,
			wantWinding: 0,
		},
	}
...
func TestIsConvexIdenticalPoints(t *testing.T) {
	...
	pts := []render.Point{render.Pt(5, 5), render.Pt(5, 5), render.Pt(5, 5)}
	if IsConvex(pts) {
...
func BenchmarkIsConvexTriangle(b *testing.B) {
	pts := []render.Point{render.Pt(0, 0), render.Pt(100, 0), render.Pt(50, 100)}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		IsConvex(pts)
	}
...
func TestDirectionFlipCheckPreservesConvexPolygons(t *testing.T) {
	tests := []struct {
		...
		{"axis_aligned_rect", []render.Point{
			render.Pt(0, 0), render.Pt(10, 0), render.Pt(10, 5), render.Pt(0, 5),
		}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func makeStrokeExpandedSineWave() []render.Point {
	const n = 100
	...
	for _, v := range outVerbs {
		...
		case stroke.VerbMoveTo, stroke.VerbLineTo:
			points = append(points, render.Pt(outCoords[ci], outCoords[ci+1]))
			ci += 2
