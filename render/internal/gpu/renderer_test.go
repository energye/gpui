...
func TestLayerStackOperations(t *testing.T) {
	...
	brush := scene.SolidBrush(render.Red)
...
func TestSceneDecoderIntegration(t *testing.T) {
	...
	s.Stroke(scene.DefaultStrokeStyle(), scene.IdentityAffine(), scene.SolidBrush(render.Blue), line)
	...
	s.Fill(scene.FillEvenOdd, scene.IdentityAffine(), scene.SolidBrush(render.Green), circle)
	s.PopLayer()
