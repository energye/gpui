...
func TestNullDeviceHandle(t *testing.T) {
	...
	if handle.SurfaceFormat() != types.TextureFormatUndefined {
		t.Error("NullDeviceHandle.SurfaceFormat() should return Undefined")
	}
}

func TestTextureDescriptorDefault(t *testing.T) {
	desc := DefaultTextureDescriptor(256, 128, types.TextureFormatRGBA8Unorm)
	...
	if desc.Format != types.TextureFormatRGBA8Unorm {
