...
//   - scale(sx [, sy])
//   - matrix(a, b, c, d, e, f)
func applyTransform(dc *render.Context, transform string) error {
...
func applyTransformFunc(dc *render.Context, name string, args []float64) error {
	switch name {
	case "translate":
		if len(args) < 1 {
