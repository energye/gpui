...
func TestSelectionHighlightSurvivesLayout(t *testing.T) {
	...
	setup := func() (*rendering.PipelineOwner, *ViewportInputBox, *MultiLineInputBox) {
		edV := New()
	...
	// NOTE: the plain single-line box used to be a separate direct-offset
	// tree; since NewInputBox shares the viewport tree with
	// NewViewportInputBox it exercises the identical path, so only the
	// viewport (with clip check) and multiline-wrap cases remain.
	cases := []struct {
		name string
		pick func(boxV *ViewportInputBox, boxM *MultiLineInputBox) (*Editor, func() []*rendering.RenderColorBox, func(t *testing.T, g []hlGeom))
	}{
		{"viewport", func(boxV *ViewportInputBox, _ *MultiLineInputBox) (*Editor, func() []*rendering.RenderColorBox, func(t *testing.T, g []hlGeom)) {
			return boxV.Editor(), func() []*rendering.RenderColorBox { return boxV.highlights }, viewportCheck(boxV)
		}},
		{"multiline-wrap", func(_ *ViewportInputBox, boxM *MultiLineInputBox) (*Editor, func() []*rendering.RenderColorBox, func(t *testing.T, g []hlGeom)) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			owner, boxV, boxM := setup()
			ed, getHls, check := tc.pick(boxV, boxM)
			ed.SelectAll()
			geoms := func() []hlGeom {
