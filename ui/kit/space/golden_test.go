...
// L3 golden: horizontal two-box space with small gap on a white canvas.
//
...
func TestSpace_PRD_SPC22_GoldenBasic(t *testing.T) {
	...
	kids := s.Children()
	if off0, off1 := kids[0].Offset(), kids[1].Offset(); off0.X != 0 || off1.X != 48 {
	...
	path := filepath.Join("testdata", "golden_space_basic.png")
