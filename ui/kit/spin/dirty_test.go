...
func TestSpin_DirtyLocality(t *testing.T) {
	...
	if !owner.FlushPaint(&rendering.PaintContext{DC: dc, PaintVisits: &visits}, false) {
		t.Fatal("expected partial paint after single spin tick")
	}
	// 全树 20+ 静态加两加载圈，局部重画访问数必须远小于全量。
	if visits >= full/2 {
		t.Fatalf("PaintVisits=%d too high for single dirty spin (full=%d)", visits, full)
	}
	if owner.LayoutCount != layouts0 {
		t.Fatalf("layout must not run on phase-only tick: %d -> %d", layouts0, owner.LayoutCount)
	}
}
