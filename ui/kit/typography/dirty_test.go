...
// 脏局部：改一个排版，只脏它自己，不刷整棵树。
// 钩子是引擎现成的 PaintVisits（ui/rendering/paint_context.go），
// 用法抄 render_matrix_test.go：全量画一遍记个数，脏一个再画，
// 第二次必须远小于第一次，且布局次数不涨。
func TestTypography_PRD_TYP29_DirtyLocality(t *testing.T) {
	const vp = 200.0
	root := rendering.NewRenderBox()
	root.FixedWidth, root.FixedHeight = vp, vp

	me := typography.NewText("locality")
	other := typography.NewText("other")
	// 两排版各占一个重绘边界，中间夹 20 个静态块。
	...
	// 只改文本色：只标画脏，不碰布局。
	me.SetType(typography.TypeDanger)

	visits = 0
	if !owner.FlushPaint(&rendering.PaintContext{DC: dc, PaintVisits: &visits}, false) {
		t.Fatal("expected partial paint after single typography dirty")
	}
	// 全树 20+ 静态加两排版，局部重画访问数必须远小于全量。
	if visits >= full/2 {
		t.Fatalf("PaintVisits=%d too high for single dirty typography (full=%d)", visits, full)
	}
