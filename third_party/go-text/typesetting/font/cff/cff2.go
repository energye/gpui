...
func ParseCFF2(src []byte) (*CFF2, error) {
	...
	for i, font := range fdIndex {
		...
		// if required, parse the local subroutines. In CFF2 the Subrs offset is
		// relative to the private dict, matching FreeType (cffload.c:
		// base_offset + private_offset + local_subrs_offset).
		if pd.subrsOffset != 0 {
			addr := int(fd.privateDictOffset) + int(pd.subrsOffset)
			out.fonts[i].localSubrs, err = parseIndex2(src, addr)
			if err != nil {
				return nil, err
			}
		}
