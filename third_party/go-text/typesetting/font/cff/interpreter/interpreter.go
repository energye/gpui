...
func (p *Machine) Run(instructions []byte, localSubrs, globalSubrs [][]byte, handler OperatorHandler) error {
	...
	for len(p.instructions) > 0 || p.callStack.top > 0 {
		// If the instructions of the current (sub)routine are exhausted but
		// we are inside a subroutine call, implicitly return to the caller.
		// FreeType (psintrp.c cf2_interpT2CharString) does the same when the
		// charstring buffer runs out: it simulates a RETURN for subroutines
		// and an ENDCHAR for the top level charstring. Some fonts (e.g.
		// Source Sans 3 VF) rely on this implicit return and omit the
		// explicit 0x0B `return' operator at the end of a subroutine.
