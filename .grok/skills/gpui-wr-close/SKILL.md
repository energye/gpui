---
name: gpui-wr-close
description: 关 R/C 完整生命周期——定标准 → 写独立真窗 → 跑 GPU → 判 §2.2 → 回写 §2（R）或 §3（C）+ §5/§10。每个主能力与每个组合窗都必须有独立 examples/ui_wr_* 包；禁止用组合窗代替单 R。3 模式：首次关闭 / 反攻重关 / 优化重关。触发词：「关闭 R7」「关 C0」「反攻 R3」「优化 R4」等。
user_invocable: true
disable_model_invocation: false
---

# gpui-wr-close — 关 R/C 完整生命周期（3 模式）

> **分工：** wr-implement 前置能力 → **本 skill 关 R/C 真窗** → metrics-audit 审指标 → 洞交 wr-engine → wr-rewrite 可调度本 skill。
>
> **真源：** `docs/ENGINE_UI_WIDGET_RENDER.md` §0 U5/U6/U7/U9/U12–U16 + **§2** + **§3** + §2.2 + §2.5 + **§5** + §10。
>
> **独立真窗：** 每个 R → `ui_wr_r*`；每个 C → `ui_wr_c*`。禁止用组合窗代替单 R。
...
- **用组合窗 JSON 绿代替单能力窗关 R**
- **该 id 没有自己的 `examples/ui_wr_*` 包**（复用他包 main 关多项）
...
- **R**：读 §2 主表「状态」列
- **C**：读 §3 组合表波次/状态备注（或 §5 分期该 W 组合列）

| 当前状态 | 触发词示例 | 选哪个模式 |
|----------|------------|------------|
| `⬜` / 目录空 | 「关闭 R7」「关 C0」 | **模式 1 首次关闭** |
| `✅` 要升维重写 | 「反攻 R3」「重写 C1」 | **模式 2 反攻重关** |
| `✅` 要优化/增量 | 「优化 R4」「R7 加场景」 | **模式 3 优化重关** |
| `🔄` | 「继续反攻 R3」 | **模式 2** |

**模式冲突处理**：优化但 ⬜ → 问是否先首次关闭；关闭但 ✅ → 问模式 2 还是 3。
...
1. `read_file docs/ENGINE_UI_WIDGET_RENDER.md`：
   - **R**：§2 主表 → `PACKAGE` / `WINDOW=1200×800` / `CLOSE_SECONDS` / `WAVE` / 状态
   - **C**：§3 组合表 → 包名 / 窗口 / 关闭用时长（`max(覆盖各 R)`，见 §2.5）/ 状态备注
2. **独立包校验**：`examples/<PACKAGE>/` 必须是本 id 专属目录；禁止复用他 R/C 的 main 关本 id。
3. 读 §2.2 FAIL 线（判定基准）；§20.2 能力专用字段语义。
4. 读 §5（该 id 所属 W 的涉及集）+ §10 修订表。
**若文档与代码矛盾**（状态 ⬜ 但已有 main.go，或状态 ✅ 但目录空），**停下报告**，不要自决。
...
**R12 / R16 等**：必须有**独立** `ui_wr_r12_*` / `ui_wr_r16_*`。C0 等组合窗按 U17 迷你壳做集成，另包另关。
...
1. 定位 §2（R）或 §3（C）状态列
2. `✅` → `🔄`（质量返工；不得宣称关闭）
3. 若属某 W：§5 该 W 行可标 `🔄`（若整波未全绿）
4. §10 修订表加一行：`<版本> | R/C<id> 质量返工：<摘要>`
若当前 ⬜，跳过降级直接重写。若已 🔄，继续重写不重复降级。
...
## 第 5 步：同 W 的组合窗（仅当本 id 是 R 且 §5 要求该 C）

查 §5：本 R 所属 W 是否「必须绿的组合窗」含某 C*。

- **C 必须有自己的独立包** `ui_wr_c*`；目录空 → 停下问：现在建 C 还是本 R 先单窗收口（W 不能标全绿直到 C 也绿）
- 组合窗**不能**代替本 R 关项（U5）；跑 C 只做集成回归
- 关 **C 本身**时：本 skill 以 C 为 `ABILITY_ID`，写/跑 `ui_wr_c*`，回写 §3 + §5，**不**把覆盖的 R 标 ✅
...
| 模式 | 原状态 | 升维后 | §10 |
|------|--------|--------|-----|
| 模式 1 | `⬜` | `✅` | `<版本> \| <R/C id> 独立真窗 GPU PASS` |
| 模式 2 | `🔄` | `✅v2` | `<版本> \| <id> 质量升维 ✅v2：…` |
| 模式 3 | `✅🔄` | `✅v2-optimized` / `✅v2-extended` | `<版本> \| <id> 优化/增量：…` |

### 6.2 回写落点

1. **R** → §2 主表状态列；**C** → §3 组合表备注/波次列
2. 查 §5：该 W「必须绿的单能力窗 + 组合窗」是否**全部**已独立真窗且 ✅ → 才把 §5 该 W 标 `✅`/`✅v2`
3. §10 加修订行
**回写后** `read_file` 确认表格未破。
...
- [ ] 没用 CPU stub 或单测单独关 R/C（U8）
- [ ] **本 id 有独立 `examples/ui_wr_*` 包**（U5/U6）
- [ ] 没用组合窗代替单 R（U5）
- [ ] 窗口 1200×800（U15）；RUN_SECONDS≥关闭用且≥5（U16）
- [ ] JSON 全族 A–J（U12）；动画窗 fps 门禁（U13）；CPU/RSS（U14）
- [ ] README 可见效果 + 门禁（U7）
- [ ] 回写了 §2（R）或 §3（C）；需要时 §5；§10（U9）
- [ ] 没降画质装绿；没用 CompositeOnly+Clear 装省绘（U11）
...
| **入：wr-rewrite 调度** | 逐 **R 与 C** 回流时调度模式 1/2/3 |
| **入：用户直接触发** | 「关闭 R7」「关 C0」「反攻 R3」→ 按状态选模式 |
| **出：wr-engine / metrics-audit** | 底层洞 / 指标诚实性 |
| **不接：能力实现 / W 矩阵调度** | wr-implement / wr-rewrite |

**关键边界：** 关 **R 与 C** 的独立真窗生命周期；每个 id 一包；状态只写 §2/§3/§5/§10。
