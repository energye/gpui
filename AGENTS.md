...
- 当前为真实为真实GPU桌面环境，可运行真窗测试。
- **真窗必须自测 + 人工二合一**：需人工看效果的真窗，先跑自动自检（断言 + 出 JSON，门禁用 `-auto-only` 直接判 PASS/FAIL），再常驻等人工操作（收真事件即打日志并改标题显示状态，关窗或 `-manual-seconds` 超时后出汇总 + JSON）；**跑完即关的一次性验收窗不能代替常驻人工窗**（如 `ui_pf_x11` 只做验收，人工拖放看效果必须用 `examples/ui_pf_x11_dnd` 这类常驻窗）。
