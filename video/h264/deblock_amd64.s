...
// S1-E16-strong whole-edge kernels: one Go->asm call filters a full
// 16-line strong edge (4 segments x 4 lines). Structure mirrors
// ffmpeg's filter_mb_edgev with bS==4: one gate, one strong kernel for
// all 16 lines. Semantics equal four deblockVStrong/deblockHStrong
// calls bit for bit: same taps, same gates, same rounding, same
// stores. Unlike the weak twin there is no tc and no skip group: every
// segment of an all-strong edge filters unconditionally. Intra MB
// edges (all bS==4) arrive here from deblockLumaEdge16; mixed edges
// (any bS!=4 or any bS<=0) stay on the per-segment path.
// The V body uses R8-R11 only, so the loop counter lives in R14; the
// H body uses R8-R15, so its loop state lives in the 16-byte frame.

// func deblockVStrong16(p unsafe.Pointer, stride, alpha, beta int)
TEXT ·deblockVStrong16(SB), NOSPLIT, $0-32
	MOVQ p+0(FP), DI
	MOVQ stride+8(FP), SI
	MOVQ $4, R14
vsgrp16:
	...
	SHRL $8, AX; MOVB AL, 2(R10)
	SHRL $8, AX; MOVB AL, 2(R11)
	// Next group: 4 rows down. SI survives the body (only AX/CX/DX/BX
	// scratch GPRs), but R8-R11/X8/X9 do not, so reload them per group.
	MOVQ SI, AX
	SHLQ $2, AX
	ADDQ AX, DI
	SUBQ $1, R14
...
TEXT ·deblockHStrong16(SB), NOSPLIT, $16-32
	MOVQ p+0(FP), DI
	MOVQ stride+8(FP), SI
	MOVQ $0, AX
	MOVQ AX, 0(SP)
hsgrp16:
	...
	MOVQ R14, R15
	ADDQ SI, R15
	// Gather p3..q3: one DWORD (4 columns) per position row.
	// X0=p3=R8, X1=p2=R9, X2=p1=R10, X3=p0=R11,
	// X4=q0=R12, X5=q1=R13, X6=q2=R14, X7=q3=R15.
	// Lane order is the stored dword order (lane 0 = first column):
	// the blend math compares taps within a lane and the store writes
	// the same order back, so lanes round-trip unchanged.
	PXOR X0, X0
	MOVL (R8), AX; MOVD AX, X0; PUNPCKLBW X15, X0
	PXOR X1, X1
	MOVL (R9), AX; MOVD AX, X1; PUNPCKLBW X15, X1
	...
	PAND X14, X15
	PXOR X15, X6
	// Store p2/p1/p0/q0/q1/q2 as DWORDs to rows R9..R14, then advance.
	// No lane reversal: gathers above kept the stored dword order, so
	// the pack-store round-trips lanes back to the same columns.
	PACKUSWB X1, X1
	MOVD X1, AX; MOVL AX, (R9)
	PACKUSWB X2, X2
	MOVD X2, AX; MOVL AX, (R10)
...
// S1-E16 whole-edge kernels: one Go->asm call filters a full 16-line
// edge (4 segments x 4 lines) with per-segment tc. Structure mirrors
// ffmpeg's filter_mb_edgev (bS[4], one gate, one h_loop_filter call):
// the Go caller passes tc[4] (filterTC per segment, -1 for bS==0 skip)
// and the kernel loops 4 groups reusing the exact 4-line bodies below.
// Semantics equal four deblockVWeak/deblockHWeak calls bit for bit:
// same taps, same gates, same rounding, same stores; skipped groups
// touch nothing (scalar filterLumaEdge returns early on bS<=0).
// Only weak edges arrive here (all bS<4, decided by the Go caller);
// strong/mixed edges stay on the per-segment path. R14/R15 survive the
// V body (it uses R8-R11 only); the H body uses R8-R15 so its loop
// state lives in the 16-byte frame instead.

// func deblockVWeak16(p unsafe.Pointer, stride, alpha, beta int, tc unsafe.Pointer)
// tc points to 4 Go ints (8 bytes each); tc[g]<0 skips group g.
TEXT ·deblockVWeak16(SB), NOSPLIT, $0-40
	MOVQ p+0(FP), DI
	MOVQ stride+8(FP), SI
	MOVQ tc+32(FP), R15
	MOVQ $4, R14
vgrp16:
	...
	PAND X9, X0
	PXOR X0, X5
	// Store p1/p0/q0/q1 (saturating pack, DWORD per ROW — the
	// scatter shape was WRONG here: rows R8..R11 sit stride apart,
	// so lane k of X2 does NOT sit 4 bytes from lane k-1; a DWORD
	// store would smear one lane across 4 columns. Kept byte
	// stores (correct), but drop the SHRL dependency chain: each
	// byte extracts from its own copy (independent MOVDs let the
	// CPU fire all four in parallel; the old SHRL chain serialized
	// them). Same bytes, less stall. S1b-W store shape (V side).
	PACKUSWB X2, X2
	MOVD X2, AX
	MOVB AL, -2(R8)
	MOVD X2, AX
	SHRL $8, AX; MOVB AL, -2(R9)
	MOVD X2, AX
	SHRL $16, AX; MOVB AL, -2(R10)
	MOVD X2, AX
	SHRL $24, AX; MOVB AL, -2(R11)
	PACKUSWB X3, X3
	MOVD X3, AX
	MOVB AL, -1(R8)
	MOVD X3, AX
	SHRL $8, AX; MOVB AL, -1(R9)
	MOVD X3, AX
	SHRL $16, AX; MOVB AL, -1(R10)
	MOVD X3, AX
	SHRL $24, AX; MOVB AL, -1(R11)
	PACKUSWB X4, X4
	MOVD X4, AX
	MOVB AL, 0(R8)
	MOVD X4, AX
	SHRL $8, AX; MOVB AL, 0(R9)
	MOVD X4, AX
	SHRL $16, AX; MOVB AL, 0(R10)
	MOVD X4, AX
	SHRL $24, AX; MOVB AL, 0(R11)
	PACKUSWB X5, X5
	MOVD X5, AX
	MOVB AL, 1(R8)
	MOVD X5, AX
...
// Horizontal twin of deblockVWeak16: 4 column groups (cols 0-3, 4-7,
// 8-11, 12-15), same row taps per group, tc[g]<0 skips group g. The H
// body uses R8-R15, so loop state (tc pointer, group index) lives in
// the 16-byte frame; DI advances 4 pixels per group.
TEXT ·deblockHWeak16(SB), NOSPLIT, $16-40
	MOVQ p+0(FP), DI
	MOVQ stride+8(FP), SI
	MOVQ tc+32(FP), AX
	MOVQ AX, 0(SP)
