...
// convertBandSIMD runs the SSE4.1 row kernel for the bulk (w &^ 3) and the
// scalar tail for the rest. It reports true for every band (the kernel
// handles all matrix/range variants via the passed taps); ancient CPUs
// without SSE4.1 must set GPUI_SCALAR_CONVERT=1 (see convert_fast.go).
func convertBandSIMD(dst, y, cb, cr []byte, w int, t coeffs, ys, ye int) bool {
	...
	for yy := ys; yy < ye; yy++ {
		...
		if bulk > 0 {
			simdRowBulk(unsafe.Pointer(&dRow[0]), unsafe.Pointer(&yRow[0]), unsafe.Pointer(&cbRow[0]), unsafe.Pointer(&crRow[0]), bulk, yMul, yOff, rCr, gCb, gCr, bCb)
		}
		// Tail (at most one pair: w is even): same math as the scalar
